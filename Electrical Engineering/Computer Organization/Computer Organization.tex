...
material.
\ssc{Network}
\sssc{Computer network or network}
...
\ssc{Performance}
\sssc{Response time, execution time, wall clock time, or elapsed time}
...
\sssc{Throughput or bandwidth}
The number of tasks completed per unit time.
\sssc{CPU execution time or CPU time}
The time the CPU spends computing for a task.
\sssc{User CPU time}
The CPU time spent in a program itself.
\sssc{System CPU time}
The CPU time spent in the operating system performing tasks on behalf of a program.
\sssc{Clock cycle, tick, clock tick, clock period, clock, or cycle}
The
time for one clock period, usually of the processor clock, which
runs at a constant rate.
\sssc{Clock rate}
The reciprocal of clock period.
\sssc{Clock cycles per instruction (CPI)}
Average number of clock cycles per instruction for a program or
program fragment.
\sssc{Instruction count}
