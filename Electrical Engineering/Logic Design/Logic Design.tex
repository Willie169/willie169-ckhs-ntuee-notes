...
\sssc{Analysis of Cross-Coupled Gates}
To get the transition table of a sequential circuit with cross-coupled gates,
\ben
\item If the circuit composed of sequential subcircuits and combination subcircuits, we may analyze each sequential subcircuit respectively. It is usually easier to analyze a latch with Moore outputs.
...
\item If there are multiple roots with them, for each present state, signal tracing may be required to determine which root it will stabilize to. For the cases where the output signals are Moore and the present combination of output signals exists in one of the roots, the original output signals are preserved.
\eit
\een
