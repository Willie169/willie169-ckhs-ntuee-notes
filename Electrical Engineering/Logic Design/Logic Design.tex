...
\titletocdoc
\sct{Logic Design}
\ssc{Introduction to Digital Systems and Switching Circuits}
\begin{itemize}
...
A logic gate is a device that performs a Boolean function. Below, electronic symbols follow ANSI standards. A bubble in a input to a gate means inverting the input before inputting to the gate; a bubble in the output from a gate means inverting the output before outputting from the gate.
