...
\item let the clock skew of it relative to the flip-flop that receive an active edge first be $k_i$,
\item for each external input $X_{ij}$ that any of its input is dependent on, let the time $X_{ij}$ needs after an active clock edge to start being stable be $x_{ij}$, and the combinational circuit propagation delay from $X_{ij}$ to the input be $c_{ij}$, in which if multiple inputs of flip-flop $i$ depend on one external input, the one such that $x_{ij}+c_{ij}$ is larger is used, and
\item for each flip-flop output $Q_{ij}$ that any of its input is dependent on, let the clock skew of flip-flop $i$ relative to the flip-flop $Q_{ij}$ belongs to be $k_{ji}$, the flip-flop output propagation delay of $Q_{ij}$ be $y_{ij}$, and the combinational circuit propagation delay from $Q_{ij}$ to the be $d_{ij}$, in which if multiple inputs of flip-flop $i$ depend on one flip-flop output, the one such that $y_{ij}+d_{ij}$ is larger is used,
\eit
then, the circuit is valid only if it satisfies:
\[t_{clk}\geq\max_i\{\max\{\max_j\{x_{ij}+c_{ij}-k_i\},\max_j\{y_{ij}+d_{ij}-k_{ij}\}+s_i\},\]
and for each $i,j$:
\[y_{ij}+d_{ij}\geq k_{ij}+h_i.\]
...
\sssc{Counter}
A counter is a sequential circuit whose state changes each active edge and repeats every fixed finite number of active edges, called count cycle. The cyclic sequence of the states is called state loop.
\sssc{State/transition/state transition diagram/graph/cycle}
A graph of states with directed edges (arc) indicating transitions from present states to next states.
...
A shift register counter is a type of synchronous counter consists of a shift register with some or all the outputs $Q$ of the flip-flops being connected to the inputs of a combinational circuit with one output whose output is connected to $D_0$. For an $n$-bit shift register counter, $D_0$ as an $n$-variable Boolean function of $Q_0,Q_1,\ldots,Q_{n-1}$ is called feedback function of the shift register counter.
\sssc{Ring Counter}
...
\sssc{Linear Feedback Shift Register (LFSR)}
...
PLACEHOLDER: definition check

PLACEHOLDER: GF(2) abstract algebra interpretation of feedback polynomial

The initial state of an LFSR is called seed.

For each $n\in\bbN$, the maximal count cycle of $n$-bit LFSRs is $2^n-1$, in which all possible states except the state where all bits are zero are included.
