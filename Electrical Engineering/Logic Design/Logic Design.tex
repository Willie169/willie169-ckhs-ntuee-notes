...
\titletocdoc
\sct{Logic Design}
\ssc{Introduction to Switching Circuits}
\begin{itemize}
...
\item Logic gate (input at the right and output at the left):\mbox{}\\\cktus{not gate}{n}
\eit 
...
\item Logic gate (input at the right and output at the left, assuming two inputs and one output):\mbox{}\\\cktus{and gate}{nn}
\eit
...
\item Logic gate (input at the right and output at the left, assuming two inputs and one output):\mbox{}\\\cktus{or gate}{nn}
\eit
...
\item Logic gate (input at the right and output at the left, assuming two inputs and one output):\mbox{}\\\cktus{xor gate}{nn}
\eit
...
\item Logic gate (input at the right and output at the left, assuming two inputs and one output):\mbox{}\\\cktus{nor gate}{nn}
\eit
\sssc{NAND / Not AND}
\begin{itemize}
\item Definition: Multiple inputs, one output. True if all inputs are false and false otherwise.
\item Logic gate (input at the right and output at the left, assuming two inputs and one output):\mbox{}\\\cktus{nand gate}{nn}
\eit
\sssc{NOR / Not OR}
\begin{itemize}
...
By using inversion bubbles at the inputs instead of the output, we have alternative gate symbols:
\bit
\item NOT gate:\mbox{}\\
\cktus{buffer gate}{i}
\item AND gate:\mbox{}\\
...
\cktus{xor gate}{ii}
\eit
Adding inversion bubbles to both ends of any interconnection does not change the Boolean function realized by the circuit. Therefore, the gate type at level $i$ and $i+1$ can be changed by inserting inversion bubbles on both sides of all connections between levels $i+1$ and $i$. After this operation, the gates at level $i$ will appear in their alternate symbols (with input bubbles) and can be replaced with their standard forms if we desire.
