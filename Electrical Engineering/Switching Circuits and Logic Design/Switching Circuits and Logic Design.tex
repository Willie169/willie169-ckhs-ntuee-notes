...
\sct{Number Systems}
\ssc{Definition}
Let the base (aka radix) $b\in\mathbb{N}_{>1}$. A base-$b$ (aka radix-$b$) number in digits:
\[(-1)^m\ldots d_2d_1d_0.d_{-1}d_{-2}\ldots,\quad m\in\{0,1\}\land\forall i\colon d_i\in\mathbb{N}_0\land d_i<b\]
represents the value:
\[N =(-1)^m\sum_id_ib^i.\]

We can only represent a base $b$ in another specified base or in decimal (base-10) and specify the base you use the represent the base if not in decimal; otherwise, every base would be base-10.
...
We want to convert a nonnegative number $N$ in base $x$ with $x\in\mathbb{N}_{>1}$,
...
and then obtaining the fractional part with radix point of the magnitude of it using vertical addition where the $i$th row is $s_i$ with the units digit of $s_i$ in the $i$th digit to the right of the radix point, for each $i$, where carry propagation is applied to ensure all digits are valid in base $y$, i.e. $<y$.
\een
...
A finite rational number with the fractional part of the magnitude of it expressed in fraction in base $x$ with $x\in\mathbb{N}_{>1}$ being $D_x$ can be expressed in finite digits with radix point in base $y\neq x$ with $y\in\mathbb{N}_{>1}$ iff the reduced denominator (denominator cancelled the greatest common factor with the numerator) of $D_x$ has no prime factor that is not a prime factor of $y$.
\subsection{Binary}
