...
\ben
\item Draw a $(n-1)$-variable K-map for input variables except $E$. In each cell, let the $n-1$-variable input combination corresponding to the cell be $P$:
\bit
\item If both $f(P,E)=f(P,E')=X$, enter $X$.
\item If $f(P,E)=1$ and $f(P,E')=0$, enter $E$.
\item If $f(P,E)=0$ and $f(P,E')=1$, enter $E'$.
\item If $f(P,E)=f(P,E')=1$, enter $1$.
\item If $f(P,E)=1$ and $f(P,E')=X$, enter $1$.
\item If $f(P,E)=X$ and $f(P,E')=1$, enter $1$.
\item If $f(P,E)=0=f(P,E')=0$, enter $0$.
\item If $f(P,E)=0$ and $f(P,E')=X$, enter $0$.
\item If $f(P,E)=X$ and $f(P,E')=0$, enter $0$.
\eit
That is, let the set of products of $n-1$ literals that corresponding to a $1$ cell be $\mathcal{O}$, the set of products of $n-1$ literals that corresponding to a $E$ cell be $\mathcal{E}$, the set of products of $n-1$ literals that corresponding to a $E'$ cell be $\mathcal{C}$, and the set of products of $n-1$ literals that corresponding to a $X$ cell be $\mathcal{X}$, we write $f$ as:
\[f=\sum m(i\mid m_i\mathcal{O})+E\sum m(i\mid m_i\in\mathcal{E})+E'\sum m(i\mid m_i\in\mathcal{C})+\sum d(i\mid m_i\in\mathcal{X}),\]
...
\eit
\sssc{Two MEVs}
Suppose we have an $n$-variable (usually six-variable) completely or incompletely specified Boolean function $f$ with $E$ and $F$  being two of the input variables of $f$ and being the MEVs.
\ben
\item Draw a $(n-2)$-variable K-map for input variables except $E$ and $F$. In each cell, let the $(n-2)$-variable input combination corresponding to the cell be $P$:
