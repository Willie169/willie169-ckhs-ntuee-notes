...
To convert a nonnegative integer
\[N=(\ldots d_1 d_0)_x\]
in base $x$ with $x\in\mathbb{N}_{>1}$ into base $y\neq x$ (written in base $x$) with $y\in\mathbb{N}_{>1}$, we take $I_1=N$ and then repeatedly take $r_i=I_i\mod y$, convert it to base $y$, and take $I_{i+1}=\frac{I-r_i}{y}$ in the $i$th (start from $1$st) time until the first time $I_{i+1}=0$, and let that time be the $k$th time, then $N$ in base $y$ is digits $(r_kr_{k-1}\ldots r_1)_y$.

To convert a rational number $q=(-1)^k\frac{a}{b}$ with $a,b\in\mathbb{N}\land k\in\{0,1\}$ in base $x$ into base $y$, we convert $a$ and $b$ to base $y$ as $a_y$ and $b_y$ separate and the wanted result is $(-1)^k\frac{a_y}{b_y}$.

To convert a base-dependent expression that is a base-independent fuction $f$ of a vector of rational numbers $(p_1,p_2,\ldots)$, we convert each $p_i$ to the wanted base as $q_i$ and the wanted result is $f(q_1,q_2,\ldots)$.
\sssc{Special cases conversions}
There is an easier way for conversion between nonnegative integers in base $b$ and base $b^n$ (with $b,n\in\mathbb{N}_{>1}$):

Convert nonnegative integers in base $b$ into base $b^n$:
\ben
...
Convert nonnegative integers in base $b^n$ into base $b$:
\ben
