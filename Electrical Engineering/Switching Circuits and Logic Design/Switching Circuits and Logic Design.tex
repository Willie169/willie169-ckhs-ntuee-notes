...
\[N=\ldots d_1 d_0\]
in base $x$ with $x\in\mathbb{N}_{>1}$ into base $y\neq x$ (written in base $x$) with $y\in\mathbb{N}_{>1}$, we take $I_1=N$ and then repeatedly take $r_i=I_i\mod y$ in the $i$th (start from $1$st) time and $I_{i+1}=\frac{I-r_i}{y}$ until the first time $I_{i+1}=0$, and let that time be the $k$th time, then the same number in base $y$ is digits $r_kr_{k-1}\ldots r_1$.
