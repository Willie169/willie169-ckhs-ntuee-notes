...
\sct{Number Systems}
\ssc{Definition}
Let $b\in\mathbb{N}_{>1}$. A number with digits $\ldots d_2d_1d_0.d_{-1}d_{-2}\ldots$ in base (or radix) $b$ represents the value:
\[N = \sum_n d_nb^n\]
where $d_n \in \mathbb{N}_0\land d_i<b$.

We call numbers in base $b$ (decimal) base-$b$ number. We can only represent a base $b$ in another base or in decimal (base-10) and specify the base you use the represent the base if not in decimal; otherwise, every base would be base-10.

Usually, when base $b>10$ (decimal), $A$ (or $a$) is used to represent the digit $9+1$; when $b>11$ (decimal), $B$ (or $b$) is used to represent the digit $9+2$; and so on.

To indicate the base of a base-$b$ number, we append subscript $_b$ after it. We also use prefix \verb|0b| (or \verb|0B|) to represent base-2 numbers, aka binary numbers, prefix \verb|0| (or \verb|0o|, \verb|0O|) to represent base-8 numbers, aka octal numbers, prefix \verb|0x| (or \verb|0X|) to represent base-16 numbers, aka hexadecimal number, and no prefix (or \verb|0d|, \verb|0D|) to represent decimal numbers, aka base-10 numbers.
\ssc{Conversion between bases}
\sssc{General conversion method}
We want to convert a nonnegative rational number
\[N=(\ldots d_2d_1d_0+D)_x,\]
where $0\leq D<1$ is a fraction or a float-point number $(0.d_{-1}d_{-2}\ldots)_x$, in base $x$ with $x\in\mathbb{N}_{>1}$ into base $y\neq x$ (written in base $x$) with $y\in\mathbb{N}_{>1}$.
\ben
\item First, we take $I_1=(\ldots d_2d_1d_0)_x$ and repeatedly:
\ben
\item take $r_i=I_i\mod y$, convert it to base $y$, which must be a single digit, and
...
\item Then, $N$ is
\[\sum_{i=1}^kr_i\times y^{i-1}+\sum_{j=1}^ls_j\times y^{-j},\]
and $N$ in base $y$ can be obtained by first taking its integer part
\[(r_kr_{k-1}\ldots r_2r_1)_y\]
and then obtaining its fractional part using vertical addition where the $i$th row is $s_i$ with its units digit in the $i$th digit to the right of the decimal point, for each $i$, where carry propagation is applied to ensure all digits are valid in base $y$, i.e. $<y$.
\een

To convert an expression that is a base-independent fuction $f$ of a vector of nonnegative rational numbers $(p_1,p_2,\ldots)$, we convert each $p_i$ to the wanted base as $q_i$ and the wanted result is $f(q_1,q_2,\ldots)$.
\sssc{Power-base conversion method}
To convert a nonnegative integer or float-point number in base $b$ into base $b^n$ with $b,n\in\mathbb{N}_{>1}$, we group the digits to the left and right of the decimal point of the base-$b$ number into blocks of $n$ digits starting from the digit closest to the decimal point and convert each block to one digit in base $b^n$.
\sssc{Root-base conversion method}
To convert a nonnegative integer or float-point number in base $b^n$ into base $b$ with $b,n\in\mathbb{N}_{>1}$, we convert each digit of the base-$b^n$ number to $n$ digits in base $b$.
\sssc{Product and quotient-base conversion method}
We want to convert a nonnegative integer or float-point number
\[N=(\ldots d_2d_1d_0.d_{-1}d_{-2}\ldots)_x\]
in base $x$ with $x\in\mathbb{N}_{>1}$ into base $y=ax$ (written in base $x$) with $a\in\mathbb{N}_{>1}\lor\frac{1}{a}\in\mathbb{N}_{>1}$.

We take $D_i=d_i\cdot a^{-i}$, which is later treated directly as in base $y$, for each integer $i$.

Then, $N$ is
\[\sum_iD_i\times y^i,\]
and $N$ in base $y$ can be obtained by vertical addition where the $i$th row is $D_i$ with its units digit in the $i+1$th digit to the left of the decimal point, for each $i$, where carry propagation is applied to ensure all digits are valid in base $y$, i.e. $<y$.
\sssc{Criterion of finite expansion after base conversion}
A rational number $0\leq D<1$ in base $x$ with $x\in\mathbb{N}_{>1}$ can be expressed in finite digits to the right of decimal points in base $y\neq x$ with $y\in\mathbb{N}_{>1}$ iff the reduced denominator (cancelled common factors with the numerator) of $D$ has only prime factors that are prime factors of $y$.
\subsection{Binary}
...
\sssc{Truth table}
A truth table, also called a table of combinations, specifies the corresponding output values for all possible combinations of input values for a Boolean function. A truth table for an $n$-variable Boolean function $f\qty(A_1,A_2,\ldots A_n)$ have $2^n$ rows and is:
...
A minterm of a completely specified Boolean function $f\colon\{1,0\}^n\to\{1,0\}$, often denoted as $m_i$ for the input combination in the $i$th row of the truth table of $f$ that is in the ON-set of $f$, is defined for any input combinations in the ON-set of $f$ as
\[m_i=\prod_{k=1}^ny_k,\]
...
Minterm expansion, canonical SOP, or standard SOP is an expression of a completely specified Boolean function as a sum of minterms of it. Let $S$ be the set of all integer $i$ such that the input combination in the $i$th row of the truth table of $f$ is in the ON-set of $f$. Then the minterm expansion, canonical SOP, or standard SOP of $f$ is
\[f=\sum_{i\in S}m_i,\]
also denoted as
\[f=\sum m(i\in S).\]
...
A maxterm of a completely specified Boolean function $f\colon\{1,0\}^n\to\{1,0\}$, often denoted as $M_i$ for the input combination in the $i$th row of the truth table of $f$ that is in the OFF-set of $f$, is defined for any input combinations in the OFF-set of $f$ as
\[M_i=\sum_{k=1}^ny_k,\]
...
Maxterm expansion, canonical POS, or standard POS is an expression of a completely specified Boolean function as a product of maxterms of it. Let $T$ be the set of all integer $i$ such that the input combination in the $i$th row of the truth table of $f$ is in the OFF-set of $f$. Then the maxterm expansion, canonical POS, or standard POS of $f$ is
\[f=\prod_{i\in T}M_i,\]
also denoted as 
\[f=\prod M(i\in T).\]
...
Let $f\colon D\to\{1,0\}$ be a completely specified Boolean function with ON-set $S$, OFF-set $T=D\setminus S$, minterm expansion
\[f=\sum_{i\in S}m_i,\]
...
\begin{longtable}[c]{c|c|c|}
\multicolum{1}{c}{\thead{\backslashbox{$B$}{$A$}} & \multicolum{1}{c}{\thead{0}} & \multicolum{1}{c}{\thead{1}} \\\cline{2-3}
\multicolum{1}{c|}{\thead{0}} & $f(0,0)$ & $f(1,0)$ \\\cline{2-3}
\multicolum{1}{c|}{\thead{1}} & $f(0,1)$ & $f(1,1)$ \\\cline{2-3}
\end{longtable}

A K-map for a $3$-variable Boolean function $f(A,B,C)$ is
\begin{longtable}[c]{c|c|c|}
\multicolum{1}{c}{\thead{\backslashbox{$BC$}{$A$}} & \multicolum{1}{c}{\thead{0}} & \multicolum{1}{c}{\thead{1}} \\\cline{2-3}
\multicolum{1}{c|}{\thead{00}} & $f(0,0,0)$ & $f(1,0,0)$ \\\cline{2-3}
\multicolum{1}{c|}{\thead{01}} & $f(0,0,1)$ & $f(1,0,1)$ \\\cline{2-3}
\multicolum{1}{c|}{\thead{11}} & $f(0,1,1)$ & $f(1,1,1)$ \\\cline{2-3}
\multicolum{1}{c|}{\thead{10}} & $f(0,1,0)$ & $f(1,1,0)$ \\\cline{2-3}
\end{longtable}

A K map for a $4$-variable Boolean function $f(A,B,C,D)$ is
\begin{longtable}[c]{c|c|c|c|c|}
\multicolum{1}{c}{\thead{\backslashbox{$CD$}{$AB$}} & \multicolum{1}{c}{\thead{00}} & \multicolum{1}{c}{\thead{01}} & \multicolum{1}{c}{\thead{11}} & \multicolum{1}{c}{\thead{10}} \\\cline{2-5}
\multicolum{1}{c|}{\thead{00}} & $f(0,0,0,0)$ & $f(0,1,0,0)$ & $f(1,1,0,0)$ & $f(1,0,0,0)$ \\\cline{2-5}
\multicolum{1}{c|}{\thead{01}} & $f(0,0,0,1)$ & $f(0,1,0,1)$ & $f(1,1,0,1)$ & $f(1,0,0,1)$ \\\cline{2-5}
\multicolum{1}{c|}{\thead{11}} & $f(0,0,1,1)$ & $f(0,1,1,1)$ & $f(1,1,1,1)$ & $f(1,0,1,1)$ \\\cline{2-5}
\multicolum{1}{c|}{\thead{10}} & $f(0,0,1,0)$ & $f(0,1,1,0)$ & $f(1,1,1,0)$ & $f(1,0,1,0)$ \\\cline{2-5}
\end{longtable}
...
& \multicolum{1}{c}{\thead{\backslashbox{$BC$}{$DE$}} & \multicolum{1}{c}{\thead{00}} & \multicolum{1}{c}{\thead{01}} & \multicolum{1}{c}{\thead{11}} & \multicolum{1}{c}{\thead{10}} \\\cline{3-6}
& \multicolum{1}{c|}{\thead{00}} & \backslashbox{$f(1,0,0,0,0)$}{$f(0,0,0,0,0)$} & \backslashbox{$f(1,0,1,0,0)$}{$f(0,0,1,0,0)$} & \backslashbox{$f(1,1,1,0,0)$}{$f(0,1,1,0,0)$} & \backslashbox{$f(1,1,0,0,0)$}{$f(0,1,0,0,0)$} \\\cline{3-6}
$A$ & \multicolum{1}{c|}{\thead{01}} & \backslashbox{$f(1,0,0,0,1)$}{$f(0,0,0,0,1)$} & \backslashbox{$f(1,0,1,0,1)$}{$f(0,0,1,0,1)$} & \backslashbox{$f(1,1,1,0,1)$}{$f(0,1,1,0,1)$} & \backslashbox{$f(1,1,0,0,1)$}{$f(0,1,0,0,1)$} \\\cline{3-6}
\backslashbox{$1$}{$0$} & \multicolum{1}{c|}{\thead{11}} & \backslashbox{$f(1,0,0,1,1)$}{$f(0,0,0,1,1)$} & \backslashbox{$f(1,0,1,1,1)$}{$f(0,0,1,1,1)$} & \backslashbox{$f(1,1,1,1,1)$}{$f(0,1,1,1,1)$} & \backslashbox{$f(1,1,0,1,1)$}{$f(0,1,0,1,1)$} \\\cline{3-6}
& \multicolum{1}{c|}{\thead{10}} & \backslashbox{$f(1,0,0,1,0)$}{$f(0,0,0,1,0)$} & \backslashbox{$f(1,0,1,1,0)$}{$f(0,0,1,1,0)$} & \backslashbox{$f(1,1,1,1,0)$}{$f(0,1,1,1,0)$} & \backslashbox{$f(1,1,0,1,0)$}{$f(0,1,0,1,0)$} \\\cline{3-6}
\end{longtable}
