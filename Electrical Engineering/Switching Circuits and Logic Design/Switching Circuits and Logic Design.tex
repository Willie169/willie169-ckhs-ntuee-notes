...
Let $b\in\mathbb{N}_{>1}$. A number with digits $\ldots d_1 d_0.d_{-1}d_{-2}\ldots$ in base (or radix) $b$ represents the value:
\[N = \sum_n d_nb^n\]
...
To indicate the base of a base-$b$ number, we append subscript $_b$ after it. We also use prefix $0b$ (or $0B$) to represent base-2 numbers, aka binary numbers, prefix $0$ to represent base-8 numbers, aka octal numbers, prefix $0x$ (or $0X$) to represent base-16 numbers, aka hexadecimal number, and no prefix to represent decimal numbers, aka base-10 numbers.
\subsubsection{General conversion method}
We want to convert a nonnegative number
\[N=(\ldots d_1 d_0.d^{-1}d^{-2}\ldots)_x\]
in base $x$ with $x\in\mathbb{N}_{>1}$ into base $y\neq x$ (written in base $x$) with $y\in\mathbb{N}_{>1}$.
\ben
\item We first take $I_1=(\ldots d_1 d_0)_x$ and repeatedly:
\ben
...
\item And then we take $J_1=(d^{-1}d^{-2}\ldots)_x$ and repeatedly
\item take $s_j=\lfloor J_j\cdot y$, which may be more than $1$ digits if $x>y$ and is later treated directly as base $y$, and
...
\item Then, $N$ in base $y$ is
\[(r_kr_{k-1}\ldots r_1.s_1s_2\ldots l_k)_y.\]
\een

To convert an expression that is a base-independent fuction $f$ of a vector of rational numbers $(p_1,p_2,\ldots)$, we convert each $p_i$ to the wanted base as $q_i$ and the wanted result is $f(q_1,q_2,\ldots)$.
\sssc{Special conversion methods}
There are easier ways for conversion between base $b$ and base $b^n$ with $b,n\in\mathbb{N}_{>1}$.

To convert numbers in base $b$ into base $b^n$, we group the digits to the left and right of the decimal point of the base-$b$ number into blocks of $n$ digits starting from the digit closest to the decimal point and convert each block to one digit in base $b^n$.

To convert numbers in base $b^n$ into base $b$, we convert each digit of the base-$b^n$ number to $n$ digits in base $b$.
\subsection{Binary}
