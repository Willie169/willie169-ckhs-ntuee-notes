...
\ssc{Digital circuit and boolean algebra}
...
\sssc{Fan-ins}
The maximum or actual number of inputs to a device is called the fan-in or fanin.
...
\sssc{Fan-out}
The maximum or actual number of inputs driven by a device's output is called the fan-out or fanout.
\sssc{Buffer}
...
\sssc{Digital circuit or switching circuit}
...
\sssc{Integrated Circuit (IC), microcip, or chip}
...
The speed can be further increased by making middle blocks larger than end blocks.
\sssc{Parallel prefix adder (PPA)}
Parallel prefix adders are a class of adders consist of three stages, preprocessing stage, prefix network or prefix computation stage, and postprocessing stage. For an $n$-bit parallel prefix adder adding operands $A_0A_1\ldots B_{n-1}$ and $B_0B_1\ldots B_{n-1}$, preprocessing stage computes generate $G_i=A_i\cdot B_i$ and propagate $P_i=A_i\oplus B_i$ for each bit, and then a prefix operation $\circ$ is defined as
\[(G_k,P_k)\circ (G_j,P_j)=(G_k+P_kG_j,P_kP_j),\]
which is associative, and the prefix network starts with $n$ roots, $(G_0,P_0)$ to $(G_{n-1},P_{n-1})$, to compute $n$ goals,
\[(G_i^*,P_i^*)\coloneq\bigcirc_{j=0}^i(G_{i-j},P_{i-j})\]
for all $i\in\bbZ\cap[0,n-1]$, where $C_{i+1}=G_i^*+P_i^*C_0$, each route from $(G_i,P_i)$ to $(G_i^*,P_i^*)$ for all $i\in\bbZ\cap[0,n-1]$ is hereafter called the $i$ span, and the data in the $i$ span is hereafter denoted $(g_i,p_i)$, and finally, the postprocessing stage computes $S_i=P_i\oplus C_i$ for all $i\in\bbZ\cap[0,n-1]$. For $i\ne j$, that the $j$ span is combined with $i$ span means the $j$ span is replaced with $(g_j,p_j)\circ(g_i,p_i)$ and the $i$ span is unchanged. This class of adder has time complexity $O(\log n)$, which is optimal. Different designs differ in the prefix network stage and can be different in area, wiring complexity, and fan-out.
\sssc{Kogge–Stone adder (KSA or KS)}
Kogge–Stone adder is a type of parallel prefix adder optimized for minimum logic depth but with larger area and wiring complexity. The prefix network of it consists of $\lceil\log_2(n)\rceil$ stages. For all $i\in[1,\lceil\log_2(n)\rceil-1]$, in the $i$th stage, for $j\in\bbZ\cap[2^{i-1},n-1]$, the $j$ span is combined with the $j-2^{i-1}$ span. The fan-out is $\lceil\log_2(n)\rceil+1$.
\sssc{Brent–Kung adder (BKA or BK)}
Brent–Kung adder is a type of parallel prefix adder with smaller area and wiring complexity but longer logic depth than Kogge–Stone adder. The prefix network of it is split into reduction or up-sweep phase, with $\lceil\log_2(n)\rceil$ stages, and distribution or down-sweep phase, with $\lceil\log_2(n)\rceil-1$ stages, which are $2\lceil\log_2(n)\rceil-1$ stages in total. Suppose $n=2^k$. For all $i\in\bbZ\cap[1,k]$, in the $i$th stage of reduction phase, for all $j\in\bbZ\cap[1,2^{k-i}]$, the $j*2^i-1$ span is combined with the $j*2^i-2^{i-1}-1$ span. For all $i\in\bbZ\cap[1,k-1]$, in the $i$th stage of distribution phase, for all $j\in\bbZ\cap[1,2^i-1]$, the $j*2^{k-i}+2^{k-i-1}-1$ span is combined with $j*2^{k-i}-1$ span. The fan-out is $\lceil\log_2(n)\rceil$.
\sssc{Han–Carlson adder (HCA)}
Han–Carlson adder is a type of parallel prefix adder that combines the ideas of Kogge–Stone and Brent–Kung adder. The prefix network of it is split into three phases, Brent–Kung reduction or up-sweep phase, Kogge–Stone phase, and Brent–Kung distribution or down-sweep phase. Suppose $n=2^k$. A particular family of one parameter $s\in\bbZ\cap[1,k)$ of Han–Carlson adder is described as follows. Both Brent–Kung reduction phase and Brent–Kung distribution phase have $s$ phases, and Kogge–Stone phase has $k-s$ phases. For all $i\in\bbZ\cap[1,s]$, in the $i$th stage of Brent–Kung reduction phase, for all $j\in\bbZ\cap[1,2^{k-i}]$, the $j*2^i-1$ span is combined with the $j*2^i-2^{i-1}-1$ span. For all $i\in[1,k-s]$, in the $i$th stage of Kogge–Stone phase, for $j\in\bbZ\cap[2^{i-1}+1,2^{k-s}]$, the $j*2^s-1$ span is combined with the $(j-2^{i-1})*2^s-1$ span. For all $i\in\bbZ\cap[1,s]$, in the $i$th stage of Brent–Kung distribution phase, for all $j\in\bbZ\cap[1,2^{k-s-1+i}-1]$, the $j*2^{s+1-i}+2^{s-i}-1$ span is combined with $j*2^{s+1-i}-1$ span. The number of stages in total is $k+s$. The fan-out is $k+1$.
\sssc{Sklansky adder (SA) or divide-or-conquer adder}
Sklansky adder is a type of parallel prefix adder optimized for minimum logic depth and with smaller area and wiring complexity than Kogge–Stone adder but has very high fan-out of $n-2^{\lceil\log_2(n)\rceil-1}+1$. The prefix network of it consists of $\lceil\log_2(n)\rceil$ stages. For all $i\in\bbZ\cap[1,2^{\lceil\log_2(n)\rceil-i}]$, for all $l\in[j*2^i-2^{i-1},\min(j*2^i-1,n-1)]$, the $l$ span is combined with the $j*2^i-2^{i-1}-1$ span, where the $j*2^i-2^{i-1}-1$ span has fan-out of $\min(2^{i-1}+1,n-2^{\lceil\log_2(n)\rceil-1}+1)$.
\sssc{Decoders}
...
\sssc{Lookup table (LUT)}
A lookup table is a possibly multidimensional array implemented in software or hardware that replaces runtime computation of a function with an array indexing retrieval, where the dimension of the array is the arity of the function.
\ssc{Devices}
\sssc{Read-only memory (ROM)}
...
\bit
\item Mask-programmable ROM: Programmed permanently during manufacturing using photolithography by selectively including or omitting the diodes at the cells. The mask used in photolithography is expensive, so the use of mask-programmable ROMs is economically feasible only if a large quantity (typically several thousand or more) is required with the same data array.
\item Programmable ROM (PROM): Manufactured with diodes at all cells and programmable once (one-time programmable (OTP)) by the user using a PROM programmer (aka PROM burner), which sends high-voltage pulses to burn tiny fuses in the cell to create permanent 1's and 0's.
\item Erasable Programmable ROM (EPROM): Manufactured with each cell contains a control gate that receives normal programming or read voltages and a floating gate that is completely insulated by oxide and can store electric charge and with quartz window on top, programmable by the user using a PROM programmer (aka PROM burner), which sends high-voltage pulses to inject electrons into floating gates, and erasable by the user by expose it to intense UV light that provides photons providing energy to remove trapped electrons from the floating gate simultaneously at all celles.
\item Electrically Erasable Programmable ROM (EEPROM): Similar to EPROM but erasing process is done electrically to remove trapped electrons from the floating gate one bit or one byte at a time (instead of all cells simultaneously) only a limited number of times, typically 100 to 1000 times.
\eit
\sssc{Programmable Logic Devices (PLDs)}
...
TODO: definition check, $\bbF_2$ abstract algebra interpretation of feedback polynomial
...
TODO: primitive, irreducible, Primitive trinomials in abstract algebra

TODO: The state graph of an $n$-bit LFSR (with $2^n-1$ vertices) consists of only disjoint cycles and loops, and the state where all bits are zero is always with a loop.

TODO: Fibonacci LFSRs, Galois LFSRs, Xorshift LFSRs
...
\end{cases}.\]
\eit
\item Define the \tb{prefix/failure function/link} $\pi\colon S\to S$ such that $\pi(S_0)=S_0$, and for any $S_i\in S\setminus\{S_0\}$ with $S_p=\pi(S_i)$, it satisfies that $p<i$, that $w_p$ equals to the string of the last $p$ characters of $w_i$, and that for any $q\in(p,i)$, $w_q$ doesn't equal to the string of the last $q$ characters of $w_i$. However for
\bit
...
\ssc{Multiplier}
\sssc{General}
Multiplication of two unsigned binary numbers $A=A_{n-1}A_{n-2}\ldots A_0$ and $B=B_{n-1}B_{n-2}\ldots B_0$ involves two steps, computing the $n$ partial products $P_i=AB_i<<i$ where $<<i$ means shifting $i$ bits towards MSB for $i\in\bbZ\cap[0,n-1]$, and computing multi-operand addition $\sum_{i=0}^{n-1}P_i$ to get the final result.

Bit heap is the heap of partial products one partial product per row, where each column is also called weight and the number of partial products containing that column is called height. For the plain $n$ partial products described above, column $i$ (starting at 0) has $i+1$ bits for $i\in\bbZ\cap[0,n-1]$ and $2n-i-1$ bits for $i\in\bbZ\cap[n,2n-2]$, and the maximum height is $n$ at column $n-1$.

A full adder can reduce 3 bits of the same column to one bit of the same column and one bit of the next column. A half adder can reduce 2 bits of the same column to one bit of the same column and one bit of the next column.
\sssc{Shift-add multiplier}
Use a shifter and an accumulator to sum each partial product one by one, trading off speed for area.
\sssc{Array multiplier}
Sum partial products with carry-save adder until the number of operands left is two, and use an adder to sum the two. The number of stage is $O(n)$.
\sssc{Wallace (tree/reduction) multiplier}
In each stage of Wallace multiplier, for each column of bit heap with height $h$ in parallel, $\floor{\frac{h}{3}}$ full adders are used, and if $h\mod 3=2$, a half adder is also used, until all columns have height $\le 2$. And then an adder is used to get the final result. The number of stage in total is $O(\log n)$.
\sssc{Dadda (tree/reduction) multiplier}
Dadda defined a sequence $\langle d_k\rangle$:
\[d_1=2,\]
\[d_{k+1}=\lfloor\frac{3}{2}d_k\rfloor,\quad k\in\bbN,\]
that is, $2,3,4,6,9,13,19,\ldots$.

In each stage of Dadda multiplier, choose the largest $k$ such that $d_k\le$ the height of the highest column, for each column of bit heap with height $h$ in parallel but with carry bit from last column in current stage (if any) counted when calculating full and half adders used, $\floor{\frac{h-d_k}{2}}$ full adders are used, and if $(h-d_k)\mod 2=1$, a half adder is also used, until all columns have height $\le 2$. And then an adder is used to get the final result. The number of stage in total is $O(\log n)$, and the gates and area used is less than that of Wallace multiplier except some small $n$.
\sssc{Booth's multiplication algorithm or Booth encoding}
Booth's multiplication algorithm or Booth encoding is a technique to reduce the number of partial products and makes them work for two's complement numbers (but not for unsigned numbers without tweak or constraint). Those partial products are then fed in to Wallace or Dadda tree. 

Suppose there are two two's complement signed numbers, multiplicand $A=A_{n-1}A_{n-2}\ldots A_0$ and multiplier $B=B_{n-1}B_{n-2}\ldots B_0$, $B_{-1}$ is defined as 0.

Consider a run of ones $B_sB_{s-1}\ldots B_t$, called starting at bit $t$ and ending at bit $s+1$. It contributes $B^{s+1}-B^t$. If bit $i$ is 0 and bit $i-1$ is 1, there is a end of a run of ones at bit $i$, contributing $2^i$. If bit $i$ is 1 and bit $i-1$ is 0, there is a start of a run of ones at bit $i$, contributing $-2^i$. If bits $i$ and $i-1$ are the same, there is no start or end of a run of ones at bit $i$, contributing $0$. Thus bit $i$ contributes $(B_{i-1}-B_i)*2^i$. If $B_0$ is 1, there is a start of a run of ones at bit $0$. This is why $B_{-1}$ is defined as $0$. Consider last bit, bit $n-1$. If bit $n-1$ is 0 and bit $n-2$ is 1, there is a end of a run of ones at bit $n-1$, contributing $2^{n-1}$. If bit $n-1$ is 1 and bit $n-2$ is 0, the number is negative, contributing $-2^{n-1}$. If bit $n-1$ is 0 and bit $n-2$ is 0, there is no end of a run of ones at bit $n-1$, contributing $0$. If bit $n-1$ is 1 and bit $n-2$ is 1, the number is negative, contributing $-2^{n-1}$, and there is an end of run of ones at bit $n-1$, contributing $2^{n-1}$, thus contributing $0$ in total. Therefore, the same rule can be applied directly to the last bit.

Consider $B_{k(i+1)-1}\ldots B_{ki-1}$. $B_{k(i+1)-1}\ldots B_{ki}$ contributes
\[\ba
m=&\sum_{j=ki}^{k(i+1)-1}(B_{j-1}-B_j)*2^j\\
=&\sum_{j=ki}^{k(i+1)-1}B_{j-1}*2^j-\sum_{j=ki}^{k(i+1)-1}B_j*2^j\\
=&B_{ki-1}*2^{ki}+\sum_{j=ki}^{k(i+1)-2}B_j*2^{j+1}-\sum_{j=ki}^{k(i+1)-2}B_j*2^j-B_{k(i+1)-1}*2^{k(i+1)-1}\\
=&B_{ki-1}*2^{ki}+\sum_{j=ki}^{k(i+1)-2}B_j*2^j-B_{k(i+1)-1}*2^{k(i+1)-1}\\
=&\qty(B_{ki-1}+\sum_{j=0}^{k-2}B_{ki+j}*2^j-B_{k(i+1)-1}*2^{k-1})*2^{ki},
\ea\]
which has range $[-2^{k-1},2^{k-1}]*2^{ki}$.

When using Booth radix-$2^k$ encoding, assuming $n$ is a multiple of $k$, the algorithm scans the multiplier $k+1$ bits at a time with one bit overlapping with last scan and starting from $B_{k-1}\ldots B_{-1}$ to $B_{n-1}\ldots B_{n-k-1}$, which are $\frac{n}{k}$ windows in total and $B_{k(i+1)-1}\ldots B_{ki-1}$ in each scan $i\in\bbZ\cap\qty[0,\frac{n}{k}-1]$, and computes partial product $m*A*2^{ki}$ where $m=B_{ki-1}+\sum_{j=0}^{k-2}B_{ki+j}*2^j-B_{k(i+1)-1}*2^{k-1}$ is retrieved from a lookup table given $B_{k(i+1)-1}\ldots B_{ki-1}$.

The $2^z$ multiples of $A$ for $z\in\bbN$ can be computed by shifting $A$ $z$ bits towards MSB. Only radix-2 and radix-4 has $m$ that ranges only on exponents of 2, which are $\{-1,0,1\}$ and $\{-2,-1,0,1,2\}$ respectively. And radix-4 has fewer windows than radix-2 and is the most used one.

If only $n$ bits is needed, we can simply discard upper bits from partial products.

Now consider unsigned $A,B$. The result is the result of two's complement plus $A*B_{n-1}*2^n$, which can be computed by adding it as another partial product, or, if only $n$ bits is needed, be ignored since it's in the overflowing bits.
\ssc{Divider}
\sssc{General}
Division of two unsigned binary number, dividend $N=N_{n-1}N_{n-2}\ldots N_0$ and divisor $D=D_{n-1}D_{n-2}\ldots D_0$, gives two unsigned binary number, quotient $Q=Q_{n-1}Q_{n-2}\ldots Q_0$ and remainder $R=R_{n-1}R_{n-2}\ldots R_0$ such that
\[N=D*Q+R\]
and
\[0\le R<D.\]
Three $n$-bit registers, remainder register $R$ holding partial remainder and finally holding remainder, quotient register $Q$ initially holding dividend $N$, and divisor register $D$ holding divisor, and one subtractor are used. Treat ${R,Q}$ as one combined register.
\sssc{Restoring Divider}
In each iteration,
\ben
\item Shift ${R,Q}$ left by one bit.
\item Assign $R-D$ to $R$.
\item Check $R$ in 2's complement.
\bit
\item If $R>0$, set the LSB of $Q$ to 1.
\item If $R<0$, assign $R+D$ to $R$, called restoring.
\eit
\een
Iterate $n$ times to obtain the final result.
\sssc{Non-restoring Divider}
Let a flag bit $F$ exist and be initially set to 0. In each iteration,
\ben
\item Shift ${R,Q}$ left by one bit.
\item Check $F$.
\bit
\item If $F=0$, assign $R-D$ to $R$.
\item If $F=1$, assign $R+D$ to $R$.
\eit
\item Check $R$ in 2's complement.
\bit
\item If $R>0$, set the LSB of $Q$ to 1 and $F$ to 0.
\item If $R<0$, set $F$ to 1.
\eit
\een
Iterate $n$ times.
\bit
\item If $F=0$, do nothing.
\item If $F=1$, assign $R+D$ to $R$.
\eit
Now we've obtained the final result.
\sssc{SRT Divider}
The algorithm is named after D. W. Sweeney of IBM, James E. Robertson of University of Illinois, and K. D. Tocher of Imperial College London. They all developed the algorithm independently at approximately the same time (published in February 1957, September 1958, and January 1958 respectively).

For a radix-$2^k$ SRT divider, assuming $n$ is a multiple of $k$, an extra $\frac{n}{k}$-cell storage $Q'$ where each cell can store an integer in $[-2^{k-1},2^{k-1}]$ is used to store intermediate quotient in that form. 

In each $i$th iteration,
\ben
\item Shift ${R,Q}$ left by $k$ bits.
\item Use a LUT with a number of leading bits of $D$ and a number of leading bits of $R$ as inputs to determine an integer $q_i\in[-2^{k-1},2^{k-1}]$ such that $\abs{R-q_iD}<D$.
\item Assign $R-q_iD$ to $R$ and store $q_i$ to the $i$th cell of $Q'$.
\een
Iterate $\frac{n}{k}$ times.

Let a flag bit $F$ exist and be initially set to 0. If $R<0$, assign $R+D$ to $R$ and $F$ to 1.

Translate $Q'$ to final quotient $Q$:
\[Q=\sum_{i=1}^{\frac{n}{k}}q_i*2^{i-1}-F.\]
Now we've obtained the final result.

A fast $Q'$ storage is carry-save storage. It consists of an $n$-bit register $Q_s$, an $n+1$-bit register $Q_c$, and an $n$-bit carry-save adder. $Q_s,Q_c$ are set to zero initially. $q_i$ are represented in $k+1$ bit two's complement. Each $i$th iteration,
\ben
\item $Q_s,Q_c$ are both shifted left $k$ bits.
\item The carry-save adder takes $Q_s,Q_c,q_i$ as inputs and $Q_s,Q_c$ as sum and carry outputs.
\een
And the final quotient $Q$ is
\[Q=Q_s+Q_c-F.\]
\ssc{Arithmetic Logic Unit (ALU)}
An arithmetic logic unit (ALU) is a combinational digital circuit that performs operations on integer binary numbers. The common operations include:
\bit
\item Unsigned and two's complement arithmetic operations: addition, subtraction, increment, decrement, negation.
\item Bitwise logical operations: AND, OR, XOR.
\item Bit shift operations: Logical right shift (SRL) (0 as new MSB), logical left shift (SLL) (0 as new LSB), arithmetic right shift (SRA) (old MSB as new MSB), arithmetic left shift (SRL) (old LSB as new LSB), rotate right (old LSB as new MSB), rotate left (old MSB as new LSB).
\eit
Comparison operations are typically done by checking flags of subtraction.

Multiplication and division usually have their seperate units.
\ssc{State Machine Chart (SM Chart)}
State machine chart, SM chart, or algorithmic state machine (ASM) chart is a type of flowchart that is equivalent to a state graph. The componenets of an SM chart include:
\bit
\item State box: A rectangular box that contains a state name, and may contain an output list written after a slash after or the state name. A state code may be placed outside the box at the top.
\item Decision box: A diamond-shaped box that contains condition, a Boolean expression that is evaluated to true or false, and two branches, true and false.
\item Conditional output box: A curved-ended rectangular box that contains conditional output list, the outputs that depend on both the state and the inputs.
\eit
A state graph for a sequential FSM can be converted to and from an equivalent SM chart. The Moore outputs become output list in a state box. The Mealy inputs become conditional output box. Derivation and realization of SM charts are similar to those of state graphs.
\end{document}
