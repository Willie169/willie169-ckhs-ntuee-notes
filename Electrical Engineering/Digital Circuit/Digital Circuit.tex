...
\eit
\sssc{Flash memory}
Derived from EEPROM but optimized to be faster, denser, cheaper, and erasable in blocks or pages.
...
\item NAND Flash: Output lines are connected in series and cells are erased in pages and with sequential access for read only. Writing and erasing is faster and cell size is smaller. Solid-State Drive (SSD) and Secure Digital (SD) card are common storage device built using NAND flash.
\eit
Type of cells:
