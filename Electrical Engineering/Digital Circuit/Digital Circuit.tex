...
Parallel prefix adders are a class of adders consist of three stages, preprocessing, prefix network, and postprocessing. For an $n$-bit parallel prefix adder adding operands $A_0A_1\ldots B_{n-1}$ and $B_0B_1\ldots B_{n-1}$, preprocessing stage computes generate $G_i=A_i\cdot B_i$ and propagate $P_i=A_i\oplus B_i$ for each bit, and then a prefix operation $\circ$ is defined as
...
Kogge-Stone adder is a type of parallel prefix adder optimized for minimum logic depth and has fan-out of 2, but with larger area and wiring complexity. The prefix network of it consists of $\lceil\log_2(n)\rceil$ stages. For all $i\in[0,\lceil\log_2(n)\rceil-1]$, in the $i$th stage, for $j\in\bbZ\cap[i,n-1]$, the $j$ span is combined with the $(j-2^{i-1})$ span.
\sssc{Brent-Kung adder (BKA or BK)}
Brent-Kung adder is a type of parallel prefix adder with fan-out of 2, and smaller area and wiring complexity but longer logic depth than Kogge-Stone adder. The prefix network of it is split into reduction phase or up-sweep phase, with $\lceil\log_2(n)\rceil$ stages, and distribution phase or down-sweep phase, with $\lceil\log_2(n)\rceil-1$ stages, which are $2\lceil\log_2(n)\rceil-1$ stages in total. Suppose $n=2^k$. For all $i\in\bbZ\cap[1,k]$, in the $i$th stage of reduction phase, for all $j\in\bbZ\cap[1,2^{k-i}]$, the $j*2^i-1$ span is combined with the $j*2^i-2^{i-1}-1$ span. For all $i\in\bbZ\cap[1,k-1]$, in the $i$th stage of distribution phase, for all $j\in\bbZ\cap[1,2^i-1]$, the $j*2^{k-i}+2^{k-i-1}-1$ span is combined with $j*2^{k-i}-1$ span.
