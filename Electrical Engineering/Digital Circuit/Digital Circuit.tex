...
Kogge-Stone adder is a type of parallel prefix adder optimized for minimum logic depth, i.e., shortest combinational path, and has fan-out of 2, but with larger area and wiring complexity. The prefix network of it consists of $\lceil\log_2(n)\rceil$ stages. For all $i\in[0,\lceil\log_2(n)\rceil-1]$, in the $(i+1)$th stage, for $j\in\bbZ\cap[i+1,n-1]$, the $j$ span is combined with the $(j-2^i)$ span.
