...
\ssc{Multiplier}
\sssc{General}
Multiplication of two unsigned binary numbers $A=A_{n-1}A_{n-2}\ldots A_0$ and $B=B_{n-1}B_{n-2}\ldots B_0$ involves two steps, computing the $n$ partial products $P_0=A_{n-1}A_{n-2}\ldots A_0*B_0$, $P_1=A_{n-1}A_{n-2}\ldots A_0*B_10$, $\ldots$, $P_{n-1}=A_{n-1}A_{n-2}\ldots A_0*B_{n-1}0\ldots0$, where $P_i=0$ if $B_i=0$ and $P_i=A<<i$ if $B_i=1$ where $<<i$ means shifting $i$ bits towards MSB, and computing multi-operand addition $\sum_{i=0}^{n-1}P_i$ to get the final result.

In the bit heap of partial products (one partial product per row), each column (also called weight) $i$ (starting at 0) has $i+1$ bits for $i\in\bbZ\cap[0,n-1]$ and $2n-i-1$ bits for $i\in\bbZ\cap[n,2n-2]$, where the maximum height is $n$ at column $n-1$.
...
Sum partial products with carry-save adder until the operands left is two, and use an adder (such as a carry-lookahead adder or parallel prefix adder) to sum the two. The number of stage is $O(n)$.
\sssc{Wallace (tree/reduction) multiplier}
In each stage of the Wallace tree, for each column of bit heap independently, if the bits left $\ge 3$, a full adder is used to reduce 3 bits of the same column to one of the same column and one bit of the next column, until all columns left $\le 2$ bits. And then an adder (such as a carry-lookahead adder or parallel prefix adder) is used to sum up the final result. The number of stage is $O(\log n)$.
...
In each stage of the Dadda tree, choose the largest $k$ such that $d_k\le$ the height of the highest column, for each column of bit heap independently, if the bits left $\ge d_k$, a full adder is used to reduce 3 bits of the same column to one of the same column and one bit of the next column, until all columns left $\le 2$ bit. And then an adder (such as a carry-lookahead adder or parallel prefix adder) is used to sum up the final result. The number of stage is $O(\log n)$ and is sometimes slightly more than that of Wallace multiplier, and the number of full adders used is usually less than that of Wallace multiplier.
