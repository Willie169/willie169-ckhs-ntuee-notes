...
Brent-Kung adder is a type of parallel prefix adder with smaller area and wiring complexity than Kogge-Stone adder but longer logic depth. The prefix network of it is split into reduction phase or up-sweep phase, with $\lceil\log_2(n)\rceil$ stages, and distribution phase or down-sweep phase, with $\lceil\log_2(n)\rceil-1$ stages, which are $2\lceil\log_2(n)\rceil-1$ stages in total. Suppose $n=2^k$. For all $i\in\bbZ\cap[1,k]$, in the $i$th stage of reduction phase, for all $j\in\bbZ\cap[1,2^{k-i}]$, the $j*2^i-1$ span is combined with the $j*2^i-2^{i-1}-1$ span. For all $i\in\bbZ\cap[1,k-1]$, in the $i$th stage of distribution phase, for all $j\in\bbZ\cap[1,2^i-1]$, the $j*2^{k-i}+2^{k-i-1}-1$ span is combined with $j*2^{k-i}-1$ span.
PLACEHOLDER
\sssc{Decoders}
