...
\sssc{Resistor}
An ideal resistor is a circuit element with a resistance and no capacitance or inductance. Resistances in a network are called loads.
\begin{center}
\begin{circuitikz}
...
The resistance $R$, capacitance $C$, and inductance $L$ of a linear circuit element is assumed to be independent of voltage and current, that is, equivalent to a one-port network consists of ideal resistors, capacitors, and inductors. The assumption for resistance is the Ohm's law.
\sssc{Sinusoidal alternating current as phasor}
...
Here, all AC discussed are sinusoidal.
\sssc{Reactance}
...
\sssc{Sources}
Ideal sources are assumed to have zero impedance.
...
\eit
Linear dependent (aka controlled) sources:
\bit
...
A linear electrical network is an electrical network which obeys the superposition principle that the output voltage is a linear combination of the input voltages, that is, consists of only linear elements.
\sssc{Linear circuit}
A linear circuit is a circuit consisting of linear netwroks, independent sources, and linear dependent sources.
...
\item Addtional linear equations of VCVS and CCVS are addded to the linear system.
\een
\ssc{Mesh Analysis of Planar Linear Circuit under DC or Sinusoidal AC}
\sssc{Supermesh}
If a current source lies on two or more meshes, remove the edge with the current source and merge the meshes into one mesh.
\sssc{Mesh analysis}
\ben
\item Indentify all meshes and form supermeshes to exlude all current sourcs.
\item For each mesh, assign a mesh current variable for it with an arbitrary but consistent direction.
\item Write the linear equation of mesh current variables given by KVL for every node.
\item Addtional linear equations of current sources are addded to the linear system.
\item Addtional linear equations of dependent sources are addded to the linear system.
\een
\ssc{Transformations}
\sssc{Y-Δ transformation and Δ-Y transformation}
\bit
\item Y (wye or star) network: a common central node $N$, three terminals $N_1,N_2,N_3$, and three circuit elements with impedances $R_1,R_2,R_3$ connecting $N_1$ and $N$, $N_2$ and $N$, and $N_3$ and $N$ respectively.
\item Δ (delta or triangle) network: three terminals $N_1,N_2,N_3$, and three circuit elements with impedances $R_{12},R_{23},R_{31}$ connecting $N_1$ and $N_2$, $N_2$ and $N_3$, and $N_3$ and $N_1$ respectively.
...
\bit
\item $n$-star network: a common central node $N$, $n$ terminals $N_1,N_2,\ldots,N_n$, and $n$ circuit elements with impedances $R_i$ connecting $N_i$ and $N$ for all $i\in\bbN\land i\leq n$.
\item $n$-mesh (complete graph) network: $n$ terminals $N_1,N_2,\ldots,N_n$, and $\frac{n(n-1)}{2}$ circuit elements  with impedances $R_{ij}$ connecting $N_i$ and $N_j$ for all $i,j\in\bbN\land i<j\leq n$.
...
\bit
\item \tb{Source deactivation method}: Replace all indepedent voltage sources with short circuits and independent current sources with open circuits, and the impedance between the two terminals is the Thévenin impedance or Norton impedance.
