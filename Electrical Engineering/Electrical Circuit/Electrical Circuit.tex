...
Here, all currents discussed are sinusoidal AC or DC.
\sssc{Reactance}
...
\item A current-controlled current source enforces a DC or sinusoidal AC cuurent $i_s(t)=\beta i_x(t)$ through its terminals, where $i_x(t)$ is control current and real for DC or complex for AC constant $\beta$ is dimensionless current gain.
\eit
\begin{center}
\begin{circuitikz}
...
A linear circuit is a circuit which obeys the superposition principle that the voltages across all one-port subnetwork, called responses or response functions, are linear combinations of the input voltages from independent voltage sources and input currents from independent current sources in it, called forcing functions, that is, consists of only linear elements, independent sources, and linear dependent sources.
...
A linear network delivers maximum power to a load impedance when it is equal to the Thévenin impedance of the network.
...
\item Sum the currents and voltages obtained from the separate analyses above. Note that power quantities do not comply with superposition principle.
\een
\ssc{SPICE}
\sssc{SPICE}
