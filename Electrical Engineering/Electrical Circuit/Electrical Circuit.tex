...
A series RC voltage low-pass filter circuit consists of an independent voltage source $V_s$, a resistor with resistance $R$, and a capacitor with capacitance $C$ and voltage $V_C$ in series connection with current $I$.
\[V_C(s)=\frac{V_s(s)+RCv_C(0^-)}{RCs+1}\]
\[I(s)=\frac{CsV_s(s)-Cv_C(0^-)}{RCs+1}\]

\[V_s(s)=RI(s)+V_C(s)\]
\[I(s)=CsV_C(s)-Cv_C(0^-)\]
\[V_s(s)+RCv_C(0^-)=(RCs+1)V_C(s)\]
\[V_C(s)=\frac{V_s(s)+RCv_C(0^-)}{RCs+1}\]
\[I(s)=\frac{CsV_s(s)-Cv_C(0^-)}{RCs+1}\]
