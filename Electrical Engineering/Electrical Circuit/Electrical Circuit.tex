...
\eit
\sssc{Time constant}
For a response function in the form $f(t)=f_0e^{-\alpha t}$ with constants $f_0\neq 0$ and $\alpha>0$, the time constant $\tau$ is defined as $\frac{1}{\alpha}$, and $f(\tau)=e^{-1}$.

For the smallest positive integer (or rational number to a number of decimals) $k$ such that $\frac{f(k\tau)}{f_0}<0.01$, we typically call that $f(t)$ decays to $0$ about $k$ times time constants.
\sssc{Series connection}
...
\sssc{RL circuit}
An RL circuit consist of a resistor with resistance $R$, an inductor with inductance $L$, and an independent voltage source $v_{in}$ in series connection.

Natural response:
\[Ri+L\dv{i}{t}=0\]
\[i=i(0^-)e^{-\frac{R}{L}t}\]
\[\tau=\frac{L}{R}\]
\[p_R=(i(0^-))^2e^{-\frac{2R}{L}t}R\]
\[\int_0^{\infty}p_R\dd{t}=\frac{1}{2}L(i(0^-))^2\]
\[RI+L(sI-i(0^-))=0\]
\[I=\frac{Li(0^-)}{R+Ls}\]
\begin{proof}
\[Ri+L\dv{i}{t}=0\]
...
\[p_R=(i(0^-))^2e^{-\frac{2R}{L}t}R\]
\[\ba
\int_0^{\infty}p_R\dd{t}&=(i(0^-))^2R\evlv(-\frac{L}{2R}e^{-\frac{2R}{L}t})_0^{\infty}\\
&=\frac{1}{2}L(i(0^-))^2
\ea\]
\[RI+L(sI-i(0^-))=0\]
\[(R+Ls)I-Li(0^-)=0\]
