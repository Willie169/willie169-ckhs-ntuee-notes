...
to obtain its voltage $v(s)$ and current $i(s)$ as a function of frequency domain $s$. Below, if not otherwise specified, $v$ and $i$ refer to $v(s)$ and $i(s)$.
...
\een
\ssc{Nodal Analysis or Node-voltage Analysis of Linear Circuit}
\sssc{Supernode}
...
An input voltage $v_{in}$ is enforced from node 0 to node 1, a resistance $R$ connects node 1 and node 2, a capacitance $C$ connects node 2 and node 0, and output voltage $v_c$ and current $i_c$ is measured from node 0 to node 2.
\[v_{in}
