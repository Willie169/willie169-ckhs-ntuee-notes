...
Analysis of circuits by equations given by KCL for nodes is called nodal analysis or node-voltage analysis.
...
\sssc{Loop analysis}
Analysis of circuits by equations given by KVL for loops is called loop analysis or loop-current analysis. If the circuit is planar, it is also called mesh analysis.
...
\sssc{Linear circuit}
A linear circuit is a circuit consisting of linear netwroks and sources.
...
\ssc{Nodal Analysis of Linear Circuit under DC or Sinusoidal AC}
\sssc{Supernode}
...
\sssc{Matrix form}
The equations given by KCL of a linear circuit is a system of linear equation:
\[\mb{Y}\mb{v}=\mb{i},\]
where $\mb{v}$ is the column vector of nodes voltages relative to a common node, $\mb{i}$ is the vector of current injected into the nodes, where each entry $i_k$ is zero if node $k$ is not connected to a current source and $i_s$ if node $k$ is connected to a current source of current $i_s$ injected to the node (negative for absorption), and $\mb{Y}$ is the admittance (or conductance for DC) matrix of the linear circuit.
