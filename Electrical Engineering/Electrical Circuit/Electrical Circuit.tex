DC analysis of resistive circuit
Laplace
...
\sssc{Convention}
\bit
\item Voltage $v$ (volt = V)
\item Current $i$ (ampere = A)
\item Complex angular frequency $s$ (Hz = 1 / s)
...
A sinusoidal AC is an AC whose voltage $v(t)$ and current $i(t)$ over time can be written in the form
\[v(t)=v_m\cos(\omega t+\phi),\]
\[i(t)=i_m\cos(\omega t+\theta).\]
They can be represented in phasors as $v=v_m\angle\phi$ and $i=i_m\angle\theta$, which can be treated as complex numbers $v_me^{i\phi}$ and $i_me^{i\theta}$.
\sssc{General current in frequency domain}
Given a general current with voltage $v(t)$ and current $i(t)$ over time, we can perform Fourier transforms,
\[v(\omega)=\mathcal{F}\{v(t)\}(\omega)=\int_{-\infty}^{\infty}v(t)e^{-i\omega t}\dd{t},\]
\[i(\omega)=\mathcal{F}\{i(t)\}(\omega)=\int_{-\infty}^{\infty}i(t)e^{-i\omega t}\dd{t},\]
to obtain its voltage $v(\omega)$ and current $i(\omega)$ as a function of frequency domain $\omega$.
...
\[p(\omega)=\frac{\qty(v(\omega))^2}{R}=\qty(i(\omega))^2R.\]
...
A one-port network is said to have net capacitive reactance if $X(\omega)<0$ and have net inductive reactance if $X(\omega)>0$.
\sssc{Impedance}
...
\sssc{Admittance}
The admittance $Y(\omega)$ of a one-port network is defined as the reciprocal of its impedance:
\[Y(\omega)=\frac{1}{Z(\omega)}.\]
\sssc{Conductance}
The conductance $G(\omega)$ of a circuit element is defined as the real part of its admittance.
\sssc{Susceptance}
The susceptance $B(\omega)$ of a circuit element is defined as the imaginary part of its admittance:
\[Y(\omega)=G(\omega)+jB(\omega).\]
\sssc{Active element}
...
\sssc{Current}
Every current can be expressed as a linear combination of a DC component and an AC component, with the later as a complex function of frequency domain that can be obtained via Laplace transform, and thus by the superposition principle, we can analyze any current by analyzing its DC component, analyzing AC component with voltages, currents, and impedances as complex functions of frequency domain and then reverse Laplace transform to time domain, and summing the voltages and currents.
...
\ssc{RC, RL, LC, and RLC Circuits}
\sssc{Series RC voltage low-pass filter circuit}
An input AC with voltage $v_{in}$ is enforced from node 0 to node 1, a resistance $R$ connects node 1 and node 2, a capacitance $C$ connects node 2 and node 0, and output voltage $v_c$ and current $i_c$ is measured at node 0 to node 2.
\[v_{in}
