...
\item For each current source, if it lies on two or more meshes, exclude the edge with the current source and merge the meshes into one mesh, called a supermesh.
...
A series RC voltage low-pass filter circuit consists of an independent voltage source $v$, a resistance $R$, and a capacitance $C$ in series connection. Let the initial voltage of $C$ be $v_c$. The current through the circuit is:
\[v(t)=Ri(t)+\frac{1}{C}\qty(v_c+\int_0^ti(\tau)\dd{\tau}).\]
\[i(s)=\frac{1}{R+\frac{1}{sC}}\qty(?+v(s)).\]PLACEHOLDER
\begin{proof}
PLACEHOLDER
\end{proof}
If $v(t)$ is a constant $v_s$.
\[i(t)=\frac{v_s-v_c}{R}e^{-\frac{t}{RC}}.\]PLACEHOLDER
\begin{proof}
\[0=Ri'(t)+\frac{1}{C}i(t)\]
\[-\frac{1}{RC}\dd{t}=\frac{1}{i(t)}\dd{i}\]
\[-\frac{t}{RC}=\ln(i(t))+A\]
\[i(t)=Ae^{-\frac{t}{RC}}\]
\[v_s=RA+v_C\]
\[A=\frac{v_s-v_c}{R}\]
