...
A edge-triggered latch is a latch that responds to changes of data inputs at the rising or falling edge of a signal.

A level-triggered latch is a latch that responds to changes of data inputs when a signal is at a specific high or low level.
\ssss{Gated, clocked, or level-sensitive latch}
A gated, clocked, or level-sensitive latch is a level-triggered latch that has an extra input signal that is not a data input called enable or clock (clk) which controls whether the latch is transparent (enabled) or hold (disabled). When enable or clock signal is at active level (1 for active-high gated latch and 0 for active-low gated latch), it is transparent, meaning the it acts as an asynchronous latch of its type; when enable is at inactive level (0 for active-high latch and 1 for active-low latch), the latch is hold, meaning the output remains at its previous state regardless of input changes.
\sssc{Flip-flops}
The term flip-flop has historically referred generically to both level-triggered and edge-triggered latch. Many modern authors, however, reserve the term flip-flop exclusively for edge-triggered latch and use the term gated latch or simply latch for level-triggered latch. The terms "edge-triggered", and "level-triggered" may be used to avoid ambiguity.
...
$(S,R,E,Q)=(1,1,0,1)\lra (S,R,E,Q)=(1,1,1,1)$. When $E$ changes from 1 to 0, the output $Q$ may changes to 1 temporarily, but will be forced back to 0 since the inputs $S_{SR}$ and $R_{SR}$ of the underlying S-R latch are both 0, thus it is simply a glitch. When $E$ changes from 0 to 1, however,  a race condition occurs.
...
\[P=((RE)'(SE+Q(R'+E')))'=RE+(SE+Q(R'+E'))'=RE+(SE)'(Q(R'+E'))'=RE+(S'+E')(Q'+RE)=S'Q'+RE+E'Q.\]
\[Q^+'=(SE+Q(R'+E'))'=(SE)'(Q(R'+E'))'=(S'+E')(Q'+RE)=S'Q'+S'RE+E'Q=P.\]
By making $S=R=1$ don't care combination, we can simplify it as:
\[Q^+'=P=RE+Q'(S+E').\]
If we plot $Q^+=SE+Q(R'+E')$ on a K-map:
...
\end{longtable}
we can find a static-1 hazard in it, 
$(S,R,E,Q)=(1,1,0,1)\lra (S,R,E,Q)=(1,1,1,1)$. When $E$ changes from 1 to 0, the output $Q$ may changes to 1 temporarily, but will be forced back to 0 since the inputs $\ol{S}_{SR}$ and $\ol{R}_{SR}$ of the underlying active-low S-R latch are both 1, thus it is simply a glitch. When $E$ changes from 0 to 1, however,  a race condition occurs.
