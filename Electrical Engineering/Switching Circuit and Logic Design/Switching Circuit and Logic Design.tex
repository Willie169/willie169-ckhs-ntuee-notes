...
\sssc{Commonly used binary codes for decimal}
...
\item\tb{Gray Code}: Each step changes only one bit, which minimizes transition errors.
\eit
\sssc{Binary encoding}
Binary encoding is an encoding for a sequence of things of which the $i$th one is assigned with a binary number $i$.
\sssc{Excess-$n$ encoding}
Excess-$n$ encoding is an encoding for a sequence of things of which the $i$th one is assigned with a binary number $i+n$. 
\sssc{Gray-like encoding}
...
A one-hot (encoding) is an encoding for any number of things of which each one is assigned with a unique binary sequence with only one 1 bit.

A one-cold (encoding) is an encoding for any number of things of which each one is assigned with a unique binary sequence with only one 0 bit.
...
\sssc{Flip-flop setup time and hold time}
...
An edge-triggered flip-flop has the same inputs and outputs of the gated latch of the same type but the enable input $E$ is replaced with clock input Clk. For rising-edge-triggered (aka active-high) flip-flops, when clock is at a rising edge, called active (clock) edge, the flip-flop is transparent; otherwise the flip-flop is hold (aka retain). For falling-edge-triggered (aka active-low) flip-flops, when clock is at a falling edge, called active (clock) edge, the flip-flop is transparent; otherwise the flip-flop is hold (aka retain).
...
\sssc{(Edge-triggered) S-R flip-flop (JKFF)}
For active-high S-R flip-flops, $(S,R)=(1,1)$ is not allowed. For active-low S-R flip-flops, $(\ol{S},\ol{R})=(0,0)$ is not allowed.
\sssc{(Edge-triggered) D flip-flop (DFF)}
When the context is clear, flip-flops (FFs) may be used to refer to D flip-flops (DFFs).
\sssc{(Edge-triggered) J-K flip-flop (JKFF)}
...
\sssc{(Edge-triggered) T flip-flop (TFF)}
...
Let the inputs and outputs of the master be with subscript $_1$ and those of the slave be with subscript $_2$. In an active-high or active-low master-slave S-R flip-flop, $Q_1$ and $Q_1'$ are connected to $S_2$ and $R_2$ respectively, and $Q_2$ and $Q_2'$ are the final outputs of it.
...
A flip-flop with clock enable (CE) input has an additional input CE and works as a normal flip-flop if CE=1, called enabled, and hold if CE=0, called disabled.
...
\ssc{Analysis of Synchronous Circuits}
...
Note that the glitches are not considered and typically cannot be detected when analyzing as automata because the inputs are read at the active clock edge, and no provision is made for extra input changes between active edges.
...
Because the output of a Moore circuit can change state only when the flip-flops change state and not when the input changes, no false outputs can appear in a Moore circuit.
\sssc{State table and transition table}
A transition table is a one-dimensional table with three (for Moore circuit) or four (for Mealy circuit) columns that lists the corresponding next states and outputs (for Mealy circuit) in the third and fourth columns for all allowed combinations of input character in the first column and present state in the second column in rows, resembling a truth table, in which the states are represented by binary or other radix numbers corresponding to the binary sequence of the state variables.
...
\sssc{State diagram or state graph for Moore circuit}
A state diagram (also called a state graph) for a Moore circuit is a directed graph with
\bit
\item Each vertex being a state, normally represented by circles with symbols of the states in the top half and output at the state in the bottom half, i.e., [state]/[output character].
\item Each directed edge being a transition, normally drawn as an arrow from present state to next state with the input character read for such transition to happen on the edge.
...
\eit
\sssc{State diagram or state graph for Mealy circuit}
A state diagram (also called a state graph) for a Mealy circuit is a directed graph with
\bit
\item Each vertex being a state, normally represented by circles with symbols of the states.
\item Each directed edge being a transition, normally drawn as an arrow from present state to next state with the input character read for such transition to happen and output character of the transition on the edge separated by /, i.e., [input character]/[output character].
\item The starting state is usually represented by an arrow with no origin pointing to the state, a subscript 0 in the symbol of the state (often $s_0$ or $S_0$), or not shown.
\eit
\sssc{Next-state Map}
A next-state map is the K-map of a next-state variable as a function of the present-state variables and for Mealy circuit also input variables.
\sssc{T input map}
Given a next-state map of a T flip-flop, to find the input needed for each present state variables and input variables combination, convert the value $Q^+_k$ in it to $T_k$ given by $T_k=Q_k\oplus Q^+_k$ for every 0 and 1 cell to get T input maps, equivalently, complement every 0 and 1 cell in the $Q_k=1$ half.
\sssc{D input map}
Given a next-state map of a D flip-flop, to find the input needed for each present state variables and input variables combination, convert the value $Q^+_k$ in it to $D_k$ given by $D_k=Q^+_k$ for every 0 and 1 cell to get D input maps, equivalently, do nothing.
...
1 & 1 & X & 0 \\\hline
\end{longtable}
for every 0 and 1 cell to get S and R input maps, equivalently, for S input maps, replace 1's in $Q_k=1$ half with X's, for R input maps, replace 0's in $Q_k=0$ half with X's and then complement all 0 and 1 cells.
...
1 & 1 & X & 0 \\\hline
\end{longtable}
for every 0 and 1 cell to get J and K input maps, equivalently, for J input maps, replace all cells in $Q_k=1$ half with X's, for K input maps, replace all cells in $Q_k=0$ half with X's and then complement all cells in $Q_k=1$ half.
...
For any flip-flop in the circuit with output propagation delay $t_p$, combinational circuit propagation delay to the flip-flop input $t_c$, the maximum time $t_x$ the inputs $X_i$ that the flip-flop input is dependent on need to be stable before the next active clock edge, the flip-flop setup time $t_s$, and clock skew $t_{sk}$ between the flip-flops that receive the clock signal first and the flip-flop, the clock period $t_{clk}$ must satisfy
...
\sssc{Synchronous Binary Up Counter}
...
\sssc{Synchronous Up/Down (U/D) Counter}
...
\een
\ssc{Window Sequence Detector of Single Word}
\sssc{Sliding and Disjoint Windows Sequence Detector}
A window sequence detector is a Mealy circuit with a serial input $X\in\{0,1\}$ is read and an output $Z$ that when the string of the last $n$ characters in the input word read is a prescribed string $w$, it outputs 1; otherwise, it outputs 0. Let $w=a_1a_2\ldots a_n$, $w_i=a_1a_2\ldots a_i$, and $w_0=\varepsilon$ (empty string).
...
To prevent $Z=1$ to occur before the $(n-m)$th input character is read where $m=0$ for those without initial state and the length of initial state $v$ for those with initial state $v$, we can AND the original output $Z$ together with the output $C$ of a combinational subcircuit that outputs 1 if a counter with $n$ states is at the $n$th state and 0 otherwise as the new output $Z$, with the clock enable signal of the counter connected to $C'$, and the original state of the counter being the $(m+1)$th state.

For non-overlapping sliding window sequence detector, connect a clear signal for all flip-flops to $Z$.

For disjoint window sequence detector, we can AND the original output $Z$ together with the output $C$ of a combinational subcircuit that outputs 1 if a counter with $n$ states with parallel input is at the $n$th state and 0 otherwise as the new output $Z$, with the clock enable signal of the counter connected to $C'$ and the parallel input connected to a constant source of first state of the counter read iff $Z=1$.
\sssc{Design as finite-state machine (FSM) based on Knuth–Morris–Pratt (KMP) algorithm}
\ben
\item Find the number of states and flip-flops needed:
\bit
\item \textbf{Moore circuit implementation:} We need $n+1$ states $S_0,S_1,\ldots,S_n$, each $S_i$ corresponding to the scenario that the string of the last $i$ input characters are the string of the first $i$ characters of $w$. Thus, we need at least $\lceil\log_2(n+1)\rceil$ flip-flops. Let the number of flip-flops be $k$.
\item \textbf{Mealy circuit implementation:} We need $n$ states $S_0,S_1,\ldots,S_{n-1}$, each $S_i$ corresponding to the scenario that the string of the last $i$ input characters are the string of the first $i$ characters of $w$. Thus, we need at least $\lceil\log_2(n)\rceil$ flip-flops. Let the number of flip-flops be $k$.
\eit
\item Assign a $k$-bit binary number corresponding to the flip-flops to each state. Binary encoding, one-hot encoding, one-code encoding, and gray-like encoding are often used. One-hot encoding or one-code encoding is typically used for FGPA.
\item Compose output:
\bit
\item\textbf{Moore circuit implementation:} Use a combinational circuit whose inputs are the outputs of the flip-flops that outputs 1 if the present state is $S_n$ and 0 otherwise.
\item\textbf{Mealy circuit implementation:} Use a combinational circuit whose inputs are the outputs of the flip-flops and $X$ that outputs 1 if the present state is $S_{n-1}$ AND $X=a_n$ and 0 otherwise.
\eit
\item For any $0\leq i\leq n-1$, define $m_i$ such that the string of the last $m_i$ characters of $w_ia_{i+1}'$ equals $w_{m_i}$ and that for any $j>m_i$, the string of the last $j$ characters of $w_ia_{i+1}'$ does not equal $w_j$. (Thus $m_0=0$.)
\item Compute transition after match:
\bit
\item \textbf{Overlapping sliding window sequence detector or disjoint window sequence detector and Moore circuit implementation}: Define $m_{n1}$ such that the string of the last $m_{n1}$ characters of $w1$ equals $w_{m_{n1}}$ and that for any $j>m_{n1}$, the string of the last $j$ characters of $w1$ does not equal $w_j$. Define $m_{n0}$ such that the string of the last $m_{n0}$ characters of $w0$ equals $w_{m_{n0}}$ and that for any $j>m_{n0}$, the string of the last $j$ characters of $w0$ does not equal $w_j$. Define $b_{n1}$ as 1 if $m_{n1}=n$ and 0 otherwise. Define $b_{n0}$ as 1 if $m_{n0}=n$ and 0 otherwise.
\item \tb{Non-overlapping sliding window sequence detector and Moore circuit implementation}: Define $m_{n1}=m_{n0}=b_{n1}=b_{n0}=0$.
\item \textbf{Mealy circuit implementation}: Define $m_n$ such that the string of the last $m_n$ characters of $w$ equals $w_{m_n}$ and that for any $j>m_n$, the string of the last $j$ characters of $w$ does not equal $w_j$.
\eit
...
\[S_i/0\xrightarrow{a_{i+1}'}S_{m_i}/0,\quad 0\leq i\leq n-1,\]
\[S_n/1\xrightarrow{1}S_{m_{n1}}/b_{n1},\]
\[S_n/1\xrightarrow{0}S_{m_{n0}}/b_{n0}.\]
\item\textbf{Mealy circuit implementation:}
\[S_i\xrightarrow{a_{i+1}/0}S_{i+1},\quad 0\leq i\leq n-2,\]
\[S_{n-1}\xrightarrow{a_n/1}S_{m_n},\]
\[S_i\xrightarrow{a_{i+1}'/0}S_{m_i},\quad 0\leq i\leq n-1.\]
\eit
