...
\een
\sssc{As automata}
A synchronous digital circuit is a deterministic finite-state machine (DFSM) that transitions every active edge, with input alphabet being $\{0,1\}^m$ where $m$ is the number of input wires (thus it recognize a regular language over $\{0,1\}^m$), states being sequences of 0's and 1's of which the length is the number of flip-flops, next-state function given by the next-state equations of the flip-flops and the combinational logic of the flip-flop inputs, and the accepted final states being all states and thus not concerned in the context of digital circuit.

A synchronous digital circuit with output is a deterministic finite-state transducer (DFST) and a Mealy machine as the accepted final states is not concerned and thus called a Mealy circuit, with the output alphabet being $\{0,1\}^n$ where $n$ is the number of output wires, and next-output function given by the next-state equations of the flip-flops and the combinational logic of the circuit outputs.

When the combinational logic of the circuit outputs depends only on the state of the circuit and independent from the circuit inputs, the circuit is a Moore machine, called a Moore circuit.
...
\sssc{State diagram}
Same as Moore and Mealy machines, state diagrams are commonly used to analyze Moore and Mealy circuits, with the states typically represented by meaningless symbol and a starting state usually represented by an arrow with no origin pointing to the state or not shown.
\sssc{Next-state Map}
...
\item Combine these maps to form the transition table.
\item If time chart (with glitches) is wanted, first
\een
\end{document}
