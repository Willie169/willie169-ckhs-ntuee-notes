...
A serial-in serial-out (SISO) shift register is a type of shift register that allows data to be shifted in and out one bit at a time. It consists of a series of edge-triggered D flip-flops connected in a chain, where the output $Q$ of one flip-flop is connected to the input $D$ of the next flip-flop. The first flip-flop in the chain receives the serial input (SI), and the last flip-flop in the chain provides the serial output (SO). The flip-flops share common clock input and clock enable/load input called shift. When shift=1 at an active edge, the data in each flip-flop is shifted to the next flip-flop in the chain, a new bit of data is loaded into the first flip-flop, and the bit in the last flip-flop is output as the serial output; otherwise, the register holds existing data.
...
\item The output of the previous flip-flop in the chain, with the first flip-flop taking serial input SI, selected when $(Sh,L)=(1,0)$ or $(1,1)$, called shift.
\eit
