...
A synchronous circuit is a finite-state machine (FSM) with the accepted final state typically being all states and also a transducer for circuits with outputs, where the next-state function is specified by the next-state equations of the flip-flops of the circuit, and thus can be analyzed with state-transition table, state diagram, etc.
