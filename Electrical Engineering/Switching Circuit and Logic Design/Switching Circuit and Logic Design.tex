...
When discussing latches and flip-flops, we the term present state $Q=Q(t)$ to denote the state of the $Q$ output of the latch or flip-flop at the time input signal changes, and the term next state $Q^+=Q(t+\epsilon)$ to denote the state of the $Q$ output after the latch or flip-flop has reacted to the input change and stabilized. The equation with the next state in the LHS and a function of present state and inputs in the RHS is called the next-state equation or characteristic equation of the sequential circuit.
\sssc{Flow table}
A flow table of a sequential circuit is a truth table with state variables (labeled with superscript $^+$) and sometimes also output values in next state as output variables and input variables and state variables in present state as input variables.
...
It is hold when $S=R=0$, set when $S=R'=1$, and reset when $S'=R=1$. It has restriction that $SR=0$ because $Q$ and $Q'$ will oscillate when $S=R=1$.
...
By making $S=R=1$ don't care combination, we can simplify them as:
\[Q^+=S+R'Q.\]
\[Q^+'=P=R+S'Q'.\]

The next state $Q^+$ will be 1 either if it is set to 1 with an $S=1$ input, or if the present state is 1 and the latch is not reset with an $R=1$ input, and be 0 otherwise.
...
It is hold when $\ol{S}=\ol{R}=1$, set when $\ol{S}=\ol{R}'=0$, and reset when $\ol{S}'=\ol{R}=0$. It has restriction that $\ol{S}+\ol{R}=1$ because $Q$ and $Q'$ will oscillate when $\ol{S}=\ol{R}=0$.
...
By making $\ol{S}=\ol{R}=0$ don't care combination, we can simplify it as:
\[Q^+'=P=\ol{R}'+\ol{S}Q'.\]

The next state $Q^+$ will be 1 either if it is set to 1 with an $\ol{S}=0$ input, or if the present state is 1 and the latch is not reset with an $\ol{R}=0$ input, and be 0 otherwise.
...
It is hold when $E=0$ or $S=R=0$, set when $E=S=R'=1$, and reset when $E=S'=R=1$. It has restriction that $SR=0$ because $Q$ and $Q'$ will oscillate when $S=R=E=1$ and a race condition where both inputs to the S-R latch changes from 0 to 1 and the propagation delays of the gates determine whether the latch stabilizes with $Q$ changed or being original state occurs when $E$ changes from 1 to 0 while $S=R=1$.
...
It is hold when $E=0$ or $S=R=0$, set when $E=S=R'=1$, and reset when $E=S'=R=1$. It has restriction that $SR=0$ because $Q$ and $Q'$ will oscillate when $S=R=E=1$ and a race condition where both inputs to the S-R latch changes from 1 to 0 and the propagation delays of the gates determine whether the latch stabilizes with $Q$ changed or being original state occurs when $E$ changes from 1 to 0 while $S=R=1$.
...
When $J=K=1$, the outputs $(Q,Q')$ become $(1,0)$ if originally $(0,1)$ and $(0,1)$ if originally $(1,0)$, called toggle. However, if $J=K=1$ stays longer than the propagation delay, the outputs toggle again, called the race-around problem.
...
It is hold when $E=0$ or $J=K=0$, set when $E=J=K'=1$, reset when $E=J'=K=1$, and toggle with race-around problem when $E=J=K=1$.
...
In the symbol of a DFF, sometimes output $Q'$ ($Q_2'$) is omitted if not used.
...
inputs are loaded into the register; otherwise, the register holds existing data.
...
\sssc{Shift Register}
A shift register is a sequential circuit composed of edge-triggered flip-flops that stores data and shifts by one bit when certain control inputs are at a certain combination at an active edge, optionally allowing serial or parallel input and output.
\bit
...
A serial-in serial-out (SISO) shift register is a type of shift register that allows data to be shifted in and out one bit at a time. An $n$-bit serial-in serial-out (SISO) shift register consists of a series of $n$ edge-triggered D flip-flops connected in a chain, where the output $Q$ of one flip-flop is connected to the input $D$ of the next flip-flop with the first flip-flop in the chain receiving the serial input (SI) and the last flip-flop in the chain providing the serial output (SO) instead. The flip-flops share common clock input and clock enable/load input called shift. When shift=1 at an active edge, the register shifts a bit; otherwise, the register holds.

If we connect the serial output to the serial input, the resulting cyclic shift register performs an end-around shift.
\sssc{Parallel-In Parallel-Out (PIPO) Shift Register}
A parallel-in parallel-out (PIPO) shift register is a type of shift register that allows data to be loaded and retrieved in parallel. An $n$-bit parallel-in parallel-out (PIPO) shift register can be implemented using $n$ edge-triggered D flip-flops each of which associated with one 4-to-1 multiplexer. The data input $D$ of each flip-flop is connected to the output of the associated multiplexer. The two select inputs of each multiplexer are connected to a common 2-bit select input called shift enable $Sh$ and load enable $L$. The four data inputs of each multiplexer are:
\bit
...
\item The output of the previous flip-flop in the chainwith the first flip-flop in the chain receiving the serial input (SI) instead, selected when $(Sh,L)=(1,0)$ or $(1,1)$, called shift.
\eit
The flip-flops share common clock input. The outputs of the flip-flops provide the parallel output (PO).

When $(Sh,L)=(1,0)$ or $(1,1)$ at an active edge, the register shifts a bit; when $(Sh,L)=(0,1)$ at an active edge, the register loads parallel input; otherwise, the register holds.
