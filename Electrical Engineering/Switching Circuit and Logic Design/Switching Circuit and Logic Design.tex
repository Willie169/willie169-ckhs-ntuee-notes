...
Sometimes two (or more) outputs are drawn on a gate with some of them (typically one of the two) with inversion bubble. This is logically equivalent to drawing one gate of that type for each of the noninverted or inverted outputs with the same input lines.
\ssc{Theorems}
...
\[D_0=A_0\oplus B_0,\]
\[D_{out_0}=A_'0B_0.\]
A logical equivalence of a parallel subtractor is a parallel adder with inversion bubbles at all $B_i$ inputs and $C_{in_0}$ being constant $1$ input.
...
A PLA with $n$ inputs, $m$ product terms, and $k$ outputs can implement $k$ functions of $n$ variables with at most $m$ product terms.
