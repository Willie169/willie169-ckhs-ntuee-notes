...
A synchronous circuit is a finite-state machine (FSM) with all alphabet being $\{0,1\}$, states being sequences of 0's and 1's, next-state function specified by the next-state equations of the flip-flops and the combinational logic of the flip-flop inputs, and the accepted final state typically being all states, and also a transducer for circuits with outputs, with the next-output function specified by the next-state equations of the flip-flops and the combinational logic of the circuit outputs.

A circuit that is a Moore machine is called a Moore circuit. A circuit that is a Mealy machine is called a Mealy circuit.

State-transition tables (typical one-dimensional, resembling truth table) and state diagrams are commonly used to analyze synchronous circuits.
