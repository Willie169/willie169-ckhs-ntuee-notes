...
\[\sum_{i=0}^{n-1}D_i\cdot m_i(S_0,S_1,\ldots S_{n-1}),\]
...
\[\prod_{i=0}^{n-1}D_i+M_i(S_0,S_1,\ldots S_{n-1}),\]
...
A MUX with active high (aka active-high) outputs outputs the selected input directly, while a MUX with active low (aka active-low) outputs outputs the inverse of the selected input. A MUX can also has an additional input $E$ called enable (signal) and works as discussed above when $E=1$ and outputs $0$ when $E=0$.
...
Each output line of a decoder corresonds to a minterm of the inputs. Thus, we can ORing them together to get the active-high outputs or NANDing them together to get the active-low outputs.
\sssc{Encoders}
An encoder is a combinational circuit that realize the inverse of a Boolean vector function realized by a decoder. A $2^n$-to-$n$ priority encoder realizes the inverse of a Boolean vector function $\{0,1\}^n\to\{0,1\}^{2^n}$ realized by a decoder. (Note that the domain of the inverse is a subset of $\{0,1\}^{2^n}$ that has $n$ elements.) 
