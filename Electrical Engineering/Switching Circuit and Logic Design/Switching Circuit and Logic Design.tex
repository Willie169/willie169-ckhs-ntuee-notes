...
\sssc{Master-slave D flip-flop (DFF or FF)}
...
\sssc{Parallel-In Parallel-Out (PIPO) Register}
A parallel-in parallel-out (PIPO) register is a register that can load data inputs in parallel and output stored data in parallel. A $n$ bit PIPO register consists of $n$ edge-triggered D flip-flops, each with $D$ being data input and $Q$ being data output. The flip-flops usually share common clock input and asynchronous clear and/or preset inputs. If using gated clocks, the clock is gated with a common signal called load; if using clock enable, the flip-flops share a common clock enable input called load. When load=1 at an active edge, the data inputs are loaded into the register; when load=0 at an active edge, the register holds existing data.

The symbol of a PIPO register in block diagrams is one underlying flip-flop symbol with bus lines for data inputs and outputs, and load is labeled as Load at the clock enable input or the gated clock input.
...
\sssc{Parallel-In Serial-Out (PISO) Register}
