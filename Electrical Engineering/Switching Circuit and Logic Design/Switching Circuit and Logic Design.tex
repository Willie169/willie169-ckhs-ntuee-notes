...
The response time or delay time $\epsilon$ of an asynchoronous or level-triggered latch is the time it needs for its state to react to a input change. If an impulse of input lasts less than $\epsilon$, the latch or flip-flop will not react to it.
...
\sssc{Transition table and state table}
A transition table is a one-dimensional state-transition table with the states represented by real binary sequences, resembling a truth table. A state table is a transition table but with the states represented by meaningless symbol.
\sssc{State diagram or state graph}
Same as Moore and Mealy machines, state diagrams (also called state graphs) are commonly used to analyze Moore and Mealy circuits, with the states typically represented by meaningless symbol and a starting state usually represented by an arrow with no origin pointing to the state, a subscript 0 in the symbol of the state (often $s_0$ or $S_0$), or not shown.
\sssc{Next-state Map}
...
\sssc{General model for synchoronous circuits without outputs}
...
\item For each 0 and 1 cell in each map, convert the value $Q^+_k$ in it to $T_k$ given by $T_k=Q_k\oplus Q^+_k$. These maps are called T input (K-)maps and are the same as the next-state maps with $Q_k=1$ half complemented.
...
\ben
\item For each $Q^+_k$, draw a next-state map.
\item For each 0 and 1 cell in each map, convert the value $Q^+_k$ in it to $D_k$ given by $D_k=Q^+_k$. These maps are called D input (K-)maps and are exactly the same as the next-state maps.
...
\ben
\item For each $Q^+_k$, draw a next-state map.
\item For each 0 and 1 cell in each map, convert the value $Q^+_k$ in it to $S_k$ and $R_k$ respectively given by the truth table below where $X$ denotes don't care:
...
\ben
\item For each $Q^+_k$, draw a next-state map.
\item For each 0 and 1 cell in each map, convert the value $Q^+_k$ in it to $J_k$ and $K_k$ respectively given by the truth table below where $X$ denotes don't care:
\begin{longtable}[c]{mm|mm}
\hline
...
\een
\ssc{Sqeuence Detector}
\sssc{Definition}
A sequence detector is a Mealy circuit with a serial input $X\in\{0,1\}$ is read and an output $Z$ that when the sequence of the last $n$ characters in the input word read is a prescribed string $w$, it outputs 1; otherwise, it outputs 0. Let $w=a_1a_2\ldots a_n$, $w_i=a_1a_2\ldots a_i$, and $w_0=\varepsilon$ (empty string).
\sssc{Design with SIPO shift register}
Use an $n$-bit SIPO shift register to read $X$ and a combinational circuit with inputs being the parallel outputs of the shift register and 1 output that is 1 if the shift register output is $w$ and 0 otherwise.
\sssc{Design as finite-state machine (FSM)}
We need $n$ states $S_0,S_1,\ldots,S_{n-1}$, each $S_i$ corresponding to the scenario that the string of the last $i$ input characters are the string of the first $i$ characters of $w$. The circuit is a deterministic finite-state machine with starting state is $S_0$ and the set of accepted (final) states at and only at which $Z=1$ is $S_{n-1}$.
\ben
\item For any $1\leq i\leq n-1$, define $m_i$ such that the string of the last $m_i$ characters of $w_{i-1}a_i'$ equals $w_{m_i}$ and that for any $j>m_i$, the string of the last $j$ characters of $w_{i-1}a_i'$ does not equal $w_j$. (Thus $m_0=0$.)

Define $m_{n1}$ such that the string of the last $m_{n1}$ characters of $w1$ equals $w_{m_{n1}}$ and that for any $j>m_{n1}$, the string of the last $j$ characters of $w1$ does not equal $w_j$. Define $m_{n0}$ such that the string of the last $m_{n0}$ characters of $w0$ equals $w_{m_{n0}}$ and that for any $j>m_{n0}$, the string of the last $j$ characters of $w0$ does not equal $w_j$.

Define $b_{n1}$ as 1 if $m_{n1}=n$ and 0 otherwise. Define $b_{n0}$ as 1 if $m_{n0}=n$ and 0 otherwise.
\item Draw a state graph where
\[S_{i-1}\xrightarrow{a_i/0}S_i,\quad 1\leq i\leq n-1,\]
\[S_{i-1}\xrightarrow{a_i'/0}S_{m_i},\quad 1\leq i\leq n-1,\]
\[S_{n-1}\xrightarrow{1/b_{n1}}S_{m_{n1}},\]
\[S_{n-1}\xrightarrow{0/b_{n0}}S_{m_{n0}}.\]
\item 
