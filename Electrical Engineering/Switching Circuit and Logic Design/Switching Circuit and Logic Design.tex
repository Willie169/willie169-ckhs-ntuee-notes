...
A $2^n$-word$\times m$-bit ROM realizes the truth table by a $n$-to-$2^n$ decoder and a grid consists of $2^n$ rows (decoder outputs) intersecting each of the $m$ columns (group to ROM outputs, outputs 0 if no 1 signal from the input lines is inputted), where in each of the $2^n\times m$ row-column intersection points, each corresponding to one bit in the memory array, a switching element is either present and connected or not. If absent or not connected (logic 0), the signal value from the row isn't passed to the column; if present and connected (logic 1), represented by a $\times$ on the diagram, the signal value from the row is passed to the column.
\sssc{Types}
...
An FPGA is an IC that consists of an array of identical logic cells, also called configurable logic blocks (CLBs), with programmable interconnections and surrounded by I/O blocks that connect the CLB signals to IC pins. The user can program the functions realized by each logic cell and the connections between the cells.

The function generators are implemented as lookup tables (LUTs), which a $n$-input LUT is essentially a reprogrammable $2^n$-word-$\times$-1-bit ROM that stores the truth table for the function being generated.
