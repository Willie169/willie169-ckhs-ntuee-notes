...
\ssc{Bus}
Several logic signals that perform a common function may be grouped together to form a bus, represented with a single heavy line optionally with a diagonal slash through it specifying the number of bits in the bus.
...
A MUX with active high (aka active-high) outputs outputs the selected input directly, while a MUX with active low (aka active-low) outputs outputs the inverse of the selected input. A MUX can also has an additional input called an enable $E$ where it works as discussed above when $E=1$ and outputs $0$ when $E=0$.
