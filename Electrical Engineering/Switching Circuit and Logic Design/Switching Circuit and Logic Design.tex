...
\sssc{Latches}
Latches are level-triggered data storage elements that store a single bit of data; one of its two states represents a "one" and the other represents a "zero". "Level-triggered" means it responds to the level (value) change of a control signal called Enable and becomes transparent when the Enable is HIGH (or sometimes LOW), called enabled.
\sssc{Flip-flops}
Flip-flops are edge-triggered data storage elements that store a single bit of data; one of its two states represents a "one" and the other represents a "zero". "Edge-triggered" means it only responds at a clock edge.
\sssc{Time}
