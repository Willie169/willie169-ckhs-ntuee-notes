...
\sssc{Clock signal}
A clock signal is an periodic logic signal (square wave)  which oscillates between a high (1) and a low (0) state at a constant frequency. A duty cycle is half a full cycle (a period), that is, how long the signal stays high or low. A Edge	The moment the signal transitions (↑ rising, ↓ falling)


Example:

If frequency = 1 MHz → period = 1 µs
That means one full clock wave repeats every microsecond.


Rising edge (↑): when clock goes from 0 → 1

Falling edge (↓): when clock goes from 1 → 0


⏱ Example:

D flip-flop samples data at the rising edge (most common).

Some designs use falling-edge flip-flops for phase balancing.
...
Clocks are generated by oscillators, which can be:

Crystal oscillator (very stable, used in CPUs, microcontrollers)

RC oscillator (simpler, less precise)

External clock generator (in test systems or FPGAs)


In digital schematics, the clock is often labeled CLK.
...
Suppose we have a D flip-flop:

Time	D	Clock	Q (output)
...
Each rising edge updates the output to the current value of D.
Between edges, Q stays constant even if D changes.
...
Clock	Periodic signal that coordinates circuit timing
Used by	Flip-flops, registers, counters, CPUs
Shape	Square wave (0 ↔ 1 repeatedly)
Purpose	Ensure all parts change state in sync
Edges	Rising (0→1) and Falling (1→0)



---

Would you like me to draw a simple timing diagram showing how the clock, D input, and Q output relate in a D flip-flop? It’s much easier to visualize that way.

\sssc{Introduction}
Flip-flops and latches are used as data storage elements to store a single bit of data; one of its two states represents a "one" and the other represents a "zero". Flip-flops are edge-triggered storage elements and latches are level-triggered ones.

A synchronous system is a digital circuit where all state changes are coordinated by a clock.
asynchronous systems, where elements change whenever inputs change
