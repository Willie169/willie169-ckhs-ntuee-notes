...
\item The starting state is usually represented by an arrow with no origin pointing to the state, a subscript 0 in the symbol of the state (often $s_0$ or $S_0$), or not shown.
\eit
\sssc{Incompletely specified state table}
A cell in a state table or transition may be don't care since such transition would never occur and is represented with - and treated the same as that in a truth table. Such a state table is called an incompletely specified state table.
...
Two states $p$ and $q$ of a Mealy machine are equivalent iff for any input $X$,
\[\delta(p,X)\equiv\delta(q,X),\quad\lambda(p,X)=\lambda(q,X).\]

Two states $p$ and $q$ of a Moore machine are equivalent iff for any input $X$,
\[\delta(p,X)\equiv\delta(q,X),\quad\lambda(p)=\lambda(q).\]

If two equivalent states exist in a state table, we can replace any occurrence of one of them by the other to eliminate a redundant state.

A deterministic finite-state transducer is minimized if no equivalent states exist.
\sssc{Finding equivalent states with implication chart}
Let the set of all states be $S=\{S_0,S_1,\ldots,S_n\}$ and the input alphabet be $\Sigma=\{X_0,X_1,\ldots,X_m\}$.
\ben
\item The implication chart, also called implication table, is a lower-triangle table with entries $a_{ij}$ for all integers $i,j\in\bbN_0\land n\geq i>j$.
...
\item For each entry $a_{ij}$ that has not been filled with $\times$, $S_j\equiv S_i$.
\een
\sssc{Equivalent deterministic finite-state transducers}
Two deterministic finite-state transducers $M_1,M_2$ with same input alphabets $\Sigma$ and output alphabets $\Gamma$, each with set of all states $S,T$ with $S\cap T=\varnothing$, transition functions $\delta_1,\delta_2$, and output function $\lambda_1,\lambda_2$, are equivalent, iff, in the combined deterministic finite-state transducer with set of states $S\cup T$, input alphabet $\Sigma$, output alphabet $\Gamma$, transition function
\[\delta(x)=\bcs\delta_1(x),\quad&x\in D_{\delta_1}\\\delta_2(x),\quad&x\in D_{\delta_2}\ecs,\]
and output function
\[\lambda(x)=\bcs\lambda_1(x),\quad&x\in D_{\lambda_1}\\\lambda_2(x),\quad&x\in D_{\lambda_2}\ecs,\]
\[\qty(\forall s\in S\colon\exists t\in T\,\text{s.t.}\,s\equiv t)\land\qty(\forall t\in T\colon\exists s\in S\,\text{s.t.}\,s\equiv t).\]
\sssc{Finding equivalent deterministic finite-state transducers with implication chart}
First minimize both deterministic finite-state transducers respectively. Let the set of all remaining states of the two deterministic finite-state transducers be $S$ and $T$. If $|S|\neq |T|$, the two deterministic finite-state transducers are not equivalent. Let $S=\{S_0,S_1,\ldots,S_n\}$ and $T=\{T_0,T_1,\ldots,T_n\}$, and the input alphabet of both deterministic finite-state transducers be $\Sigma=\{X_0,X_1,\ldots,X_m\}$.
\ben
...
\item Define a bipartite graph with two partitions $S$ and $T$ with, an edge between $S_i\in S$ and $T_j\in T$ exists iff $a_{ij}$ is not filled with $\times$. The two deterministic finite-state transducers are equivalent iff the bipartite graph has a perfect matching.
\een
\end{document}
