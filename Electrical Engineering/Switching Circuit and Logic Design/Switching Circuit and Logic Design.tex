...
An edge-triggered flip-flop has the same inputs and outputs of the gated latch of the same type but the enable input $E$ is replaced with clock input $C$. For rising-edge-triggered (aka active-high) flip-flops, when clock is at a rising edge, the flip-flop is transparent; otherwise the flip-flop is hold (aka retain). For falling-edge-triggered (aka active-low) flip-flops, when clock is at a falling edge, the flip-flop is transparent; otherwise the flip-flop is hold (aka retain).
