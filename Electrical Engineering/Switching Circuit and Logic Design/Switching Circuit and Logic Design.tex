...
Same as Moore and Mealy machines, state diagrams (also called state graphs) are commonly used to analyze Moore and Mealy circuits, with the states typically represented by meaningless symbol and a starting state usually represented by an arrow with no origin pointing to the state or not shown.
\sssc{Next-state Map}
...
general model of the Mealy circuitWe can determine the fastest clock speed (the minimum clock period) from the general model of the Mealy circuit.
the clock period $t_{clk}$ 
For any flip-flop in the circuit with output propagation delay $t_p$, combinational circuit propagation delay to the flip-flop input $t_c$, the maximum time $t_x$ the inputs $X_i$ that the flip-flop input is dependent on need to be stable before the next active clock edge, the flip-flop setup time $t_s$, and clock skew $t_{sk}$ between the flip-flops that receive the clock signal first and the flip-flop.
\[t_{clk}\geq\max\{t_p,t_x\}+t_c+t_s+t_{sk}.\]
Another constraint is that for any flip-flop in the circuit with also hold time $t_h$ and the maximum clock skew $t_{msk}$ between it and any flip-flop whose inputs are dependent on the flip-flop output,
\[t_p+t_c\geq t_{msk}+t_h.\]
