...
The symbol of a J-K latch in block diagrams is the same as an active-high S-R latch with an $S$ and $R$ renamed $J$ and $K$ respectively.
...
\sssc{T Latch}
A toggle (aka T) latch is a gated J-K latch with $J$ and $K$ replaced with $T$. It has two inputs, toggle $T$ and enable $E$, and two outputs, $Q$ and $Q'$.

It is hold when $E=0$ or $T=0$ and toggle when $E=T=1$.
