...
When shift=1 at an active edge, the register shifts a bit; otherwise, the register holds.
\sssc{Serial-In Parallel-Out (SIPO) Shift Register}
A serial-in parallel-out (SIPO) shift register is a type of shift register that allows data to be retrieved in parallel. An $n$-bit SIPO shift register consists of an $n$-bit SISO register with additional $n-1$ outputs connected to the outputs $Q$ of the flip-flops except the last one. Those outputs together with the original SO is the parallel output of the register.
...
\bit
\item the output of the associated flip-flop that is selected when $(Sh,L)=(0,0)$, called hold,
\item the corresponding bit of the parallel input (PI) that is selected when $(Sh,L)=(0,1)$, called load, and
\item the output of the previous flip-flop in the chain with the first flip-flop in the chain receiving the serial input (SI) instead that is selected when $(Sh,L)=(1,0)$ or $(1,1)$, called shift.
\eit
...
A parallel-in parallel-out (PIPO) shift register is a type of shift register that allows data to be loaded and retrieved in parallel. An $n$-bit PIPO shift register consists of an $n$-bit PISO register with additional $n-1$ outputs connected to the outputs $Q$ of the flip-flops except the last one. Those outputs together with the original SO is the parallel output of the register.
...
\sssc{Ring Counter}
If we connect the output $Q$ of the last flip-flop in the chain to the input $D$ of the first flip-flop in the chain of an $n$-bit shift register, the resulting cyclic shift register performs an end-around shift, that is, the bits stored circulate around the register, and the state of the register repeats each $\frac{n}{m}$ shifts with $m\in\bbN$ being the largest number of bits of the repeating subpattern in a state.

If we connect the output $Q'$ of the last flip-flop in the chain to the input $D$ of the first flip-flop in the chain, the resulting shift register with inverted feedback transitions through distinct states repeatedly.
