...
A clock signal is an periodic logic signal (square wave)  which oscillates between a high, representing a "one", and a low, representing a "zero", state at a constant frequency. A duty cycle is half a full cycle (a period), that is, how long the signal stays high or low. A rising edge is a moment the signal goes from 0 to 1; a falling edge is a moment the signal goes from 1 to 0.

Clock signals are generated by oscillators, which can be crystal oscillator (very stable, used in CPUs and microcontrollers), RC oscillator (simpler and less precise), external clock generator, etc., often labeled CLK.
\sssc{Synchronous system} 
...
\sssc{Latches}
Latches are level-triggered data storage elements that store a single bit of data; one of its two states represents a "one" and the other represents a "zero". "Level-triggered" means it changes state at a level change of input signals.
\sss{Gated latch}
A gated latch is a latch with an extra input signal called enable or clock (clk) that controls whether the latch is transparent (enabled) or hold (disabled). When enable is at active level (1 for active-high latch and 0 for active-low latch), the latch is transparent, meaning the output follows the input; when enable is at inactive level (0 for active-high latch and 1 for active-low latch), the latch is hold, meaning the output remains at its previous state regardless of input changes.
\sssc{Flip-flops}
Flip-flops are edge-triggered data storage elements that store a single bit of data; one of its two states represents a "one" and the other represents a "zero". "Edge-triggered" means it only changes state at a clock edge.
\sssc{Time}
...
When discussing latches and flip-flops, we the term \textit{present state} $Q=Q(t)$ to denote the state of the $Q$ output of the latch or flip-flop at the time input signal changes, and the term \textit{next state} $Q^+=Q(t+\epsilon)$ to denote the state of the $Q$ output after the latch or flip-flop has reacted to the input change and stabilized.
\sssc{Metastable state}
A metastable state is a state where the signal in a closed circuit
\bit
\item is a constant and can theoretically stay as is if no noise occur, and
\item transits away if any noise occurs.
\eit
The probability that metastability lasts decays exponentially over time.

Take a circuit consist of two inverters with the output of one being the input of another for example. There are two stable states (excluding metastable state), 0 and 1, and one metastable state, a voltage right in the middle of 0 and 1.
\sssc{(Active-high or NOR-gate) Set-Reset (S-R or SR) Latch}
An (active-high or NOR-gate) set-reset latch has two inputs, set $S$ and reset $R$, and two outputs, $Q$ and $Q'$, and stores one bit, $Q$. It consists two NOR gates with $S$ and $Q$ being the inputs and $Q'$ being the output of one NOR gate, and $R$ and $Q'$ being the inputs and $Q$ begin the output of the other NOR gate. It has restriction that $SR=0$ because $Q$ and $Q'$ will oscillate when $S=R=1$.
...
The symbol of an active-high S-R latch in block diagrams is an rectangle with L or S-R latch in it, two input lines with label $S$ and $R$ in the rectangle adjacent to it on one long side, output line with label $Q$ in the rectangle adjacent to it at the position opposite to $S$ on the opposite side, and output line with label $Q'$ in the rectangle adjacent to it at the position opposite to $R$ on the opposite side.
\sssc{Active-low or NAND-gate Set-Reset (S-R or SR) Latch, $\ol{\tx{S}}\ol{\tx{R}}$ Latch, or $\ol{\tx{S}}$-$\ol{\tx{R}}$ Latch}
An alternative form of S-R latch is the active-low or NAND-gate set-reset latch, aka $\ol{\tx{S}}\ol{\tx{R}}$ latch or $\ol{\tx{S}}$-$\ol{\tx{R}}$ latch. It has two inputs, $\ol{S}$ and $\ol{R}$, and two outputs, $Q$ and $Q'$, and stores one bit, $Q$. It consists two NAND gates with $\ol{S}$ and $Q'$ being the inputs and $Q$ being the output of one NAND gate, and $\ol{R}$ and $Q$ being the inputs and $Q'$ begin the output of the other NAND gate. It has restriction that $\ol{S}+\ol{R}=1$ because $Q$ and $Q'$ will oscillate when $\ol{S}=\ol{R}=0$.
...
The symbol of an active-low S-R latch in block diagrams is an rectangle with L or S-R latch in it, two input lines with label $\ol{S}$ and $\ol{R}$ in the rectangle adjacent to it on one long side, output line with label $Q$ in the rectangle adjacent to it at the position opposite to $\ol{S}$ on the opposite side, and output line with label $Q'$ in the rectangle adjacent to it at the position opposite to $\ol{R}$ on the opposite side.
...
\een
\sssc{Active-high or NOR-gate Gated S-R Latch}
An active-high or NOR-gate gated S-R latch has three inputs, set $S$, reset $R$, and enable $E$, and two outputs, $Q$ and $Q'$, and stores one bit, $Q$. It consists of an active-high S-R latch with $S$ and $R$ being replaced with outputs of two AND gates, each of which takes $E$ and the original input as inputs respectively. When $E=0$, the gated S-R latch is hold, i.e., it functions as an active-high S-R latch; when $E=1$, the gated S-R latch is transparent.

It has restriction that $SR=0$ because $Q$ and $Q'$ will oscillate when $E=S=R=1$ and a race condition where both inputs to the S-R latch changes from 0 to 1 and the propagation delays of the gates determine whether the latch stabilizes with $Q=1$ or $Q=0$ occurs when $E$ changes from 1 to 0 while $S=R=1$.

With present state $Q$, next state $Q^+$, and next state of $Q'$ $P$, the next-state equation of a gated S-R latch is:
\[Q^+=(RE+(SE+Q)')'=(RE)'(SE+Q)=(R'+E')(SE+Q)=SR'E+R'Q+QE'.\]
By making $S=R=1$ don't care combination, we can simplify it as:
\[Q^+=SE+Q(R'+E').\]
\[P=(SE+(SE+Q(R'+E')))'=(SE)'(SE+Q(R'+E'))'=(S'+E')(SE)'(Q(R'+E'))'=(S'+E')(Q'+RE)=S'Q+S'RE+E'Q'.\]
\[Q^+'=(SE+Q(R'+E'))'=(SE)'(Q(R'+E'))'=(S'+E')(Q'+RE)=S'Q'+S'RE+E'Q=P.\]
By making $S=R=1$ don't care combination, we can simplify it as:
\[Q^+'=P=RE+Q'(S+E').\]
If we plot $Q^+=SE+Q(R'+E')$ on a K-map:
\begin{longtable}[c]{mm|m|m|m|m|}
& & \multicolumn{4}{|c|}{SR}\\
& & 00 & 01 & 11 & 10\\\hline
\multirow{4}{EQ} & 00 & 0 & 0 & 0 & 0\\\cline{3-6}
& 01 & 1 & 1 & 1 & 1\\\cline{3-6}
& 11 & 1 & 0 & 1 & 1\\\cline{3-6}
& 10 & 0 & 0 & 1 & 1\\\cline{3-6}
\end{longtable}
we can find a static-1 hazard in it, 
$(S,R,E,Q)=(1,1,0,1)\lra (S,R,E,Q)=(1,1,1,1)$. When $E$ changes from 1 to 0, the output $Q$ may changes to 1 temporarily, but will be forced back to 0 since the inputs $S_{SR}$ and $R_{SR}$ of the underlying S-R latch are both 0, which is simply a glitch. When $E$ changes from 0 to 1, however,  a race condition occurs.

Another restriction of a NOR-gate gated S-R latch is that when a 
called 0's catching problem, 0 catching problem, or race condition at 0 problem.
