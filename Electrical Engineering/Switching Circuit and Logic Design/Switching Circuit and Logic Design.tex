...
\end{longtable}\FB
\sssc{Binary codes}
Binary codes are encodings that encode things into binary numbers.
\sssc{Binary code}
...
\sssc{Weighted counter}
A weighted code is a counter that counts in weighted code.
...
\item For each entry $a_{ij}$ that has not been filled with $\times$, $S_j\equiv S_i$. For all equivalence classes under $\equiv$, replace all elements in it with one arbitrary state in it.
\een
...
Compatibility of states is reflexive and symmetric but not transitive. Two states are compatible if they are equivalent.

Two states are distinguishable iff they can't be compatible in any assignment of legal compatible states.

If two compatible states exist in a state table, we can replace any occurrence of one of them by the other to eliminate a redundant state.
...
\sssc{Merger chart or merger table}
Merger chart, also called merger table, is a method to minimize incompletely specified deterministic finite-state transducer.

First eliminate all unreachable states. Let the set of all remaining states be $S=\{S_0,S_1,\ldots,S_n\}$ and the input alphabet be $\Sigma=\{X_0,X_1,\ldots,X_m\}$.
\ben
\item The merger chart is a lower-triangle table with entries $a_{ij}$ for all integers $i,j\in\bbN_0\land n\geq i>j$.
\item Fill each entry $a_{ij}$ with
\bit
\item $\times$ (means distinguishable) iff for Mealy machine, there exists $X_k\in\Sigma$ such that $\lambda(S_j,X_k)\neq\lambda(S_i,X_k)$, and for Moore machine, $\lambda(S_j)\neq\lambda(S_i)$,
\item pairs of states $(\delta(S_j,X_k),\delta(S_i,X_k))$ for all $X_k\in\Sigma$ such that
...
\item Find all maximal cliques of the graph using the Bron–Kerbosch algorithm. For all maximal cliques, replace all elements in it with one arbitrary state in it.
\een
...
\sssc{Given encoding}
Binary encoding, one-hot encoding, one-code encoding, and gray-like encoding are often used. One-hot encoding or one-code encoding is typically used for FGPA.
\sssc{Cost}
...
\item Iterate through the remaining state assignments and find the one(s) of the lowest cost.
\een
