...
\sct{Vector Space and Functional}
\ssc{Spaces and maps}
\sssc{Vector space (向量空間) or Linear space (線性空間)}
...
When a vector consists of $n$ scalar components, we typically denote it as
\[(x_1,x_2,\ldots,x_n)\]
or
...
\sssc{Linear map (線性映射) or linear operator}
For vector spaces $V,W$, a map $T\colon V\to W$ between vector spaces $V$ and $W$ over field $F$ is linear if
...
A linear transformation is a bijection from an vector space onto itself that is an linear map.
...
The dimension of an affine space is defined as the dimension of its associated vector space.
...
\end{enumerate}
\sssc{Operator norm or induced norm}
...
\sssc{(Algebraic) dual (對偶) (vector) (space)}
...
\[(\varphi+\psi)(x)=\varphi (x)+\psi (x),\]
\[(a\varphi )(x)=a\left(\varphi (x)\right).\]
\sssc{Transpose, dual map, or adjoint}
For two vector spaces $X,Y$ and a continuous linear map $A\colon X\to Y$, the transpose, dual map, or adjoint of $A$ is defined as the unique map $^tA\colon Y'\to X'$ such that for all $x\in X$, for all $y\in Y'$,
\[\langle y,A(x)\rangle=\lange^tA(y),x\ranlge.\]
\sssc{Strong dual (space)}
A strong dual space is a continuous dual space of a TVS equipped with the strong (dual) topology, also called topology of uniform convergence on bounded subsets. Suppose $X$ is a TVS over field $\bbF$ and $\mathcal{B}$ is a family of bounded subsets of $X$ such that every bounded subset of $X$ is a subset of some $B\in\mathcal{B}$. Define the family $T$ of seminorms $\|\cdot\|_B$ on $X'$ for all $x'\in X'$ as
...
\sssc{Cartesian coordinate system (笛卡爾座標系統) or rectangular coordinate system (直角座標系統)}
...
\sssc{Curvilinear coordinate system}
...
\sssc{Flat, hyperplane, or affine subspace (仿射子空間)}
...
\sssc{Elementwise multiplication}
Fot two vectors $\mb{u}=(u_1,\ldots,u_n)$ and $\mb{v}=(v_1,\ldots,v_n)$, their elementwise multiplication, denoted as $\mb{u}\odot\mb{v}$, is defined as
...
\sssc{Standard simplex or probability simplex}
A standard simplex or probability simplex is a simplex in $\bbR^{k+1}$ whose vertices are unit vectors, that is, the set
\[\{(p_0,p_1,\ldots,p_k)\mid\sum_{i=0}^kp_i=1\}.\]
And each point $(p_0,p_1,\ldots,p_k)$ corresponds to a PMF $p$ with target sample space $\{x_0,x_2,\ldots,x_k\}$ and $p(x_i)=p_i$.
...
\sssc{Basic properties of cross product}
\bit
\item Self cross product is the zero vector:
\[\mb{a}\times\mb{a}=\mb{0}.\]
\item Anticommutative law (反交換律):
\[\mathbf{a}\times\mathbf{b}=-\mathbf{b}\times\mathbf{a}.\]
\item Left and right distributive over addition and substraction:
...
\eit
\sssc{Triple product identity (三重積恆等式)/Jacobi identity (雅可比恆等式)}
...
Let $A\in\bbR^{n\times n}$ and $\mb{x}\in\bbR^n$. $A$ is called
\bit
\item Positive definite: if
\[\mb{x}^\top A\mb{x}>0,\quad\forall\mb{x}\neq 0.\]
\item Negative definite: if
\[\mb{x}^\top A\mb{x}<0,\quad\forall\mb{x}\neq 0.\]
\item Positive semidefinite: if
\[\mb{x}^\top A\mb{x}\geq 0,\quad\forall\mb{x},\]
\[\mb{x}^\top A\mb{x}>0,\quad\exists\mb{x}.\]
\item Negative semidefinite: if
\[\mb{x}^\top A\mb{x}\leq 0,\quad\forall\mb{x},\]
\[\mb{x}^\top A\mb{x}<0,\quad\exists\mb{x}.\]
\item Neutral/Zero semidefinite: if
\[\mb{x}^\top A\mb{x}=0,\quad\forall\mb{x}.\]
...
\sssc{Rodrigues' rotation formula}
