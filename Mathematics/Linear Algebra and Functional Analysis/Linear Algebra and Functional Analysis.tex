...
\sssc{Functional (泛函)}
A functional is a function from a vector space into the field of real or complex numbers. For a vector space $X$, $x\in X$ and a functional on $f$ on $X$, $f(x)$ is also denoted as $\langle f,x\rangle$.
\sssc{Seminorm (半範數)}
