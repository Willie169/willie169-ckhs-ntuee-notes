...
\sssc{Ordinary and Singular Points}
...
For any $a\in I$, $a$ is called an ordinary point iff $p$ and $q$ are both analytic at $a$ and called a singular point otherwise.
...
\[p(x)=\frac{g(x)}{f(x)},\quad q(x)=\frac{h(x)}{f(x)}.\]
\ben
\item We seek a solution in the form:
...
network.trr.mode\item Solve the indicial equation for $r$. Let the roots be $r_1\geq r_2$.
