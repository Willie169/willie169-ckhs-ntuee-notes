...
\item Bessel function of the first kind:
    ...
    $J_0(0)=1$, $J_{\nu}(0)=0$ for $\nu>0$ or $-\nu\in\bbN$, $J_{\nu}(x)$ has a branching-point singularity at 0 otherwise.

    $J_{\nu}$ and $J_{-\nu}$ are two linearly independent solutions of the Bessel's PLACEHOLDER 
\item Bessel function of the second kind: If $\Re(\nu)-\Im(\nu)\notin\bbZ$,
    \[Y_{\nu}(x)=\frac{J_{\nu}(x)\cos(\nu\pi)-J_{-\nu}(x)}{\sin(\nu\pi)}\]
    otherwise,
