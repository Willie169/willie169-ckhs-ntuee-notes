...
\[y^{(n)}+\sum_{i=0}^{n-1}p_i(x)y^{(i)}=0.\]
The particular solution $y_p$ such that the general solutions are $y=y_c+y_p$ of the original ODE (or general solutions $y$ with the integral constants treated as parameters of the family) is given by:
\[\sum_{i=1}^ny_i(x)\int\frac{W_i(x)}{W(x)}\dd{x},\]
where
\bit
