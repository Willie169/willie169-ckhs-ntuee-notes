...
\sssc{Filter}
A continuous-time linear time-invariant system is a low-pass (低通) filter (LPF) iff $H(i\omega)$ where $i=\sqrt{-1}$ as a function of a real variable $\omega$ is such that
\[\lim_{\omega\to 0}\abs{H(i\omega)}=1\]
and
\[\lim_{\omega\to\infty}\abs{H(i\omega)}=0.\]
A continuous-time linear time-invariant system is a high-pass (高通) filter (HPF) iff $H(i\omega)$ where $i=\sqrt{-1}$ as a function of a real variable $\omega$ is such that
