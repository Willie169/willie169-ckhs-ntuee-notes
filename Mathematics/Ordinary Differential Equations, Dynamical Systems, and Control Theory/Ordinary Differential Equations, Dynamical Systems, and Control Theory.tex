...
\item The two-parameter family of general solutions for all $0<|x-x_0|<R$ where $R$ is the distance between $x_0$ and the nearest singular point with parameters $a_0,a_1$ is:
\[y=\sum_{n=0}^{\infty}a_n(x-x_0)^n.\]
\een
\bpr
PLACEHOLDER
...
\item Let $R$ be the distance between $x_0$ and the nearest regular singular point. Depending on the relation of $r_1$ and $r_2$:
\bit
...
\ssc{Bessel function}
Bessel functions are a family of special functions that are solutions to the Bessel's differential equation:
\[x^2y''+xy'+(x^2-\nu^2)y=0,\]
where $x$ is the independent variable and $\nu\in\bbC$ is called order.
\bit
\item Bessel function of the first kind:
    \[J_{\nu}(x)=\sum_{m=0}^{\infty}\frac{(-1)^m}{m!\Gamma(m+\nu-1)}\qty(\frac{x}{2})^{2m+\nu}.\]
    $J_0(0)=1$, $J_{\nu}(0)=0$ for $\nu\in\bbR_{>0}\cup\bbN$, $J_{\nu}(x)$ has a branching-point singularity at 0, which is the only singularity, otherwise.

    $J_{\nu}$ and $J_{-\nu}$ are two linearly independent solutions of the Bessel's equation for integer order.  $J_{-\nu}(x)=(-1)^{\abs{\nu}}J_{\nu}(x)$ for integer $\nu$.
\item Bessel function of the second kind: If $\nu\notin\bbZ$,
    \[Y_{\nu}(x)=\frac{J_{\nu}(x)\cos(\nu\pi)-J_{-\nu}(x)}{\sin(\nu\pi)},\]
    which is a solution of the Bessel's equation that is linearly independent of $J_{\nu}$.
