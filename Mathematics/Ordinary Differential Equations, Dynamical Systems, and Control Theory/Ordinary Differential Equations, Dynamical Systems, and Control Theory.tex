...
Differentiating $n-1$ times gives
\[\Omega\begin{bmatrix}c_1\\c_2\\\vdots\\c_n\end{bmatrix}=\begin{bmatrix}0\\0\\\vdots\\0\end{bmatrix}\]
Prove by contradiction. If $\det(\Omega)=0$, by Cramer formula, $c_1\\c_2\\\vdots\\c_n\end{bmatrix}=\begin{bmatrix}0\\0\\\vdots\\0\end{bmatrix}$.
\epr
\sssc{Solutions of linear homogenous ODE as vector space}
The general solutions of an $n$th-order linear homogenous ODE is a vector space of dimension $n$. A basis of it is called a fundamental set of solutions.
...
\sssc{Solutions of linear nonhomogenous ODE as affine space}
The general solutions of a linear nonhomogenous ODE is an affine space whose associated vector space is the vector space of the solutions of its complementary equation.
\sssc{Annihilator}
...
Prove by Laplace transform.
\[\ba
\mathcal{L}\{x^{m_j}e^{\alpha_jx}\qty(k_j\cos(\beta_jx)+l_j\sin(\beta_jx))\}&=\mathcal{L}\{x^{m_j}e^{\alpla_jx}\qty((k_j+l_j)e^{i\beta_jx}-(k_j-l_j)ie^{-i\beta_jx})\}\\
&=(k_j+l_j)\mathcal{L}\{x^{m_j}e^{\alpla_jx+i\beta_jx}\}-(k_j-l_j)i\mathcal{L}\{x^{m_j}e^{\alpla_jx-i\beta_jx}\}\\
...
PLACEHOLDER
\ssc{Variation of parameters or variation of constants method of solution of linear nonhomogeneous equation}
\sssc{Method of solution}
Given an $n$th-order linear nonhomogeneous ODE
\[y^{(n)}+\sum_{i=0}^{n-1}p_i(x)y^{(i)}=q(x),\]
...
where
\bit
\item $x_0$ is an arbitraty point in $I$,
...
\ssc{Bessel function}
Here we discuss in real numbers.
\sssc{Bessel function}
...
The general solutions are the linear combinations of two linearly independent solutions.
\bit
\item Bessel function of the first kind:
    \[J_{\nu}(x)=\sum_{m=0}^{\infty}\frac{(-1)^m}{m!\Gamma(m+\nu+1)}\qty(\frac{x}{2})^{2m+\nu}.\]
    $J_{\nu}(x)$ is defined on $\bbR$ for $\nu\in\bbZ$, on $\bbR_{>=0}$ for $\nu\in\bbR_{>0}\setminus\bbZ$, and on $\bbR_{>0}$ for $\bbR_{<0}$. $J_0(0)=1$. $J_{\nu}(0)=0$ for $\nu\in\bbR_{>0}\cup\bbZ$.

    $J_{\nu}$ and $J_{-\nu}$ are two linearly independent solutions of the Bessel's equation for $\nu\in\bbR\setminus\bbZ$.  $J_{-\nu}(x)=(-1)^{\nu}J_{\nu}(x)$ for $\nu\in\bbZ$.
...
\[y_2''(x)=\sum_{n=0}^{\infty}B_n(n-\nu)(n-\nu-1)x^{n-\nu-2}\]
\[\sum_{n=0}^{\infty}A_n(n+\nu)(n+\nu-1)x^{n+\nu}+\sum_{n=0}^{\infty}A_n(n+\nu)x^{n+\nu}+\sum_{n=0}^{\infty}A_nx^{n+\nu+2}-\nu^2\sum_{n=0}^{\infty}A_nx^{n+\nu}=0\]
\[A_1(1+2\nu)x^{1+\nu}+\sum_{n=2}^{\infty}\qty(A_n(n^2+2n\nu)+A_{n-2})x^{n+\nu}=0\]
\[A_n=0,\quad\frac{n+1}{2}\in\bbN\]
\[A_n(n^2+2n\nu)+A_{n-2}=0,\quad\frac{n}{2}\in\bbN\]
\[A_{2m}=\frac{(-1)^mA_0}{\prod_{i=1}^m(2i)(2i+2\nu)}=\frac{(-1)^mA_0\Gamma(\nu+1)}{2^{2m}m!\Gamma(m+\nu+1)},\quad m\in\bbN\]
Let
\[A_0=\frac{1}{2^\nu\Gamma(\nu+1)}\]
\[A_{2m}=\frac{(-1)^m}{2^{2m+\nu}m!\Gamma(m+\nu+1)},\quad m\in\bbN\]
\[y_1(x)=\sum_{m=0}^{\infty}\frac{(-1)^m}{2^{2m+\nu}m!\Gamma(m+\nu+1)}x^{2m+\nu}=\sum_{m=0}^{\infty}\frac{(-1)^m}{m!\Gamma(m+\nu+1)}\qty(\frac{x}{2})^{2m+\nu}=J_{\nu}(x)\]
\[y_2(x)=\sum_{m=0}^{\infty}\frac{(-1)^m}{m!\Gamma(m-\nu+1)}\qty(\frac{x}{2})^{2m-\nu}=J_{-\nu}(x)\]
When $2\nu\in\bbZ$, test whether $y_1(x),y_2(x)$ are linearly dependent.
\[J_{\nu}'(x)=\sum_{m=0}^{\infty}\frac{(-1)^m}{m!\Gamma(m+\nu+1)}\qty(\frac{x}{2})^{2m+\nu-1}\frac{2m+\nu}{2}\]
\[J_{-\nu}'(x)=\sum_{m=0}^{\infty}\frac{(-1)^m}{m!\Gamma(m-\nu+1)}\qty(\frac{x}{2})^{2m-\nu-1}\frac{2m-\nu}{2}\]
\[W(J_{\nu},J_{-\nu})(x)=\sum_{m=0}^{\infty}\sum_{n=0}^{\infty}\frac{(-1)^{m+n}(m-n-\nu)}{m!n!\Gamma(m-\nu+1)\Gamma(n+\nu+1)}\qty(\frac{x}{2})^{2m+2n-1}\]
The coefficient of the $x^{-1}$ term is
\[\frac{-2\nu}{\Gamma(1-\nu)\Gamma(1+\nu)}=\frac{-2}{\Gamma(\nu)\Gamma(1-\nu)}=\frac{-2\sin(\pi\nu)}{\pi}\]
Thus $W(J_{\nu},J_{-\nu})(0)=0$ for $\nu\in\bbZ$.

\epr
\sssc{Modified Bessel function}
...
\item Modified Bessel function of the first kind:
    ...
    $I_{\nu}(x)$ is defined on $\bbR$ for $\nu\in\bbZ$, on $\bbR_{>=0}$ for $\nu\in\bbR_{>0}\setminus\bbZ$, and on $\bbR_{>0}$ for $\bbR_{<0}$. $I_0(0)=1$. $I_{\nu}(0)=0$ for $\nu\in\bbR_{>0}\cup\bbZ$.

    $I_{\nu}$ and $I_{-\nu}$ are two linearly independent solutions of the Bessel's equation for $\nu\in\bbR\setminus\bbZ$.  $I_{-\nu}(x)=(-1)^{\nu}I_{\nu}(x)$ for $\nu\in\bbZ$.
...
PLACEHOLDER: BVP can have many, one, or no solution
PLACEHOLDER: end
