...
TODO 
\end{proof}
\ssc{System of linear first-order ODEs or linear first-order system}
\sssc{Definition}
For $n$ unknown functions $x_1(t),\ldots x_n(t)$, a linear first-order system can be written as normal form
\[x_j'(t)=\sum_{i=1}^na_{j,i}(t)x_i(t)+f_j(t),\quad\forall j\in\bbN_{\le n}\]
or matrix form
\[\mb{x}'(t)=A(t)\mb{x}(t)+\mb{f}(t),\]
where
\[\mb{x}(t)=\begin{bmatrix}x_1(t)\\\vdots\\x_n(t)\end{bmatrix}\]
is called solution vector,
\[\mb{A}(t)=\begin{bmatrix}a_{1,1}(t) & \cdots & a_{1,n}(t)\\\vdots & \ddots & \vdots\\a_{n,1}(t) & \cdots & a_{n,n}(t)\end{bmatrix}\]
is called coefficient matrix, and
\[\mb{f}(t)=\begin{bmatrix}f_1(t)\\\vdots\\f_n(t)\end{bmatrix}\]
If $\mb{f}(t)=\mb{0}$, that is, all of the ODEs are homogeneous, then the system is homogeneous; otherwise the system is non-homogeneous.
TODO
