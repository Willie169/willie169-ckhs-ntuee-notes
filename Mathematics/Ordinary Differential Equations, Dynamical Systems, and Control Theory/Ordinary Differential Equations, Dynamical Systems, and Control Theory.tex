...
\sssc{Band-stop filter or band-rejection filter}
A continuous-time linear time-invariant system is a band-stop filter iff $H(i\omega)$ where $i=\sqrt{-1}$ as a function of a real variable $\omega$ is such that
\[\lim_{\omega\to 0}\abs{H(i\omega)}=K\in\bbR_{>0}\]
\[\lim_{\omega\to\infty}\abs{H(i\omega)}=K\in\bbR_{>0}\]
and
\[\max_{\omega}\abs{H(i\omega)}=0,\]
where $K$ is called band-stop gain. The filter is called prototype or canonical if $K=1$.
