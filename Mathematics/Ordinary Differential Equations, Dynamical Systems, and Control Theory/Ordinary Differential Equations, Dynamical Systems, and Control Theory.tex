...
The formation of its general solutions using the method of undetermined coefficients is as follows:
\ben
\item For each term, an annihilator is listed:
\bit
\item $x^me^{\alpla x}$: $(D-\alpha)^{m+1}$
\bpr
Prove by Laplace transform.
\[\mathcal{L}\{x^me^{\alpla x}\}=\frac{m!}{(s-\alpha)^{m+1}}\]
\[\mathcal{L}^{-1}\{m!\}=0\]
\epr
\item $x^me^{\alpla x}\qty(k\cos(\beta x)+l\sin(\beta x))$: $(D^2-2\alpha D+\alpha^2+\beta^2)^{m+1}$
\bpr
Prove by Laplace transform.
\[\ba
\mathcal{L}\{x^me^{\alpla x}\qty(k\cos(\beta x)+l\sin(\beta x))\}&=\mathcal{L}\{x^me^{\alpla x}\qty((k+l)e^{i\beta x}-(k-l)ie^{-i\beta x})\}\\
&=(k+l)\mathcal{L}\{x^me^{\alpla x+i\beta x}\}-(k-l)i\mathcal{L}\{x^me^{\alpla x-i\beta x}\}\\
&=\frac{(k+l)m!}{(s-\alpha-i\beta)^{m+1}}-\frac{(k-l)im!}{(s-\alpha+i\beta)^{m+1}}\\
&=\frac{(k+l)m!(s-\alpha+i\beta)^{m+1}-(k-l)im!(s-\alpha-i\beta)^{m+1}}{(s-\alpha-i\beta)^{m+1}(s-\alpha+i\beta)^{m+1}}\\
&=\frac{(k+l)m!(s-\alpha+i\beta)^{m+1}-(k-l)im!(s-\alpha-i\beta)^{m+1}}{(s^2-2\alpha+\alpha^2+\beta^2)^{m+1}}
\ea\]
\[\mathcal{L}^{-1}\{(k+l)m!(s-\alpha+i\beta)^{m+1}-(k-l)im!(s-\alpha-i\beta)^{m+1}\}=0\]
\epr
\eit
