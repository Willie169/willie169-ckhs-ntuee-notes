...
\sssc{Homogeneity}
A linear ODE is called a linear homgeneous ODE iff the forcing term is 0 for all $x$; otherwise, it is called a linear non-homogeneous ODE.
...
TODO
\ssc{Solving Systms of Linear ODEs by Elimination}
\sssc{Method of solution}
\ben
...
Laplace transform both sides, solve the polynomial equation to get the function of $s$, and inverse Laplace transform it to get the solutions.
...
Legendre polynomials are the polynomial solutions of Legendre's differential equations:
...
Use power series method at 0 and then Frobenius method at $\pm 1$.
\[y=\sum_{m=0}^{\infty}a_mx^m\]
\[y'=\sum_{m=1}^{\infty}a_mmx^{m-1}\]
\[y''=\sum_{m=2}^{\infty}a_mm(m-1)x^{m-2}\]
\[\sum_{m=2}^{\infty}a_mm(m-1)x^{m-2}-\sum_{m=2}^{\infty}a_mm(m-1)x^m-2\sum_{m=1}^{\infty}a_mmx^m+n(n+1)\sum_{m=0}^{\infty}a_mx^m=0\]
\[2a_2+6a_3x+\sum_{m=2}^{\infty}a_{m+2}(m+1)(m+2)x^m-2a_1x-\sum_{m=2}^{\infty}a_mm(m+1)x^m+n(n+1)(a_0+a_1x)+n(n+1)\sum_{m=2}^{\infty}a_mx^m=0\]
\[2a_2+na_0=0\]
\[6a_3-2a_1+n(n+1)a_1=0\]
\[a_{m+2}(m+1)(m+2)-m(m+1)a_n+n(n+1)a_m\]
\[a_2=-\frac{n(n+1)}{2}a_0\]
\[a_3=-\frac{(n-1)(n+2)}{6}a_1\]
\[a_{m+2}=-\frac{(n-m)(n+m+1)}{(m+1)(m+2)}a_m\]
\[a_m=-\frac{(n-m+2)(n+m-1)}{(m-1)m}a_{m-2}\]
\[a_{2m}=(-1)^m\frac{\prod_{i=0}^m(n-i)(n+i+1)}{(2m)!}a_0,\quad m\in\bbN\]
\[a_{2m+1}=(-1)^m\frac{\prod_{i=1}^m(n-i)(n+i+1)}{(2m+1)!}a_1,\quad m\in\bbN\]
When $n$ is even, $a_{2m}$ terminates at $m=n$. When $n$ is odd, $a_{2m+1}$ terminates at $m=n$. These yield the polynomial solutions.

For even $n$, take
\[a_0=(-1)^{\frac{n}{2}}\frac{\prod_{i=1}^{\frac{n}{2}}(2i-1)}{\prod_{i=1}^{\frac{n}{2}}(2i)}=(-1)^{\frac{n}{2}}\frac{n!}{2^n\qty(\qty(\frac{n}{2})!)^2}\]
Let $k=\frac{n}{2}$.
\[a_0=(-1)^k\frac{n!}{2^n\qty(k!)^2}\]
\[a_{2k}=(-1)^k\frac{\prod_{i=0}^k(n-i)(n+i+1)}{n!}a_0=\frac{(n+k+1)!k}{2^n\qty(k!)^3}\]
\[a_m=-\frac{(n-m+2)(n+m-1)}{(m-1)m}a_{m-2}\]
