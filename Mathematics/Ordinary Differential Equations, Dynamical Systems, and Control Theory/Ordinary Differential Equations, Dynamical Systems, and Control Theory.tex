...
TODO
\sssc{Helmholtz equation}
Helmholtz equation is
\[\nabla^2f+k^2f=0.\]
\sssc{Helmholtz equation assumed separation of variables in Cartesian coordinate}
\[\nabla^2=\pdv[2]{x}+\pdv[2]{y}+\pdv[2]{z}\]
Assume separation of variables $f(x,y,z)=X(x)Y(y)Z(z)$.

The $x$-equation is a homogeneous linear equation:
\[X''+\lambda X=0\]
The solution is
\[X=C\cos(\sqrt{\lambda}x)+D\sin(\sqrt{\lambda}x)\]
\sssc{Helmholtz equation assumed separation of variables in cylindrical coordinate}
\[\nabla^2=\frac{1}{r}\pdv{r}\qty(r\pdv{}{r})+\frac{1}{r^2}\pdv[2]{\theta}+\pdv[2]{z}\]
Assume separation of variables $f(r,\theta,z)=R(r)\Theta(\theta)Z(z)$.

The $z$-equation is a homogeneous linear equation:
\[Z''+\lambda Z=0\]
The solution is
\[Z=C\cos(\sqrt{\lambda}z)+D\sin(\sqrt{\lambda}z)\]
$\Theta$ must be periodic of period $2\pi$. This leads to
\[\Theta=C\cos(m\theta)+D\sin(m\theta)\]
\[\Theta''+m^2\Theta=0\]
where $m\in\bbN_0$.
...
&=x^{-\nu}\sum_{m=0}^{\infty}\lim_{\mu\to\nu}\frac{(-1)^{m+1}}{m!\Gamma(m+\mu)}\qty(\frac{x}{2})^{2m+\nu+1}\\
&=-x^{-\nu}J_{\nu+1}(x)
\ea\]
\epr
\[\dv{}{x}\qty(x^{\nu}J_{\nu}(x))=x^{\nu}J_{\nu-1}(x).\]
