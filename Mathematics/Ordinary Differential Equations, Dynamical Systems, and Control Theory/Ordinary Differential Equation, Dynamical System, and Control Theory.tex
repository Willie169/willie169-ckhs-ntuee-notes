...
\begin{document}
\title{Ordinary Differential Equation, Dynamical System, and Control Theory}
\author{沈威宇}
\date{\temtoday}
\titletocdoc
\ch{Ordinary Differential Equation (常微分方程), Dynamical System (動態系統 or 動力系統), and Control Theory (控制理論)}
\sct{Ordinary Differential Equation}
\ssc{Definition}
...
\sct{Linear ODE}
\ssc{Definition}
\sssc{Linear ODE and system of linear ODEs or linear system}
An $n$th-order linear ODE or system of linear ODEs of a unknown function or dependent variable unknown functions or dependent variables $\mb{y}\colon\bbR\to\bbR^m$ with respect to an independent variable $x\in\bbR$ can be written in the form, called normal form,
\[\sum_{i=0}^na_i(x)\mb{y}^{(i)}=\mb{f}(x),\]
in which $a_i(x)\colon\Omega_i\subseteq\bbR^{m\times m}\to\bbR$ are called coefficients and $\mb{f}(x)\colon\bbR\to\bbR^m$ is called forcing term.
\sssc{Linear differential operator}
A linear differential operator is a polynomial of the differential operators $D$, i.e., $\dv{}{x}$, and its order is the degree of the polynomial.
\sssc{Linearity}
...
\sssc{Homogeneity}
A linear ODE is called a linear homogeneous ODE iff the forcing term is 0 for all $x$; otherwise, it is called a linear non-homogeneous ODE.
\sssc{Complementary or associated (homogeneous) equation, natural, complementary, or homogeneous solution or function, and particularly solution}
Given an $n$th-order linear non-homogeneous ODE
...
If $y_1(x)$ and $y_2(x)$ are both solutions of a linear homogeneous ODE on an interval, then $c_1y_1(x)+c_2y_2(x)$ is also a solution of the ODE on the interval for any constants $c_1,c_2$.
\sssc{Linear independence}
$n$ functions of the same domain and codomain $y_1(x),y_2(x),\ldots y_n(x)$ are said to be linearly independent iff
\[\sum_{i=1}^nc_iy_i(x)=0\implies\forall i\in[1,n]\cap\bbZ\colon c_i=0\]
\sssc{System of linear first-order ODEs or linear first-order system}
...
For $n$ $(n-1)$ times differentiable functions $y_1,y_2,\ldots,y_n$ on an interval $I$, for any $x_0\in I$, $W(y_1,y_2,\ldots,y_n)(x_0)\ne 0$ implies $y_1,y_2,\ldots,y_n$ are linearly independent.
\bpr
Suppose
...
Since $W(y_1,y_2,\ldots,y_n)(x_0)=\det(\Omega)\ne0$, $\Omega$ is invertible.
\[\begin{bmatrix}c_1\\c_2\\\vdots\\c_n\end{bmatrix}=\Omega^{-1}\begin{bmatrix}0\\0\\\vdots\\0\end{bmatrix}=0\]
\epr
\sssc{Non-zero Wrońskian of solutions of linear homogeneous equation iff linear independence of them}
...
\epr
\sssc{Solutions of linear homogeneous ODE as vector space}
The general solutions of an $n$th-order linear homogeneous ODE is a vector space of dimension $n$. A basis of it is called a fundamental set of solutions.

If $y_1(x),y_2(x),\ldots y_n(x)$ are $n$ linearly independent solutions of an $n$th-order linear homogeneous ODE on an interval, then they are a basis of the vector space of the general solutions of the ODE on the interval, and $\sum_{i=1}^nc_iy_i(x)$ is the family of general solutions of the ODE on the interval with $n$ parameter $c_1,c_2,\ldots c_n$.
\sssc{Solutions of linear non-homogeneous ODE as affine space}
The general solutions of a linear non-homogeneous ODE is an affine space whose associated vector space is the vector space of the solutions of its complementary equation.
\sssc{Annihilator}
...
For any solution $f(t)$ of any linear non-homogeneous ODE of one variable $t$, there exists some $M,T\geq 0$ and $\alpha\in\bbR$ such that for all $t>T$ $\abs{f(t)}\leq e^{M\alpha t}$, and the smallest such $\alpha$ is the largest real part among all roots of its characteristic equation.
...
\end{proof}
\ssc{Method of undetermined coefficients (MUC) for constant coefficient linear non-homogeneous ODE}
\sssc{Superposition approach}
Consider an $n$th-order constant coefficient linear non-homogeneous ODE
...
\item Solve for $C_{jk}$ such that $y_j$ is a particular solution of the constant coefficient linear non-homogeneous ODE
...
\item Solve for $C_{jk},D_{jk}$ such that $y_j$ is a particular solution of the constant coefficient linear non-homogeneous ODE
...
\ea\]
\epr
\item Find a lowest-order annihilator of $g(x)$, $L_1$, and apply it on both sides to obtain a linear homogeneous ODE
...
TODO
\ssc{Variation of parameters or variation of constants method of solution of linear non-homogeneous equation}
\sssc{Method of solution}
Given an $n$th-order linear non-homogeneous ODE
\[y^{(n)}+\sum_{i=0}^{n-1}p_i(x)y^{(i)}=q(x),\]
...
\[y=y_c+y_p.\]
\een
\ssc{Power series solutions and Frobenius method for second-order linear homogeneous ODE}
\sssc{Type of Points}
...
\end{proof}
\sssc{Example for complex roots: TODO}
TODO
...
\sssc{Existence of a unique solution of linear ODE}
...
TODO: BVP can have many, one, or no solution
TODO: end
...
Applying unilateral Laplace transform \[Y(s)=X(s)\cdot H(s),\]
...
where $H(s)$ is called transfer function, and its absolute value is called gain (增益), or attenuation (衰減量) if $<1$.
\sssc{Transient response}
A response component that converges to an equilibrium as $t$ gets large.
\sssc{Time constant}
...
We assume the system is stable, that is, all poles $a_1,\ldots,a_n$ of $Y(s)$ except $s=i\omega$ are in the left half-plane.
\[y(t)=\frac{1}{2\pi i}\lim_{T\to\infty}\int_{-iT}^{iT}\frac{H(s)e^{st}}{s-i\omega}\dd{s}\]
...
The other residues contribute multiple of $e^{-a_kt}$, which vanish as $t\to\infty$.

Thus, the steady response is
\[H(i\omega)e^{i\omega t}\]
$H(i\omega)$ is called frequency response, $\abs{H(i\omega)}$ is called magnitude response, and $\Arg(H(i\omega))$ is called phase (angle) response.
...
\[\lim_{\omega\to 0}\abs{H(i\omega)}=K\in\bbR_{>0}\]
and
...
and
\[\lim_{\omega\to\infty}\abs{H(i\omega)}=K\in\bbR_{>0},\]
where $K$ is called high-pass gain. The filter is called prototype or canonical if $K=1$.
\sssc{Band-pass (BP) filter (BPF)}
A continuous-time linear time-invariant system is a band-pass filter iff $H(i\omega)$ where $i=\sqrt{-1}$ as a function of a real variable $\omega$ is such that
\[\lim_{\omega\to 0}\abs{H(i\omega)}=0\]
\[\lim_{\omega\to\infty}\abs{H(i\omega)}=0\]
and
\[\max_{\omega}\abs{H(i\omega)}=K\in\bbR_{>0},\]
where $K$ is called band-pass gain. The filter is called prototype or canonical if $K=1$.
\sssc{Pole–zero plot or pole–zero constellation}
