...
\[\lim_{\omega\to 0}\abs{H(i\omega)}=0\]
\[\lim_{\omega\to\infty}\abs{H(i\omega)}=0\]
\[\arg_{\omega}\left\network.captive-portal-service.enabled{\abs{H(i\omega)}=\frac{1}{\sqrt{2}}\right\}=\omega_L,\omega_H\]
...
\ssc{First-order transfer function factors}
\sssc{Ramp function}
\[H_r(s;W)\coloneq\frac{s}{W},\quad W\in\bbR_{>0}\]
\[H_r(i\omega;W)=\frac{i\omega}{W}\]
\[\abs{H_r(i\omega;W)}=\frac{\omega}{W}\]
\[g_r(\omega;W)=20\log_{10}(\omega)-20\log_{10}(W)=20\log_{10}(2)\log_2(\omega)-20\log_{10}(W)\]
...
\[H_{HP}(s;W)\coloneq\frac{s}{s+W},\quad W\in\bbR_{>0}\]
\[H_{HP}(i\omega;W)=\frac{i\omega}{i\omega+W}\]
\[\abs{H_{HP}(i\omega;W)}=\frac{\omega}{\sqrt{\omega^2+W^2}}\]
\[g_{HP}(\omega;W)=20\log_{10}(\omega)-10\log_{10}(\omega^2+W^2)\]
\[\theta_{HP}(\omega;W)=\frac{\pi}{2}-\arg(i\omega+W)\]
Asymptotic Bode plot and cutoff frequency:
\bit
\item For $\omega\ll W$:
\[g_{HP}(\omega;W)\approx 20\log_{10}(\omega)-20\log_{10}(W),\]
which has slope $20$ dB/dec, $y$ intercept $-20\log_{10}(W)$, and $x$ intercept $\log_{10}(W)$.
\[\theta_{HP}(\omega;W)\approx\frac{\pi}{2}.\]
\item For $\omega=W$:
\[g_{HP}(W;W)=20\log_{10}(W)-10\log_{10}(2W^2)=-10\log_{10}(2).\]
Thus the cutoff frequency is $\omega_c=W$.
\[\theta_{HP}(W;W)=\frac{\pi}{2}-\frac{\pi}{4}=\frac{\pi}{4}.\]
\item For $\omega\gg W$:
\[g_{HP}(\omega;W)=0,\]
which is horizontal equilibrium $g_{HP}=0$, i.e., $\abs{H(i\omega)}=1$. 
\[\theta_{HP}(\omega;W)\approx\frac{\pi}{2}-\frac{\pi}{2}=0.\]
\eit
