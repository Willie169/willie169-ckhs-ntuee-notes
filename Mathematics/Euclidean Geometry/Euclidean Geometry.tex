...
\date{\temtoday}
\titletocdoc
\sct{Euclidean Geometry (歐幾里德幾何學)}
...
\ssc{錐體}
\sssc{中心}
錐體中心在其中心軸上底面上方$\frac{1}{4}$高處。
\begin{proof}
令底面面積$A$，高$L$：
\[\frac{\int_0^LlA\qty(\frac{l}{L})^2\dd{l}}{\int_0^LA\qty(\frac{l}{L})^2\dd{l}}=\frac{\int_0^Ll^3\dd{l}}{\int_0^Ll^2\dd{l}}=\frac{3L}{4}\]
\end{proof}
...
\sssc{Polar coordinate system (極座標系)}
...
\ssc{正整數維空間}
$\bbR^n,\quad n\in\mathbb{N}$
\sssc{仿射子空間一般式}
$\mathbb{R}^n$中，一$n-1$維仿射子空間可以表示成：
\[\mathbf{n}\cdot\mathbf{x}+c=0\]
\sssc{仿射子空間向量式（vector equation）}
$\mathbb{R}^n$中，一$n-1$維仿射子空間可以表示成：
\[\mathbf{n}\cdot\qty(\mathbf{x}-\mb{x}_0)=0\]
\sssc{仿射子空間純量式（scalar equation）}
$\mathbb{R}^n$中，一$n-1$維仿射子空間可以表示成：
\[\sum_{k=1}^na_k\qty(x_k-x_{k_0})=0\]
\sssc{仿射子空間線性式（linear equation）}
$\mathbb{R}^n$中，一$n-1$維仿射子空間可以表示成：
\[\sum_{k=1}^na_kx_k+c=0\]
\sssc{仿射子空間截距式}
...
\sssc{直線向量式（Vector equation）}
$\mathbb{R}^n$中，一直線可表示成：
\[\mathbf{x}=\mathbf{a}+\mathbf{v}t,\quad t\in\mathbb{R}\]
稱$\mathbf{v}$為此直線之方向向量（direction vector）, and each component of it is called direction number。
\sssc{直線參數式（Parametric equation）}
$\mathbb{R}^n$中，一直線可表示成：
\[\bcs
x_1&=a_1+v_1t\\
x_2&=a_2+v_2t\\
&\vdots
x_n&=a_n+v_nt\\
\ecs,\quad t\in\mathbb{R}\]
\sssc{直線對稱式（symmetric equation）/比例式}
$\mathbb{R}^n$中，直線：
...
相同，稱前一種表達方法為（對稱）比例式。
\sssc{Line segment}
The line segment between $\mb{r}_0$ and $\mb{r}_1$ is given by the vector equation
\[\mb{r}_0+(\mb{r}_1-\mb{r}_0)t,\quad t\in[0,1].\]
\sssc{點與仿射子空間關係}
