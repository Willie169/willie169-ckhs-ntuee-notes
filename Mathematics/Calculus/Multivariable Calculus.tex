...
\item Positive orientation: For boundary $\ell$ of a compact surface $S$, $\dd{\boldsymbol{\ell}}$ is used as a shorthand for $\hat{T}\dd{S}$, where $\hat{T}$ is the unit tangent vector at almost each point on $\ell$ transverses counterclockwise around $S$, with respect to the orientation of $S$. In $\bbR^2$, $S$ is oriented upward, that is, $\dd{\boldsymbol{\ell}}=\qty(\dv{y}{\ell}-\dv{x}{\ell})\dd{\ell}$ where $\ell$ is arc length.
\item Outward orientation: For boundary $S$ of a region in $\bbR^3$, $\dd{\mb{S}}$ is used as a shorthand for $\hat{n}\dd{S}$, where $\hat{n}$ is the outward pointing unit normal vector at almost each point on $S$.
\end{itemize}
