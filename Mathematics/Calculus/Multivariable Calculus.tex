...
\item Curl (旋度) (three dimension) operator: $\nabla \times$ or $\operatorname{curl}$
...
\[
\nabla f = \pdv{f}{(x_1,x_2,\ldots,x_n)} = \begin{pmatrix}\qty(\pdv{f}{x_1}) & \qty(\pdv{f}{x_2}) & \dots  & \qty(\pdv{f}{x_n})\end{pmatrix}
\]
...
\]
And the gradient of an $m$-dimensional vector field $\mathbf{F}$, also called the Jacobian matrix (雅可比矩陣) of $\mb{F}$, is a matrix whose $( i,j )$th entry is
\[(J(\mb{F}))_{ij}=\frac{\partial F_i}{\partial x_j},\]
The determinant of Jacobian matrix is called Jacobian determinant, or Jacobian.

The Jacobian matrix or sometimes Jacobian is also denoted as
...
T_1 & T_2 & T_3
\end{pmatrix}\\
=&\qty(\pdv{T_3}{x_2}-\pdv{T_2}{x_3},\pdv{T_1}{x_3}-\pdv{T_3}{x_1},\pdv{T_2}{x_1}-\pdv{T_1}{x_2})
\ea\]
The curl of a three-dimensional vector field is a three-dimensional vector field.
\subsubsection{Curl (two dimension)}
In two dimensions, $\pdv{T_2}{x_1}-\pdv{T_1}{x_2}$ is called curl.
