...
\sssc{Independence of path}
We say $\int_Cf\cdot\dd{\mb{x}}$ is independent of path if for any path $D$ with the same initial and terminal points as $C$, $\int_Cf\cdot\dd{\mb{x}}=\int_Df\cdot\dd{\mb{x}}$.
\subsubsection{Conservative field (保守場)}
A field $f$ on a Euclidean tensor space is conservative on a subset $U$ iff for any paths $C,D\in U$  with the same initial and terminal points,
\[\int_Cf(\mathbf{x})\cdot\mathrm{d}\mathbf{x}=\int_Df(\mathbf{x})\cdot\mathrm{d}\mathbf{x}\]
equivalently, for any closed path $C\in U$, 
...
\eit
\sssc{Gradient}
If $f\colon\bbR^n\to\bbR$ is a scalar field with continuous first partial derivatives, then its gradient $\nabla f$ is conservative.

If $F\colon\bbR^n\to\bbR^n$ is a continuous conservative field on a connected open set $U$, then there exists a scalar field $f\colon\bbR^n\to\bbR$ with continuous first partial derivatives whose gradient $\nabla f$ equals $F$ on $U$.
