...
Suppose $r$ is a piecewise $C^1$ oriented curve with initial point $a$ and terminal point $b$. If $\mb{F}$ is a tensor field differentiable on a neighborhood of $C$, then
\[\int_r(\nabla\mathbf{F})\cdot\dd{\mb{r}}=\mb{F}(b)-\mb{F}(a).\]
\bpr
Let $r$ be $\mb{r}(t),\quad t\in[\alpha,\beta]$, where $\mb{r}(\alpha)=a$ and $\mb{r}(\beta)=b$.
\[\int_r(\nabla\mathbf{F})\cdot\dd{\mb{r}}=\int_{\alpha}^{\beta}(\nabla\mb{F})(\mb{r}(t))\mb{r}'(t)\dd{t}\]
Let $\mb{F}=(F_1,\ldots,F_n)$. For each component $F_i(\mb{r}(t))$, by multivariable chain rule:
...
\[\nabla\times\mb{F}=\qty(\pdv{F_3}{x_2}-\pdv{F_2}{x_3},\pdv{F_1}{x_3}-\pdv{F_3}{x_2},\pdv{F_2}{x_1}-\pdv{F_1}{x_2})\]
PLACEHOLDER
\epr
