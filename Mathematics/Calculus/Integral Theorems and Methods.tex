...
Suppose $C\colon\mb{r}(t),\quad t\in[a,b]$ is a piecewise $C^1$, oriented curve. If $\mb{F}$ is a scalar or vector field differentiable on a neighborhood of $C$, then
\[\int_C(\nabla\mathbf{F})\cdot\dd{\mb{r}}=\mb{F}(\mb{r}(b))-\mb{F}(\mb{r}(a)),\]
where $\nabla\mathbf{F}$ denotes gradient for scalar field $\mb{F}$ and Jacobian matrix for vector field $\mb{F}$.
\bpr
\[\int_C(\nabla\mathbf{F})\cdot\dd{\mb{r}}=\int_a^b(\nabla\mb{F})(\mb{r}(t))\cdot\mb{r}'(t)\dd{t}\]
Let $\mb{F}=(F_1,\ldots,F_n)$. For each component $F_i(\mb{r}(t))$,
\[\dv{}{t}F_i(\mb{r}(t))=\nabla F_i(\mb{r}(t))\mb{r}_i'(t)\]
Thus
\[(\nabla\mb{F})(\mb{r}(t))\cdot\mb{r}'(t)=\dv{}{t}\mb{F}(\mb{r}(t))\]
By FTC part 2,
...
For a twice differentiable, three-dimensional field $\mb{T}$, $\nabla\cdot(\nabla\times\mb{T})=0.\]
\bpr
\[\ba
