...
For a point $a$ in a topological space and real-valued functions $f$ and $g$ defined on some neighborhood of $a$ except possibly at $a$, iff there exists some neighborhood $I$ of $a$ and positive real numbers $M,M_1,M_2$ such that
\bit
\item $\abs{f(x)}\le M\abs{g(x)}$, then $f(x)\in O(g(x))$ as $x\to a$.
\item $\abs{f(x)}\ge M\abs{g(x)}$, then $f(x)\in \Omega(g(x))$ as $x\to a$.
\item $M_1\abs{g(x)}\le\abs{f(x)}\le M_2\abs{g(x)}$, then $f(x)\in \Theta(g(x))$ as $x\to a$.
\item $\lim_{x\to a}\abs{\frac{f(x)}{g(x)}}=0$, then $f(x)\in o(g(x))$ as $x\to a$.
\item $\lim_{x\to a}\abs{\frac{g(x)}{f(x)}}=0$, then $f(x)\in \omega(g(x))$ as $x\to a$.
\eit
For real-valued functions $f$ and $g$ defined on some interval $[a,\infty)$, if there exists some real number $T$ and positive real numbers $M,M_1,M_2$ such that for all $x\geq T$,
\bit
\item $\abs{f(x)}\le M\abs{g(x)}$, then $f(x)\in O(g(x))$ as $x\to\infty$ and $\abs{g(x)}$ is an asymptotic upper bound of $\abs{f(x)}$.
\item $\abs{f(x)}\ge M\abs{g(x)}$, then $f(x)\in \Omega(g(x))$ as $x\to\infty$ and $\abs{g(x)}$ is an asymptotic lower bound of $\abs{f(x)}$.
\item $M_1\abs{g(x)}\le\abs{f(x)}\le M_2\abs{g(x)}$, then $f(x)\in \Theta(g(x))$ as $x\to\infty$.
\item $\lim_{x\to\infty}\abs{\frac{f(x)}{g(x)}}=0$, then $f(x)\in o(g(x))$ as $x\to\infty$.
\item $\lim_{x\to\infty}\abs{\frac{g(x)}{f(x)}}=0$, then $f(x)\in \omega(g(x))$ as $x\to\infty$.
\eit
...
\bpr
Counterexample
\[\frac{f(x)}{g(x)}=\bcs 1,\quad & x\in\bbQ\\\abs{x-a},\quad & x\notin\bbQ\ecs\]
