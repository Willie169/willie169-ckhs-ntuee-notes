...
If $f$ is differentiable at all point in $I\subeteq U$, we say $f$ is differentiable on $I$; if $f$ is differentiable on $U$, we say $f$ is differentiable. If $D^{(n-1)}f$ exists and is differentiable at all point in $I\subeteq U$, we say $f$ is $n$-times differentiable on $I$; if $D^{(n-1)}f$ exists and is differentiable on $U$, we say $f$ is $n$-times differentiable.
...
\end{proof}
\sssc{General form of L'Hôpital's rule (羅必達法則) or Bernoulli's rule}
...
\[f'(c)=0.\]
\end{proof}
\sssc{Mean Value Theorem (MVT) (均值定理)}
...
\end{proof}
\sssc{Concavity (凹性)}
Let $V$ be a vector space, $f\colon J\subseteq V\to\bbR$ be a function, and $I\subseteq J$ be a convex set.
\bit
\item If for any $x,y\in I$ and $\alpha\in(0,1)$, $f((1-\alpha)x+\alpha y)>(1-\alpha)f(x)+\alpha f(y)$, often called "$f$ lies strictly above all its secant lines on $I$", then $f$ is called to be strictly concave upwards or strictly convex downwards.
\item If for any $x,y\in I$ and $\alpha\in(0,1)$, $f((1-\alpha)x+\alpha y)\geq (1-\alpha)f(x)+\alpha f(y)$, often called "$f$ lies weakly above all of its tangent lines on $I$", then $f$ is called to be weakly concave upwards or weakly convex downwards.
\item If for any $x,y\in I$ and $\alpha\in(0,1)$, $f((1-\alpha)x+\alpha y)<(1-\alpha)f(x)+\alpha f(y)$, often called "$f$ lies strictly below all of its tangent lines on $I$", then $f$ is called to be strictly concave (downwards) or strictly convex (upwards).
\item If for any $x,y\in I$ and $\alpha\in(0,1)$, $f((1-\alpha)x+\alpha y)\leq (1-\alpha)f(x)+\alpha f(y)$, often called "$f$ lies weakly below all of its tangent lines on $I$", then $f$ is called to be weakly concave (downwards) or weakly convex (upwards).
\eit
Concave upward (CU) or convex downward and concave downward (CD) are define to be either weakly or strictly concave (upward) and concave downward in different contexts, here we define them to be strictly concave (upward) and concave downward.
...
\bit
\item $f'$ is strictly increasing on $I$ iff $f$ is strictly concave upward on $I$.
\item $f'$ is non-decreasing on $I$ iff $f$ is weakly concave upward on $I$.
\item $f'$ is strictly decreasing on $I$ iff $f$ is strictly concave downward on $I$.
\item $f'$ is non-increasing on $I$ iff $f$ is weakly concave downward on $I$.
\eit
\begin{proof}
    For the first statement, we want to show that for any $x,y\in I$ and $\alpha\in(0,1)$, $f((1-\alpha)x+\alpha y)>(1-\alpha)f(x)+\alpha f(y)$.
    ...
    \[(c-a)f(b)<(b-a)f(c)+(c-b)f(a).\]
    \[f(b)<\frac{b-a}{c-a}f(c)+\frac{c-b}{c-a}f(a).\]
    Set
    \[\alpha=\frac{b-a}{c-a}\in(0,1).\]
    Then
    \[b=(1-\alpha)a+\alpha c.\]
    Thus the inequality becomes
    \[f\qty((1-\alpha)a+\alpha c)<(1-\alpha)f(a)+\alpha f(c).\]
...
\bit
\item If $f''>0$ for any $x\in I$, then the graph of $f$ is concave upward on $I$.
\item If $f''<0$ for any $x\in I$, then the graph of $f$ is concave downward on $I$.
\eit
\begin{proof}
