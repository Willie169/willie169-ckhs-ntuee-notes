...
For a point $a$ in a topological space and non-negative functions $f$ and $g$ defined on some neighborhood of $a$ except possibly at $a$, iff there exists some neighborhood $I$ of $a$ and positive real numbers $M,M_1,M_2$ such that
\bit
\item $f(x)\le Mg(x)$, then $f(x)=O(g(x))$ as $x\to a$.
\item $f(x)\ge Mg(x)$, then $f(x)=\Omega(g(x))$ as $x\to a$.
\item $M_1g(x)\le f(x)\le M_2g(x)$, then $f(x)=\Theta(g(x))$ as $x\to a$.
\item $\lim_{x\to a}\frac{f(x)}{g(x)}=0$, then $f(x)=o(g(x))$ as $x\to a$.
\item $\lim_{x\to a}\frac{g(x)}{f(x)}=0$, then $f(x)=\omega(g(x))$ as $x\to a$.
\eit
For non-negative functions $f$ and $g$ defined on some interval $[a,\infty)$, if there exists some real number $T$ and positive real numbers $M,M_1,M_2$ such that for all $x\geq T$,
\bit
\item $f(x)\le Mg(x)$, then $f(x)=O(g(x))$ as $x\to\infty$.
\item $f(x)\ge Mg(x)$, then $f(x)=\Omega(g(x))$ as $x\to\infty$.
\item $M_1g(x)\le f(x)\le M_2g(x)$, then $f(x)=\Theta(g(x))$ as $x\to\infty$.
\item $\lim_{x\to\infty}\frac{f(x)}{g(x)}=0$, then $f(x)=o(g(x))$ as $x\to\infty$.
\item $\lim_{x\to\infty}\frac{g(x)}{f(x)}=0$, then $f(x)=\omega(g(x))$ as $x\to\infty$.
\eit
