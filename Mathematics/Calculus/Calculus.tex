...
\[(\mb{F}\cdot\nabla)\mb{A}=D_{\mb{F}}\mb{A}=\lim_{h\to 0}\frac{\mb{A}(\mb{x}+h\mb{F}(\mb{x}))-\mb{A}(\mb{x})}{h}=\sum_{i=1}^nF_i\pdv{\mb{A}}{x_i}=\mb{F}\cdot\nabla\mb{A}.\]
In $D_{\mb{F}}\mb{A}$ notation, $\mb{F}$ is often assumed constant unit vector.
