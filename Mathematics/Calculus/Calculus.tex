...
\ssc{Exponential Function (指數函數)}
\sssc{Natural exponential function}
...
\sssc{Intermediate value theorem for real functions}
...
\sssc{Intermediate value theorem for functions between topological spaces}
...
\sssc{Uniform continuity for functions between metric spaces}
...
\sssc{Relationships}
Let $(X,d_1)$ and $(Y,d_2)$ be metric spaces, and $f\colon O\subseteq X\to Y$ be a function. $f$ is uniform continuous and locally Lipschitz if it is Lipschitz continuous. $f$ is uniform continuous on $D\subseteq O$ if it is Lipschitz continuous on $D\subseteq O$.
...
Suppose a dependent variable $y$ represents a function $f$ of an independent variable vector $\mb{x}=(x_1,x_2,\ldotsx_n)$, that is,
\[y=f(\mb{x}).\]
...
Given a function $f$ of an independent variable vector $\mb{x}=(x_1,x_2,\ldotsx_n)$, that is,
\[f(\mb{x}).\]
Then, the partial derivative of the function $f$ with respect to $x_i$ can be written as
...
\ssc{Derivatives of Functions with Real Domain}
...
\sssc{Piecewise smoothness for real functions}
Let $f\colon D\subseteq\mathbb{R}\to\mathbb{R}$ be a function. If there exists a family $\{[a_i,b_i]\mid i\in I\}$ of closed intervals such that:
\begin{itemize}
\item for every closed interval $V$, the set $\{i\in I\mid V\cap [a_i,b_i]\neq\varnothing\}$ is finite (some sources require instead that $n(I)$ is finite, which is stronger),
\item $D\subseteq\bigcup_{i\in I}[a_i,b_i]$,
...
\sssc{Piecewise smoothness for functions between topological spaces}
Let $X$ and $Y$ be topological spaces, and $f\colon D\subseteq X\to Y$ be a function. If there exists a locally finite (some sources require instead finite, which is stronger) family $\{U_i\mid i\in I\}$ of closed subsets of $X$ such that:
\begin{itemize}
\item $D\subseteq\bigcup_{i\in I}U_i$, and
...
\ssc{Real analyticity (實解析性) for real-codomain functions}
...
\ssc{Points in graph}
\sssc{Critical point (臨界點), equilibrium point (平衡點), or stationary point (駐點)}
...
\ssc{Lebesgue-Vitali theorem (of characterization of the Riemann integrable functions)}
...
For $s>\alpha$, since $f(t)$ is piecewise continuous, there exist $t_0=0,t_1,\ldotst_k\in\mathbb{R}$ such that:
\begin{itemize}
\item the integral
\[\int_{t_i}^{t_{i+1}} |f(t)| e^{-st}\dd{t}$\]
exists and is finite for every $i\in\mathbb{Z}\land 0\leq i\leq k-1$, and that
\item the family $\{[t_0,t_1],[t_1,t_2],\ldots[t_{k-1},t_k]$ is locally finite, and that
...
where $t,T\in\mathbb{R}$, $y=(y_1, y_2,\ldots y_n)\in\mathbb{R}^n$, $Y=(Y_1, Y_2,\ldots Y_n)\in\mathbb{R}^n$, and constant $a,b_i\in\mathbb{R}_{>0}$.
...
Let
\[D=\{(t,y)\mid|t-T|\leq a\land\forall\tx{\ integer\ }1\leq i\leq n\colon |y_i-Y_i|\leq b_i\}\]
be a rectangle in $\mathbb{R}\times\mathbb{R}^n$ where $t,T\in\mathbb{R}$, $y=(y_1, y_2,\ldots y_n)\in\mathbb{R}^n$, $Y=(Y_1, Y_2,\ldots Y_n)\in\mathbb{R}^n$, and constant $a,b_i\in\mathbb{R}_{>0}$, and $f\colon D\to \mathbb {R}$ be a function that is:
\bit
...
\sssc{Direction field or slope field}
...
\ssc{Autonomous first-order ODE}
\sssc{Autonomous first-order ODE}
An autonomous first-order ODE is an ODE in the form 
\[\dv{y}{x}=f(y),\quad y\in D\subseteq\mathbb{R}^n\land f\colon D\to\mathbb{R}^n.\]
\sssc{Critical point (臨界點), equilibrium point (平衡點), or stationary point (駐點)}
For a system of autonomous first-order ODEs $\dv{y}{x}=f(y)$, we say that a point $c$ in the domain of $f$ is a critical point, equilibrium point, or stationary point if $f(c)=0$.
...
\sct{Systems theory (系統理論)}
\ssc{Dynamical systems theory (動力系統理論 or 動態系統理論)}
\sssc{Dynamical system (動力系統 or 動態系統)}
A dynamical system is a tuple $(T, X, \Phi)$ where $T$ is a monoid, written additively and with the independent variable $t$ in it called evolution parameter (演化參數), $X$ is a non-empty set, called the phase space (相空間) or state space (狀態空間) with the independent variable $x$ in it called phase (相) or state (狀態), and $\Phi$ is a function, called the evolution function (演化函數) or flow,
...
An orbit which consists of a single point is called constant orbit; a non-constant orbit is called closed or periodic if there exists $t\neq 0\in I(x)$ such that $\Phi (t,x)=x$ for any $x\in X$.
\sssc{Velocity field (速度場)}
The velocity field of a dynamical system $(T, X, \Phi)$ with
\[\Phi(t,x)\colon U\subseteq(T\times X)\to X\]
is the function
\[f(x)=\pdv{\Phi}{t}\big\vert_{t=0}.\]
\sssc{Equilibrium, fixed point, or fixpoint}
An equilibrium, fixed point, or fixpoint of a dynamical system $(T, X, \Phi)$ is a point $x^*\in X$ such that the velocity field $f$ vanishes there, that is, $f(x^*)=0$.
...
For $X=\mathbb{R}^n$ and continuous velocity field $f$, one usually plots the space $X=\mathbb{R}^n$, marks equilibria, and draw arrows at each point $x$ in a grid indicating $f(x)$ at it.

Specifically, a phase portrait for $X=\mathbb{R}$ is also called a phase line.
\sssc{Systems of autonomous first-order ODEs as dynamical systems}
...
Then the dynamical system represented by it has phase space $\mathbb{R}^n$, evolution parameter $t$, phase or state $x$, velocity field $f$, and evlotion function $\Phi\colon U\to\mathbb{R}^n;\Phi(t,x_0)=x(t;x_0)$.
\ssc{Control theory (控制理論)}
\sssc{Proportional–integral–derivative (PID) controller (PID 控制器){{{
