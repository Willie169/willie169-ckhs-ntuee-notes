...
\subsection{Continuity (連續性) of Real-valued Function}
\sssc{Definition}
\begin{itemize}
\item If $\exists \lim_{x\to a} f(x)$ and $\lim_{x\to a} f(x)=f(a)$, we say $f(x)$ is continuous (連續的) at $a$; otherwise, we say $f(x)$ is discontinuous (不連續的) at $a$.
