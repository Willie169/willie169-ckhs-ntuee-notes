...
\bit
\item $f$ is called non-decreasing, monotone increasing, or monotonically increasing on an interval $I\subseteq X$ if
\[\forall a,b\in I\colon a<b\implies f(a)\leq f(b).\]
\item $f$ is called non-decreasing, monotone increasing, or monotonically increasing if
\[\forall a,b\in X\colon a<b\implies f(a)\leq f(b).\]
\item $f$ is called non-increasing, monotone decreasing, or monotonically decreasing on an interval $I\subseteq X$ if
\[\forall a,b\in I\colon a<b\implies f(a)\geq f(b).\]
\item $f$ is called non-increasing, monotone decreasing, or monotonically decreasing if
...
$f(-x)$, called reflection of $f$ (through $x=0$), is also denoted as $\tilde{f}$ or $\hat{R}f$, where $\tilde{\phantom{f}}$ or $\hat{R}$ is reflection operator.
...
\subsection{Inverse of Composition}
...
Some sources define phase as $\varphi$ and/or phase angle as $\varphi$, which is not used here.

For two periodic functions $f(t)$ and $g(t)$ such that $g(t)=Af(t+\tau)$ for some real number $A,\tau$, the phase shift (相位移) $\Delta\varphi$ and phase angle shift (相位角移) $\Delta\varphi$ of $g(t)$ relative to $f(t)$ are defined as
\[\Delta\varphi=\frac{\tau}{T}-\left\lfloor\frac{\tau}{T}\right\rfloor\]
\[\Delta\varphi=2\pi(\Delta\varphi).\]
$g(t)$ is called to lag $f(t)$ by phase $\tau$ or phase angle $2\pi(\Delta\varphi)$. $f(t)$ is called to lead $g(t)$ by phase $\tau$ or phase angle $2\pi(\Delta\varphi)$. The phase difference (相位差) and phase angle difference (相位角差) between them are defined as the magnitude of $\Delta\varphi$ and $\Delta\varphi$ respectively.
...
\subsubsection{Extended real number system}
...
\subsubsection{Notation and reading}
...
If such $M$ exists for some $L\in\bbR$, we say the limit exists and the sequence converges (收斂) to $L$; otherwise, we say the limit does not exist (DNE) and the sequence diverges (發散).
...
\subsubsection{Radius and interval of convergence for power series}
For a power series $S_n(x)=\sum_{n=0}^{\infty}c_n(x-a)^n$, the interval of $x$ such that $S_n(x)$ is convergent is called interval of convergence, and if $S_n(x)$ is convergent for $|x-a|<R$ and divergent for $|x-a|>R$, for some $R\in\bar{\bbR}_{>0}$, then $R$ is called radius of convergence.
...
If such $a_0$ exists for some $L\in\bbR$, we say the limit exists or the net converges to $L$; otherwise, we say the limit does not exist (DNE).
...
If such $\delta$ exist for some $L\in\bbR$, we say the limit exists; otherwise, we say the limit does not exist (DNE).
...
If such $M$ exists for some $L\in\bbR$, we say the limit exists and the function converges to $L$ as \(x\) becomes arbitrarily large; otherwise, we say the limit does not exist (DNE) and the function diverges as \(x\) becomes arbitrarily large.
...
If such $M$ exists for some $L\in\bbR$, we say the limit exists and the function converges to $L$ as \(x\) becomes arbitrarily small; otherwise, we say the limit does not exist (DNE) and the function diverges as \(x\) becomes arbitrarily small.
...
If such $U$ exist for some $L\in\bbR$, we say the limit exists; otherwise, we say the limit does not exist (DNE).
...
\subsection{Big O, little o, theta, big Omega, and small omega notation}
For real-valued nets $f$ and $g$,
\bit
\item $\exists M>0,T\in\bbR\colon x\ge T\implies\abs{f(x)}\le M\abs{g(x)}$ iff $f(x)\in O(g(x))$ and $\abs{g(x)}$ is an asymptotic upper bound of $\abs{f(x)}$.
\item $\exists M>0,T\in\bbR\colon x\ge T\implies\abs{f(x)}\ge M\abs{g(x)}$ iff $f(x)\in \Omega(g(x))$ and $\abs{g(x)}$ is an asymptotic lower bound of $\abs{f(x)}$.
\item $\exists M_1,M_2>0,T\in\bbR\colon x\ge T\implies M_1\abs{g(x)}\le\abs{f(x)}\le M_2\abs{g(x)}$ iff $f(x)\in \Theta(g(x))$.
\item $\forall M>0\colon\exists T\in\bbR\colon x\ge T\implies\abs{f(x)}<M\abs{g(x)}$ iff $f(x)\in o(g(x))$.
\item $\forall M>0\colon\exists T\in\bbR\colon x\ge T\implies\abs{f(x)}>M\abs{g(x)}$ iff $f(x)\in \omega(g(x))$.
\eit
...
the $n$th mixed partial derivative function of $y$ or $f$ $m_{i_k}$ times with respect to $x_{i_k}$, and then $m_{i_{k-1}}$ times with respect to $x_{i_{k-1}}$, $\ldots$, and then $m_{i_1}$ times with respect to $x_{i_1}$, in which $\sum_{j=1}^km_{i_j}=n$, can be written as
...
\subsubsection{With compact support}
...
\subsubsection{Function of bounded variation (BV)}
$f$ is called of bounded variation or called a (globally) BV function over $[a,b]$ iff $V_a^b(f)$ is finite.
\subsubsection{Function of locally bounded variation}
...
\subsubsection{Space of functions of bounded variation}
...
\subsubsection{Jordan decomposition of BV function}
...
\subsection{Of real fields}
TODO
\subsection{In measure theory}
TODO
...
\[\max_i\left(x_{i+1}-x_{i}\right)\]
for every integer $i\in [0,n-1]$.
\subsubsection{Tagged partition}
...
we say that $f$ is Lebesgue integrable or absolutely integrable.
...
Let $U\subseteq\bbR$ and $u\in L^1_{\tx{loc}}(U)$. We say that $v\in L^1_{\tx{loc}}(U)$ is a weak derivative of $u$ if
\[\int_Uu\varphi'\dd{\mu}=-\int_Uv\phi\dd{\mu}\]
for all $\varphi\in C^{\infty}_c(U)$.

Let $U\subseteq\bbR^n$, $u\in L^1_{\tx{loc}}(U)$, and $\alpha$ be a muti-index. We say that $v\in L^1_{\tx{loc}}(U)$ is a $\alpha$th weak derivative of $u$ if
\[\int_UuD^{\alpha}\varphi\dd{\mu}=(-1)^{\abs{\alpha}}\int_Uv\phi\dd{\mu}\]
for all $\varphi\in C^{\infty}_c(U)$.
\subsection{Sobolev space}
For $k,p\in\bbN\cup\{\infty\}$, the Sobolev space $W^{k,p}(\bbR)$ is defined as the subset of functions $f$ in $L^p(\bbR)$ such that $f$ and its weak derivatives up to order $k$ have a finite $L^p$ norm.

The Sobolev spaces are equipped with a norm
\[\|f\|_{k,p}=\qty(\sum_{i=0}^k\norm{f^{(k)}}_p^{\pht{p}p})^{\frac{1}{p}},\quad p\in\bbN,\]
and
\[\|f\|_{k,\infty}=\max_{i=0,1,\ldots,k}\|f^{(i)}\|_{\infty}.\]
...
In $\bbR^n$, the area of the surface formed by rotating the curve $\mb{r}(t),\quad t\in[a,b]$ about a rotational axis $\mb{P}+\mb{k}u,\quad u\in\bbR$ with $\norm{\mb{k}}=1$ is
...
\ea\]
\epr
\subsubsection{Frenet-Serret formulas or Frenet–Serret theorem}
\[\dv{T}{s}=\kappa\mb{N},\]
...
\subsubsection{Tagent space (切空間)}
...
\subsubsection{Dimension of tangent space}
The dimension of $T_xM$ is the same as $M$.
TODO?
\bpr
TODO 
\epr
\subsubsection{Tagent bundle (切叢)}
...
\subsubsection{Normal space (法空間)}
...
Let $M$ be a smooth manifold of dimension $n$. A metric tensor or tensor $g_p$ at a point $p$ is a function $g_p\colon (T_pM)^2\to\bbR$ that is
\bit
\item Bilinear:
...
\item Nondegenerate: For any $X_p\ne0$ in $T_pM$, there exists $Y_p\in T_pM$ such that
\[g_p(X_p,Y_p)\ne0.\]
\eit
\subsubsection{Signature of metric tensor}
...
\subsection{Differentiable map between differentiable manifolds}
Let $\varphi\colon M\to N$ be a differentiable map between differentiable manifolds $M,N$.
\subsubsection{Differential, derivative, or total derivative}
...
\subsubsection{Regular or smooth}
...
\subsection{Parametric manifold}
\subsubsection{Parametric manifold}
...
\[\pdv{\mb{r}}{s}\times\pdv{\mb{r}}{t}\ne\mb{0}.\]

A parametric manifold whose image is differentiable is called regular or smooth if it's regular at all points.
\subsubsection{Fundamental tangent vector}
For an $m$-dimensional differentiable parametric manifold $\mb{r}(\mb{x})$, each $\pdv{\mb{r}}{x_i}$ is a fundamental tangent vector.
\subsection{Orientation of hypersurface}
...
\item Unit normal vector: $\hat{\mb{n}}$ or $\mb{n}$
\item Vector field: A function, often denoted with bold uppercase Latin letter, whose domain is a subset of $\bbR^n$ and codomain is $\bbR^m$.
\item Vector component: For a type $(r,1)$ tensor in multidimensional array representation, its $i$th component is often denoted as the same character with subscript $i$, e.g., $F_i$ or $\mb{F}_i$ for $\mb{F}$.
...
\item Directional derivative (方向導數) operator: $(\mb{F}\cdot\nabla)\mb{G}$, $D_{\mb{F}}\mb{G}$, $\pdv{\mb{G}}{\mb{F}}$, or $\nabla_{\mb{F}}\mb{G}$ for directional derivative of tensor field $\mb{G}$ along/in the direction of a vector field $\mb{F}\colon\bbR^n\to\bbR^n$. For constant vector $\mb{F}$, $D_{\mb{F}}\mb{G}$ notation is typically used and $\mb{F}$ is typically a unit vector.
...
\item Vector arc length element: $\dd{\boldsymbol{\ell}}$, which is defined as $\mb{n}\dd{\ell}$.
\item Surface element: $\dd{S}$
\item Vector surface element: $\dd{\mb{S}}$, which is defined as $\mb{n}\dd{S}$ and is only defined for orientable surface equipped with an orientation. For a vector field $\mb{F}$, $\iint_S\mb{F}\cdot\dd{\mb{S}}$ is called flux of $f$ across $S$.
...
\item Outward/Positive orientation: The boundary region $S$ of a compact region is oriented outward unless otherwise specified.
\end{itemize}
...
The Jacobian matrix of a vector field $\mb{F}\colon U\subseteq\bbR^n\to\bbR^n$ is a $m\times n$ matrix
\[J(\mb{F})=\begin{bmatrix}
...
For tensor fields $\mb{G}\colon U\subseteq\bbR^n\to X$ and $\mb{F}\colon V\subseteq\bbR^n\to\bbR^n$, the directional derivative of $\mb{G}$ in the direction of $\mb{F}$ is a function $((\mb{F}\cdot\nabla)\mb{G})\colon W\subseteq\bbR^n\to X$ defined as
...
For a type $(r,1)$ tensor field $\mb{F}$,
...
F_1 & F_2 & F_3
\end{bmatrix}\\
=&\qty(\pdv{F_3}{y}-\pdv{F_2}{z},\pdv{F_1}{z}-\pdv{F_3}{x},\pdv{F_2}{x}-\pdv{F_1}{y})
\ea\]
...
\[\nabla^2 f = \tr(H(f))=\sum_{i=1}^n\frac{\partial^2 f}{\partial x_i^{\phantom{i}2}}.\]
For $m$-dimensional $f$, let $f=\mb{F}=(F_1,\ldots,F_m)$,
\[\ba
...
\pdv{F_m}{x_1} & \cdots & \pdv{F_m}{x_n}
\end{bmatrix}\\
=&\qty(\sum_{i=1}^n\frac{\partial^2F_1}{\partial x_i^{\pht{i}2}},\ldots,\sum_{i=1}^n\frac{\partial^2F_m}{\partial x_i^{\pht{i}2}})\\
=&\qty(\nabla^2F_1,\ldots,\nabla^2F_n)
\ea\]
...
\subsubsection{Higher-order mixed partial derivatives}
...
Taylor polynomial of at $0$ is called Maclaurin polynomial.
\subsection{Graph}
\subsubsection{Level curve, level slice, or contour line}
...
\[f(\mb{x})=k.\]
\subsubsection{Contour map or contour plot}
...
\subsubsection{Conservative field (保守場) or gradient field}
An $n$-dimensional vector field $\mb{v}$ defined on an open subset $U$ of $\bbR^n$ is said to be a conservative field or gradient field on $U$ if there exists a $C^1$ scalar field $\varphi$ on $U$ such that
\[\mb{v}=\nabla\varphi\]
on $U$. $\varphi$ is called the (scalar) potential function.
...
\subsection{Type of region}
\subsubsection{Simple region}
...
\subsubsection{Type of region in two dimensions}
...
where $g_1,g_2$ are continuous functions defined on $[a,b]$.
\subsubsection{Type of region in three dimensions}
...
where $E$ is a plane $g_1,g_2$ are continuous functions defined on $E$.
\subsection{Manifold element in terms of Hausdorff measure}
\subsubsection{Differentiable parametric manifold}
...
\subsubsection{Differentiable parametric surface}
A differentiable surface $S$ in $\bbR^3$ parameterized as $\mb{r}(s,t)$ on domain $D$ has two-dimensional Hausdorff measure
\[\int_D\abs{\pdv{\mb{r}}{s}\times\pdv{\mb{r}}{t}}\dd{s}\dd{t}\]
and surface element
\[\dd{S}=\abs{\pdv{\mb{r}}{s}\times\pdv{\mb{r}}{t}}\dd{s}\dd{t}.\]
If $S$ is orientable and equipped with an orientation, the vector surface element is
\[\dd{\mb{S}}=\pm\pdv{\mb{r}}{s}\times\pdv{\mb{r}}{t}\dd{s}\dd{t},\]
where $\pm$ is such that $\pm\UnitVector(\pdv{\mb{r}}{s}\times\pdv{\mb{r}}{t})$ is in the orientation.
\subsubsection{Hypersurface as graph}
...
\subsection{Polar coordinate system (極座標系)}
\subsubsection{Definition}
...
\subsubsection{Conversion from and to Cartesian coordinate system}
...
\subsubsection{Local orthogonal basis}
...
\epr
\subsection{Cylindrical coordinate system (圓柱座標系)}
\subsubsection{ISO 31-11 Definition}
...
\subsection{Spherical coordinate system (球座標系)}
\subsubsection{Physics convention, ISO 80000-2, or inclination-azimuth convention definition}
...
\item the polar angle, zenith angle, or inclination angle $\theta\in[0,\pi]$ defined as the angle between the ray from the origin to the point and the positive half of the polar axis and undefined at the origin; and
...
\eit
\subsubsection{Mathematics convention or azimuth-inclination convention definition}
A spherical coordinate system is a three-dimensional coordinate system that specifies point positions by a polar axis (a chosen directed line) and an auxiliary axis (a reference ray) with the intersection of the two axes called the origin and the plane passing through the origin and perpendicular to the polar axis equipped with polar coordinate system with the polar axis being the auxiliary axis called the reference plane. The three spherical coordinates $(\rho,\theta,\varphi)$ are:
\bit
\item the radial distance or radius $\rho\in\bbR_{\geq 0}$ defined as the distance of the point from the origin;
\item the azimuth or azimuthal angle $\theta\in(-\pi,\pi]$ defined as the polar angle of the projection of the point on the reference plane and undefined on the polar axis (of the coordinate system instead of the reference plane); and
\item the polar angle, zenith angle, or inclination angle $\varphi\in[0,\pi]$ defined as the angle between the ray from the origin to the point and the positive half of the polar axis and undefined at the origin.
\eit
\subsubsection{Conversion from and to Cartesian coordinate system (physics convention)}
...
\[\varphi=\operatorname{atan2}(y,x)=\begin{cases}\sgn(y)\arccos(\frac{x}{\sqrt{x^2+y^2}}),\quad&y\neq 0\lor x>0\\\pi,\quad&y=0\land x<0\end{cases}.\]
\subsubsection{Conversion from and to Cartesian coordinate system (mathematics convention)}
If the polar axis and the auxiliary axis of a spherical coordinate system is the $z$ axis and the positive direction of the $x$ axis of a Cartesian coordinate system, then:
...
\[\theta=\operatorname{atan2}(y,x)=\begin{cases}\sgn(y)\arccos(\frac{x}{\sqrt{x^2+y^2}}),\quad&y\neq 0\lor x>0\\\pi,\quad&y=0\land x<0\end{cases},\]
\[\varphi=\arccos(\frac{z}{r}).\]
\subsubsection{Conversion from and to cylindrical coordinate system (physics convention)}
...
\[\varphi=\theta.\]
\subsubsection{Conversion from and to cylindrical coordinate system (mathematics convention)}
If the polar axis and the auxiliary axis of a spherical coordinate system is the main axis and the auxiliary axis of a cylindrical coordinate system, then:
...
\[\rho=\sqrt{r^2+z^2},\]
\[\theta=\theta,\]
\[\varphi=\arctan(\frac{r}{z}).\]
\subsubsection{Local orthogonal basis (physics convention)}
Radial unit vector $\hat{\mb{e}}_r$ is $(\sin\theta\cos\varphi,\sin\theta\sin\varphi,\cos\theta)$ (Cartesian). Zenith unit vector $\hat{\mb{e}}_{\theta}$ is $(\cos\theta\cos\varphi,\cos\theta\sin\varphi,-\sin\theta)$ (Cartesian). Azimuthal unit vector $\hat{\mb{e}}_{\varphi}$ is $(-\sin\varphi,\cos\varphi,0)$ (Cartesian).
\subsubsection{Local orthogonal basis (mathematics convention)}
Radial unit vector $\hat{\mb{e}}_{\rho}$ is $(\sin\varphi\cos\theta,\sin\varphi\sin\theta,\cos\varphi)$ (Cartesian). Azimuthal unit vector $\hat{\mb{e}}_{\theta}$ is $(-\sin\theta,\cos\theta,0)$ (Cartesian). Zenith unit vector $\hat{\mb{e}}_{\varphi}$ is $(\cos\varphi\cos\theta,\cos\varphi\sin\theta,-\sin\varphi)$ (Cartesian).
\subsubsection{Scale factor (physics convention)}
\[(h_r,h_{\theta},h_{\varphi})=(1,r,r\sin\theta)\]
\subsubsection{Scale factor (mathematics convention)}
\[(h_r,h_{\theta},h_{\varphi})=(1,\rho\sin\varphi,\rho)\]
\subsubsection{Line element (physics convention)}
\[\dd{\mb{r}}=\dd{r}\hat{\mb{e}}_r+r\dd{\theta}\hat{\mb{e}}_{\theta}+r\sin\theta\dd{\varphi}\hat{\mb{e}}_{\varphi}\]
\subsubsection{Line element (mathematics convention)}
\[\dd{\mb{r}}=\dd{\rho}\hat{\mb{e}}_{\rho}+\rho\sin\varphi\dd{\theta}\hat{\mb{e}}_{\theta}+\rho\dd{\varphi}\hat{\mb{e}}_{\varphi}\]
\subsubsection{Volume element (physics convention)}
\[\dd{V}=r^2\sin\theta\dd{r}\dd{\theta}\dd{\varphi}\]
\subsubsection{Volume element (mathematics convention)}
\[\dd{V}=\rho^2\sin\varphi\dd{r}\dd{\theta}\dd{\varphi}\]
\subsubsection{Surface elements (physics convention)}
$r=$ constant:
...
\subsubsection{Surface elements (mathematics convention)}
$\rho=$ constant:
\[\dd{\mb{S}}=\hat{\mb{e}}_{\rho}\rho^2\sin\varphi\dd{\theta}\dd{\varphi}\]
$\theta=$ constant:
\[\dd{\mb{S}}=\hat{\mb{e}}_{\theta}\rho\dd{\rho}\dd{\varphi}\]
$\varphi=$ constant:
\[\dd{\mb{S}}=\hat{\mb{e}}_{\varphi}\rho\sin\varphi\dd{\rho}\dd{\theta}\]
...
\item If a limit exists in $\bbR$, it is unique, and the limit inferior, limit superior, and the only cluster point are it.
\eit
\bpr
Let $(a_n)_{n\in A}$ be a real-valued net. Let
\[i_n=\inf_{k\ge n}a_k\]
\[s_n=\sup_{k\ge n}a_k\]
$i_n$ is non-decreasing, and $s_n$ is non-increasing. 

By Monotone Convergence Theorem, both
\[\liminf_{n\to\infty}a_n=\lim_{n\to\infty}i_n\]
and
\[\limsup_{n\to\infty}a_n=\lim_{n\to\infty}s_n\]
exist in $[-\infty,\infty]$.

If $\liminf_{n\to\infty}a_n=\infty$, all $a_n$ must be $\infty$, but $a_n\in\bbR$.

If $\limsup_{n\to\infty}a_n=-\infty$, all $a_n$ must be $-\infty$, but $a_n\in\bbR$.

For any fixed $n$, for all $m>n,m\in A$,
\[i_n\le a_m\le s_n\]
Suppose
\[\lim_{n \to \infty} a_n = L\in\bbR\]
For any fixed $\varepsilon>0$,
\[\exists M\in A\colon n\ge M, n\in A\implies\abs{a_n-L}<\varepsilon\implies L-\varepsilon<a_n<L+\varepsilon\]
\[(n\ge M\implies L-\varepsilon<a_n)\implies L-\varepsilon\le i_n\]
\[(n\ge M\implies a_n<L+\varepsilon)\implies s_n\le L+\varepsilon\]
Thus
\[\exists M\in A\colon n\ge M, n\in A\implies L-\varepsilon\le i_n\le a_n\le s_n\le L+\varepsilon\]
Thus
...
\[\lim_{x\to a}f(x)\]
doesn't exist.
\subsubsection{Polar coordinate method}
...
\subsubsection{Cylindrical coordinate method}
...
\subsubsection{Spherical coordinate method}
In $\bbR^3$, for any normed vector-valued function $f(x,y,z)$, if there exists a constant $L$ in the codomain of $f$ such that
\[\lim_{r\to 0}f(r\sin\theta\cos\phi,r\sin\theta\sin\phi,r\cos\theta)=L\]
then
\[\lim_{(x,y,z)\to(0,0,0)}f(x,y,z)=L.\]
\subsubsection{Generalized polar coodinate method}
...
\subsubsection{homogeneous decomposition}
...
\item If $P(\mb{a})=Q(\mb{a})=0$, use homogeneous decomposition. Let
...
\eit
\eit
\subsection{$\qty(1+\frac{a_n}{n})^n$}
For a sequence $(a_n)_{n=1}^{\infty}$, if $\lim_{n\to\infty}a_n=a\in\bbR$, then
\[\lim_{n\to\infty}\qty(1+\frac{a_n}{n})^n=e^a.\]
\bpr
Define
\[x_n=\frac{a_n}{n}\]
\[\lim_{n\to\infty}x_n=0\]
\[\lim_{x\to0}\frac{\ln(1+x)}{x}=1\]
\[\lim_{n\to\infty}n\ln(1+x_n)=\lim_{n\to\infty}a_n=a\]
\[\lim_{n\to\infty}\qty(1+\frac{a_n}{n})^n=\lim_{n\to\infty}\exp(n\ln(1+x_n))=e^a\]
\epr
\subsection{Big O, little o, theta, big Omega, and small omega notation}
\subsubsection{Limit}
If $\exists k\in\bbR\colon x\ge k\implies g(x)\ne0$,
\bit
\item $\limsup\abs{\frac{f(x)}{g(x)}}<\infty$ iff $f(x)\in O(g(x))$.
\bpr
...
\epr
\item $\liminf\abs{\frac{f(x)}{g(x)}}>0$ iff $f(x)\in \Omega(g(x))$.
\bpr
...
\epr
\item $\infty>\limsup\abs{\frac{f(x)}{g(x)}}\ge\liminf\abs{\frac{f(x)}{g(x)}}>0$ iff $f(x)\in \Theta(g(x))$.
\bpr
...
\epr
\item $\lim_{x\to\infty}\abs{\frac{f(x)}{g(x)}}=0$ iff $f(x)\in o(g(x))$.
\[\abs{f(x)}<M\abs{g(x)}\]
\[\abs{\frac{f(x)}{g(x)}}<M\]
\[\lim_{x\to\infty}\abs{\frac{f(x)}{g(x)}}=0\]
\item $\lim_{x\to\infty}\abs{\frac{f(x)}{g(x)}}=\infty$ iff $f(x)\in \omega(g(x))$.
\[\abs{f(x)}>M\abs{g(x)}\]
\[\abs{\frac{f(x)}{g(x)}}>M\]
...
$o(g(x))$, $\Theta(g(x))$, and $\omega(g(x))$ are mutually exclusive.
\bpr
Prove by contradiction.

If $f\in o(g(x))\cap\Theta(g(x))$,
...
If $f\in \omega(g(x))\cap\Theta(g(x))$,
\[\forall M>0\colon\exists T\in\bbR\colon x\ge T\implies\abs{f(x)}>M\abs{g(x)}\]
\[\exists M_1,M_2>0,U\in\bbR\colon x\ge U\implies M_1\abs{g(x)}\le\abs{f(x)}\le M_2\abs{g(x)}\]
...
\bit
\item $\Theta(g(x))$ is an equivalence class.
\bpr
Transitive:
\[\exists M_1,M_2>0,T\in\bbR\colon x\ge T\implies M_1\abs{g(x)}\le\abs{f(x)}\le M_2\abs{g(x)}\]
...
\epr
\item $O(g(x))$ is partial-ordered set with $\Theta$ as $=$.
\bpr
Transitive:
\[\exists M>0,T\in\bbR\colon x\ge T\implies\abs{f(x)}\le M\abs{g(x)}\]
...
\epr
\item $\Omega(g(x))$ is partial-ordered set with $\Theta$ as $=$.
\bpr
Similar to that of $O$.
\epr
\item $o(g(x))$ is a strictly partially-ordered set.
\bpr
Asymmetry:
...
\epr
\item $\omega(g(x))$ is a strictly partially-ordered set.
\bpr
Similar to that of $o$.
\epr
\eit
...
\subsubsection{Jacobian of inverse}
...
\subsubsection{Monotone function is differentiable a.e.}
If a real function $f$ is monotone on an interval $I$, it is differentiable a.e. on $I$.
\bpr
TODO
\epr
\subsubsection{Existence of total derivative}
...
that is,
\[\forall\varepsilon>0\colon\exists\delta>0\text{\ s.t.\ }\|h\|_V<\delta\implies\|f(a+h)-f(a)-A(h)\|_W\leq\varepsilon\|h\|_V.\]
...
\end{proof}
\subsubsection{Extremum of direction derivatives}
For each point $\mb{x}$,
...
\end{itemize}
TODO: Newton's method converges quadratically or faster if and proof
TODO: Given convergence, suppose that we want the error to be less than a given value $v$, we can take $x_n$ such that $\abs{x_n-x_{n-1}}<v$?
...
\subsubsection{TODO}
TODO
\subsection{Absolutely continuous antiderivative}
...
\subsection{Jacobian matrix of injective function}
If $\mb{F}\colon U\subseteq\bbR^n\to\bbR^n$ is injective and differentiable, and $\mb{F}^{-1}\colon V\subseteq\bbR^n\to\bbR^n$ is differentiable, then for all $\mb{x}\in U$,
\[(J(\mb{F}))(\mb{x})(J(\mb{F}^{-1}))(\mb{F}(\mb{x}))=I_n,\]
and for all $\mb{y}\in V$,
\[(J(\mb{F}))(\mb{F}^{-1}(\mb{y}))(J(\mb{F}^{-1}))(\mb{y})=I_n.\]
\bpr
\[(\mb{F}\circ\mb{F}^{-1})(\mb{x})=\mb{x},\quad\mb{x}\in V\]
\[(J(\mb{F})\circ\mb{F}^{-1}\cdot J(\mb{F}^{-1}))(\mb{x})=I_n,\quad\mb{x}\in V\]
\[(\mb{F}^{-1}\circ\mb{F})(\mb{x})=\mb{x},\quad\mb{x}\in U\]
\[(J(\mb{F}^{-1})\circ\mb{F}\cdot J(\mb{F}))(\mb{x})=I_n,\quad\mb{x}\in U\]
\epr
...
\end{proof}
\subsubsection{Multivariate form}
Let $D\subseteq\mathbb{R}^n$ be an open set, $\mathbf{F}\colon D\to\mathbb{R}^n$ be injective and differentiable on $D$ and $C^1$ a.e. on $D$, $\abs{\det(J(\mb{F}))}$ be nonzero on $D$. Then for any piecewise continuous function $f\colon\mb{F}(D)\to\mathbb{R}$ with compact support on $D$,
\[\int_Df(\mb{F}(\mb{x}))\abs{\det((J(\mb{F}))(\mb{x}))}\dd{\mb{x}}=\int_{\mb{F}(D)}f(\mb{y})\dd{\mb{y}}\]
and
\[\int f(\mb{F}(\mb{x}))\abs{\det((J(\mb{F}))(\mb{x}))}\dd{\mb{x}}=\int f(\mb{y})\dd{\mb{y}}.\]
Substitutions from RHS to LHS are also called inverse substitution.
\bpr
TODO
\epr
If also that $\mb{F}^{-1}$ is differentiable on $\mb{F}(D)$ and $C^1$ a.e. on $\mb{F}(D)$, since
\[(J(\mb{F}))(\mb{x})(J(\mb{F}^{-1}))(\mb{F}(\mb{x}))=I_n,\]
\[\int_Df(\mb{F}(\mb{x}))\dd{\mb{x}}=\int_{\mb{F}(D)}f(\mb{y})\abs{\det((J(\mb{F}^{-1}))(\mb{y}))}\dd{\mb{y}}\]
and
\[\int f(\mb{F}(\mb{x}))\dd{\mb{x}}=\int f(\mb{y})\abs{\det((J(\mb{F}^{-1}))(\mb{y}))}\dd{\mb{y}}.\]
...
\item use substitution $u=\frac{x}{\sqrt{|k|}}+\frac{h}{\sqrt{|k|}}$, which makes the integrand,
...
\subsection{Lebesgue integral theorems}
\subsubsection{Fubini's theorem (富比尼定理)}
...
\subsubsection{(Lebesgue's) Dominated convergence theorem (DCT) (（勒貝格）控制/受制收斂定理)}
...
Suppose $C\colon\mb{r}(t),\quad t\in[a,b]$ is a piecewise $C^1$, oriented curve. If $\mb{F}$ is a tensor field differentiable on a neighborhood of $C$, then
\[\int_C\nabla\mathbf{F}\dd{\mb{r}}=\mb{F}(\mb{r}(b))-\mb{F}(\mb{r}(a)).\]
\bpr
\[\int_C\nabla\mathbf{F}\dd{\mb{r}}=\int_a^b(\nabla\mb{F})(\mb{r}(t))\mb{r}'(t)\dd{t}\]
Let $\mb{F}=(F_1,\ldots,F_n)$. For each component $F_i(\mb{r}(t))$,
\[\dv{}{t}F_i(\mb{r}(t))=(\nabla F_i)(\mb{r}(t))\mb{r}_i'(t)\]
Thus
\[(\nabla\mb{F})(\mb{r}(t))\mb{r}'(t)=\dv{}{t}\mb{F}(\mb{r}(t))\]
By FTC part 2,
\[\int_a^b\dv{}{t}\mb{F}(\mb{r}(t))\dd{t}=\mb{F}(\mb{r}(b))-\mb{F}(\mb{r}(a))\]
\epr
\subsubsection{Divergence theorem, Gauss's theorem, or Ostrogradsky's theorem (高斯散度定理)}
Suppose $V\subseteq\mathbb{R}^n$ is a compact region with outward-oriented, piecewise $C^1$ boundary $\partial V=S$. If $\mb{F}$ is a vector field differentiable and piecewise $C^1$ on a neighborhood of $V$, then
\[\int_V\nabla\cdot\mb{F}\dd{V}=\oint_S\mb{F}\cdot\dd{\mb{S}}.\]
\bpr
Simple regions case: Let $\sigma$ be a permutation on $(x_1,\ldots,x_n)$ and
\[V=\left\{(x_1,\ldots,x_n)\colon\forall i\in\bbZ\cap[1,n]\colon\varphi_i\qty(\qty(x_{\sigma(j)})_{j=1}^i)\le x_{\sigma(i)}\le\psi_i\qty(\qty(x_{\sigma(j)})_{j=1}^i)\right\},\]
...
\epr
\subsubsection{Divergence product rule}
For a scalar field $f$ and a vector field $\mb{G}$,
\[\nabla\cdot(f\mb{G})=\nabla f\cdot\mb{G}+f\nabla\cdot\mb{G}.\]
...
\[\int_Vf\nabla^2g+\nabla f\cdot\nabla g\dd{V}=\oint_Sf\nabla g\cdot\dd{\mb{S}}.\]
\bpr
By divergence theorem and divergence product rule,
\[\ba
\oint_S(f\nabla g)\cdot\dd{\mb{S}}=&\int_V\nabla\cdot(f\nabla g)\dd{V}\\
=&\int_V\nabla f\cdot\nabla g+f\nabla\cdot(\nabla g)\dd{V}\\
=&\int_Vf\nabla^2g+\nabla f\cdot\nabla g\dd{V}
\ea\]
\epr
\subsubsection{Green's second identity}
Suppose $V\subseteq\mathbb{R}^n$ is a compact region with outward-oriented, piecewise $C^1$ boundary $\partial V=S$. If two scalar fields $f,g$ are $C^1$ and piecewise $C^2$ on a neighborhood of $V$, and a scalar field $\varepsilon$ is continuous and piecewise $C^1$ on a neighborhood of $V$, then
\[\int_Vf\nabla\cdot(\varepsilon\nabla g)-g\nabla\cdot(\varepsilon\nabla f)\dd{V}=\oint_S\varepsilon\qty(f\nabla g-g\nabla f)\cdot\dd{\mb{S}}.\]
\bpr
By divergence theorem and divergence product rule,
\[\ba
\oint_S\varepsilon\qty(f\nabla g-g\nabla f)\dd{\mb{S}}=&\int_V\nabla\cdot\qty(\varepsilon\qty(f\nabla g-g\nabla f))\dd{V}\\
=&\int_V\nabla\varepsilon\cdot\qty(f\nabla g-g\nabla f)+\varepsilon\nabla\cdot\qty(f\nabla g-g\nabla f)\dd{V}\\
=&\int_Vf\nabla\varepsilon\cdot\nabla g-g\nabla\varepsilon\cdot\nabla f+\varepsilon\qty(\nabla f\cdot\nabla g+f\nabla^2g-\nabla g\cdot\nabla f-g\nabla^2f)\dd{V}\\
=&\int_Vf\nabla\cdot(\varepsilon\nabla g)-f\varepsilon\nabla^2g-g\nabla\cdot(\varepsilon\nabla f)+g\varepsilon\nabla^2f+\varepsilon\qty(f\nabla^2g-g\nabla^2f)\dd{V}\\
=&\int_Vf\nabla\cdot(\varepsilon\nabla g)-g\nabla\cdot(\varepsilon\nabla f)\dd{V}
\ea\]
\epr
For $\varepsilon=1$,
\[\int_Vf\nabla^2g-g\nabla^2f\dd{V}=\oint_S\qty(f\nabla g-g\nabla f)\cdot\dd{\mb{S}}.\]
...
\[\iint_S\pdv{Q}{x}-\pdv{P}{y}\dd{S}=\oint_{\ell}P\dd{x}+Q\dd{y},\]
that is, let $\mb{F}=(Q,-P)$,
...
By Green's theorem,
\[\ba
\iint_T\pdv{\mb{Q}}{u}-\pdv{\mb{P}}{v}\dd{u}\dd{v}=&\oint_{\omega}\mb{P}\dd{u}+\mb{Q}\dd{v}\\
...
also statement
\epr
\subsubsection{Curl of curl}
For a vector field $\mb{F}\colon U\subseteq\bbR^3\to\bbR^3$, at a point where it's $C^2$,
\[\nabla\times(\nabla\times\mb{F})=\nabla(\nabla\cdot\mb{F})-\nabla^2\mb{F}.\]
\bpr
\[\ba
\nabla\times(\nabla\times\mb{F})=&\nabla\times\qty(\pdv{F_3}{y}-\pdv{F_2}{z},\pdv{F_1}{z}-\pdv{F_3}{x},\pdv{F_2}{x}-\pdv{F_1}{y})\\
=&\qty(\pdv{\pdv{F_2}{x}-\pdv{F_1}{y}}{y}-\pdv{\pdv{F_1}{z}-\pdv{F_3}{x}}{z},\pdv{\pdv{F_3}{y}-\pdv{F_2}{z}}{z}-\pdv{\pdv{F_2}{x}-\pdv{F_1}{y}}{x},\pdv{\pdv{F_1}{z}-\pdv{F_3}{x}}{x}-\pdv{\pdv{F_3}{y}-\pdv{F_2}{z}}{y})\\
=&\qty(\frac{\partial^2F_2}{\partial x\partial y}-\frac{\partial^2F_1}{\partial y^2}+\frac{\partial^2F_3}{\partial x\partial z}-\frac{\partial^2F_1}{\partial z^2},\frac{\partial^2F_3}{\partial y\partial z}-\frac{\partial^2F_2}{\partial z^2}+\frac{\partial^2F_1}{\partial x\partial y}-\frac{\partial^2F_2}{\partial x^2},\frac{\partial^2F_1}{\partial x\partial z}-\frac{\partial^2F_3}{\partial x^2}+\frac{\partial^2F_2}{\partial y\partial z}-\frac{\partial^2F_3}{\partial y^2})
\ea\]
\[\ba
\nabla(\nabla\cdot\mb{F})=&\nabla\qty(\pdv{F_1}{x}+\pdv{F_2}{y}+\pdv{F_3}{z})\\
=&\qty(\frac{\partial^2F_1}{\partial x^2}+\frac{\partial^2F_2}{\partial x\partial y}+\frac{\partial^2F_3}{\partial x\partial z},\frac{\partial^2F_1}{\partial x\partial y}+\frac{\partial^2F_2}{\partial y^2}+\frac{\partial^2F_3}{\partial y\partial z},\frac{\partial^2F_1}{\partial x\partial z}+\frac{\partial^2F_2}{\partial y\partial z}+\frac{\partial^2F_3}{\partial z^2})
\ea\]
\[\ba
\nabla\times(\nabla\times\mb{F})-\nabla(\nabla\cdot\mb{F})=&\qty(-\frac{\partial^2F_1}{\partial x^2}-\frac{\partial^2F_1}{\partial y^2}-\frac{\partial^2F_1}{\partial z^2},-\frac{\partial^2F_2}{\partial x^2}-\frac{\partial^2F_2}{\partial y^2}-\frac{\partial^2F_2}{\partial z^2},-\frac{\partial^2F_3}{\partial x^2}-\frac{\partial^2F_3}{\partial y^2}-\frac{\partial^2F_3}{\partial z^2})\\
=&-\qty(\nabla^2F_1,\nabla^2F_2,\nabla^2F_3)\\
=&-\nabla^2\mb{F}
\ea\]
\epr
\subsection{Conservative field}
\subsubsection{Path independence equivalent to zero closed path integral}
For an $n$-dimensional vector field $\mb{v}$ in $L^1(U)$ for an open subset $U$ of $\bbR^n$, for any two piecewise $C^1$ paths $C,D\subseteq U$ with same initial and terminal points,
\[\int_C\mb{v}\cdot\dd{\mb{x}}=\int_D\mb{v}\cdot\dd{\mb{x}}\]
...
\epr
\subsubsection{Conservative equivalent to path independence}
...
on $U$.
\bpr
If: Any simply-connected surface $S\subseteq U$ is bounded by a piecewise $C^1$, simple, closed curve $C$, by Green's theorem,
\[\iint_S\pdv{f_2}{x}-\pdv{f_1}{y}\dd{S}=\oint_Cf\cdot\dd{(x,y)}=0\]
\epr
...
on $U$.
\bpr
If: Any simply-connected surface $S\subseteq U$ is bounded by a piecewise $C^1$, simple, closed curve $C$, by Stokes' theorem,
\[\iint_S\nabla\times f\dd{S}=\oint_Cf\cdot\dd{(x,y,z)}=0\]
\epr
\subsubsection{Divergence of curl}
For a scalar field $\mb{F}$ $C^2$ on $U\subseteq\bbR^3$,
\[\nabla\cdot(\nabla\times\mb{F})=0.\]
\bpr
\[\ba
\nabla\cdot(\nabla\times\mb{F})=&\nabla\cdot\qty(\pdv{F_3}{y}-\pdv{F_2}{z},\pdv{F_1}{z}-\pdv{F_3}{x},\pdv{F_2}{x}-\pdv{F_1}{y})\\
=&\pdv[2]{F_3}{xy}-\pdv[2]{F_2}{xz}+\pdv{F_1}{yz}-\pdv{F_3}{xy}+\pdv{F_2}{xz}-\pdv{F_1}{yz}\\
=&0
\ea\]
\epr
\subsection{Two-dimensional curve rotates about principal axis on the same plane}
The area of the surface formed by rotating the simple curve $\mb{r}(t)=(x(t),y(t)),\quad t\in[a,b]$ about $y=d$ is
...
The area of the surface formed by rotating a two-dimensional simple curve with arc length $s$ and centroid $\mb{P}$ about a rotational axis on the same plane of the curve that doesn't cut the curve in a distance $d$ from $\mb{P}$ is
\[A=2\pi sd.\]
\begin{proof}
...
for some $\xi\in[a,b]$.
\subsection{Symmetry function integral}
...
\epr
\subsection{Multiple integral theorems}
\subsubsection{Multiple integral to multiplication of integrals}
For a rectangle
...
\subsubsection{Multiple integral to iterated integral theorem}
For a region $D$ in $\bbR^n$ of the form
\[D=\left\{(x_1,x_2,\ldots,x_n)\middle|\bigwedge_{i=0}^{n-1}f_i\qty(x_{\sigma(1)},x_{\sigma(2)},\ldots,x_{\sigma(i)})\le x_{\sigma(i+1)}\le g_i\qty(x_{\sigma(1)},x_{\sigma(2)},\ldots,x_{\sigma(i)})\right\},\]
...
\section{Total Variation Theorems}
\subsection{Bounded}
If real function is BV over an interval, it's bounded on it.
\bpr
Obviously.
\epr
\subsection{Implying or implied by bounded variation}
...
\section{Series and Improper Integral Theorems and Methods}
\subsection{Limit of real series theorems}
\subsubsection{Integral test (for convergence) or Maclaurin–Cauchy test}
...
\bit
\item If $\int_a^{\infty}f(x)\dd{x}$ is convergent and $f(x)$ is monotone decreasing, for any $n\in\bbN_{\ge a}$,
...
\item If $\int_a^{\infty}f(x)\dd{x}$ is convergent and $f(x)$ is monotone increasing, for any $n\in\bbN_{\ge a}$,
...
\epr
\subsubsection{Series conditional convergence implies squared-index-multiplied-termed series divergence}
...
\epr
\subsubsection{Series conditional convergence can not determine index-multiplied-termed series divergence}
...
\epr
\subsubsection{(d'Alembert's ratio or Cauchy) ratio test}
\ben
\item If $\limsup_{n\to\infty}\abs{\frac{a_{n+1}}{a_n}}<1$, then the series $\sum_{n=1}^{\infty}a_n$ is absolutely convergent.
\item If $\liminf_{n\to\infty}\abs{\frac{a_{n+1}}{a_n}}>1$, then the series $\sum_{n=1}^{\infty}a_n$ is divergent.
\item Otherwise, the test is inconclusive.
\een
\bpr
For the first statement, choose an arbitrary $r$ such that $\limsup_{n\to\infty}\abs{\frac{a_{n+1}}{a_n}}<r<1$. There exists an integer $N$ such that
...
\epr
\subsubsection{Ratio test for power series}
Suppose that the power series $\sum_{n=0}^{\infty}c_n(x-a)^n$ satisfies $c_n\ne0$ for all $n$. If $\lim_{n\to\infty}\abs{c_n/c_{n+1}}$ exists in $\bar{\bbR}_{\ge0}$, then it is equal to the radius of convergence of $\sum_{n=0}^{\infty}c_n(x-a)^n$.
\bpr
If
...
If
\[\lim_{n\to\infty}\abs{c_n/c_{n+1}}=0,\]
for all $x\ne a$,
\[\lim_{n\to\infty}\abs{\frac{c_{n+1}(x-a)^{n+1}}{c_n(x-a)^n}}=\infty.\]
Use ratio test. For all $x\ne a$, the series is divergent.
...
\item If $\liminf_{n\to\infty}\sqrt[n]{\abs{a_n}}>1$, then the series $\sum_{n=1}^{\infty}a_n$ is divergent.
\item Otherwise, the test is inconclusive.
\een
\bpr
For the first statement, choose an arbitrary $r$ such that $\limsup_{n\to\infty}\sqrt[n]{\abs{a_n}}<r<1$. There exists an integer $N$ such that
...
Thus $\sum_{n=1}^{\infty}a_n$ is divergent.
\epr
\subsubsection{Raabe's test or Rabbe–Duhamel test}
For a series $\sum_{n=1}^{\infty}a_n$ of positive terms, define
\[R_n=n\qty(\frac{a_n}{a_{n+1}}-1)\]
\bit
\item If $\limsup_{n\to\infty}R_n<1$, then the series converges.
\item If $\liminf_{n\to\infty}R_n>1$, then the series converges.
\item Otherwise, the test is inconclusive.
\eit
\bpr
Case $\limsup_{n\to\infty}R_n<0$: $a_{n+1}\ge a_n$ for sufficiently large $n$, so the series diverges.

Case $0\le\limsup_{n\to\infty}R_n<1$: There exists $R<1$ and $N\in\bbN$ such that $R_n\le R$ for all $n\ge N$, that is,
\[\frac{a_n}{a_{n+1}}\le\qty(1+\frac{R}{n})\le e^{\frac{R}{n}}.\]
Thus
\[a_{n+1}\ge a_ne^{-\frac{R}{n}}\]
This implies that
\[a_{n+1}\ge a_Ne^{-R\sum_{i=N}^ni^{-1}}\ge ca_Ne^{-R\ln n}=ca_Nn^{-R}\]
Since $R<1$, by p-series test, the series diverges.

Case $\liminf_{n\to\infty}R_n>1$: There exists $R>1$ and $N\in\bbN$ such that $R_n\ge R$ for all $n\ge N$, that is,
\[\frac{a_n}{a_{n+1}}\ge1+\frac{R}{n}\]
Thus
\[\ln a_n-\ln a_{n+1}\ge\ln(1+\frac{R}{n})\]
\[\ln a_{n+1}\le\ln a_N-\sum_{i=N}^n\ln(1+\frac{R}{i})\le -R\ln N+O(1)\]
This implies that
\[a_{n+1}\le cN^{-R}\]
Since $R>1$, by p-series test, the series diverges.
\epr
\subsubsection{Riemann series theorem or Riemann rearrangement theorem}
...
TODO
\epr
\subsubsection{Power series multiplying rational function of $n$ does not affect radius of convergence}
...
Let
\[m=\deg P-\deg S.\]
There exists $A, B\in\bbR$ such that
\[An^m\le Q(n)\le Bn^m\]
for sufficiently large $n$.
\[\lim_{n\to\infty}\sqrt[n]{\abs{n^m}}=\lim_{n\to\infty}n^{m/n}=\lim_{n\to\infty}e^{m\ln n/n}=1\]
\[\lim_{n\to\infty}\sqrt[n]{\abs{A}}=\lim_{n\to\infty}\sqrt[n]{\abs{B}}=1\]
Thus
\[\lim_{n\to\infty}\sqrt[n]{\abs{Q(n)}}=1\]
\[\limsup_{n\to\infty}\sqrt[n]{\abs{Q(n)c_n}}=\limsup_{n\to\infty}\sqrt[n]{\abs{c_n}}=\frac{1}{R}\]
...
TODO
\epr
\subsection{p-test and Master Theorem}
\subsubsection{p-test (for improper integral)}
...
\end{proof}
\subsubsection{p-test for series or p-series test}
...
\epr
\subsubsection{Generalized p-test (for improper integral)}
Define iterated logarithmic function $L_n(x)$ as
\[L_{0}(x)=x,\quad x\in\bbR_{>0}\]
\[L_{n}(x)=\ln(L_{n-1}(x)),\quad n\in\bbN,x\in\bbR_{>0}.\]
For any fixed $a>1,k\in\bbN$,
\[\int_a^{\infty}\frac{1}{\qty(\prod_{i=0}^{k-1}L_{i}(x))\qty(L_{k}(x))^m}\dd{x}\]
converges iff $m>1$.

For any fixed $0<a<1,k\in\bbN$,
\[\int_0^a\frac{1}{\qty(\prod_{i=0}^{k-1}L_{i}(x))\qty(L_{k}(x))^m}\dd{x}\]
converges iff $m<1$.
\bpr
Let
\[u=L_k(x)\]
\[\dd{u}=\frac{\dd{x}}{\prod_{i=0}^{k-1}L_{i}(x)}\]
\[\ba
  {}&{}\int\frac{1}{\qty(\prod_{i=0}^{k-1}L_{i}(x))\qty(L_{k}(x))^m}\dd{x}\\
  =&\int\frac{1}{u^m}\dd{u}\\
...
\epr
\subsubsection{Generalized p-test for series or generalized p-series test}
Define iterated logarithmic function $L_{a}(x)$ as
\[L_{0}(x)=x,\quad x\in\bbR_{>0}\]
\[L_{n}(x)=\ln(L_{n-1}(x)),\quad n\in\bbN,x\in\bbR_{>0}.\]
For any fixed $a\in\bbN$,
\[\sum_{n=a}^{\infty}\frac{1}{\qty(\prod_{i=0}^{k-1}L_{i}(x))\qty(L_{k}(x))^m}\]
converges iff $m>1$.
\bpr
...
PLACEHOLDER
\epr
\subsubsection{Generalized Master Theorem}
For a positive valued net $T(n)$ defined on $n=b^h$ for any $h\in\bbN_0$ as
\[T(n)=aT\qty(\frac{n}{b})+f(n)\]
for some constants $a>0$ and $b>1$, and non-negative-valued function $f(n)$,
\ben
\item If for some $c<\log_ba$,
\[f(n)=O(n^c),\]
then
\[T(n)=\Theta(n^{\log_ba}).\]
\item Define iterated logarithmic function $L_{a}(x)$ as
\[L_{0}(x)=x,\quad x\in\bbR_{>0}\]
\[L_{n}(x)=\ln(L_{n-1}(x)),\quad n\in\bbN,x\in\bbR_{>0}.\]
\ben
\item If for some positive integers $k$ and positive number $m$, 
\[f(n)=\Theta(n^{\log_ba}\qty(\prod_{i=1}^{k-1}(L_{i}(n))^{-1})\qty(L_{k}(n))^{m-1}),\]
then
\[T(n)=\Theta(n^{\log_ba}(L_{k}(n))^m).\]
\item If for some positive integer $k$, 
\[f(n)=o(n^{\log_ba}\prod_{i=1}^k(L_{i}(n))^{-1}),\]
then
\[T(n)=\Theta(n^{\log_ba}L_{k-1}(n)).\]
\item If for some positive integers $k$ and some positive number $m$,
\[f(n)=\Theta(n^{\log_ba}\qty(\prod_{i=1}^{k-1}(L_{i}(n))^{-1})\qty(L_{k}(n))^{-m-1}),\]
then
\[T(n)=\Theta(n^{\log_ba}).\]
\een
\item If $f(n)=\Omega(n^c)$ for some $c>\log_ba$, and the regularity condition that $\exists m\in\bbR,k<1$ such that $n\ge m$ implies
\[af\qty(\frac{n}{b})\le kf(n)\]
is satisfied, then
\[T(n)=\Theta(f(n)).\]
\een
\bpr
Let $s=\log_ba$, $n=b^h$, and $f(x)=x^sg(x)$. At level $j$, there are $a^j$ nodes, each of size $\frac{n}{b^j}$. Thus, the total non-recursive work at level $j$ is
\[a^jf\qty(\frac{n}{b^j}).\]
Hence
\[\ba
T(n)=&\sum_{j=0}^{h-1}a^jf\qty(\frac{n}{b^j})+n^sT(1)\\
...
\[T(n)=\Theta(n^{\log_ba}).\]
Case 2:
\[L_{k}(b^x)=\Theta(L_{k-1}(x))\]
\[f(n)=\Theta(n^{\log_ba}\qty(\prod_{i=1}^{k-1}(L_{i}(n))^{-1})\qty(L_{k}(n))^{m-1})\]
\[g(b^r)=\Theta(\qty(\prod_{i=1}^{k-2}(L_{i}(r))^{-1})\qty(L_{k-1}(r))^{m-1})\]


If $\int_a^{\infty}f(x)\dd{x}$ is convergent and $f(x)$ is monotone decreasing, for any $n\in\bbN_{\ge a}$,
\[\int_n^{\infty}f(x)\dd{x}\le\sum_{k=n}^{\infty}f(k)\le f(n)+\int_n^{\infty}f(x)\dd{x},\]

Sub-case 1 $m>0$: By generalized p-test for series
\[\sum_{r=1}^hg\qty(b^r)\]
is divergent. Thus,
\[T(n)=\Theta(n^{\log_ba}(L_{k}(n))^m).\]
Sub-case 2 $m=0$:
...
Case 3: If $f(n)=\Omega(n^c)$ for some $c>\log_ba$, and the regularity condition that $\exists m\in\bbR,0<k<1$ such that $n\ge m$ implies
\[af\qty(\frac{n}{b})\le kf(n)\]
is satisfied, then
\[af\qty(\frac{n}{b})\le kf(n)\]
\[a^jf\qty(\frac{n}{b^j})\le a^{j-1}kf\qty(\frac{n}{b^{j-1}})\le k^jf(n)\]
\[\ba
T(n)=&\sum_{j=0}^{h-1}a^jf\qty(\frac{n}{b^j})+n^sT(1)\\
=&f(n)+\sum_{j=1}^{h-1}O(k^jf(n))+\Theta\qty(n^s)
\ea\]
\[\sum_{j=1}^{\infty}k^jf(n)=\frac{f(n)}{1-k}\]
\[T(n)=f(n)+O(f(n))+\Theta\qty(n^s)=\Theta(f(n))\]
\epr
Terms:
\bit
\item $h$ is called recursion level.
\item $\log_ba$ is called critical exponent $c_{crit}$.
\item $T(1)$ is called leaf cost.
\item At each node, the first term is called recursive work, and the second term is called non-recursive work.
\eit
For original master theorem, the second case is
\ben
\item If for some positive integers $m$, 
\[f(n)=\Theta(n^{\log_ba}(\ln n)^{m-1}),\]
then
\[T(n)=\Theta(n^{\log_ba}(\ln n)^m).\]
\item If
\[f(n)=o(n^{\log_ba}(\ln n)^{-1}),\]
then
\[T(n)=\Theta(n^{\log_ba}\ln(\ln n)).\]
\item If for some positive integers $m$,
\[f(n)=\Theta(n^{\log_ba}(\ln n)^{-m-1}),\]
then
\[T(n)=\Theta(n^{\log_ba}).\]
...
\subsubsection{Convergence of improper integral of monotone increasing/decreasing integrand implies non-positive/non-negative integrand}
...
\subsubsection{Convergence of improper integral of monotone integrand implies limit of product of integrand and independent variable converges to 0}
...
Then, $\int_M^{\infty}f^-(x)$ diverges.
\epr
\subsection{(Direct) comparison (test) (theorem)}
\subsubsection{For real series}
...
Statement 2 is the contrapositive of statement 1.
\end{proof}
\subsection{Limit comparison (test) (theorem) (LCT)}
\subsubsection{For real series}
...
\subsubsection{$\qty(1-\frac{1}{n})^n$}
\[\lim_{n\to\infty}\qty(1-\frac{1}{n})^n=e^{-1}\]
\bpr
Let
\[\lim_{n\to\infty}\qty(1-\frac{1}{n})^n=L\]
\[\lim_{n\to\infty}n\ln(1-\frac{1}{n})=\ln L\]
Near 0,
\[\ln(1-x)=-x+O(x^2)\]
\[\lim_{n\to\infty}n\qty(-\frac{1}{n}+O\qty(\frac{1}{n^2}))=-1=\ln L\]
\[L=e^{-1}\]
\epr
...
The truncation $\operatorname{truncate}\colon\bbR\to\bbR$ is defined as:
...
\[f(t)=A_0+\qty(A_1-A_0)\qty(\left\lfloor\frac{t}{T}+D\right\rfloor-\left\lfloor\frac{t}{T}\right\rfloor).\]
...
\ea\]
\end{proof}
\subsubsection{Iterated logarithmic function}
Define iterated logarithmic function $L_{a}(x)$ as
\[L_{0}(x)=x,\quad x\in\bbR_{>0}\]
\[L_{n}(x)=\ln(L_{n-1}(x)),\quad n\in\bbN,x\in\bbR_{>0}.\]
\[\dv{}{x}L_{n}(x)=\frac{1}{\prod_{i=0}^{n-1}L_{i}(x)},\quad n\in\bbN,x\in\bbR_{>0}.\]
\bpr
\[\dv{}{x}L_n(x)=\dv{}{x}\ln(L_{n-1}(x))=\frac{1}{L_{n-1}(x)}\dv{}{x}L_{n-1}(x)\]
\[\dv{}{x}L_0(x)=1\]
\epr
...
\subsubsection{Inverse secant function}
\[\int\arcsec(x)\dd{x}=x\arcsec(x)-\sgn(x)\ln\abs{x+\sqrt{x^2-1}}+C,\quad\abs{x}\ge1.\]
\subsubsection{Inverse cosecant function}
\[\int\arccsc(x)\dd{x}=x\arccsc(x)+\sgn(x)\ln\abs{x+\sqrt{x^2-1}}+C,\quad\abs{x}\geq 1.\]
\subsubsection{Hyperbolic sine function}
...
&=\frac{2}{a}\qty(x^nR-n\int (ax+b)\frac{x^{n-1}}{R}\dd{x})\\
&=\frac{2}{a}\qty(x^nR-bn\int \frac{x^{n-1}}{R}\dd{x})-2n\int\frac{x^n}{R}\dd{x}\\
...
\subsubsection{$(1+x)^r$}
\[(1+x)^r=\sum_{n=0}^{\infty}\binom{r}{n}x^n,\quad r\in\bbN_0\lor-1<x<1\lor(-1\le x\le1\land r>0)\lor(x=1\land-1<r<0).\]
\bpr
Apply ratio test,
\[\lim_{n\to\infty}\abs{\frac{\binom{r}{n+1}x^{n+1}}{\binom{r}{n}x^n}}=\lim_{n\to\infty}\abs{\frac{r-n}{n+1}x}=\abs{x}\]
For $\abs{x}=1$, apply Raabe's test,
\[\ba
\lim_{n\to\infty}n\qty(\abs{\frac{\binom{r}{n}}{\binom{r}{n+1}}}-1)=&\lim_{n\to\infty}n\qty(\frac{n+1}{n-r}-1)\\
=&\lim_{n\to\infty}\frac{n}{n-r}(1+r)\\
=&1+r
\ea\]
...
\[\frac{1}{1+x}=\frac{1}{1+a}\sum_{n=0}^{\infty}\qty(\frac{a-x}{1+a})^n,\quad |x-a|<|1+a|.\]
\begin{proof}
...
\end{proof}
\[\sqrt{a^2+x^2}=\sum_{n=0}^{\infty}\frac{(-1)^{n+1}(2n)!}{4^n(n!)^2(2n-1)}\frac{x^{2n}}{a^{2n-1}},\quad\abs{x}<\abs{a}.\]
\subsubsection{$(1-x)^r$}
\[(1-x)^r=\sum_{n=0}^{\infty}\binom{r}{n}(-x)^n,\quad r\in\bbN_0\lor-1<x<1\lor(-1\le x\le1\land r>0)\lor(x=-1\land-1<r<0).\]
\bpr
Similar to that of $(1+x)^r$.
\epr
\[\frac{1}{1-x}=\sum_{n=0}^{\infty}x^n,\quad |x|<1.\]
...
\end{proof}
\[\sqrt{a^2-x^2}=-\sum_{n=0}^{\infty}\frac{(2n)!}{4^n(n!)^2(2n-1)}\frac{x^{2n}}{a^{2n-1}},\quad\abs{x}<\abs{a}.\]
\subsubsection{Logarithmic function}
\[\ln(x)=\ln(a)-\sum_{n=1}^{\infty}\frac{1}{n}\qty(\frac{a-x}{a})^n,\quad 0\le x\le 2a\land a>0.\]
\begin{proof}
\[\dv{\ln(x)}{x}=x^{-1}.\]
...
\end{proof}
\[\ln(1+x)=-\sum_{n=1}^{\infty}\frac{1}{n}(-x)^n,\quad -1<x\le 1.\]
\[\ln(1+x)=\ln(1+a)-\sum_{n=1}^{\infty}\frac{1}{n}\qty(\frac{a-x}{1+a})^n,\quad -1<x\le 1+2a\land a>-1.\]
\[\ln(1-x)=-\sum_{n=1}^{\infty}\frac{1}{n}x^n,\quad -1\le x<1.\]
\[\ln(1-x)=\ln(1-a)-\sum_{n=1}^{\infty}\frac{1}{n}\qty(\frac{x-a}{1-a})^n,\quad 2a-1\le x<1\land a>1.\]
\subsubsection{Exponential function}
...
\end{proof}
\subsubsection{Inverse sine function}
\[\arcsin(x)=\sum_{n=0}^{\infty}\frac{(2n)!}{4^n(n!)^2(2n+1)}x^{2n+1},\quad\abs{x}\le1.\]
\subsubsection{Inverse cosine function}
\[\arccos(x)=\frac{\pi}{2}-\sum_{n=0}^{\infty}\frac{(2n)!}{4^n(n!)^2(2n+1)}x^{2n+1},\quad\abs{x}\le1.\]
\subsubsection{Inverse tangent function}
\[\arctan(x)=\sum_{n=0}^{\infty}\frac{(-1)^n}{2n+1}x^{2n+1},\quad\abs{x}\le1.\]
\subsubsection{Inverse cotangent function}
\[\arccot(x)=\frac{\pi}{2}-\sum_{n=0}^{\infty}\frac{(-1)^n}{2n+1}x^{2n+1},\quad\abs{x}\le1.\]
\subsubsection{Hyperbolic sine function}
\[\sinh(x)=\sum_{n=0}^{\infty}\frac{1}{(2n+1)!}x^{2n+1}.\]
\[\sinh(x)=\sum_{n=0}^{\infty}\frac{((n+1)\mod 2)\sinh a+(n\mod 2)\cosh a}{n!}(x-a)^n.\]
\subsubsection{Hyperbolic cosine function}
\[\cosh(x)=\sum_{n=0}^{\infty}\frac{1}{(2n)!}x^{2n}.\]
\[\cosh(x)=\sum_{n=0}^{\infty}\frac{((n+1)\mod 2)\cosh a+(n\mod 2)\sinh a}{n!}(x-a)^n.\]
\subsection{List of Nablas}
Let $\mb{r}=(x,y,z)$, $r=\abs{\mb{r}}$.
\subsubsection{Gradient of $r$}
\[\nabla r^n=nr^{n-2}\mb{r}\]
\bpr
\[\nabla r=nr^{n-1}\nabla r\]
\[\nabla r=\qty(\frac{x}{r},\frac{y}{r},\frac{z}{r})=\frac{\mb{r}}{r}\]
\epr
\subsubsection{Curl of position}
\[\nabla\times\mb{r}=\mb{0}\]
\bpr
\[\nabla\times\mb{r}=\begin{bmatrix}\mb{i}&\mb{j}&\mb{k}\\\pdv{}{x}&\pdv{}{y}&\pdv{}{z}\\x&y&z\end{bmatrix}=\mb{0}\]
\epr
\subsubsection{Gradient of $\ln r$}
\[\nabla\ln r=\frac{\mb{r}}{r^2}\]
\bpr
\[\nabla\ln r=\frac{1}{r}\nabla r=\frac{\mb{r}}{r^2}\]
\epr
...
\[\qty(-1+\varepsilon)e^{-1+\varepsilon}=-\frac{1}{e}+\delta.\]
\[\qty(-1+\varepsilon)e^{\varepsilon}=-1+e\delta\]
\[\qty(-1+\varepsilon)\sum_{i=0}^{\infty}\frac{\varepsilon^i}{i!}=-1+e\delta\]
...
\[W_0\qty(-\frac{1}{e}+\delta)=-1+\varepsilon=-1+O(\sqrt{\delta})\]
\[\frac{W\qty(-\frac{1}{e}+\delta)-W\qty(-\frac{1}{e})}{\delta}=O\qty((\delta)^{-1/2})\]
...
\subsubsection{Lemniscate of Bernoulli}
