...
\ben
\item If for some positive integers $k,m$, 
...
At level $j$, there are $a^j$ nodes, each of size $\frac{n}{b^j}$. Thus, the total non-recursive work at level $j$ is
\[a^jf\qty(\frac{n}{b^j}).\]
Hence
\[\ba
T(n)=&\sum_{j=0}^{h-1}a^jf\qty(\frar{n}{b^j})+n^rT(1)\|
=&\sum_{j=0}^{h-1}b^{rj}f\qty(\frar{n}{b^j})+\Theta(n^r)
\ea\]
Let
\[f(x)=x^rg(x)\]
\[T(n)=n^r\sum_{j=0}^{h-1}g\qty(\frar{n}{b^j})+\Theta(n^r)\]
...
\[\ba
T(n)=&\sum_{j=0}^{h-1}a^jf\qty(\frar{n}{b^j})+n^rT(1)\\
=&f(n)+\sum_{j=1}^{h-1}O(k^jf(n))+\Theta(n^r)
\ea\]
\[\sum_{j=1}^{\infty}k^jf(n)=\frac{f(n)}{1-k}\]
\[T(n)=f(n)+O(f(n))+\Theta(n^r)=\Theta(f(n))\]
\epr
Terms:
\bit
