...
\sssc{Dirichlet function{{{
\sssc{Weierstrass function{{{
\sssc{Blancmange curve{{{
...
A point is a singularity or a singular point of a function $f\colon I\subseteq\mathbb{R}\to\mathbb{R}$ if it is a cluster point of $I$ but not in $I$, or it is a discontinuity of $f$.
...
\eit
\sssc{Type of singularities for real functions}
\bit
\item \tb{Removable discontinuity}: If $\exists\lim_{x\to a}f(x)\land\lim_{x\to a}f(x)\neq f(a)$, we call $f(a)$ a removable discontinuity.
\item \tb{Removable singularity}: If $\exists\lim_{x\to a}f(x)$ and either that $f(a)$ is undefined or $\lim_{x\to a}f(x)\neq f(a)$, we call $f(a)$ a removable singularity.
\item \tb{Jump discontinuity}: If $\exists\lim_{x\to a^-}f(x)\land\exists\lim_{x\to a^+}f(x)\land\lim_{x\to a^-}f(x)\neq\lim_{x\to a^+}f(x)$ and that $f(a)$ is defined, we call $f(a)$ a jump discontinuity.
\item \tb{Jump singularity}: If $\exists\lim_{x\to a^-}f(x)\land\exists\lim_{x\to a^+}f(x)\land\lim_{x\to a^-}f(x)\neq\lim_{x\to a^+}f(x)$, we call $f(a)$ a jump singularity.
\item \tb{Type I discontinuity}: Removable discontinuities and jump discontinuities are collectively called type I discontinuities.
\item \tb{Type I singularity}: Removable singularities and jump singularities are collectively called type I singularities.
\item \tb{Type II discontinuity}: At least one of $\lim_{x\to a^-}f(x)$ and $\lim_{x\to a^+}f(x)$ doesn't exist in $\mathbb{R}$ and that $f(a)$ is defined, we call $f(a)$ a type II discontinuity.
\item \tb{Type II singularity}: At least one of $\lim_{x\to a^-}f(x)$ and $\lim_{x\to a^+}f(x)$ doesn't exist in $\mathbb{R}$, we call $f(a)$ a type II singularity.
\item\tb{Infinite discontinuity}: If at least one of $\lim_{x\to a^-}f(x)$ and $\lim_{x\to a^+}f(x)$ does not exist, and that those in $\lim_{x\to a^-}f(x)$ and $\lim_{x\to a^+}f(x)$ that do not exist are either $\infty$ or $-\infty$, and that $f(a)$ is defined, we call $f(a)$ infinite discontinuity.
\item\tb{Infinite singularity}: If at least one of $\lim_{x\to a^-}f(x)$ and $\lim_{x\to a^+}f(x)$ does not exist, and that those in $\lim_{x\to a^-}f(x)$ and $\lim_{x\to a^+}f(x)$ that do not exist are either $\infty$ or $-\infty$, we call $f(a)$ infinite singularity.
\item\tb{Essential singularity}: An essential singularity that is not an infinite singularity is called an essential singularity.
\eit
\sssc{Piecewise continuity for real functions}
Let $f\colon D\subseteq\mathbb{R}\to\mathbb{R}$ be a function. We say $f$ is piecewise-continuous if there exists a family $\{[a_i,b_i]\mid i\in I\}$ of closed intervals such that for every closed interval $V$, the set $\{i\in I\mid V\cap [a_i,b_i]\neq\varnothing\}$ is finite, and $D\subseteq\bigcup_{i\in I}[a_i,b_i]$, and that function $f\big\vert_{(a_i,b_i)}\colon(a_i,b_i)\to Y$ is continuous for every $i\in I$.
\sssc{Piecewise continuity for functions between topological spaces}
Let $X$ and $Y$ be topological spaces, and $f\colon D\subseteq X\to Y$ be a function. We say $f$ is piecewise-continuous if there exists a locally finite family $\{U_i\mid i\in I\}$ of closed subsets of $X$ such that $D\subseteq\bigcup_{i\in I}U_i$ and that function $f\big\vert_{\operatorname{int}\left(U_i\right)}\colon\operatorname{int}\left(U_i\right)\to Y$ is continuous for every $i\in I$.
