...
Let $W$ be a topological vector space. The ordinary derivative (常導數) or derivative (導數) of a function $f(u)\colon U\subseteq\mathbb{R}\to W$ (with respect to $u$) at $x\in U$, denoted as $f'(x)$, is defined as
...
If there exists such an operator $A$, it is unique, so we define the (first(-order)) ordinary derivative or derivative of $f$ (with respect to $u$) at $x$, denoted as $Df(x)$, as $A$.
...
\eit
\subsubsection{For real-domain function at finity}
\bit
