...
\item $\forall M>0\colon\exists T\ge a\colon\abs{f(x)}<M\abs{g(x)}$ iff $f(x)\in o(g(x))$.
\item $\forall M>0\colon\exists T\ge a\colon\abs{f(x)}>M\abs{g(x)}$ iff $f(x)\in \omega(g(x))$.
\eit
\sssc{Limit}
If $g(x)\ne0$,
\bit
...
\bpr
\[\abs{f(x)}\le M\abs{g(x)}\qc xge T\]
\[\abs{\frac{f(x)}{g(x)}}\le M\qc xge T\]
\[\sup\abs{\frac{f(x)}{g(x)}}\le M\qc xge T\]
\[\limsup\abs{\frac{f(x)}{g(x)}}\le M<\infty\]
\epr
\item $\liminf\abs{\frac{f(x)}{g(x)}}>0$ iff $f(x)\in \Omega(g(x))$.
\bpr
\[\abs{f(x)}\ge M\abs{g(x)}\qc xge T\]
\[\abs{\frac{f(x)}{g(x)}}\ge M\qc xge T\]
\[\inf\abs{\frac{f(x)}{g(x)}}\ge M\qc xge T\]
\[\liminf\abs{\frac{f(x)}{g(x)}}\ge M>0\]
\epr
\item $\infty>\limsup\abs{\frac{f(x)}{g(x)}}\ge\liminf\abs{\frac{f(x)}{g(x)}}>0$ iff $f(x)\in \Theta(g(x))$.
\bpr
\[\abs{f(x)}\ge M_1\abs{g(x)}\qc xge T\]
\[\abs{\frac{f(x)}{g(x)}}\ge M_1\qc xge T\]
\[\inf\abs{\frac{f(x)}{g(x)}}\ge M_1\qc xge T\]
\[\liminf\abs{\frac{f(x)}{g(x)}}\ge M_1>0\]
\[\abs{f(x)}\le M_2\abs{g(x)}\qc xge T\]
\[\abs{\frac{f(x)}{g(x)}}\le M_2\qc xge T\]
\[\sup\abs{\frac{f(x)}{g(x)}}\le M_2\qc xge T\]
...
\eit
\sssc{Relation}
$o$, $\Theta$, and $\omega$ are mutually exclusive.
\[\Theta\subseteq O\cap\Omega\]
\[O\subseteq o\cup\Theta\]
\[\Omega\subseteq\omega\cup\Theta\]
\bpr
Obvious.
\epr
\[O\setminus o\not\subseteq\Theta\]
...
\bpr
Counterexample
\[\frac{f(x)}{g(x)}=\bcs 1,\quad & x\in\bbQ\\\frac{1}{x},\quad & x\notin\bbQ\ecs\]
\epr
Binary relation:
\bit
\item $\Theta$ is an equivalence class
\item $O$ and $\Omega$ are partial-ordered sets.
\item $o$ and $\omega$ are strictly partially-ordered sets.
\eit
