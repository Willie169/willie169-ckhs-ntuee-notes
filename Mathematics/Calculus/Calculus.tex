lune of Hippocrates area (?):
```
7.3 49. Find the area of the crescent-shaped region (called a lune) bounded by arcs of circles with radii $r$ and $R$. (See the figure.)

Blue circle:
\[x^2+y^2=r^2\]
\[y=\sqrt{r^2-x^2}\]
Red circle:
\[x^2+(y+\sqrt{R^2-r^2})^2=R^2\]
\[y=\sqrt{R^2-x^2}-\sqrt{R^2-r^2}\]
The area of the lune in the blue circle outside of the red circle is
...
bounded variation
Dirichlet test and Cauchy condensation test for series and integral

Improper:
If $\int_a^{\infty}\abs{f(x)}\dd{x}$ converges, then $\lim_{x\to\infty}f(x)=0$.
...
\[tf(t)<4\varepsilon.\]
Thus, $\forall\varepsilon>0$, there exists $M>0$ such that $\forall t>M$,
\[tf(t)<\varepsilon,\]
that is,
\[\lim_{x\to\infty}xf(x)=0.\]
\end{proof}
mb nonnegative replace with BV still works?


Comparison test: If there exists $g(x)$ such that $\int_a^{\infty}g(x)\dd{x}$ converges and $\exists b\geq a$ such that $\forall x\in(b,\infty)\colon g(x)\geq\abs{f(x)}$, then $\int_a^{\infty}\abs{f(x)}\dd{x}$ converges.
p-test: $\int_a^{\infty}x^p\dd{x}$ converges iff $p<-1$.
Dirichlet test (?): If $f(x)$ is such that there exist $g(x)$ that is non-increasing on $[a,\infty)$ and $h(x)$ that is of bounded variation on $[a,\infty)$ such that $f(x)=g(x)h(x)$ for all $x\in(a,\infty)$, then $\int_a^{\infty}f(x)\dd{x}$ converges.
