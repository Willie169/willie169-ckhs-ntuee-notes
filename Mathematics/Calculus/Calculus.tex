...
\sssc{Radius and interval of convergence for power series}
...
\ssc{Big O, little o, theta, big Omega, and small omega notation}
...
\[g_p(X_p,Y_p)\ne0.\]
\eit
\sssc{Signature of metric tensor}
...
\ssc{Orientation of hypersurface}
...
\sssc{Conservative field (保守場) or gradient field}
...
\epr
\ssc{Cylindrical coordinate system (圓柱座標系)}
\sssc{ISO 31-11 Definition}
...
\ssc{Spherical coordinate system (球座標系)}
\sssc{Physics convention, ISO 80000-2, or inclination-azimuth convention definition}
...
\eit
\sssc{Mathematics convention or azimuth-inclination convention definition}
...
\eit
\sssc{Conversion from and to Cartesian coordinate system (physics convention)}
...
\sssc{Conversion from and to Cartesian coordinate system (mathematics convention)}
If the polar axis and the auxiliary axis of a spherical coordinate system is the $z$ axis and the positive direction of the $x$ axis of a Cartesian coordinate system, then:
...
\sssc{Conversion from and to cylindrical coordinate system (physics convention)}
...
\[\varphi=\theta.\]
\sssc{Conversion from and to cylindrical coordinate system (mathematics convention)}
If the polar axis and the auxiliary axis of a spherical coordinate system is the main axis and the auxiliary axis of a cylindrical coordinate system, then:
...
\sssc{Local orthogonal basis (physics convention)}
...
\sssc{Local orthogonal basis (mathematics convention)}
...
\sssc{Surface elements (mathematics convention)}
$\rho=$ constant:
...
TODO
\end{proof}
\ssc{Limit of function between normed spaces theorem}
\sssc{Different paths criterion for nonexistence of a limit}
...
\sssc{homogeneous decomposition}
...
\epr
\eit
\sssc{Generalized Master Theorem}
...
then
\[T(n)=\Theta(n^{\log_ba}).\]
\item Define iterated logarithm $\operatorname{ilog}(a,x)$ as
\[\operatorname{ilog}(0,x)=x,\]
\[\operatorname{ilog}(n,x)=\ln(\operatorname{ilog}(n-1,x)),\quad n\in\bbN.\]
\ben
\item If for some positive integers $k$ and positive number $m$, 
\[f(n)=\Theta(n^{\log_ba}\qty(\prod_{i=1}^{k-1}(\operatorname{ilog}(i,n))^{-1})\qty(\operatorname{ilog}(k,n))^{m-1}),\]
then
\[T(n)=\Theta(n^{\log_ba}(\operatorname{ilog}(k,n))^m).\]
\item If for some positive integer $k$, 
\[f(n)=o(n^{\log_ba}\prod_{i=1}^k(\operatorname{ilog}(i,n))^{-1}),\]
then
\[T(n)=\Theta(n^{\log_ba}\operatorname{ilog}(k-1,n)).\]
...
\[T(n)=\Theta(n^{\log_ba}).\]
Case 2:
\[\operatorname{ilog}(k,b^x)=\Theta(\operatorname{ilog}(k-1,x))\]
\[f(n)=\Theta(n^{\log_ba}\qty(\prod_{i=1}^{k-1}(\operatorname{ilog}(i,n))^{-1})\qty(\operatorname{ilog}(k,n))^{m-1})\]
\[g(b^r)=\Theta(\qty(\prod_{i=1}^{k-2}(\operatorname{ilog}(i,r))^{-1})\qty(\operatorname{ilog}(k-1,r))^{m-1})\]
...
\end{proof}
\ssc{Existence}
\sssc{Monotone function is differentiable a.e.}
...
\ssc{Jacobian matrix of injective function}
...
\epr
\sssc{Divergence product rule}
For a scalar field $f$ and a vector field $\mb{G}$,
\[\nabla\cdot(f\mb{G})=\nabla f\cdot\mb{G}+f\nabla\cdot\mb{G}.\]
\sssc{Green's first identity}
...
\epr
\sssc{Series conditional convergence implies squared-index-multiplied-termed series divergence}
...
\epr
\sssc{Series conditional convergence can not determine index-multiplied-termed series divergence}
...
\epr
\sssc{(d'Alembert's ratio or Cauchy) ratio test}
\ben
...
Thus $\sum_{n=1}^{\infty}a_n$ is divergent.
\epr
\sssc{Raabe's test or Rabbe–Duhamel test}
...
TODO
\epr
\sssc{Power series multiplying rational function of $n$ does not affect radius of convergence}
...
\end{proof}
\sssc{p-test for series or p-series test}
...
\sssc{$\frac{1}{x^p(\ln x)^q}$-test for series or $\frac{1}{x^p(\ln x)^q}$-series test}
...
\epr
\sssc{Generalized p-test (for improper integral)}
Define iterated logarithm $\operatorname{ilog}(a,x)$ as
\[\operatorname{ilog}(0,x)=x,\]
\[\operatorname{ilog}(n,x)=\ln(\operatorname{ilog}(n-1,x)),\quad n\in\bbN.\]
For any fixed $a>1,k\in\bbN$,
\[\int_a^{\infty}\frac{1}{\qty(\prod_{i=0}^{k-1}\operatorname{ilog}(i,x))\qty(\operatorname{ilog}(k,x))^m}\dd{x}\]
converges iff $m>1$.

For any fixed $0<a<1,k\in\bbN$,
\[\int_0^a\frac{1}{\qty(\prod_{i=0}^{k-1}\operatorname{ilog}(i,x))\qty(\operatorname{ilog}(k,x))^m}\dd{x}\]
converges iff $m<1$.
\bpr
Lemma:
...
By p-test and lemma,
\epr
\sssc{Generalized p-test for series or generalized p-series test}
Define iterated logarithm $\operatorname{ilog}(a,x)$ as
\[\operatorname{ilog}(0,x)=x,\]
\[\operatorname{ilog}(n,x)=\ln(\operatorname{ilog}(n-1,x)),\quad n\in\bbN.\]
For any fixed $a\in\bbN$,
\[\sum_{n=a}^{\infty}\frac{1}{\qty(\prod_{i=0}^{k-1}\operatorname{ilog}(i,x))\qty(\operatorname{ilog}(k,x))^m}\]
converges iff $m>1$.
\bpr
PLACEHOLDER
\epr
