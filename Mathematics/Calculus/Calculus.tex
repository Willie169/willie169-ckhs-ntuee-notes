...
\sct{Lists}
\ssc{List of Derivatives and as definite integrals}
\sssc{Power function}
\[\dv{x^n}{x}=nx^{n-1}.\]
...
\[\dv{\tan(x)}{x}=\sec^2(x).\]
\[\tan(x)=\int_{\operatorname{round}\qty(\frac{x}{\pi})\pi}^x\sec^2(t)\dd{t}.\]
\begin{proof}
\[\begin{aligned}
...
\[\dv{\cot(x)}{x}=-\csc^2(x).\]
\[\cot(x)=-\int_{\frac{\pi}{2}+\operatorname{round}\qty(\frac{x}{\pi}-\frac{1}{2})\pi}^x\csc^2(t)\dd{t}.\]
\begin{proof}
...
\[\sec(x)=\int_{\operatorname{round}\qty(\frac{x}{\pi})\pi}^x\sec(t)\tan(t)\dd{t}.\]
\begin{proof}
\[\ba
