...
\ssc{Autonomous continuous-time dynamical system}
...
\sssc{Autonomous continuous-time dynamical system}
...
\sssc{Critical points (臨界點), equilibria, equilibrium points (平衡點), or stationary points (駐點) of systems of autonomous first-order ODEs}
...
\sssc{Systems of first-order ODEs as evolution rule of continuous-time dynamical systems}
...
\sssc{Systems of autonomous first-order ODEs as evolution rule of autonomous continuous-time dynamical systems}
Consider a system of autonomous first-order ODEs:
\[\dv{x}{t}=f(x), \quad x\in\mathbb{R}^n\land f\colon\mathbb{R}^n\to\mathbb{R}^n.\]

Let $U\subseteq\mathbb{R}\times\mathbb{R}^n$ be the set such that for any $(t_0,x_0)$ in $U$, $t_0$ is in the interval of definition of the solution of it with the initial condition $x(0)=x_0$ and that the solution is unique, and $x(t;x_0)$ be the solution of it with the initial condition $x(0)=x_0$.

Then the dynamical system represented by it has phase space $\mathbb{R}^n$, evolution parameter $t$, phase or state $x$, and evlotion function $\Phi\colon U\to\mathbb{R}^n;\Phi(t,x_0)=x(t;x_0)$. And the system of ODEs is called the evol
ution rule of the dynamical system.
\sssc{(Lyapunov) stable, Asymptotically stable, Exponentially stable, Semi-stable, and Unstable equilibria}
Let $x^*$ be an equilibrium of an autonomous continuous-time dynamical system defined by the evlotion rule $\dv{x}{t}=f(x)$. Then
\bit
\item $x^*$ is called to be Lyapunov stable if
    \[
        {{{
