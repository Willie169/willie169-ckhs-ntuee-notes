...
\bit
\item $\exists M>0,T\ge a\colon x\ge T\implies\abs{f(x)}\le M\abs{g(x)}$, i.e., $\limsup\abs{\frac{f(x)}{g(x)}}<\infty$, then $f(x)\in O(g(x))$ as $x\to\infty$ and $\abs{g(x)}$ is an asymptotic upper bound of $\abs{f(x)}$.
\item $\exists M>0,T\ge a\colon x\ge T\implies\abs{f(x)}\ge M\abs{g(x)}$, i.e., $\liminf\abs{\frac{f(x)}{g(x)}}>0$, then $f(x)\in \Omega(g(x))$ as $x\to\infty$ and $\abs{g(x)}$ is an asymptotic lower bound of $\abs{f(x)}$.
\item $\exists M_1,M_2>0,T\ge a\colon x\ge T\implies M_1\abs{g(x)}\le\abs{f(x)}\le M_2\abs{g(x)}$, then $f(x)\in \Theta(g(x))$ as $x\to\infty$.
\item $\forall M>0\colon\exists T\ge a\colon\abs{f(x)}<M\abs{g(x)}$, i.e., $\lim_{x\to\infty}\abs{\frac{f(x)}{g(x)}}=0$, then $f(x)\in o(g(x))$ as $x\to\infty$.
\item $\forall M>0\colon\exists T\ge a\colon\abs{f(x)}>M\abs{g(x)}$, i.e., $\lim_{x\to\infty}\abs{\frac{f(x)}{g(x)}}=\infty$, then $f(x)\in \omega(g(x))$ as $x\to\infty$.
\eit
