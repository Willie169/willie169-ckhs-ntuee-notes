...
\ssc{(Topological) curve}
\sssc{Definition}
A (parameterized) (topological) curve in a topological space $X$ is a continuous map $\gamma\colon I\to X$ for some interval $I$. In some context, the curve refers to the image and the parameterization refers to the map.
...
\sssc{As fake function with informally assigned properties}
The Dirac delta function $\delta(x)$ defined on $\bbR$ is a fake function denoted as
\[\delta(x)
\begin{cases}
\infty, & x = 0, \\
0, & x \neq 0.
\end{cases}\]
\[0\delta(x)=0.\]
For all $0\in\Omega\subseteq D_f\subseteq\bbR$, for all indexed family $A\colon I\to\mathcal{A}\subseteq D_f;\;i\mapsto A_i$ with $I=J\cap\bbN$ for some interval $J$,
\[\int f(x)\sum_{i\in I}\delta(A_i)\dd{x}\coloneq\sum_{i\in I}u(f(A_i)),\]
\[\int_{\Omega}f(x)\sum_{i\in I}\delta(A_i)\dd{x}\coloneq\sum_{i\in I}f(A_i).\]
Let
\[u_U(x)\coloneq\sum_{i\in I}u(A_i),\]
\[u_U'(x)\coloneq\sum_{i\in I}\delta(A_i).\]
\sssc{In higher dimensions}
For any $\mb{x}=(x_1,x_2,\ldots,x_n)\in\bbR^n$ with $n\in\bbN$:
\[\delta(\mb{x})=\sum_{k=1}^n\delta(x_i).\]
\sssc{As a measure}
For all indexed family $A\colon I\to\mathcal{A}\subseteq\bbR;\;i\mapsto A_i$ with $I=J\cap\bbN$ for some interval $J$, the definite integrals involving the sum of Dirac delta functions $\sum_{i\in I}\delta(A_i)$ can be rigorously defined via a Radon measure called Dirac measure $\delta_A$ that for any $\Omega\in\bbR$,
\[\delta_A(\Omega)\coloneq\abs{\Omega\cap A(I)}.\]
Thus for any function $f$ such that $0\in\Omega\subseteq D_f\subseteq\bbR$, for all such indexed family with $A(I)\subseteq D_f$,
\[\int_{\Omega}f(x)\sum_{i\in I}\delta(A_i)\dd{x}\coloneq\int_{\Omega}f(x)\dd{\delta_A(x)}=\sum_{i\in I}f(A_i).\]
\sssc{As proper or improper Riemann–Stieltjes integral}
For any function $f$ such that $0\in\Omega\subseteq D_f\subseteq\bbR$, for all indexed family $A\colon I\to\mathcal{A}\subseteq D_f;\;i\mapsto A_i$ with $I=J\cap\bbN$ for some interval $J$, with
\[u_A(x)\coloneq\sum_{i\in I}u(A_i),\]
where $u$ is the unit step function, we define
\[\int_{\Omega}f(x)\sum_{i\in I}\delta(A_i)\dd{x}\coloneq\int_{\Omega}f(x)\dd{u_A(x)}=\sum_{i\in I}f(A_i),\]
where $\int_{\Omega}f(x)\dd{u_A(x)}$ is a proper or improper Riemann–Stieltjes integral.
\sssc{As a distribution}
PLACEHOLDER
\sssc{Sinc function as nascent delta function}
For every Laplace-transformable complex function $\varphi(x)$:
\[\lim_{a\to 0}\int_{-\infty}^{\infty}\frac{\sin(\frac{\pi x}{a})}{\pi x}\varphi(x)\dd{x}=\lim_{a\to 0}\int_{-\infty}^{\infty}\frac{1}{a}\frac{\sin(\pi x)}{\pi x}\varphi(x)\dd{x}=\varphi(0).\]
