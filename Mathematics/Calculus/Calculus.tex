...
\begin{proof}\mbox{}\\
    ...
    Thus:
    \[\dv[k]{x^n}{x}=\qty(\prod_{j=0}^{k-1}(n-j))x^{n-k},\quad k\in\mnathbb{N}.\]
\end{proof}
<<<<<<< HEAD
...
\begin{proof}
    ...
    \ea\]
    Thus:
    \[\dv[k]{a^x}{x}=(\ln a)^ka^x,\quad k\in\mnathbb{N}.\]
\end{proof}
\sssc{Logarithmic function}
For any $x\in\mathbb{R}_{>0}$:
\[\ln(x)=\ln(a)+\sum_{n=1}^{\infty}(-1)^{n-1}\frac{(x-a)^n}{na^n},\quad 0<x\leq a.\]
\begin{proof}
    First, by the Fundamental Theorem of Calculus and the definition of $\ln(x)$:
    \[\dv{\ln(x)}{x}=\frac{1}{x}.\]
...
Lemma 1.0:
\[\dv{f(x)g(x)}{x}=f'(x)g(x)+f(x)g'(x).\]

Recall lemma 0, we obtain:
\[\lim_{h\to 0}\frac{\cos h-1}{h}=0.\]
...
Thus,
\[\dv[k]{\sin(x)}{x}=\sin\qty(x+\frac{n\pi}{2}).\]
\end{proof}
...
\begin{proof}
First, show that:
\[\dv{\tan(x)}{x}=\sec^2(x)=1+\tan^2(x).\]
\[\tan(x)=\frac{\sin(x)}{\cos(x)}.\]
Lemma 0: For differentiable functionS $f(x)$ and $g(x)$,
\[\dv{}{x}\qty(\frac{f(x)}{g(x)})=\frac{f'(x)g(x)-f(x)g'(x)}{g(x)^2},\quad g(x)\neq 0.\]
\[\begin{aligned}
...
Define the derivative polynomials $P_n$ such that $P_n\qty(\sec(x))=\sec^{(n)}(x)$ with recursion as:
\[\begin{cases}
&P_0(y)=y,\\
&P_n(y)=yP_{n-1}'(y),\quad n\in\mathbb{N}.
\end{cases}\]

The Taylor expansion of the cotangent function at $a$ is:
\[\sec(x)=\sum_{n=0}^{\infty}\frac{P_n\qty(\sec(x))(a)}{n!}(x-a)^n.\]
=======
...
>>>>>>> 68cb5be74d382753677f53f4fc44ad168f6d4169
