...
\sct{Ordinary differential equation (ODE) (常微分方程) and continuous-time dynamical system (連續時間動力系統)}
...
\sssc{Autonomous first-order ODE or autonomous DE}
An autonomous first-order ODE (aka an autonomous DE) is an ODE in the form 
...
If $c$ is a critical point, then $y(x)=c$ is a constant solution of the system of autonomous first-order ODEs, also called an equilibrium solution.
...
\sssc{Translation property or shift property of solutions of an autonomous first-order ODE}
Let function $\Phi(t,y)$ denotes the solution of the autonomous first-order ODE
\[\dv{x}{t}=f(x),\quad x\in D\subseteq\mathbb{R}^n\land f\colon D\to\mathbb{R}^n,\]
with IC $x(0)=y$.

Then the translation property or shift property states that
\[\forall s\in\mathbb{R}\colon\Phi(t+s,y)=\Phi(t,\Phi(s,y)).\]
\ssc{Dynamical system (動力系統 or 動態系統)}
...
Let $x^*$ be an equilibrium of an autonomous continuous-time dynamical system defined by the evlotion rule $\dv{x}{t}=f(x)$ and $A=\mathbb{R}_{\geq 0}$ or $\mathbb{R}$ be the domain of the evolution function. Then
\bit
\item $x^*$ is called to be (Lyapunov) stable if and only if
...
\item $x^*$ is called to be exponentially stable if and only if
...
\item $x^*$ is called to be marginally stable if it is (Lyapunov) stable but not asymptotically stable, typically for linear autonomous continuous-time dynamical system.
\item $x^*$ is called to be unstable if and only if it is not (Lyapunov) stable.
