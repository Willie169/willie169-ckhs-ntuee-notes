...
The arithmetic mean $\bar{f}$ of a function $f$ defined on a bounded interval $I\subseteq D_f$ is defined as the integral of $f$ over $I$ divided by the length of $I$.
\sssc{Measurable function}
The arithmetic mean of a measurable function $f$ defined on a measurable set $\Omega\subseteq D_f$ with $\mu(\Omega)\in\bbR$ is defined as
\[\bar{f}=\frac{1}{\mu(\Omega)}\int_{\Omega}f\dd{\mu}.\]
\ssc{Cesàro sum or Cesàro mean}
The Cesàro sum or Cesàro mean of a function $f$ over $[a,\infty)\subseteq D_f$ for $a\in\bbR$ with $\int_a^bf(x)\dd{x}$ being a proper integral for all $b\in(a,\infty)$ is defined as
...
The RMS $f_{rms}$ of a function $f$ defined on a bounded interval $I\subseteq D_f$ is defined as the square root of the arithmetic mean of $f^2$.
