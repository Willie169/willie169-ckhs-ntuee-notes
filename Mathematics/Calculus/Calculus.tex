...
The Heaviside step function or unit step function $u(x)\colon\bbR\to\bbR$ is defined as:
\[u(x)=
\begin{cases}0,\quad &x<0\\
1,\quad &x\geq 0
\end{cases},\]
where $u(0)$ is sometimes defined to be $\frac{1}{2}$ or other values instead.
...
\[\int_0^x\delta(t)\,\mathrm{d}t=u(x).\]
\[u'(x)=\delta(x).\]
...
\sssc{First shifting theorem, Time delay, or Time shift}
