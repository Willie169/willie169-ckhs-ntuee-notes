...
We say a series $S_n=\sum_{i=1}^{\infty}a_i$ converges absolutely to $L$ iff the series $\sum_{i=1}^{\infty}\abs{a_i}$ converges to $L$. If $S_n$ is convergent but not convergent absolutely, we say it is convergent conditionally.
...
\[\PV{\int_{a}^{b} f(x)\dd{x}}\coloneq\lim_{\epsilon\to 0}\qty(\int_a^{c-\epsilon}f(x)\dd{x}+\int_{c+\epsilon}^bf(x)\dd{x}),\]
if the limit exists.
...
An improper integral of a real function is convergent if it is absolutely convergent.
\ssc{Deleting head}
\sssc{For real series}
...
\end{proof}
If real sequences $\langle a_n\rangle_{n=1}^{\infty}$ and $\langle b_n\rangle_{n=1}^{\infty}$ are such that $a_n\leq b_n\leq 0$ for all $n\geq m$ for some $m\in\bbN$, then
\ben
\item $\sum_{n=1}^{\infty}a_n$ is convergent implies $\sum_{n=1}^{\infty}b_n$ is convergent.
\item $\sum_{n=1}^{\infty}b_n$ is divergent implies $\sum_{n=1}^{\infty}a_n$ is divergent.
\een
\sssc{For improper integral}
If real functions $f$ and $g$ are measurable on $ and $f(x)\geq g(x)\geq 0$ a.e. for $x\geq a$, then
\ben
\item $\int_a^{\infty}f(x)\dd{x}$ is convergent implies $\int_a^{\infty}g(x)\dd{x}$ is convergent.
\item $\int_a^{\infty}g(x)\dd{x}$ is divergent implies $\int_a^{\infty}f(x)\dd{x}$ is divergent.
\een
\begin{proof}
Let
\[F(b)=\int_a^bf(x)\dd{x},\quad b\geq a,\]
\[G(b)=\int_a^bg(x)\dd{x},\quad b\geq a.\]
$F$ and $G$ are monotone increasing since $f(x),g(x)\geq 0$ a.e. for $x\geq 0$.

$F(x)\geq G(x)$ since $f(x)\geq g(x)$ a.e. for $x\geq 0$.

Statement 1:
\[\int_a^{\infty}f(x)\dd{x}=\lim_{x\to\infty}F(x)=L.\]
\[0\leq G(x)\leq F(x)\]
Thus,
\[\int_a^bg(x)\dd{x}=\lim_{x\to\infty}G(x)\]
exists and $\geq 0$ and $\leq L$.

Statement 2:
\[\int_a^bg(x)\dd{x}=\lim_{x\to\infty}G(x)=\infty\]
\[0\leq G(x)\leq F(x)\]
Thus,
\[\int_a^bf(x)\dd{x}=\lim_{x\to\infty}F(x)=\infty.\]
\end{proof}
If real functions $f$ and $g$ are measurable on $ and $f(x)\leq g(x)\leq 0$ a.e. for $x\geq a$, then
\ben
\item $\int_a^{\infty}f(x)\dd{x}$ is convergent implies $\int_a^{\infty}g(x)\dd{x}$ is convergent.
\item $\int_a^{\infty}g(x)\dd{x}$ is divergent implies $\int_a^{\infty}f(x)\dd{x}$ is divergent.
\een
If real functions $f$ and $g$ are measurable and $f(x)\geq g(x)\geq 0$ a.e. for $x\leq a$, then
\ben
\item $\int_{-\infty}^af(x)\dd{x}$ is convergent implies $\int_{-\infty}^ag(x)\dd{x}$ is convergent.
\item $\int_{-\infty}^ag(x)\dd{x}$ is divergent implies $\int_{-\infty}^af(x)\dd{x}$ is divergent.
\een
If real functions $f$ and $g$ are measurable and $f(x)\leq g(x)\leq 0$ a.e. for $x\leq a$, then
\ben
\item $\int_{-\infty}^af(x)\dd{x}$ is convergent implies $\int_{-\infty}^ag(x)\dd{x}$ is convergent.
\item $\int_{-\infty}^ag(x)\dd{x}$ is divergent implies $\int_{-\infty}^af(x)\dd{x}$ is divergent.
\een
...
\end{proof}
\sssc{Secant function}
\[\int\sec(x)\dd{x}=\ln|\tan(x)+\sec(x)|+C=\ln|\tan(\frac{x}{2}+\frac{\pi}{4})+C.\]
\begin{proof}
\[\ba
...
\end{proof}
\sssc{Cosecant function}
\[\int\csc(x)\dd{x}=-\ln|\cot(x)+\csc(x)|+C=-\ln|\cot(\frac{x}{2})|+C.\]
\begin{proof}
\[\ba
