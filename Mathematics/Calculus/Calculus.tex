...
\[y^{(n)},\quad\tx{or\ }f^{(n)}(x),\]
in which $^{(n)}$ can be replaced with $n$ primes ($'$) (For example, $y''$ is equivalent to $y^{(2)}$, and $f''$ is equivalent to $f^{(2)}$.).
\sssc{Newton's notation (牛頓符號), dot notation, or fluxions for differentiation}
...
in which subscript are $n$ $x_i$s, $^{(n)}$ can be replaced with $n$ primes ($'$) (For example, $\partial''$ is equivalent to $\partial^{(2)}$, and $D''$ is equivalent to $D^{(2)}$.), and $\partial_{x_ix_i\ldots x_i}$, $\partial^{(n)}_{\pht{(n)}x_ix_i\ldots x_i}$, $D_{x_ix_i\ldots x_i}$, and $D^{(n)}_{\pht{(n)}x_ix_i\ldots x_i}$ are called partial differential operators or partial derivative operators;
...
in which subscript are $m_{i_j}$ $x_{i_j}$s for all $x_{i_j}$, $^{(n)}$ can be replaced with $n$ primes ($'$) (For example, $\partial''$ is equivalent to $\partial^{(2)}$, and $D''$ is equivalent to $D^{(2)}$.), and $\partial_{x_ix_i\ldots x_i}$, $\partial^{(n)}_{\pht{(n)}x_ix_i\ldots x_i}$, $D_{x_ix_i\ldots x_i}$, and $D^{(n)}_{\pht{(n)}x_ix_i\ldots x_i}$ are called partial differential operators or partial derivative operators.
...
If $f$ is differentiable at all point in $I\subeteq U$, we say $f$ is differentiable on $I$; if $f$ is differentiable at all point in its domain, we say $f$ is differentiable.
...
\end{itemize}
\ssc{Leibniz integral rule (萊布尼茲積分法則)}{{{
\ssc{Fubini's theorem (富比尼定理)}{{{
\ssc{(Lebesgue’s) Dominated convergence theorem ((勒貝格)控制/受制收斂定理)}{{{
...
A real function is Laplace-transformable if and only if it is piecewise continuous and of exponential order.
\begin{proof}{{{
...
We define a sequence of functions $f_n$ as 
\[\int_0^\infty f\qty(\frac{u}{s_n}) e^{-u} \dd{u},\]
where $\lim_{n\to\infty}s_n=\infty$.

For fixed $u \ge 0$, $\frac{u}{s_n} \to 0^+$ as $n\to\infty$, so $f_n\qty(\frac{u}{s})$ pointwise converges to $f(0^+)$.
...
$$
\int_0^\infty f(0^+) e^{-u} du = f(0^+) \int_0^\infty e^{-u} du = f(0^+).
$$
...
which is exactly the **initial value theorem**.

---

This is fully rigorous: the change of variables plus the dominated convergence theorem justifies the exchange of limit and integral.

---

If you want, I can also give the **rigorous proof using $f'(t)$** directly without the change of variables—it’s another standard way. It’s actually a bit shorter. Do you want me to show that too?
...
If poles of $sF(s)$ are in the left half-plane (except possibly at $s=0$):
...
Do you want me to also make a **summary table of these properties** (with formula + short description) for quick reference?
