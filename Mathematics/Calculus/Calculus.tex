...
\ssc{Of function between normed spaces}
Let $V,W$ be two normed spaces. Let \(I\) be an open subset of $V$ containing the point \(a\). Let $f$ be a function with codomain $W$ defined on \(I\), except possibly at \(a\) itself. The limit of \( f(x) \) as \( x \) approaches \( a \) is denoted as $\lim_{x \to a} f(x)$ or $\lim_{\|x-a\|_V \to 0} f(x)$. For $L\in W$:
\[\lim_{x \to a} f(x) = L \equiv \forall a_n\colon\bbN\to I\setminus\{a\} \tx{\ s.t.\ }\lim_{n\to\infty}\|a_n-a\|_V=0:\,\forall \varepsilon>0:\, \exists M \in\bbN \text{\ s.t.\ } n\geq M \implies \|f(a_n) - L\|_W < \varepsilon.\]
...
\sssc{Local Lipschitz (continuity) of functions between metric spaces}
...
\ssc{Ordinary Derivative of Functions with Real Domain}
...
\[f'_+(x)=\lim_{h\to 0^+}\frac{f(x+h)-f(x)}{h}\]
if the limit exists.
\ssc{Ordinary derivative of Functions with Real Vector Domain}
...
\ssc{Partial derivative of Functions with Real Vector Domain}
...
\ssc{Taylor series}
\sssc{Taylor series (泰勒級數) or Taylor expansion (泰勒展開)}
...
in which the first degree one is also called linear approximation, linearization, or tangent line, and
\[R_n(x)=f(x)-T_n(x)\]
is called remainder.

Taylor polynomial of at $0$ is called Maclaurin polynomial.
\sssc{Real analyticity (實解析性) of real-codomain functions}
...
\ssc{Extensions of Riemann–Stieltjes integral}
...
\sssc{Connection with improper Riemann integral}
...
\sssc{$L^p$ spaces ($L^p$ 空間) or Lebesgue space}
...
\sssc{Locally integrable $L^p$ space}
...
\sssc{Mesure induced by function}
...
\sssc{Function induced by measure}
...
\ssc{Lebesgue–Stieltjes integral}
\sssc{Definition}
...
\sssc{Connection with improper Riemann–Stieltjes integral}
...
\ssc{Root mean square (RMS or rms) or quadratic mean}
\sssc{Sequence}
...
\sssc{Curve of equality}
A curve of equality is defined with a real-valued function $f(\mb{x})$ and a real number $k$ called level as
\[f(\mb{x})=k,\]
also called level curve of $f$ at level $k$.
\sssc{Curve in Enclidean space}
...
\ssc{Frenet-Serret frame}
\sssc{Unit tangent vector or tangent}
The unit tangent vector of a curve $\mb{r}(t)$ at $t$ is
\[\mb{T}(t)=\frac{\mb{r}'(t)}{\abs{\mb{r}'(t)}}.\]
\sssc{Unit normal vector or normal}
\[\mb{N}(t)=\frac{\mb{T}'(t)}{\abs{\mb{T}'(t)}}.\]
\[\mb{T}\cdot\mb{N}=0.\]
\bpr
\[\ba
...
\ea\]
\[\mb{N}=\frac{\norm{\mb{r}'}^2\mb{r}''-\qty(\mb{r}'\cdot\mb{r}'')\mb{r}'}{\norm{\norm{\mb{r}'}^2\mb{r}''-\qty(\mb{r}'\cdot\mb{r}'')\mb{r}'}}\]
\[\norm{\mb{r}'}\norm{\norm{\mb{r}'}^2\mb{r}''-\qty(\mb{r}'\cdot\mb{r}'')\mb{r}'}\mb{T}\cdot\mb{N}=\norm{\mb{r}'}^2\qty(\mb{r}\cdot\mb{r}'')-\qty(\mb{r}'\cdot\mb{r}'')\qty(\mb{r}'\cdot\mb{r}')=0\]
\epr
In three dimensions,
\[\mb{N}=\frac{\mb{r}'\times\mb{r}''}{\norm{\mb{r}'\times\mb{r}''}}.\]
\sssc{Unit binormal vector or binormal}
In three dimensions,
\[\ba
\mb{B}(t)&\coloneq\mb{T}(t)\times\mb{N}(t)\\
&=\frac{\mb{r}'\times\qty(\mb{r}'\times\mb{r}'')}{\norm{\mb{r}'}\norm{\mb{r}'\times\mb{r}''}}
\ea\]
\sssc{Frenet-Serret frame}
...
\ssc{Curvature}
\sssc{Definition}
The curvature $\kappa$ of a curve $\mb{r}$ at $t$ is defined, with unit tagent vector $\mb{T}$ and arc length $s$, as
\[\kappa\coloneq\norm{\dv{\mb{T}}{s}}.\]
\sssc{In all dimensions}
\[\ba
\kappa&=\norm{\flatfrac{\dv{T}{t}}{\dv{s}{t}}\\
&=\frac{\norm{\norm{\mb{r}'}^2\mb{r}''-\qty(\mb{r}'\cdot\mb{r}'')\mb{r}'}}{\norm{\mb{r}'}^4}\\
&=\frac{\abs{\mb{r}''_{\perp\mb{r}'}}}{\norm{\mb{r}'}^2}
\ea\]
\bpr
\[\ba
\dv{}{t}\norm{\mb{r}'(t)}&=\dv{\sqrt{\mb{r}'\cdot\mb{r}'}}{t}\\
&=\frac{2\mb{r}'\cdot\mb{r}''}{2\sqrt{\mb{r}'\cdot\mb{r}'}}\\
&=\frac{\mb{r}'\cdot\mb{r}''}{\norm{\mb{r}'}}
\ea\]
\[\ba
\dv{T}{t}&=\dv{}{t}\qty(\frac{\mb{r}'}{\norm{\mb{r}'}})\\
&=\frac{\mb{r}''\norm{\mb{r}'}-\mb{r}'\frac{\mb{r}'\cdot\mb{r}''}{\norm{\mb{r}'}}}{\norm{\mb{r}'}^2}\\
&=\frac{\norm{\mb{r}'}^2\mb{r}''-\qty(\mb{r}'\cdot\mb{r}'')\mb{r}'}{\norm{\mb{r}'}^3}
\ea\]
\[\dv{s}{t}=\norm{\mb{r}'}\]
...
\ea\]
\epr
\sssc{In two dimensions}
Let $\mb{r}(t)=(x(t),y(t))$.
\[\kappa=\frac{\norm{x''y'-x'y''}{\qty(x'^2+y'^2)^{3/2}}.\]
\sssc{In three dimensions}
Let $\mb{r}\in\bbR_3$.
\[\kappa=\frac{\norm{\mb{r}'\times\mb{r}''}}{\norm{\mb{r}'}^3}.\]
\ssc{Torsion}
The torsion $\tau$ of a curve $\mb{r}$ at $t$ is defined, with unit normal vector $\mb{N}$ and unit binormal vector $\mb{B}$, as
\[\ba
\tau&\coloneq-\mb{B}'\cdot\mb{N}\\
&=\frac{\det(\mb{r}',\mb{r}'',\mb{r}''')}{\norm{\mb{r}'\times\mb{r}''}^2}
\ea\]
\bpr
\[\mb{B}=\frac{\mb{r}'\times\qty(\mb{r}'\times\mb{r}'')}{\norm{\mb{r}'}\norm{\mb{r}'\times\mb{r}''}}\]
\[\dv{}{t}\qty(\mb{r}'\times\qty(\mb{r}'\times\mb{r}''))=\mb{r}''\times\qty(\mb{r}'\times\mb{r}'')+\mb{r}'\times\qty(\mb{r}'\times\mb{r}''')\]
\[\dv{}{t}\qty(\norm{\mb{r}'}\norm{\mb{r}'\times\mb{r}''})=\frac{\mb{r}'\cdot\mb{r}''}{\norm{\mb{r}'}}\norm{\mb{r}'\times\mb{r}''}+\norm{\mb{r}'}\frac{\mb{r}'\times\mb{r}'''}{\norm{\mb{r}'\times\mb{r}''}}\]
\[\ba
...
\sct{Multivariable (多變數 or 多變量 or 多元) Calculus}
...
\sssc{Taylor series or Taylor expansion}
...
Therefore, $f(c)=N$.
\end{proof}
\sssc{For functions between topological spaces}
...
\ssc{Monotone function have at most countably many discontinuities}
...
\epr
\ssc{Uniform continuity implies continuity}
\sssc{For functions between metric spaces}
PLACEHOLDER
\sssc{For functions between topological vector spaces}
PLACEHOLDER
\ssc{Absolute continuity implies uniform continuity for functions from an interval to a metric space}
PLACEHOLDER
\ssc{Lipschitz continuity theorems}
\sssc{Lipschitz continuity implies absolute continuity for functions from an interval to a metric space}
PLACEHOLDER
\sssc{Local Lipschitz continuity on compact set implies global Lipschitz continuity on it}
PLACEHOLDER
...
\[\lim_{x\to a}h(x)=L.\]
\end{proof}
\ssc{Limit of real-valued net theorems}
...
\sct{Differential theorems and methods}
...
\sssc{Product rule for dot product}
...
\end{aligned}\]
\end{proof}
\sssc{Product rule for cross product}
...
\[D^{\beta}f(a)=c_{\beta}.\]
\epr
\sssc{Taylor's theorem on the reals}
...
PLACEHOLDER
\epr
\sssc{Taylor's theorem on higher dimensions}
...
\sct{Integral theorems and methods}
...
\sct{Numerical differentiation and integration}
...
\end{aligned}\]
\ssc{List of piecewise Functions}
...
\sssc{Rounding half to even (四捨六入五成雙) or banker's rounding}
...
\sssc{Limit of sinc function at zero}
...
\[\lim_{h\to 0}\frac{\sin h}{h}=1.\]
\end{proof}
\sssc{Limit of power over factorial}
...
\end{proof}
\sssc{Generalized complex binomial theorem}
...
\sssc{Lambert W function, omega function, or product logarithm}
...
\ea\]
\end{proof}
\ssc{List of integral defined functions}
...
\sct{Riemann zeta function or Euler–Riemann zeta function}
\ssc{Harmonic series (調和級數) and alternating harmonic series}
\sssc{Harmonic series}
\[H_M=\sum_{n=1}^M\frac{1}{n}.\]
\sssc{Alternating harmonic series}
...
\ea\]
\epr
\sssc{Euler–Mascheroni constant or Euler's constant}
\[\begin{aligned}
...
\epr
\sssc{Alternating harmonic series convergence}
...
\ea\]
\epr
\ssc{Riemann zeta function or Euler–Riemann zeta function}
\sssc{Definition}
