...
\bit
\item If for any $x,y\in I$ and $\alpha\in(0,1)$, $f((1-\alpha)x+\alpha y)\leq (1-\alpha)f(x)+\alpha f(y)$, often called "$f$ lies weakly below all of its secant lines on $I$", then $f$ is called to be weakly concave upward/convex (upwards)/CU/cup-shape.
\item If for any $x,y\in I$ and $\alpha\in(0,1)$, $f((1-\alpha)x+\alpha y)<(1-\alpha)f(x)+\alpha f(y)$, often called "$f$ lies strictly below all of its secant lines on $I$", then $f$ is called to be strictly concave upward/convex (upwards)/CU/cup-shape.
\item If for any $x,y\in I$ and $\alpha\in(0,1)$, $f((1-\alpha)x+\alpha y)\geq (1-\alpha)f(x)+\alpha f(y)$, often called "$f$ lies weakly above all of its secant lines on $I$", then $f$ is called to be weakly concave (downwards)/convex downwards/CD/cap-shape.
\item If for any $x,y\in I$ and $\alpha\in(0,1)$, $f((1-\alpha)x+\alpha y)>(1-\alpha)f(x)+\alpha f(y)$, often called "$f$ lies strictly above all its secant lines on $I$", then $f$ is called to be strictly concave (downwards)/convex downwards/CD/cap-shape.
\eit
The terms concave upward/convex (upwards)/CU/cup-shape and concave (downwards)/convex downwards/CD/cap-shape are define to be either weakly or strictly in different contexts, here we define them to be strictly.
...
\eit
\ssc{General form of L'Hôpital's rule (羅必達法則) or Bernoulli's rule}
