...
\epr
\ssc{Integral test}
Suppose $f(x)\colon [1,\infty)$ is bounded and monotone, and $\lim_{x\to\infty}f(x)=0$. Let $\langle a_n\rangle_{n\in\bbN}$ be the sequence defined by $a_n=f(n)$. Then, $\int_1^{\infty}f(x)\dd{x}$ is convergent iff $\sum_{n=1}^{\infty}a_n$ is convergent.
\bpr
Since $f$ is monotone, it is 
