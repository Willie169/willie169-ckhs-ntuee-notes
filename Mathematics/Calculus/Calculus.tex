...
\[f(t+T)=f(t)\]
for any $t$ in the domain of $f$, we say that $f$ is periodic and that the period (週期) of $f$ is any $T$, or sometimes the smallest $T$.
\ssc{Symmetry}
...
\sssc{Cesàro sum or Cesàro mean}
The Cesàro sum or Cesàro mean of a sequence $\langle a_i\rangle_{i=l}^{\infty}$ is defined as
...
The arithmetic mean $\bar{f}$ of a real-domain function $f$ over a bounded interval $I\subseteq D_f$ is defined as the integral of $f$ over $I$ divided by the length of $I$.
\sssc{Measurable function}
The arithmetic mean of a measurable function $f$ over a measurable set $\Omega\subseteq D_f$ with $\mu(\Omega)\in\bbR$ is defined as
\[\bar{f}=\frac{1}{\mu(\Omega)}\int_{\Omega}f\dd{\mu}.\]
\ssc{Cesàro sum or Cesàro mean}
The Cesàro sum or Cesàro mean of a real-domain function $f$ over $[a,\infty)\subseteq D_f$ for $a\in\bbR$ with $\int_a^bf(x)\dd{x}$ being a proper integral for all $b\in(a,\infty)$ is defined as
\[\lim_{n\to\infty}\frac{1}{n}\int_a^nf(x)\dd{x}.\]
If the Cesàro sum is finite, the function is called Cesàro-summable over $[a,\infty)$.

The Cesàro sum or Cesàro mean of a real-domain function $f$ over $(-\infty,b]\subseteq D_f$ for $b\in\bbR$ with $\int_a^bf(x)\dd{x}$ being a proper integral for all $a\in(-\infty,b)$ is defined as
...
\sssc{For set of values}
For set of values $\{x_1,x_2,\ldots,x_n\}$, the RMS $x_{rms}$ of it is given by:
\[x_{rms}=\sqrt{\frac{1}{n}\sum_{i=1}^nx_i^{\pht{i}2}}.\]
\sssc{For real-domain function}
The RMS $f_{rms}$ of a real-domain function $f$ over a bounded interval $I\subseteq D_f$ is defined as the square root of the arithmetic mean of $f^2$.

The RMS $f_{rms}$ of a real-domain function $f$ over an unbounded interval $I\subseteq D_f$ is defined as the square root of the Cesàro mean of $f^2$.

If $f$ is a periodic function with period $T\in\bbR_{>0}$, then the RMS of it over any unbounded interval or bounded interval of length $nT$ for some $n\in\bbN$ that is a subset of $D_f$ is the same.
\ssc{Sinc function as nascent delta function}
