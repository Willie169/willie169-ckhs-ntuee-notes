...
Let $f\colon X\subseteq\mathbb{R}\to Y$. When $(-\infty,a]\subseteq X$, for some $a\in\bbR$, we sometimes denote $\lim_{x\to-\infty}f(x)$ as $f(-\infty)$. When $[a,\infty)\subseteq X$, for some $a\in\bbR$, we sometimes denote $\lim_{x\to\infty}f(x)$ as $f(\infty)$.
...
\item\tb{Sequence of partial sums}: The sequence of partial sums of a sequence $\langle a_n\rangle_{n=l}^{\infty}$ is the sequence $\langle s_n=\sum_{i=l-1}^na_i\rangle_{n=l-1}^{\infty}$ where $s_{l-1}=0$ or $\langle s_n=\sum_{i=l}^na_i\rangle_{n=l}^{\infty}$.
\end{itemize}
\ssc{Series (級數)}
\begin{itemize}
...
\[\forall \varepsilon > 0:\,\int_{a-\varepsilon}^{a+\varepsilon} f(x) \delta(x-a) \, \mathrm{d}x = f(a).\]
...
For a real series
\[S_n = \sum_{i=l}^n a_i,\]
where \(a_i\) are terms of a real sequence \(\langle a_n\rangle_{n=l}^{\infty}\). The limit of \(S_n\) is denoted as \(\lim_{n\to\infty}S_n\) or \(\sum_{i=1}^{\infty}a_i\) and defined as the limit of the sequence \(\langle S_n\rangle_{n=l}^{\infty}\). If the limit exists and equal to $L$, we say the series converges to $L$; otherwise, we say the series diverges.
...
\sssc{(Pointwise) limit inferior, infimum limit, limit infimum, liminf, inferior limit, lower limit, or inner limit function, and limit superior, supremum limit, limit supremum, limsup, superior limit, upper limit, or outer limit function}
Let $X$ be a set and $Y$ be a topological space. A net of functions $\langle f_a\rangle_{a\in A}$ all having the same domain $X$ and codomain $Y$ is said to have a (pointwise) limit inferior function $f\colon X\to Y$, denoted as 
\[\liminf_af_a=f\text{\ pointwise}\]
or
\[\liminf_af_a=f,\]
iff
\[\forall x\in X\colon\liminf_af_a(x)=f(x),\]
and is said to have a (pointwise) limit superior function $f\colon X\to Y$, denoted as 
\[\limsup_af_a=f\text{\ pointwise}\]
or
\[\limsup_af_a=f,\]
iff
\[\forall x\in X\colon\limsup_af_a(x)=f(x).\]
\sssc{Uniform limit}
...
\sssc{Definition}
Below, we will define the Riemann–Stieltjes integral of a function $f\colon X\subseteq\bbR\to\bbR$ over another function $g\colon Y\subseteq\bbR\to\bbR$ over an interval $[a,b]\subseteq (X\cap Y)$,
...
\bit
\item If a limit inferior exists, it is unique.
\item If a limit superior exists, it is unique.
...
\sssc{For real net}
\bit
\item If a limit inferior exists, it is unique, and all cluster points are greater than or equal to it.
\item If a limit superior exists, it is unique, and all cluster points are less than or equal to it.
...
\bit
\item If a pointwise limit inferior function exists, it is unique.
\item If a pointwise limit superior function exists, it is unique.
\item If a pointwise limit function exists, it is unique, and the pointwise limit inferior function and pointwise limit superior function are it.
\item If a uniform limit function exists, it is unique, and the pointwise limit function, pointwise limit inferior function, and pointwise limit superior function are it.
\eit
\ssc{Limit Laws}
...
\end{proof}
\sssc{Convergent improper integral implies limit of product of integrand and independent variable converges to 0 for nonnegative and non-increasing integrand}
Let $f$ be a real function.

If $\int_a^{\infty}f(x)\dd{x}$ converges and $f(x)$ is nonnegative and non-increasing on $(a,\infty)$, then $\lim_{x\to\infty}xf(x)=0$.
\begin{proof}
...
\end{proof}
If $\int_a^{\infty}f(x)\dd{x}$ converges and $f(x)$ is nonpositive and non-decreasing on $(a,\infty)$, then $\lim_{x\to\infty}xf(x)=0$.

If $\int_{-\infty}^af(x)\dd{x}$ converges and $f(x)$ is nonnegative and non-increasing on $(-\infty,a)$, then $\lim_{x\to-\infty}xf(x)=0$.

If $\int_{-\infty}^af(x)\dd{x}$ converges and $f(x)$ is nonpositive and non-decreasing on $(-\infty,a)$, then $\lim_{x\to-\infty}xf(x)=0$.
...
\sssc{For improper integral}
For real function $f$ defined on $[a,\infty)$, $\int_a^{\infty}f(x)\dd{x}$ converges iff $\int_{a+b}^{\infty}f(x)\dd{x}$ converges, for all $b\in\bbR_{>0}$.

For real function $f$ defined on $(-\infty,a]$, $\int_{-\infty}^af(x)\dd{x}$ converges iff $\int_{-\infty}^{a-b}f(x)\dd{x}$ converges, for all $b\in\bbR_{>0}$.
\ssc{Direct comparison theorem}
\sssc{For real series}
If real sequences $\langle a_n\rangle_{n=1}^{\infty}$ and $\langle b_n\rangle_{n=1}^{\infty}$ are such that $a_n\geq b_n\geq 0$ for all $n\geq m$ for some $m\in\bbN$, then
\ben
...
\een
\begin{proof}
Let $\langle s_n=\sum_{i=1}^na_i\rangle_{i=1}^{\infty}$ and $\langle t_n=\sum_{i=1}^na_i\rangle_{i=1}^{\infty}$ be the sequences of partial sums of $\langle a_n\rangle_{n=1}^{\infty}$ and $\langle b_n\rangle_{n=1}^{\infty}$.

$s_n$ and $t_n$ are monotone increasing for all $n\geq m$ since $a_n,b_n\geq 0$ for all $n\geq m$.

$s_n\geq t_n$ for all $n\geq p$ for some $p\in\bbN$ since $a_n\geq b_n$ for all $n\geq m$.
...
exists and $\geq 0$ and $\leq L$.

Statement 2:
\[\sum_{n=1}^{\infty}b_n=\lim_{n\to\infty}t_n=\infty\]
\[0\leq t_n\leq s_n\]
for all $n\geq p$.

Thus,
\[\sum_{n=1}^{\infty}a_n=\lim_{n\to\infty}s_n=\infty\]
\end{proof}
\sssc{For improper integral}
If functions $f\colon [a,b)\to\bbR$ and $g\colon [a,b)\to\bbR$ are measurable with $b\in(a,\infty]$, $\int_a^bf(x)\dd{x}$ and $\int_a^bg(x)\dd{x}$ are improper integral, $\int_a^cf(x)\dd{x}$ and $\int_a^cg(x)\dd{x}$ are proper integrals for any $a<c<b$,
...
Statement 2:
\[\int_a^bg(x)\dd{x}=\lim_{t\to b}G(t)=\infty\]
\[0\leq G(x)\leq F(x)\]
Thus,
\[\int_a^bf(x)\dd{x}=\lim_{t\to b}F(t)=\infty.\]
\end{proof}
\ssc{Limit comparison theorem (LCT)}
\sssc{For real series}
If real sequences $\langle a_n\rangle_{n=1}^{\infty}$ and $\langle b_n\rangle_{n=1}^{\infty}$ are such that $a_n\geq 0$ and $b_n>0$ for all $n\geq m$ for some $m\in\bbN$, let
\[L=\lim_{n\to\infty}\frac{a_n}{b_n},\]
then:
\ben
\item If $0<L<\infty$, then $\sum_{n=1}^{\infty}a_n$ is convergent iff $\sum_{n=1}^{\infty}b_n$ is convergent. (classical LCT)
\item if $L=0$, then $\sum_{n=1}^{\infty}b_n$ is convergent implies $\sum_{n=1}^{\infty}a_n$ is convergent. (one-sided LCT)
\item If $L=\infty$, then $\sum_{n=1}^{\infty}b_n$ is divergent implies $\sum_{n=1}^{\infty}a_n$ is divergent. (one-sided LCT)
\een
\begin{proof}
Statement 1:
...
As $L>0$, choose $\varepsilon$ such that $L-\varepsilon>0$, so
\[\frac{a_n}{L+\varepsilon}<b_n<\frac{a_n}{L-\varepsilon}.\]
$\sum_{n=1}^{\infty}\frac{a_n}{L+\varepsilon}$ converges iff $\sum_{n=1}^{\infty}a_n$ converges iff $\sum_{n=1}^{\infty}\frac{a_n}{L-\varepsilon}$ converges.
...
For every $\varepsilon_1>0$, there exists $N_1\in\bbN$ such that for all $n\geq N_1$, $\frac{a_n}{b_n}<\varepsilon_1$.

For every $\varepsilon_2>0$, there exists $N_2\in\bbN$ such that for all $n\geq N_2$, $b_n<\varepsilon_2$.

Take $N_2$ for $varepsilon_2\leq 1$, for every $\varepsilon_1>0$, there exists $N=\max(N_1,N_2)\in\bbN$ such that for all $n\geq N$, $a_n<\varepsilon_1$

Statement 3:

For every $M_1>0$, there exists $N_1\in\bbN$ such that for all $n\geq N_1$, $\frac{a_n}{b_n}>M_1$.

For every $M_2>0$, there exists $N_2\in\bbN$ such that for all $n\geq N_2$, $b_n>M_2$.

Take $N_2$ for $M_2\geq 1$, for every $M_1>0$, there exists $N=\max(N_1,N_2)\in\bbN$ such that for all $n\geq N$, $a_n>M_1$.
\end{proof}
\sssc{For improper integral}
If functions $f\colon [a,b)\to\bbR$ and $g\colon [a,b)\to\bbR$ are measurable with $b\in(a,\infty]$, $\int_a^bf(x)\dd{x}$ and $\int_a^bg(x)\dd{x}$ are improper integral, $\int_a^cf(x)\dd{x}$ and $\int_a^cg(x)\dd{x}$ are proper integrals for any $a<c<b$, $f(x)\geq 0$ and $g(x)>0$ a.e. for $x\in[a,b)$, let
\[L=\frac{\int_a^bf(x)\dd{x}}{\int_a^bg(x)\dd{x}},\]
then:
\ben
\item If $0<L<\infty$, then $\int_a^bf(x)\dd{x}$ is convergent iff $\int_a^bg(x)\dd{x}$ is convergent. (classical LCT)
\item if $L=0$, then $\int_a^bg(x)\dd{x}$ is convergent implies $\int_a^bf(x)\dd{x}$ is convergent. (one-sided LCT)
\item If $L=\infty$, then $\int_a^bg(x)\dd{x}$ is divergent implies $\int_a^bf(x)\dd{x}$ is divergent. (one-sided LCT)
\een
\begin{proof}
待
\end{proof}
...
\end{proof}
\sssc{For improper integral}
If $f\colon [a,\infty)\to\bbR$ is a function such that the function
\[F(x)=\int_a^xf(t)\dd{t},\quad x\geq a\]
is bounded and function $g\colon [a,\infty)\to\bbR$ is monotone such that $\lim_{x\to\infty}g(x)=0$, then
\[\int_a^{\infty}f(x)g(x)\dd{x}\]
converges.
\begin{proof}
Rewrite as Riemann–Stieltjes integral
\[\int_a^{\infty}f(x)g(x)\dd{x}=\int_{x=a}^{\infty}g(x)\dd{F(x)}.\]

Choose $M$ such that $\abs{F(x)}$ is bounded above by $M$.

For any $\varepsilon>0$, there is $N\in\bbR$ such that $\abs{g(N)}<\frac{\varepsilon}{2M}$ for all $m\geq N$.

For any $N\leq p\leq q$, using integration by parts of Riemann–Stieltjes integral,
\[\ba
...
\sssc{Integration by parts or partial integration of Riemann–Stieltjes integral}
...
\[\sqrt{ax^2+bx+c}=\sqrt{|ak|}\sqrt{\operatorname{sgn}\qty(a)\qty(\frac{x}{\sqrt{|k|}}+\frac{h}{\sqrt{|k|}})^2+\operatorname{sgn}\qty(k)},\]
\iten use substitution $u=\frac{x}{\sqrt{|k|}}+\frac{h}{\sqrt{|k|}}$, which makes the integrand,
\[\sqrt{|k|}R\qty(\sqrt{|k|}u-h,\sqrt{|ak|}\sqrt{\operatorname{sgn}\qty(a)u^2+\operatorname{sgn}\qty(k)}),\]
which is a rational function of $u$ and $\sqrt{\operatorname{sgn}\qty(a)u^2+\operatorname{sgn}\qty(k)}$, and,
\item depending on $\operatorname{sgn}\qty(a)$ and $\operatorname{sgn}\qty(k)$,
\bit
...
\ssc{Area of Lune}
The area of the crescent-shaped region (called a lune) bounded by arcs of circles $C:x^2+y^2=r^2$ and $D:x^2+(y+\sqrt{R^2-r^2})^2=R^2$ inside the circle $C$ outside of the circle $D$ is
\[\frac{\pi r^2}{2}+r\sqrt{R^2-r^2}-R^2\arcsin(\frac{r}{R}).\]
\begin{proof}
\[y=\sqrt{r^2-x^2}\]
\[y=\sqrt{R^2-x^2}-\sqrt{R^2-r^2}\]
\[\int_{-r}^r\sqrt{r^2-x^2}-\sqrt{R^2-x^2}+\sqrt{R^2-r^2}\dd{x}=\frac{\pi r^2}{2}+2r\sqrt{R^2-r^2}-R^2\int_{-r/R}^{r/R}\sqrt{1-u^2}\dd{u},\quad u=\frac{x}{R},\quad\dd{u}=\frac{1}{R}\dd{x}\]
\[\ba
\int_{-r/R}^{r/R}\sqrt{1-u^2}\dd{u}&=\int_{\arcsin(-r/R)}^{\arcsin(r/R)}\cos^2t\dd{t},\quad u=\sin t,\quad\dd{u}=\cos t\dd{t}\\
&=\frac{1}{2}\int_{\arcsin(-r/R)}^{\arcsin(r/R)}1+\cos(2t)\dd{t}\\
&=\arcsin(\frac{r}{R})+\frac{r}{R}\sqrt{1-\qty(\frac{r}{R})^2}
\ea\]
\[\int_{-r}^r\sqrt{r^2-x^2}-\sqrt{R^2-x^2}+\sqrt{R^2-r^2}\dd{x}=\frac{\pi r^2}{2}+r\sqrt{R^2-r^2}-R^2\arcsin(\frac{r}{R})\]
\end{proof}
...
\[\dv{|x|}{x}=\operatorname{sgn}\qty(x),\quad x\neq 0.\]
...
\[\arcsec(x)=\operatorname{sgn}\qty(x)\int_1^{\abs{x}}\frac{1}{t\sqrt{t^2-1}}\dd{t}+(1-\operatorname{sgn}\qty(x))\frac{\pi}{2},\quad\abs{x}\geq 1.\]
\begin{proof}
\[\ba
...
\[\arccsc(x)=\operatorname{sgn}\qty(x)\int_{\abs{x}}^{\infty}\frac{1}{t\sqrt{t^2-1}}\dd{t},\quad\abs{x}\geq 1.\]
\begin{proof}
\[\ba
...
\begin{proof}
\[\ba
\dv{\operatorname{arcsech}\qty(x)}{x}&=\dv{}{x}\ln(\frac{1}{x}+\frac{\sqrt{1-x^2}}{x})\\
...
\begin{proof}
\[\ba
\dv{\operatorname{arccsch}\qty(x)}{x}&=\dv{}{x}\ln(\frac{1}{x}+\frac{\sqrt{1+x^2}}{|x|})\\
&=\dv{}{x}\ln(\frac{1+\operatorname{sgn}\qty(x)\sqrt{1+x^2}}{x})\\
&=\frac{x}{1+\operatorname{sgn}\qty(x)\sqrt{1+x^2}}\frac{\frac{\operatorname{sgn}\qty(x)x}{\sqrt{1+x^2}}x-\qty(1+\operatorname{sgn}\qty(x)\sqrt{1+x^2})}{x^2}\\
&=\frac{1}{1+\operatorname{sgn}\qty(x)\sqrt{1+x^2}}\frac{-\sqrt{1+x^2}-\operatorname{sgn}\qty(x)}{x\sqrt{1+x^2}}\\
&=\frac{-\operatorname{sgn}\qty(x)}{x\sqrt{1+x^2}}\\
&=-\frac{1}{|x|\sqrt{1+x^2}}
\ea\]
\end{proof}
...
&=t+C=\operatorname{sgn}\qty(u)\arccosh u+C\\
&=\operatorname{sgn}\qty(x)\arccosh\abs{\frac{x}{a}}+C\\
