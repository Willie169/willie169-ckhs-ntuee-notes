...
\sssc{Heaviside step function (黑維塞階躍函數)/unit step function (單位階躍函數)}
\[H(x)=
...
\sssc{Squeeze (夾擠) or Sandwich (三明治) theorem}
...
\sssc{Squeeze or Sandwich theorem}
Given sequences $\langle a_n\rangle$, $\langle b_n\rangle$, and $\langle c_n\rangle$ which for all $k$ that is greater than a specific positive integer $j$:
...
\subsubsection{Squeeze or Sandwich theorem at finity}
...
\[\lim_{x\to a^-}f(x)\leq\lim_{x\to a^-}g(x).\]
\subsubsection{Squeeze or Sandwich theorem at finity of one-sided limit}
...
\[\lim_{x\to-\infty}f(x)\leq\lim_{x\to-\infty}g(x).\]
\subsubsection{Squeeze or Sandwich theorem at infinity}
...
Let $X$ be a set and $Y$ be topological space. A sequence of functions $f_n$ all having the same domain $X$ and codomain $Y$ is said to converge pointwise to a given function $f\colon X\to Y$, denoted as 
...
\eit
\sssc{Piecewise continuity}
For a function $f$ defined on an interval $I$, if there is a finite set $J$ (i.e. $n(J)\in\mathbb{N}) of open subintervals of $I$ such that $n(I\setminus\bigcup_{j\in J}j)\in\mathbb{N}_0$ and that $f$ is continuous on each $j\in J$, we say $f$ is piecewise-continuous.
...
\sssc{Examples of continuous functions}
The following types of functions are continuous at every number in their domain: rational functions, trigonometric functions, inverse trigonometric functions, exponential functions, logarithmic functions.
\sssc{Intermediate Value Theorem, IVT (中間值定理)}
...
\ssc{Inverse Laplace transform (反拉普拉斯變換)}
The inverse Laplace transform of a complex function $F(s)$, denoted as $\mathrm{L}^{-1}\{F(s)\}(t)$ or $f(t)$, is defined as a real function such that
\[\mathcal{L}\{f(t)\}(s) = F(s),\]
where $\mathcal {L}$ denotes the Laplace transform.
\sssc{Mellin's inverse formula, Bromwich integral, or Fourier–Mellin integral}
The inverse Laplace transform of a complex function $F(s)$ is given by the line integral:
\[\mathrm{L}^{-1}\{F(s)\}(t)=f(t)=\frac{1}{2\pi i}\lim_{T\to\infty}\int_{\gamma-iT}^{\gamma+iT}e^{st}F(s)\dd{s},\]
...
\ssc{Laplace Transform Table}

\ssc{Fourier Transform Table}
