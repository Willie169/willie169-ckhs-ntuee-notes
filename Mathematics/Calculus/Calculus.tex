...
\end{proof}
\sssc{Racetrack principle}
If $f'(x)>g'(x)$ for all $x>a$, and $f(a)=g(a)$, then $f(x)>g(x)$ for all $x>a$.

If $f'(x)\geq g'(x)$ for all $x>a$, and $f(a)=g(a)$, then $f(x)\geq g(x)$ for all $x>a$.
\begin{proof}
For the first statement, let $h(x)=f(x)-g(x)$.
\[h'(x)=f'(x)-g'(x)>0,\quad\forall x>a.\]
\[h(a)=f(a)-g(a)=0.\]
By MVT, there exists $c\in(a,x)$ such that
\[\frac{h(x)-h(a)}{x-a}=h'(c)>0,\quad\forall x>a.\]
\[h(x)=(x-a)h'(c)>0,\quad\forall x>a.\]
...
\end{proof}
Let functions $f\colon I\subseteq\mathbb{R}\to\mathbb{R}$ be differentiable on an open interval $(a,b)\subseteq I$ and continuous on $[a,b]$ with $a<b$.
\bit
