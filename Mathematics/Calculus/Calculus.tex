...
\epr
\item $O(g(x))$ is partial-ordered set.
\bpr
Transitive:
\[\exists M>0,T\ge a\colon x\ge T\implies\abs{f(x)}\le M\abs{g(x)}\]
...
\[\exists M>0,T\ge a\colon x\ge T\implies\abs{f(x)}\le M\abs{g(x)}\]
\epr
\item $\Omega(g(x))$ is partial-ordered set.
$\exists M>0,T\ge a\colon x\ge T\implies\abs{f(x)}\ge M\abs{g(x)}$ iff $f(x)\in \Omega(g(x))$ and $\abs{g(x)}$ is an asymptotic lower bound of $\abs{f(x)}$.
\item $o(g(x))$ and $\omega(g(x))$ are strictly partially-ordered sets.
\bpr
PLACEHOLDER
...
\item Irreflexivity or anti-reflexive: $\neq aRa$,
\item Symmetry: $aRb\implies bRa$,
\item Asymmetry: $aRb\implies \neg bRa$ (which implies irreflexivity),
\item Antisymmetry: $aRb\land bRa\implies a=b$,
