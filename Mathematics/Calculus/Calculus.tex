...
Below, we are discussing limits of sequences in the set of real number and $\mathbb{N}$ denotes the domain of them.
\subsubsection{Definition}
For a real sequence \(\langle a_n\rangle\), the limit of \(\langle a_n\rangle\), denoted as $\lim_{n \to \infty} a_n$ or $\lim_n a_n$, is defined as follows:
\[\lim_{n \to \infty} a_n = L \equiv \forall \epsilon > 0:\, \exists N \in\mathbb{N}\text{\ s.t.\ } n\in\mathbb{N}\land n \geq N\implies |a_n - L| < \epsilon.\]
In other words, as \(n\) becomes arbitrarily large, \(a_n\) gets arbitrarily close to \(L\).

If such $N$ exists, we say the limit exists or the sequence converges (收斂) to $L$; otherwise, we say the limit doesn't exist or the sequence diverge (發散).
\subsubsection{Infinite Limits}
\[\lim_{n\to \infty}a_n=\infty \equiv \forall M > 0, \exists N \in\mathbb{N} \text{\ s.t.\ } n\in\mathbb{N}\land n \geq N \implies a_n > M.\]
\[\lim_{n\to \infty}a_n=-\infty \equiv \forall M < 0, \exists N \in\mathbb{N} \text{\ s.t.\ } n\in\mathbb{N}\land n \geq N \implies a_n < M.\]
...
\[\forall\epsilon>0:\,\exists N\text{\ s.t.\ }\forall n\geq N:\,|c-a_n|=c-a_n\leq c-a_N=|c-a_N|<\epsilon.\]
...
If such $N$ exists, we say the limit exists or the series converges to $L$; otherwise, we say the limit doesn't exist or the series diverge.
...
If such $N$ exists, we say the limit exists or the net converges to $L$; otherwise, we say the limit doesn't exist.
\subsubsection{Infinite Limits}
...
\begin{itemize}
\item For a point $a$ in the domain of a function $f$, if and only if $\exists\lim_{x\to a}f(x)$ and $\lim_{x\to a}f(x)=f(a)$, we say $f(x)$ is continuous (連續的) at $a$; otherwise, we say $f(x)$ is discontinuous (不連續的) at $a$. A point that $f$ is discontinuous at is called a singularity or singular point (奇點).
...
\[y=f(\mb{x}).\]
Then the partial derivative of the function $f$ with respect to $x_i$ can be written as
\[y_{x_i},\quad y'_{x_i},\quad f_{x_i},\quad f'_{x_i},\quad\partial_{x_i}f,\quad \tx{or\ }D_{x_i}f;\]
the $n$th partial derivative of the function $f$ with respect to $x_i$ can be written as below, in which subscript are $n$ $x_i$s
\[y_{x_ix_i\ldots x_i},\quad y'_{x_ix_i\ldots x_i},\quad f_{x_ix_i\ldots x_i},\quad f'_{x_ix_i\ldots x_i},\quad\partial_{x_ix_i\ldots x_i}f,\quad \tx{or\ }D_{x_ix_i\ldots x_i}f;\]
...
\[y_{x_{i_1}x_{i_1}\ldots x_{i_2}x_{i_2}x_{i_2}\ldots x_{i_2}\ldots x_{i_k}x_{i_k}\ldots x_{i_k}},\quad y'_{x_{i_1}x_{i_1}\ldots x_{i_2}x_{i_2}x_{i_2}\ldots x_{i_2}\ldots x_{i_k}x_{i_k}\ldots x_{i_k}},\]
\[f_{x_{i_1}x_{i_1}\ldots x_{i_2}x_{i_2}x_{i_2}\ldots x_{i_2}\ldots x_{i_k}x_{i_k}\ldots x_{i_k}},\quad f'_{x_{i_1}x_{i_1}\ldots x_{i_2}x_{i_2}x_{i_2}\ldots x_{i_2}\ldots x_{i_k}x_{i_k}\ldots x_{i_k}},\]
\[\partial_{x_{i_1}x_{i_1}\ldots x_{i_2}x_{i_2}x_{i_2}\ldots x_{i_2}\ldots x_{i_k}x_{i_k}\ldots x_{i_k}}f,\quad \tx{or\ }D_{x_{i_1}x_{i_1}\ldots x_{i_2}x_{i_2}x_{i_2}\ldots x_{i_2}\ldots x_{i_k}x_{i_k}\ldots x_{i_k}}f.\]
\ssc{Fréchet Differentiation (弗蘭歇微分)}
\sssc{Fréchet derivative (弗蘭歇導數), ordinary derivative (常導數), or derivative (導數)}
Let $V$ and $W$ be normed vector spaces and $U$ be an open subset of $U$. A function $f\colon U\to W$ is Fréchet differentiable (弗蘭歇可微的) or differentiable at $x\in U$ if there exists a bounded linear operator $A\colon V\to W$ such that
...
We define the (first) Fréchet derivative function (弗蘭歇導函數) or derivative function (導函數) of $f$, denoted as $Df$, as a function $Df\colon U\to B(V,W)$, in which $B(V,W)$ is the space of all bounded linear operators from $V$ to $W$, such that for any $x\in U$ at which $f$ is differentiable, $Df$ maps $x$ to the derivative of $f$ at $x$.

If such derivative exists, the derivative of the $k$th ($k\in\mathbb{N}$) derivative function of $f$ at $x\in U$ is called the $k+1$th derivative of $f$ at $x\in U$. If such function exists, the derivative function of the $k$th ($k\in\mathbb{N}$) derivative function of $f$ is called the $k+1$th derivative function of $f$.
...
If $f$ is differentiable at all point in its domain, we say $f$ is differentiable.

The operation of finding the derivative or derivative function is called Fréchet differentiation (弗蘭歇微分), ordinary differentiation (常微分), or differentiation (微分).
...
\sssc{Gateaux derivative (加托導數) and partial derivatives (偏導數)}
Let $V$ be a locally convex topological vector spaces (LCTVS), $W$ be a topological vector space, and $U$ be an open subset of $V$. Given a function $F\colon U\to W$, the (first) Gateaux derivative $dF(u;\,\psi)$ of $F$ at $u\in U$ in the direction $\psi \in V$ is defined to be
\[\begin{aligned}
...
\ssc{Lebesgue’s Dominated convergence theorem (勒貝格控制收斂定理 or 勒貝格受制收斂定理)}{{{
