...
PLACEHOLDER待
\sssc{Constant}
\[\mathcal{L}\{C\}=\frac{C}{s},\quad C\in\bbC.\]
ROC: $\Re(s)>0$.
\sssc{Unit impulses}
For any indexed family $A\colon I\to\mathcal{A}\subseteq D;\;i\mapsto A_i$ with $I=J\cap\bbN$ for some interval $J\ni 1$, the function:
\[\mathcal{L}\{\sum_{i\in I}\delta(t-A_i)\}=\sum_{i\in I}e^{-sA_i},\quad \mathcal{A}\subseteq\bbR_{\geq 0}.\]
ROC: $s\in\bbC$ if $\mathcal{A}\cap\bbR_{>0}=\varnothing$, $\Re(s)>0$ otherwise.
\sssc{Unit step}
