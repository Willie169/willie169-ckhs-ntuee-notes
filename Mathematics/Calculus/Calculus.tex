...
\subsubsection{Limit}
For a real-valued net $\langle x_a\rangle_{a\in A}$ in which $A$ is a directed set, the limit of $\langle x_a\rangle_{a\in A}$ is denoted as $\lim_a x_a$. For $L\in\bbR$:
...
\subsection{Absolute continuity (絕對連續) of functions from an interval to a metric space}
...
\eit
\subsection{Differential, total differential, or infinitesimal change}
...
\[\dd{f}=f'\dd{x}.\]
For a normed vector-valued function $f$ of independent variables $x_1,x_2,\ldots,x_n$, we define the differential of it, $\dd{f}$, in terms of the differentials of $x_1,x_2,\ldots,x_n$, denoted as $\dd{x_1},\dd{x_2},\ldots,\dd{x_n}$, as
...
\subsubsection{Real analyticity (實解析性) of real-codomain functions}
...
\subsubsection{Definition}
Plane curve or polar equation is a plane curve in the form $r=f(\theta)$, also denoted as $r(\theta)$.
...
The determinant of Jacobian matrix is called Jacobian determinant, or Jacobian for short.\subsubsection{Divergence (散度)}
\[
...
\[\lim_af(x_a)=L.\]
\subsubsection{Monotone Convergence Theorem (單調收斂定理)}
...
\subsection{Theorems and methods of differentiation}
...
\subsubsection{Implicit function theorem (隱函數定理) of real-valued function of two real variables}
If $f(x,y)$ is a function that is continuously differentiable on a neighborhood of a point $(x_0,y_0)$ and $\pdv{f}{y}(x_0,y_0)\neq 0$ on a neighborhood of $(x_0,y_0)$, then $f$ is injective w.r.t. $y$ on a neighborhood of $(x_0,y_0)$, and there exists a unique differentiable function $g$ such that $y_0=g(x_0)$ and $f(x,g(x))=0$ in a neighborhood of $x_0$. Moreover,
\[g'(x)=-\frac{\pdv{f}{x}(x,g(x))}{\pdv{f}{y}(x,\varphi(x))}.\]
\bpr
Differentiate both sides of $f(x,g(x))=0$.
\epr


\subsubsection{Implicit function theorem of real-valued function of multiple real variables}
\subsubsection{Implicit function theorem of real vector-valued function of multiple real variables}




Given a function $f\colon D\subseteq\bbR^{(n+m)}\to\bbR^m$ that is continuously differentiable on a neighborhood of a point $(\mb{a},\mb{b})$ and such that $f(\mb{a},\mb{b})=\mb{0}$. If $J_{f,\mb{y})$ is invertible, then there exists a neighborhood $U$ of $\mb{a}$ such that there exists a unique function $g\colon U\to\bbR^m$ such that $g(\mb{a})=\mb{b}$ and $f(\mb{x},g(\mb{x}))=\mb{0}$ for all $x\in U$. Moreover,
\[(\nabla g)(\mb{x})=-\qty[J_{f,\mb{y}}(\mb{x},g(\mb{x}))]^{-1}\qty[J_{f,\mb{x}}(\mb{x},g(\mb{x}))].\]
\bpr
PLACEHOLDER
\epr
\subsubsection{Implicit function theorem of multiple variables}
Below, we will denote $(\mb{x},\mb{y})$ as an element of $\bbR^{(n+m)}$ for some $\mb{x}\in\bbR^n$ and $\mb{y}\in\bbR^m$, and denote the left $n$ columns of $\nabla f(\mb{a},\mb{b})$ as $J_{f,\mb{x})$, and the right $m$ columns of $\nabla f(\mb{a},\mb{b})$ as $J_{f,\mb{y})$. Given a function $f\colon D\subseteq\bbR^{(n+m)}\to\bbR^m$ that is continuously differentiable on a neighborhood of a point $(\mb{a},\mb{b})$ and such that $f(\mb{a},\mb{b})=\mb{0}$. If $J_{f,\mb{y})$ is invertible, then there exists a neighborhood $U$ of $\mb{a}$ such that there exists a unique function $g\colon U\to\bbR^m$ such that $g(\mb{a})=\mb{b}$ and $f(\mb{x},g(\mb{x}))=\mb{0}$ for all $x\in U$. Moreover,
\[(\nabla g)(\mb{x})=-\qty[J_{f,\mb{y}}(\mb{x},g(\mb{x}))]^{-1}\qty[J_{f,\mb{x}}(\mb{x},g(\mb{x}))].\]
\bpr
PLACEHOLDER
\epr
...
For a real vector-valued function and a point $a$ in its domain, all $n$th order partial derivatives exist at $a$, iff $n$th order total derivative exists at $a$.
\subsection{Graph-related}
...
\end{proof}
\subsubsection{Second Derivative Test for Local Extrema Theorem of Real Functions}
...
\eit
\subsubsection{Second Derivative Test for Local Extrema Theorem of Scalar Fields}
...
\subsection{Fubini's theorem (富比尼定理)}
If a function is Lebesgue integrable on $X\times Y$, then:
\[\iint_{X\times Y}f(x,y)\dd{(x,y)}=\int_X\qty\int_Yf(x,y)\dd{y}\dd{x}=\int_Y\int_Xf(x,y)\dd{x}\dd{y}.\]
\bpr
PLACEHOLDER
\epr
\subsection{(Lebesgue's) Dominated convergence theorem (DCT) (（勒貝格）控制/受制收斂定理)}
...
\subsubsection{Gradient theorem (梯度定理)}
...
\subsection{Theorem of total variation of differentiable function}
\subsubsection{Theorem of total variation of differentiable real function}
...
\subsubsection{Theorem of total variation of differentiable real fields}
PLACEHOLDER
...
\end{proof}
\subsubsection{Generalized complex binomial theorem}
For $k,x\in\bbC$, if either 
\bit
\item $|x|<1$, or
\item $-1<\Re(k)$ and $x=1$, or
\item $k\ge 0$ and $|x|=1$, or
\item $k\in\bbN_0$,
\eit
\[\qty(1+x)^k=\sum_{n=0}^{\infty}\binom{k}{n}x^n,\]
