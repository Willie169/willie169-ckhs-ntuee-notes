...
\eit
\ssc{Differentials (微分子) and Differential equation (微分方程)}
\sssc{Differentials of dependent variables or functions of an independent variable}
Suppose a dependent variable $y$ represents a function $f$ of an independent variable $x$, that is,
\[y=f(x),\]
...
\sssc{Differentials without assuming independent variables}
For an equation involving differentials $\mathrm{d}x_1,\mathrm{d}x_2,\ldots \mathrm{d}x_n$
\[\sum_{k=1}^nM_k(x_1,x_2,\ldots x_n)\mathrm{d}x_i,\]
if without assuming independent variables, its solutions is defined as any curve $\qty((x_1(t),x_2(t),\ldots x_n(t))$ with respect to an arbitary parameter $t$ that satisfies
\[\sum_{k=1}^nM_k(x_1,x_2,\ldots x_n)\dv{x_i}{t}\]
for any $t$ in its domain.
...
\ssc{Logarithmic differentiation}
...
\ssc{Derivative of variable base, constant exponent funcion}
...
\ssc{Derivative of constant base, variable exponent funcion}
...
\ssc{Derivative of variable base, variable exponent funcion}
