...
Let $f$ be a real-valued function defined on a interval $[a,b]$. The lower and upper Darboux sums, $L(f,P)$ and $U(f,P)$, of $f$ with respect to a partition $P(x,n)$ of $[a,b]$, are two specific Riemann sums of which the tags are chosen to be the infimum and supremum (respectively) of $f$ on each subinterval:
\[\begin{aligned}
...
Let $f$ be a real-valued function defined on a interval $[a,b]$. The Riemann left (aka left-sided), right (aka right-sided), and midpoint sums, $R_L(f,P)$, $R_R(f,P)$, and $R_M(f,P)$, of $f$ with respect to a partition $P(x,n)$ of $[a,b]$, are three specific Riemann sums of which the tags are chosen to be the left endpoint, right endpoint, and midpoint (respectively) on each subinterval:
\[\begin{aligned}
...
\end{aligned}\]
\subsubsection{Riemann sums with respect to partition that divides interval equally}
A Riemann sum of $f$ with respect to a tagged partition $P(x,n,\xi)$ that divides an interval into subintervals of equal length is denoted as $R_n(f)$.

A lower Darboux sum, an upper Darboux sum, a Riemann left sum, a Riemann right sum, and a Riemann midpoint sum of $f$ with respect to a partition $P(x,n)$ that divides an interval into subintervals of equal length are sometimes denoted as $L_n(f),U_n(f),{R_L}_n(f),{R_R}_n(f)$, and ${R_M}_n(f)$ respectively.
