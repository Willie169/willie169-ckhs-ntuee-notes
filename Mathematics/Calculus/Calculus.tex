...
\item\tb{Sequence of partial sums}: The sequence of partial sums of a sequence $\langle a_n\rangle_{n=l}^{\infty}$ is the sequence $\left\langle s_n=\sum_{i=l-1}^na_i\right\rangle_{n=l-1}^{\infty}$ where $s_{l-1}=0$ or $\left\langle s_n=\sum_{i=l}^na_i\right\rangle_{n=l}^{\infty}$.
\end{itemize}
\ssc{Series (級數)}
\begin{itemize}
...
is bounded and function $g\colon [a,b)\to\bbR$ is monotone such that $\lim_{x\to b^-}g(x)=0$, then
\[\int_a^bf(x)g(x)\dd{x}\]
converges.
\begin{proof}
...
\ssc{Arc length}
\sssc{Limit-of-polygons definition}
Given a curve $\gamma(t)=(x(t),y(t))$ between $t=a$ and $t=b$ with $a<b$. Let $P(x,n)$ be a partition of interval $[a,b]$. Define
\[L(P)=\sum_{i=1}^n\|\gamm(i)-\gamma(i-1)\|.\]
The arc length of the curve between $t=a$ and $t=b$ is defined as
\[L=\lim_{n\to\infty}L(P).\]
\sssc{Integral for differentiable curve}
The arc length of a differentiable curve $\gamma(t)=(x(t),y(t))$ between $t=a$ and $t=b$ with $a<b$ is
\[L=\int_a^b\sqrt{\qty(\dv{x}{t})^2+\qty(\dv{y}{t})^2}\dd{t}.\]

In a special case of curve $y=f(x)$, the arc length of it between $x=a$ and $x=b$ with $a<b$ is
\[L=\int_a^b\sqrt{1+\qty(\dv{f}{x})^2}\dd{x}.\]
\ssc{Average value}
