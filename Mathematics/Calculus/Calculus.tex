...
\ssc{Sequence}
\begin{itemize}
\item\textbf{Sequence}: A sequence is a function of which the domain is a set $\{x\in\mathbb{Z}\land l\leq x\leq m\}$, in which $l$ usually equals to $0$ or $1$, usually denoted as $\langle a_n\rangle$, $\{a_n\}$, or $(a_n)$, sometimes denoted with domain as $\langle a_n\rangle_{n=l}^m$, $\{a_n\}_{n=l}^m$, or $(a_n)_{n=l}^m$, where the subscript $n$ refers to the $n$th element of the sequence, that is, the function value when the independent variable is $n$.
...
\section{Laplace Transform (拉普拉斯變換) and Fourier Transform (傅立葉變換)}
\ssc{(Unilateral) Laplace Transform}
The (unilateral) Laplace transform is an integral transform that converts a function of a real variable (usually $t$, in the time domain) to a function of a complex variable (usually $s$, in the complex-valued frequency domain, also known as $s$-domain, or $s$-plane). The functions are often denoted in lowercase for the time-domain representation and uppercase for the frequency-domain.
...
The set of values for which $F(s)$ converges absolutely, called region of convergence (ROC), is either of the form Re(s) > a or Re(s) ≥ a, where a is an extended real constant with −∞ ≤ a ≤ ∞ 
.

---

Would you like me to also explain **how this differs from the Fourier transform** (since Laplace is kind of a generalization of Fourier)?

The **Fourier transform** is another integral transform, closely related to the Laplace transform. Formally, for a function $f : \mathbb{R} \to \mathbb{C}$, its Fourier transform is defined as
...
* $t \in \mathbb{R}$ is the **time (or space) variable**,
* $\omega \in \mathbb{R}$ is the **angular frequency variable**,
* the integral is understood in the sense of convergence (absolutely, in $L^1$, or in the distributional sense for broader classes of functions).

---

### Convergence conditions

* If $f \in L^1(\mathbb{R})$ (i.e. $\int_{-\infty}^\infty |f(t)|\, dt < \infty$), then $F(\omega)$ exists and is bounded and continuous.
* More generally, the Fourier transform can be extended to square-integrable functions $f \in L^2(\mathbb{R})$ (using Plancherel’s theorem).
* In advanced theory, it is even extended to **tempered distributions**.

---

🔑 Difference from Laplace:

* Fourier uses $e^{-i \omega t}$ (purely oscillatory kernel).
* Laplace uses $e^{-st}$ with $s = \sigma + i\omega$, which includes a **decay/growth factor** $e^{-\sigma t}$ for convergence control.

Do you want me to also write the **inverse Fourier transform** (the formal recovery formula)?
