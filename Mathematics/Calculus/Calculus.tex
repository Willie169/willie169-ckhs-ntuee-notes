...
\[\int\frac{1}{\qty(\prod_{i=0}^{k-1}L_{i}(x))\qty(L_{k}(x))^m}\dd{x}=\int\frac{1}{u^m}\dd{u}\]
...
then
\[T(n)=\Theta(n^{\log_ba}).\]
\item Define iterated logarithmic function $L_n(x)$ as
\[L_{0}(x)=x,\quad x\in\bbR_{>0}\]
\[L_{n}(x)=\ln(L_{n-1}(x)),\quad n\in\bbN,x\in\bbR_{>0}.\]
\ben
...
By generalized p-test for series:
