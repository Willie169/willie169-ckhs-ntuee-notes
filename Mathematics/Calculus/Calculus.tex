...
\bit
\item $\abs{f(x)}\le M\abs{g(x)}$, then $f(x)=O(g(x))$ as $x\to a$.
\item $\abs{f(x)}\ge M\abs{g(x)}$, then $f(x)=\Omega(g(x))$ as $x\to a$.
\item $M_1\abs{g(x)}\le\abs{f(x)}\le M_2\abs{g(x)}$, then $f(x)=\Theta(g(x))$ as $x\to a$.
\item $\lim_{x\to a}\abs{\frac{f(x)}{g(x)}}=0$, then $f(x)=o(g(x))$ as $x\to a$.
\item $\lim_{x\to a}\abs{\frac{g(x)}{f(x)}}=0$, then $f(x)=\omega(g(x))$ as $x\to a$.
\eit
...
\bit
\item $\abs{f(x)}\le M\abs{g(x)}$, then $f(x)=O(g(x))$ as $x\to\infty$.
\item $\abs{f(x)}\ge M\abs{g(x)}$, then $f(x)=\Omega(g(x))$ as $x\to\infty$.
\item $M_1\abs{g(x)}\le\abs{f(x)}\le M_2\abs{g(x)}$, then $f(x)=\Theta(g(x))$ as $x\to\infty$.
\item $\lim_{x\to\infty}\abs{\frac{f(x)}{g(x)}}=0$, then $f(x)=o(g(x))$ as $x\to\infty$.
\item $\lim_{x\to\infty}\abs{\frac{g(x)}{f(x)}}=0$, then $f(x)=\omega(g(x))$ as $x\to\infty$.
\eit
...
Obvious.
\epr
$o\lor\Theta\implies O$, but the inverse is not true.

$\omega\lor\Theta\implies\Omega$, but the inverse is not true.
\bpr
Obvious.

Counterexample for the inverse:
\[\frac{f(x)}{g(x)}=\bcs 1,\quad & x\in\bbQ\\\abs{x-a},\quad & x\notin\bbQ\ecs\]
\[\frac{f(x)}{g(x)}=\bcs 1,\quad & x\in\bbQ\\\frac{1}{x},\quad & x\notin\bbQ\ecs\]
\epr
Binary relation:
\bit
\item $\Theta$ is an equivalent relation.
\item $O$ and $\Omega$ are partial orderings.
\item $o$ and $\omega$ are strict partial orderings.
\eit
