...
\item\tb{Infinite (無窮) discontinuity}: If at least one of $\lim_{x\to a^-}f(x)$ and $\lim_{x\to a^+}f(x)$ does not exist, and that those in $\lim_{x\to a^-}f(x)$ and $\lim_{x\to a^+}f(x)$ that do not exist are either $\infty$ or $-\infty$, and that $f(a)$ is defined, we call $f(a)$ an infinite discontinuity.
...
\item\tb{Essential (本質) discontinuity or type II discontinuity}: A discontinuity of $f$ that is neither a removable discontinuity nor a jump discontinuity is called an essential discontinuity or a type II discontinuity of $f$.
\eit
\sssc{Singularity or singular point (奇點)}
A point is a singularity or a singular point of a function $f$ if it is a cluster point of its domain but not in its domain, or it is a discontinuity of $f$.
...
Let $X$ and $Y$ be topological spaces, and $f\colon D\subseteq X\to Y$ be a function. If there exists a locally finite family $\{U_i\mid i\in I\}$ of closed subsets of $X$ such that:
\begin{itemize}
...
\sssc{Lipschitz continuity for functions between metric spaces}
...
\sssc{Relationship with uniform continuity}
Let $(X,d_1)$ and $(Y,d_2)$ be metric spaces, and $f\colon O\subseteq X\to Y$ be a function. $f$ is uniform continuous if it is Lipschitz continuous. $f$ is uniform continuous on $D\subseteq O$ if it is Lipschitz continuous on $D\subseteq O$.
\sssc{Lipschitz constant (利普希茨常數)}
If a function $f\colon D\subseteq X\to Y$ between metric spaces $(X,d_1)$ and $(Y,d_2)$ is such that there exists a real constant $K$ such that $\forall x,y\in D$, $d_{2}(f(x),f(y))\leq Kd_{1}(x,y)$, then, any such $K$ is sometimes called a Lipschitz constant of the function $f$, $f$ is sometimes called to be $K$-Lipschitz, and the smallest such $K$ is sometimes called the best Lipschitz constant, Lipschitz constant, or the dilation of $f$.
\sssc{Short map}
...
\item $a=-\infty$,
\item $b=\infty$,
\item $f(x)$ is unbounded on $[a,b]$, or
\item $f(x)$ is undefined somewhere in $[a,b]$.
\een
...
\item for $f(x)$ with singularity $c\in(a,b)$:
