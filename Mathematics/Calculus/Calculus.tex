\documentclass[a4paper,12pt]{rearticle\setcounter{secnumdepth}{5}
...
\bpr
For $p\le0$, $\sum_{n=a}^{\infty}\frac{1}{n^p}=\infty$. For $p>0$, $f\qty(x)=\frac{1}{x^p}$ is monotone decreasing on $[1,\infty)$. By integral test and p-test, the statement holds.
\epr
...
\[\int_0^a\frac{1}{\qty(\prod_{i=0}^{k-1}L_{i}\qty(x))\qty(L_{k}\qty(x))^m}\dd{x}\]
converges iff $m<1$.
\bpr
Let
\[u=L_k\qty(x)\]
...
\[\sum_{n=a}^{\infty}\frac{1}{\qty(\prod_{i=0}^{k-1}L_{i}\qty(x))\qty(L_{k}\qty(x))^m}\]
converges iff $m>1$.
\bpr
...
\item Define iterated logarithmic function $L_n\qty(x)$ as
\[L_{0}\qty(x)=x,\quad x\in\bbR_{>0}\]
\[L_{n}\qty(x)=\ln\qty(L_{n-1}\qty(x)),\quad n\in\bbN,x\in\bbR_{>0}.\]
\ben
...
\[f\qty(n)=o\qty(n^{\log_ba}\prod_{i=1}^k\qty(L_{i}\qty(n))^{-1}),\]
then
...
\[T\qty(n)=\Theta\qty(n^{\log_ba}).\]
\een
\item If $f\qty(n)=\Omega\qty(n^c)$ for some $c>\log_ba$, and the regularity condition that $\exists m\in\bbR,k<1$ such that $n\ge m$ implies
...
\item $T\qty(1)$ is called leaf cost.
