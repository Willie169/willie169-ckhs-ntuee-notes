...
\sssc{Closed curve orientation}
Unless otherwise specified, closed curve is counterclockwise oriented (if applicable).
...
\ssc{Parameterized manifold}
\sssc{Fundamental tangent vector}
For an $n$-dimensional parameterized manifold $\mb{r}(\mb{x})$ where $\mb{x}\in\bbR^n$ and $\mb{r}\in\bbR^m$ with $n<m$, each $\pdv{\mb{r}}{x_i}$ is a fundamental tangent vector.
\sssc{Fundamental vector product}
For surface $S=\mb{r}(s,t)$ in $\bbR^3$, the outer product of the fundamental tangent vectors $\mb{r}_s\times\mb{r}_t$ is called fundamental vector product.
\sssc{Outward-pointing surface element}
For surface $S=\mb{r}(\theta,\varphi)$ in $\bbR^3$ equipped with coordinate $(r,\thera,\varphi)$ such that $r$ is the radial distance from origin, the outward-pointing surface element $\dd{\mb{S}}$ is
\[\dd{\mb{S}}=\mb{r}_s\times\mb{r}_t\dd{s}\dd{t}\]
Unless otherwise specified, vector surface element is outward-pointing.