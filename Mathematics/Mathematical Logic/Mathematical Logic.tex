...
A structure can be defined with a triple $(A,\sigma,I)$ of a domain, also called universe, carrier (set), underlying set, domain of discourse, or universe of discourse, $A$, denoted as $|\mathcal{A}|$ or simply $\mathcal{A}$, a signature $\sigma$, and an interpretation function $I$ that, for all $n\in\bbN_0$, maps each $f\in F_n$ to an $n$-ary operation $I(f)$ on $A$, called interpretation of $f$ and often denoted as $f$, and each $p\in P_n$ to a subset $I(p)$ of $A^n$, called interpretation of $p$ and often denoted as $p$.
\sssc{Substructure}
