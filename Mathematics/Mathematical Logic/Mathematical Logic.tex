...
If a deductive system $S$ over language $L$ and a deductive system $T$ over language $M$ are such that there exists a function $H\colon L\to M$ that replaces each symbol in $L$ in the input with a symbol of the same type in $M$ such that for every formula $l\in L$, $S\vdash l\implies T\vdash H(l)$, then we say $T$ inteprets $S$, $S$ is weaker than $T$, or $T$ is stronger than $S$.
