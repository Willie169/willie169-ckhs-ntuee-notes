...
\sssc{Introduction}
Zermelo set theory (Z) is a first-order formal thoery in which every variable is called a set defined by the proof system with axiom of extensionality, axiom of regularity, axiom schema of specification, axiom of paring, axiom of union, axiom of infinity, axiom of power set, and typical rules of inference of first-order logic.
...
These theories, also abbreviated as set theory, have been the standard way to provide rigorous foundations for all branches of mathematics since the first half of the 20th century. Derivable in ZF $\vdash_{ZF}$ and derivable in ZFC $\vdash_{ZFC}$ are denoted $\vdash$ when the context is clear.
...
\[\forall z\forall w_1\forall w_2\ldots\forall w_n\qty[\forall x\qty(x\in z\implies\exists y\qty(\varphi\qty(x,y,z,w_1,w_2,\ldots,w_n)\land\forall v\qty(\varphi\qty(x,v,z,w_1,w_2,\ldots,w_n)\implies v=y)))\implies\exists U\forall x\qty(x\in U\iff\exists y\qty(y\in U\land\varphi\qty(x,y,z,w_1,w_2,\ldots,w_n)))],\]
can be obtained by applying the axiom schema of specification:
\[U=\{x\in u\mid\exists y(y\in U\land\varphi(x,y,z,w_1,w_2,\ldots,w_n))\}.\]
...
ZF $\cup\{\exists R\forall x(x\in R\iff x\ni x)\}$ is inconsistent and proves $R\in R\iff R\ni R$.
\sssc{Non-existence of univere}
ZF $\cup\{\exists U\forall x(x\in U)\}$ is inconsistent and with $R=\{x\in U\mid x\ni x\}$, which is a set by axiom schema of specification, proves $R\in R\iff R\ni R$.
...
\sssc{Pairwise disjoint or mutually exclusive sets}
Sets in a set $x$ are pairwise disjoint, also called mutually exclusive, iff
\[\forall p\neq q\in x\colon p\cap q=\varnothing,\]
that is,
...
\sssc{Inclusion map}
An inclusion map is a map from a set $A$ to its superset $B$ that maps each element in $A$ to the same element in $B$.
\sssc{Permutation}
A permutation over a set is a bijection $\sigma$ from it to itself.

The identity permutation is $\sigma(x)=x$.

The set of permutations over a set is a group with multiplication
\[\sigma\tau\colon x\mapsto\sigma(\tau(x)).\]
...
\sssc{Collectively exhaustive}
Subsets $(A_i)_{i\in I}$ of a set $X$ is collectively exhaustive iff
\[\bigcup_{i\in I}A_i=X.\]
\sssc{Partition (分割或劃分)}
A partition of a set $X$ is a collection, often indexed family, $(A_i)_{i\in I}$, of nonempty subsets $A_i$ of $X$ that are mutually exclusive and collectively exhaustive.
\sssc{Ring of sets}
We call a family $\mathcal{R}$ of subsets of a set $\Omega$ a ring of sets  $\Omega$ if it has the following properties:
\begin{enumerate}
...
\sssc{Universe and complement}
When the sets under discussion are all subsets of a given set, the given set is called the universe, denoted as $U$, and for any set $A\subseteq U$, $U\setminus A$ is denoted as $A'$, $\overline{A}$, $A^c$, or $A^C$, called complement.
\sssc{Disjoint union or discriminated union}
\[\bigsqcup_{i=1}^nA_i=\bigcup_{i\in I}\{(x,i)\mid x\in A_i\}.\]
\sssc{Element operation notation}
For set $A$, scalar $s$, and operation $*$ defined for all $s*a\mid a\in A$, $s*A$ sometimes denotes the set $\{s*a\mid a\in A\}$.

For set $A$, scalar $s$, and operation $*$ defined for all $a*s\mid a\in A$, $A*s$ sometimes denotes the set $\{a*s\mid a\in A\}$.

For indexed family $(a_i)_{i\in I},(b_i)_{i\in I}$, and operation $*$ defined for all $a_i*b_i\mid i\in I$, $(a_i)_{i\in I}*(b_i)_{i\in I}$ sometimes denotes the indexed family $(a_i*b_i)_{i\in I}$.
