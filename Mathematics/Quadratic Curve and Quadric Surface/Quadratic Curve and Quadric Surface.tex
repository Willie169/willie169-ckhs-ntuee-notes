...
\begin{document}
\title{Quadratic Curve and Quadric Surface}
\author{沈威宇}
\date{\temtoday}
\titletocdoc
\ch{Quadratic Curve and Quadric Surface}
\sct{Quadratic Equation}
\ssc{General form}
In $\bbR^n$,
\[C+\sum_{j=1}^nx_j\qty(B_j+\sum_{k=j}^nA_{jk}x_k),\quad A_{jk},B_j,C\in\bbR\]
or
\[\mb{x}^{\top}\mb{A}\mb{x}+\mb{B}^{\top}\mb{x}+C=0,\quad\mb{A}\in\bbR^{n\times n},\mb{B}\in\bbR^n,C\in\bbR\]
where
\[\mb{A}_{jj}=A_{jj},\]
\[\mb{A}_{jk}=\mb{A}_{kj}=\frac{A_{jk}}{2},\quad j\neq k,\]
\[\mb{B}_j=B_j,\]
\[\prod_{j=1}^n\prod_{k=j}^nA_{jk}\neq 0.\]
$\mb{A}$ is symmetric.
\ssc{Reduction to standard form}
If $\mb{A}$ is invertible, translate
\[\mb{x}=\mb{y}-\frac{1}{2}\mb{A}^{-1}\mb{B},\]
diagonalize with orthogonal matrix $Q$ such that
\[Q^{\top}\mb{A}Q=\diag(\lambda_1,\lambda_2,\ldots,\lambda_n),\]
rotate to
\[\mb{y}=Q\mb{z},\]
and obtain the standard form
\[\sum_{i=1}^n\lambda_i\mb{z}_i^{\pht{i}2}+C-\frac{1}{4}\mb{B}^{\top}\mb{A}^{-1}\mb{B}=0.\]
\bpr
If $\mb{A}$ is invertible, first remove linear form. Let $\mb{x}=\mb{y}+\mb{x}_0$,
\[\qty(\mb{y}+\mb{x}_0)^{\top}\mb{A}\qty(\mb{y}+\mb{x}_0)+\mb{B}^{\top}\qty(\mb{y}+\mb{x}_0)+C=0\]
\[\mb{y}^{\top}\mb{A}\mb{y}+\qty(2\mb{A}\mb{x}_0+\mb{B})^{\top}\mb{y}+\qty(\mb{x}_0\mb{A}\mb{x}_0+\mb{B}^{\top}\mb{x}_0+C)=0\]
\[2\mb{A}\mb{x}_0+\mb{B}=0\]
\[\mb{x}_0=-\frac{1}{2}\mb{A}^{-1}\mb{B}\]
\[\mb{y}^{\top}\mb{A}\mb{y}+C'=0\]
\[C'=C-\frac{1}{4}\mb{B}^{\top}\mb{A}^{-1}\mb{B}\]
By spectral theorem, there exists orthogonal matrix $Q$ such that
\[Q^{\top}\mb{A}Q=\Lambda=\diag(\lambda_1,\lambda_2,\ldots,\lambda_n).\]
Assign new coordinate
\[\mb{y}=Q\mb{z}\]
\[\mb{z}^{\top}\Lambda\mb{z}+C'=0\]
\[\sum_{i=1}^n\lambda_i\mb{z}_i^{\pht{i}2}+C'=0\]
\epr
If $\mb{A}$ is singular, diagonalize with orthogonal matrix $Q$ such that
\[Q^{\top}\mb{A}Q=\diag(\lambda_1,\lambda_2,\ldots,\lambda_r,0,\ldots,0),\quad r=\rank(\mb{A}),\]
rotate
\[\mb{x}=Q\mb{y}\]
with $b=Q^{\top}B$, translate
\[y_i=z_i-\frac{b_i}{\lambda_i},\quad i\le r\]
\[y_i=z_i,\quad i>r,\]
obtain
\[\sum_{i=1}^r\lambda_i\mb{z}_i^{\pht{i}2}+\sum_{i=r+1}^nb_i\mb{z}_i+C-\sum_{i=1}^rb_i^{\pht{i}2}=0\]
if there exists any $b_k\neq 0$ for $k>r$, pick any one of them and translate
\[\mb{w}_k=\mb{z}_k+C-\sum_{i=1}^rb_i^{\pht{i}2}\]
\[\mb{w}_j=\mb{z}_j,\quad j\neq k,\]
obtain paranoid-type standard form
\[\sum_{i=1}^r\lambda_i\mb{w}_i^{\pht{i}2}+\sum_{i=r+1}^nb_i\mb{w}_i=0\]
if $b_k=0$ for all $k>r$, it is degenerate
\[\sum_{i=1}^r\lambda_i\mb{z}_i^{\pht{i}2}+C-\sum_{i=1}^rb_i^{\pht{i}2}=0.\]
\bpr
\[\mb{x}=Q\mb{y}\]
\[\mb{y}^{\top}\Lambda\mb{y}+\qty(Q^{\top}B)^{\top}\mb{y}+C=0\]
\[\mb{y}^{\top}\Lambda\mb{y}+\qty(Q^{\top}B)^{\top}\mb{y}+C=0\]
\[\mb{z}^{\top}\Lambda\mb{z}+C-\sum_{i=1}^rb_i^{\pht{i}2}=0\]
\epr
...
\item 笛卡爾方程定義：圓錐曲線指平面上，任取原點定義笛卡爾座標後，$Ax^2+Bxy+Cy^2+Dx+Ey+F=0$的圖形，且$\det\begin{pmatrix} A & B/2 & D/2 \\ B/2 & C & E/2 \\ D/2 & E/2 & F \end{pmatrix}<0$。
\item 極方程定義：圓錐曲線指平面上，取極點於圓錐曲線一焦點定義極座標後，$r=\frac{\ell}{1-\varepsilon\cos(\theta+\varphi)$的圖形，其中半正焦弦$\ell>0$，離心率$\varepsilon\geq0$。
...
\item 極方程：
\[r=\frac{\frac{b^2}{a}}{1-\frac{\sqrt{a^2-b^2}}{a}\cos(\theta)}=\frac{a\qty(1-\varepsilon^2)}{1-\varepsilon\cos(\theta)},\quad h=\sqrt{a^2-b^2}\land k=0\]
\[r=\frac{\frac{b^2}{a}}{1-\frac{\sqrt{a^2-b^2}}{a}\cos(\pi-\theta)}=\frac{a\qty(1-\varepsilon^2)}{1-\varepsilon\cos(\pi-\theta)},\quad h=-\sqrt{a^2-b^2}\land k=0\]
\item 準線方程：
...
\[(y-k)^2=4f(x-h)\]
\item 極方程：
\[r=\frac{2f}{1-\cos(\theta)},\quad h=-f\land k=0\]
\item 準線方程：
\[x=-f+h\]
\item 中心/頂點：$(h,k)$
...
\[\left(\frac{x-h}{a}\right)^2-\left(\frac{y-k}{b}\right)^2=1\]
\item 極方程：
\[r=\frac{\frac{b^2}{a}}{1-\frac{\sqrt{a^2+b^2}}{a}\cos(\theta)}=\frac{a\qty(\varepsilon^2-1)}{1-\varepsilon\cos(\theta)},\quad h=-\sqrt{a^2+b^2}\land k=0\]
\[r=\frac{\frac{b^2}{a}}{1-\frac{\sqrt{a^2+b^2}}{a}\cos(\pi-\theta)}=\frac{a\qty(\varepsilon^2-1)}{1-\varepsilon\cos(\pi-\theta)},\quad h=\sqrt{a^2+b^2}\land k=0\]
\item 準線方程：
\[x=\pm\frac{a^2}{\sqrt{a^2+b^2}}+h\]
\item 中心：$(h,k)$
\item 頂點：$(h\pm a,k)$
...
\end{itemize}
\subsection{雙葉圓錐/雙重圓錐（double-napped cone, double cone, or cone）}
\subsubsection{定義}
...
\ssc{Quadric surface}
\sssc{General equation}
A quadric surface is the graph of a second-degree equation in three variables, that is,
\[Ax^2+By^2+Cz^2+Dxy+Eyz+Fxz+Gx+Hy+Iz+J=0.\]
By translation and rotation, it can be brought into either of the two standard forms (axis being $z$-axis and vertex being origin)
\[Ax^2+By^2+Cz^2+J=0\]
or
\[Ax^2+By^2+Iz=0.\]
\sssc{Trace}
Horizontal trace is the section of a plane parallel to the $xy$ plane and the quadric surface.

Vertical trace is the section of a plane parallel to the $z$-axis and the quadric surface.
\sssc{Types}
In standard form, traces exclude trivial cases.
\bit
\item Ellipsoid:
\[\frac{x^2}{a^2}+\frac{y^2}{b^2}+\frac{z^2}{c^2}=1\]
All traces that pass through it are ellipses. If $a=b=c$, it's a sphere.
\item Hyperboloid of one sheet:
\[\frac{x^2}{a^2}+\frac{y^2}{b^2}-\frac{z^2}{c^2}=1\]
Horizontal traces are ellipses. Vertical traces are hyperbolas. If $a=b$, horizontal traces are circles.
\item Hyperboloid of two sheets:
\[-\frac{x^2}{a^2}-\frac{y^2}{b^2}+\frac{z^2}{c^2}=1\]
Horizontal traces for $|z|>c$ are ellipses. Vertical traces are hyperbolas. If $a=b$, horizontal traces are circles.
\item (Elliptic) cone:
\[\frac{x^2}{a^2}+\frac{y^2}{b^2}-\frac{z^2}{c^2}=0\]
Horizontal traces are ellipses. Vertical traces are hyperbolas. If $a=b$, it's circular cone.
\item Elliptic paraboloid:
\[\frac{x^2}{a^2}+\frac{y^2}{b^2}=\frac{z}{c}\]
Horizontal traces are ellipses. Vertical traces are parabolas. If $a=b$, it's circular paraboloid.
\item Hyperbolic paraboloid:
\[\frac{x^2}{a^2}-\frac{y^2}{b^2}=\frac{z}{c}\]
Horizontal traces are hyperbolas. Vertical traces are parabolas.
\item Parabolic cylinder:
\[\frac{x^2}{a^2}=\frac{z}{c}\]
Horizontal traces are two parallel lines. Vertical traces are parabolas.
\item Elliptic cylinder:
\[\frac{x^2}{a^2}+\frac{y^2}{b^2}=1\]
Horizontal traces are ellipses. Vertical traces are two parallel lines. If $a=b$, it's circular cylinder.
\item Two parallel planes:
\[\frac{x^2}{a^2}=1\]
All traces are two parallel lines.
\item Hyperbolic cylinder:
\[\frac{x^2}{a^2}-\frac{y^2}{b^2}=1\]
Horizontal traces are hyperbolas. Vertical traces are two lines.
\eit
