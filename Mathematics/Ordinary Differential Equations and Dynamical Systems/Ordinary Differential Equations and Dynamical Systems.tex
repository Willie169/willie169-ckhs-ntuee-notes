...
An $n$-parameter family of solutions of an ODE of a dependent variable $y$ with respect to an independent variable $x$ is of the form $y=f(x,c_1,c_2,\dots c_n)$ (explicit) or $G(x,y,c_1,c_2,\dots c_n)=0$ (implicit), in which $c_1,c_2,\dots c_n$ are parameters that are arbitrary given that the solution obtained is a solution of the ODE. The arbitrary parameters are usually denoted as $A,B,\ldots$.
...
\sssc{Remarks}
Occasionally, a first-order differential equation is not linear in one variable but is linear in the other variable. If a first-order ODE that can be wriiten of the form
...
The general solutions of a linear homogenous ODE form a vector space.
...
\ssc{Basis}
If $y_1(x),y_2(x),\ldots y_n(x)$ are linearly independent solutions of an $n$th-order linear homgeneous ODE on an interval, then they form a basis of the vector space of the general solutions of the ODE on the interval, and $\sum_{i=1}^nc_iy_i(x)$ is the family general solutions of the ODE on the interval with $n$ parameter $c_1,c_2,\ldots c_n$.
...
is called its complementary equation, and the general solutions of the original equation is the sum of the general solutions $y_c$ of the complementary equation and a particular solution $y_p$.
...
\sssc{Example: SHM}
PLACEHOLDER
\sssc{Example: damped SHM}
PLACEHOLDER
\sssc{Example: constant voltage series RLC and LC resonance circuit}
PLACEHOLDER
\sssc{Example: constant current parallel RLC and LC resonance circuit}
PLACEHOLDER
...
\item The ansatz is
\[y_j=x^{s_j}e^{\alpha_jx}\sum_{k=0}^{m_j}x^k\qty(C_{jk}\cos(\beta_jx)+D_{jk}\sin(\beta_jx)),\]
...
\sssc{Wrońskian}
For $n$ functions $f_1,f_2,\ldots,f_n$ that are $(n-1)$ times differentiable on an interval $I$, the Wrońskian of them, $W(f_1,f_2,\ldots,f_n)$, is a function on $I$ defined by
...
\[y^{(n)}+\sum_{i=0}^{n-1}p_i(x)y^{(i)}=0,\]
which can be obtained by variation of parameters of the family of the solutions of the complementary equation. The particular solution of the original ODE is given by:
\[y_p=\sum_{i=1}^ny_i(x)\int\frac{W_i(x)}{W(x)}\dd{x},\]
where
\bit
\item $W(x)$ is the Wrońskian of $y_1(x),y_2(x),\ldots,y_n(x)$,
\item $W_i(x)$ is the Wrońskian of $y_1(x),y_2(x),\ldots,y_n(x)$ but with the $i$th column replaced by $(0,0,\ldots,0,q(x))^\top$.
\eit
\begin{proof}
...
\ben
\item The power series method seeks a solution for all ordinary points $x$ of the form
...
\[y'=\sum_{n=0}^{\infty}a_nn(x-x_0)^{n-1},\]
\[y''=\sum_{n=0}^{\infty}a_nn(n-1)(x-x_0)^{n-2}.\]
\item Substitute into the ODE:
\[\sum_{n=0}^{\infty}\qty(n(n-1)+n(x-x_0)p(x)+(x-x_0)^2q(x))a_n(x-x_0)^{n-2}=0.\]
\item The two-parameter family of general solutions for all ordinary points $x$ with parameters $a_0,a_1$ is
\[y=\sum_{n=0}^{\infty}a_n(x-x_0)^n.\]
Solve it into the ODE to obtain a recurrence relation for $a_n$.
\een
\sssc{Frobenius method}
Consider a second-order linear homogenous ODE
\[y''+p(x)y'+q(x)y=0.\]
For any $a$ in the intersection of the domain of $p$ and the domain of $q$, $a$ is called a regular singular point iff it is not an ordinary point and $(x-a)p(x)$ and $(x-a)^2q(x)$ are both analytic at $a$.
\ben
\item The Frobenius method seeks a solution for all ordinary points and regular singular points $x$ of the form
\[y=(x-x_0)^r\sum_{n=0}^{\infty}a_n(x-x_0)^n,\quad a_0\neq 0,\]
where $x_0$ is a fixed point called expansion point and $a_n$ are constants to be determined, called Frobenius series.
\item Differentiate the series term by term:
\[y'=\sum_{n=0}^{\infty}a_n(n+r)(x-x_0)^{n+r-1},\]
\[y''=\sum_{n=0}^{\infty}a_n(n+r)(n+r-1)(x-x_0)^{n+r-2}.\]
\item Substitute into the ODE:
\[\sum_{n=0}^{\infty}\qty((n+r)(n+r-1)+(n+r)(x-x_0)p(x)+(x-x_0)^2q(x))a_n(x-x_0)^{n+r-2}=0.\]
\item Collect the lowest power term in the series (often the $n=0$ term but possibly other), let it be the $n=n$ term, let
\[p_0=\lim_{x\to x_0}(x-x_0)p(x),\]
\[q_0=\lim_{x\to x_0}(x-x_0)^2q(x),\]
the indicial polynomial is defined as
\[I(r)=(n+r)(n+r-1)+(n+r)p_0+q_0,\]
and the indicial equation is defined as
\[I(r)=0,\]
...
\bit
\item Case $r_1-r_2\notin\bbZ$: The two one-parameter families of solutions for all ordinary points and regular singular points $x$, $y_1$ with parameter $A_0\neq 0$ and $y_2$ with parameter $B_0\neq 0$, are defined as
\[y_1(x)=(x-x_0)^{r_1}\sum_{n=0}^{\infty}A_n(x-x_0)^n,\]
\[y_2(x)=(x-x_0)^{r_2}\sum_{n=0}^{\infty}B_n(x-x_0)^n.\]
Solve them into ODE respectively to obtain recurrence relations for $A_n$ and $B_n$. 
\item Case $r_1-r_2\in\bbN_0$: The two one-parameter families of solutions for all ordinary points and regular singular points $x$, $y_1$ with parameter $A_0\neq 0$ and $y_2$ with parameter $B_0\neq 0$, are defined as
\[y_1(x)=(x-x_0)^{r_1}\sum_{n=0}^{\infty}A_n(x-x_0)^n,\]
\[y_2(x)=y_1(x)\ln(x-x_0)+(x-x_0)^{r_2}\sum_{n=0}^{\infty}B_n(x-x_0)^n.\]
Solve them into ODE respectively to obtain recurrence relations for $A_n$ and $B_n$.
\eit 
\item The two-parameter family of general solutions for all ordinary points and regular singular points $x$ is given by
\[y=y_1(x)+y_2(x).\]
\een
\sssc{Example:}
PLACEHOLDER
...
\sssc{Phase trajectory and phase portrait (相圖)}
Given a dynamical system $(T, X, \Phi)$, a phase trajectory is a geometric representation of an orbit $\gamma$, the phase portrait is a geometric representation of the collection of all orbits $\gamma$ through any $x\in X$.
\sssc{Attractor (吸引子)}
