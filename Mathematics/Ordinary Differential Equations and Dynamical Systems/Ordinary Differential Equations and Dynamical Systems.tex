...
\sssc{Example: constant voltage series RL circuit}
IVP:
...
\[I(t)=\frac{V}{R}\qty(1-e^{-Rt/L})+I_0e^{-Rt/L}\]
\end{proof}
\sssc{Example: constant current parallel RC circuit}
IVP:
...
\[\sum_{j=0}^na_j(x)\dv[j]{y}{x}=g(x),\]
where $a_0(x),\dots a_n(x)$, called the coefficients, and $g(x)$, called the forcing term, are functions of $x$.

An ODE is nonlinear if it is not linear.
\ssc{Homogeneity}
A linear ODE is called a linear homgeneous ODE if $g(x)=0$; otherwise, it is called a linear nonhomogeneous ODE.
\ssc{Linearity}
Let $L[y]=\sum_{j=0}^na_j(x)\dv[j]{y}{x}$ denotes the left-hand side, then $L[\alpha y_1+\beta y_2]=\alpha L[y_1]+\beta L[y_2]$. That's why it's called to be linear.
...
\ssc{Theorem of general solutions as linear combinations}
If $y_1(x),y_2(x),\ldots y_n(x)$ are linearly independent solutions of an $n$th-order linear homgeneous ODE on an interval, then $\sum_{i=1}^nc_iy_i(x)$ is the family general solutions of the ODE on the interval with $n$ parameter $c_1,c_2,\ldots c_n$.
\begin{proof}
PLACEHOLDER
\end{proof}
\ssc{Characteristic equation or auxiliary equation}
Given an $n$th-order linear homogeneous ODE
\[\sum_{j=0}^na_j\dv[j]{y}{x}=0,\quad a_n\neq 0,\]
