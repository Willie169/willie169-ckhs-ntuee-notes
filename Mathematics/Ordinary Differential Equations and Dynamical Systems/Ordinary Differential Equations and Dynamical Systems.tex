...
\[V(t)=IR\qty(1-e^{-t/(RC)})+V_0e^{-t/(RC)}\]
\end{proof}
\ssc{First-order Exact Equation}
\sssc{Definition}
A first-order ODE of the form
\[M(x,y)\dd{x}+N(x,y)\dd{y}=0\]
with
...
and $\frac{\partial^2F}{\partial x\partial y}$. Such $F$ is called the potential function.
...
\ssc{Integration factors to make non-exact first-order ODEs exact}
\sssc{Definition}
...
\sssc{Integration factor of one variable}
If
...
\[\dv{y}{t}=-\gamma y+\delta xy,\]
\[x(0)=x_0,\quad y(0)=y_0.\]

Solution:
PLACEHOLDER
\[\dv{y}{t}=\dv{y}{x}\dv{x}{t}\]
\[\dv{y}{x}=\frac{-\gamma y+\delta xy}{\alpha x-\beta xy}\]
\[\qty(\gamma y-\delta xy)\dd{x}+\qty(\alpha x-\beta xy)\dd{y}=0\]
PLACEHOLDER


\ssc{First-order Homogeneous Equation}
\sssc{Definition}
A first-order ODE of the form
\[\dv{y}{x}=F(x,y)\]
