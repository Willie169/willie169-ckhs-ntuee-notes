...
\ssc{Introduction}
\sssc{Differential Equation (微分方程)}
...
\[\dv{y}{x}=F(x,y).\]
Take functions $u(x,y)$, $g(u)$, and $h(x)$ such that for almost everywhere in the domain of $F$,
\begin{itemize}
    \item there exists a function $p$ with $y=p(x,u)$ and $\pdv{p}{u}$ exists,
    \item there exists a function $q$ such that $q(u)=\pdv{p}{x}$,
    \item $u(x,y)$ is defined almost everywhere in the domain of $F$,
    \item $g(u)h(x)=F(x,y)$ almost everywhere in the domain of $F$.
\end{itemize}

Substitution gives
\[\pdv{p}{x}+\pdv{p}{u}\pdv{u}{x}=g(u)h(x),\]
which is separable.
\[\int\frac{1}{g(u)}\dv{y}{u}\dd{u}=\int h(x)\dd{x}+C.\]
And $g(u)=0$ is a constant solution.

Substituting $u(x,y)$ back gives the solutions.

Finally, we should consider the singular solutions when $u(x,y)$ is not defined or $g(u)h(x)!=F(x,y)$.
