...
\FB}
\sssc{Sinusoid}
A function $f(t)=A\cos(\omega t+\theta)$ for some real non-negative constants $A,\omega,\theta$ is called a sinusoid.
\sssc{Sinc function}
...
\sssc{Definition}
The phasor or complex amplitude of a sinusoid $A\cos(\omega t+\theta)$ for some real non-negative constants $A,\omega,\theta$ is a complex number
...
The $A\angle\theta$ notation is often called phasor and the $Ae^{i\theta}$ notation if often called complex number or complex amplitude.

Phasor denoting $A\cos(\omega t+\theta)$ is called rotating phasor. Phasor denoting $A\cos(\theta)$ is called non-rotating phasor.
\sssc{Addition}
Consider $X_1\cos(\omega t+\varphi_1)$ and $X_2\cos(\omega t+\varphi_2)$ and associated phasors $\ul{X}_1=X_1\angle\varphi_1$ and $\ul{X}_2=X_2\angle\varphi_2$.
\[X_1\cos(\omega t+\varphi_1)+X_2\cos(\omega t+\varphi_2)=X_m\cos(\omega t+\varphi),\]
where
\[X_m=\abs{\ul{X}_1+\ul{X}_2}\]
\[\varphi=\mathrm{arg}\qty(\ul{X}_1+\ul{X}_2)\]
\sssc{Multiplication}
