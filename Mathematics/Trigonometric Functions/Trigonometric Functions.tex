...
\FB}
\sssc{Sinusoid}
A function $f(t)=A\cos(\omega t+\theta)$ for some real nonnegaative constants $A,\omega,\theta$ is called a sinusoid.
\sssc{Phasor}
A sinusoid $A\cos(\omega t+\theta)$ for some real nonnegaative constants $A,\omega,\theta$ when included an imaginary part $i\cdot A\cos(\omega t+\theta)$ leads to
\[A\cos(\omega t+\theta)+i\cdot A\sin(\omega t+\theta)=Ae^{i\theta}\cdot e^{i\omega t}.\]
The phasor or complex amplitude:
\[A\angle\theta\]
can represent the sinusoid $A\cos(\omega t+\theta)$, the complex number $Ae^{i\theta}$, or the two-dimensional vector $(A\cos\theta,A\sin\theta)$.
