...
TODO
\epr
\sssc{Holomorphic or analytic function}
...
A simple or single pole is a pole of order 1. A repeated or multiple pole is a pole of order $>1$.
\sssc{Zeros}
...
A simple or single zero is a zero of order -1. A repeated or multiple zero is a zero of order $<-1$.
...
\sssc{Pole-zero constellation or plot}
...
\bit
\item $\forall n\in\bbN\colon a_{-n}=0$: either $f$ is holomorphic at $z_0$ or $z_0$ is a removable singularity of $f$.
\item $\exists N\in\bbN\tx{\ s.t.\ }a_{-N}\neq 0\land\forall n\in\bbN_{>N}\colon a_{-n}=0$: $z_0$ is a pole of order $N$ of $f$.
...
\sssc{(Cauchy's) residue theorem}
Let $U$ be a simply connected open subset of the complex plane containing a finite list of points $a_1,\ldots,a_n$, $U_0=U\setminus\{a_1,\ldots,a_n\}$, and a function $f$ is holomorphic on $U_0$. Let $\gamma$ be a closed rectifiable curve in $U_0$. Let the winding number of $\gamma$ around $a_k$ be $I(\gamma,a_k)$.
\[\oint_{\gamma}f(z)\dd{z}=2\pi i\sum_{k=1}^nI(\gamma,a_k)\Res(f,a_k).\]
\bpr
By divergence theorem, obviously.
\epr
\end{document}
