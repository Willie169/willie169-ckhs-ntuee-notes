...
\sssc{Definition}
For any complex-valued functions $f,g$ defined on $\bbR_{\geq0}$ for one-sided Laplace transform and $\bbR$ for two-sided Laplace transform, for any indexed family with $(A_i)_{i\in I}\subseteq\bbR_{\geq0}$ for one-sided Laplace transform and $\bbR$ for two-sided Laplace transform, for any real number $a$, the one-sided Laplace transform of $f(t)+g(t)\sum_{i\in I}\delta(t-A_i)$ is defined as
\[\int_0^{\infty}e^{-st}\qty(f(t)+g(t)\sum_{i\in I}\delta(t-A_i))\dd{t},\]
and the two-sided Laplace transform of the notation $f(t)+g(t)\sum_{i\in I}\delta(t-A_i)$ is defined as
\[\int_{-\infty}^{\infty}e^{-st}f(t)\qty(f(t)+g(t)\sum_{i\in I}\delta(t-A_i))\dd{t},\]
where convergence of the Laplace transform is determined by $f(t)$ term, where at a value of $s$, convergence is defined as the existence of such integral at it, and, if $I\neq\varnothing$, since the support of $\sum_{i\in I}\delta(t-A_i)$ is a locally finite set, we can always multiply $e^{-st}g(t)$ with a function $\chi(t)$ that is 1 at all $A_i$ such that for all $s$, $e^{-st}g(t)\chi(t)\in\mathcal{D}(\bbR_{\geq 0})$ for one-sided Laplace transform and $e^{-st}g(t)\chi(t)\in\mathcal{D}(\bbR)$ for two-sided Laplace transform, and the integral of $e^{-st}g(t)\sum_{i\in I}\delta(t-A_i)$ is to be interpreted as
