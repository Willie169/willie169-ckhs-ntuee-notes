...
\sssc{Definition}
Let $R$ be $\bbR_{\geq0}$ for one-sided Laplace transform and $\bbR$ for two-sided Laplace transform. The vector space over $\bbC$, $\mathscr{L}$ of Laplace transform integrands consists of all $f(t)+\sum_{j=0}^{\infty}\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji})$, for any complex-valued functions $f$ defined on $R$, for any intervals $K_j\ni 0$, for $I_j=\bbZ\cap R\cap K_j$, for any indexed families $(A_{ji})_{i\in I_j}\subseteq R$ with $A_{ji}<A_{j(i+1)}$ for any $i$ such that $i\in I_j\land i+1\in I_j$, for any indexed families $(a_{ji})_{i\in I_j}\subseteq\bbC_{\neq 0}$, the one-sided Laplace transform is defined as
\[\int_0^{\infty}e^{-st}\qty(f(t)+\sum_{j=0}^{\infty}\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji}))\dd{t},\]
and the two-sided Laplace transform is defined as
\[\int_{-\infty}^{\infty}e^{-st}\qty(f(t)+\sum_{j=0}^{\infty}\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji}))\dd{t},\]
where:
\bit
\item Convergence of the Laplace transform at a value $z$ of $s$ is defined as
\[e^{-zt}f(t)\in L^1(R)\land\sum_{i\in I_0}a_{0i}\exp(-zA_{0i})<\infty.\]
\item Since the support of $\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji})$ is a locally finite set $\{A_{ji}\}$, we can always find a function $\chi(t)$ that is $a_{ji}$ at $A_{ji}$ such that $e^{-st}\chi(t)\in\mathcal{D}(R)$, and the integral of $e^{-st}\chi(t)\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji})$ is to be interpreted as
\[\langle\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji}),e^{-st}\chi(t)\rangle,\]
and the derivative of $\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji})$ is taken distributionally as
\[\dv{}{t}(\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji}))=\sum_{i\in I_j}a_{ji}\dv[j+1]{}{t}\delta(t-A_{ji}).\]
\eit
