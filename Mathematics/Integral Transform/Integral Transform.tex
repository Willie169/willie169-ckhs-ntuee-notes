...
\sssc{Integral Transform}
An integral transform is in the form
\[\int_a^b K(s,t)f(t)\dd{t}\]
where $K(s,t)$ is called kernel.
\sssc{Linear transform}
An integral transform
\[\int_a^b K(s,t)f(t)\dd{t}\]
is a linear transform iff
\[C\int_a^bK(s,t)f(t)\dd{t}+D\int_a^bK(s,t)g(t)\dd{t}+E\int_a^bK(s,t)h(t)\dd{t}=\int_a^bK(s,t)(Cf(t)+Dg(t)+Eh(t))\dd{t}\]
whenever the LHS exists.
\ssc{Laplace Transform (拉普拉斯變換)}
\sssc{Introduction}
The one-sided or unilateral Laplace transform and two-sided or bilateral Laplace transform, where Laplace transform may refer to either of them or both of them depending on context and often refer to the former when not clear, are two integral transform that convert a complex-valued signal, for both one-sided and two-sided, often denoted in lowercase such as $f$ or $f(t)$, of a real variable $t$, called time domain, to a complex-valued function, often denoted in uppercase of the same letter, $F$ or $F(s)$, or as $\mathcal{L}\{f\}$, $\mathcal{L}\{f(t)\}$, $\mathcal{L}\{f\}(s)$, or $\mathcal{L}\{f(t)\}(s)$, and for two-sided, also denoted as $\mathcal{B}\{f\}$, $\mathcal{B}\{f(t)\}$, $\mathcal{B}\{f\}(s)$, or $\mathcal{B}\{f(t)\}(s)$, of a complex variable $s$, called (complex) frequency domain, (complex) frequency plane, $s$-domain, $s$-plane, Laplace domain, or Laplace plane.
\sssc{Definition}
Let $R$ be $\bbR_{\geq0}$ for one-sided Laplace transform and $\bbR$ for two-sided Laplace transform.

For any complex-valued (or real-valued) functional $f$ with support on $R$, the Laplace transform is defined as
\[\int_Re^{-st}f(t)\dd{t},\]
of which convergence is defined as the finiteness of the integral, on the subset of $\bbC$ where it converges.

For any complex-valued (or real-valued) distribution $T\in\mathcal{D}(\bbR)$ with compact support on $R$, the Laplace transform is defined as
\[\langle T,e^{-st}\rangle,\]
which is always convergent, on $\bbC$.

Distributions are often denoted in function inside integral notation, notably such as Dirac distribution, but should be interpreted in distributional definition.
...
\bit
\item Restriction of functional or distribution on $R$ is represented with the same symbol of the functional or distribution and interpreted as either depending on context.
\item $\int_0^{\infty}$ is also denoted as $\int_{0^-}^{\infty}$, i.e., $\lim_{\varepsilon\to 0^+}\int_{-\varepsilon}^{\infty}$, and $0$ as input of a time domain signal is also denoted as $0^-$. This notation is intuitive in Riemann–Stieltjes integral interpretation of Dirac delta.
\eit
...
For one-sided Laplace transform, ROC is a class of the form of either $\Re(s) > a$ or $\Re(s) \geq a$ for some extended real constant $a$, called abscissa of convergence.

For two-sided Laplace transform, ROC is a class of the form of either $\Re(s) > a$ or $\Re(s) \geq a$ and either $\Re(s) <  $ or $\Re(s) \leq b$ for some extended real constants $a$ and $b$, called abscissas of convergence.
...
\eit
\sssc{Linearity}
\[\mathcal{L}\{af(t)+bg(t)\}=a\mathcal{L}\{f(t)\}+b\mathcal{L}\{g(t)\},\quad a,b\in\bbC_{\neq0}.\]
The ROC is the intersection of the ROC of $\mathcal{L}\{f(t)\}$ and $\mathcal{L}\{g(t)\}$.
\sssc{Unit step function}
\[\mathcal{L}\{u(t-a)\} = \frac{e^{-as}}{s},\quad a\in R.\]
The ROC is $\Re(s)>0$.
\sssc{Delta distribution}
\[\mathcal{L}\{\dv[n]{}{t}\delta(t-a)\}=s^ne^{-sa},\quad n\in\bbN_0\land a\in R.\]
The ROC is $\bbC$.
\sssc{Exponential function}
\[\mathcal{L}\left\{e^{at}\right\} = \frac{1}{s-a},\quad a\in\bbC_{\neq0}.\]
...
\[\mathcal{L}\left\{e^{at}f(t)\right\}=F(s-a),\quad a\in\bbC_{\neq0}.\]
...
For one-sided Laplace transform:
\[\mathcal{L}\left\{f(t-a)\right\}=e^{-as}F(s),\quad a\in\bbR_{\geq 0}.\]
The ROC doesn't change.
\begin{proof}
For functional:
\[\ba
\mathcal{L}\left\{f(t-a)\right\}&=\int_a^{\infty}e^{-st}f(t-a)\dd{t}\\
...
\[\mathcal{L}\left\{f(t-a)\right\}=e^{-as}F(s),\quad a\in\bbR_{\neq 0}.\]
The ROC doesn't change.
\begin{proof}
For functional:
\[\ba
\mathcal{L}\left\{f(t-a)\right\}&=\int_{-\infty}^{\infty}e^{-st}f(t-a)\dd{t}\\
...
\[\mathcal{L}\left\{f'(t)\right\}=sF(s)+\lim_{t\to\infty}e^{-st}f(t)-f(0),\]
\[\mathcal{L}\left\{\dv[n]{f}{t}(t)\right\}=s^nF(s)+\sum_{i=0}^{n-1}s^{n-1-i}\qty(\lim_{t\to\infty}e^{-st}\dv[i]{f}{t}(t)-\dv[i]{f}{t}(0)).\]
\bpr
\[u=e^{-st},\quad\dd{u}=-se^{-st}\dd{t}.\]
\[\dd{v}=f'(t)\dd{t},\quad v=f(t).\]
\[\ba
\mathcal{L}\left\{f'(t)\right\}&=\int_0^\infty e^{-st}f'(t)\dd{t}\\
...
\[\mathcal{L}\left\{\dv[n]{f}{t}(t)\right\}=s^nF(s)-\sum_{i=0}^{n-1}s^{n-1-i}\dv[i]{f}{t}(0),\]
and the ROC is a superset of that of $F(s)$.

For two-sided Laplace transform of functional:
\[\mathcal{L}\left\{f'(t)\right\}=sF(s)+\lim_{t\to\infty}e^{-st}f(t)-\lim_{t\to-\infty}e^{-st}f(t),\]
\[\mathcal{L}\left\{\dv[n]{f}{t}(t)\right\}=s^nF(s)+\sum_{i=0}^{n-1}s^{n-1-i}\qty(\lim_{t\to\infty}e^{-st}\dv[i]{f}{t}(t)-\lim_{t\to-\infty}e^{-st}\dv[i]{f}{t}(t)).\]
\bpr
\[u=e^{-st},\quad\dd{u}=-se^{-st}\dd{t}.\]
\[\dd{v}=f'(t)\dd{t},\quad v=f(t).\]
\[\ba
\mathcal{L}\{f'(t)\}&=\int_{-\infty}^\infty e^{-st}f'(t)\dd{t}\\
...
\[\mathcal{L}\{f'(t)\}=sF(s),\]
\[\mathcal{L}\left\{\dv[n]{f}{t}(t)\right\}=s^nF(s),\]
and the ROC is a superset of that of $F(s)$.
...
For one-sided Laplace transform of functional:
\[\mathcal{L}\left\{\int_0^tf(\tau)\dd{\tau}\right\}=\frac{1}{s}F(s)-\frac{1}{s}\lim_{t\to\infty}e^{-st}\int_0^tf(\tau)\dd{\tau}.\]
\bpr
\[\ba
\mathcal{L}\left\{\int_0^tf(\tau)\dd{\tau}\right\}&=\int_0^{\infty}e^{-st}\int_0^tf(\tau)\dd{\tau}\dd{t}\\
...
\[\mathcal{L}\left\{\int_0^tf(\tau)\dd{\tau}\right\}=\frac{1}{s}F(s),\]
and the ROC is $F(s)$ if there is a zero at $0$ of $F(s)$ and $F(s)\cap\{s\in\bbC\mid\Re(s)>0\}$ otherwise.

For two-sided Laplace transform of functional:
\[\mathcal{L}\left\{\int_{-\infty}^tf(\tau)\dd{\tau}\right\}=\frac{1}{s}F(s)-\frac{1}{s}\lim_{t\to\infty}e^{-st}\int_{-\infty}^tf(\tau)\dd{\tau}.\]
\bpr
\[\ba
\mathcal{L}\left\{\int_{-\infty}^tf(\tau)\dd{\tau}\right\}&=\int_{-\infty}^{\infty}e^{-st}\int_{-\infty}^tf(\tau)\dd{\tau}\dd{t}\\
...
\[\mathcal{L}\left\{\int_{-\infty}^tf(\tau)\dd{\tau}\right\}=\frac{1}{s}F(s),\]
and the ROC is $F(s)$ if there is a zero at $0$ of $F(s)$ and $F(s)\cap\{s\in\bbC\mid\Re(s)>0\}$ otherwise.
\[\mathcal{L}\left\{\int_t^{\infty}f(\tau)\dd{\tau}\right\}=-\frac{1}{s}F(s)-\frac{1}{s}\lim_{t\to-\infty}e^{-st}\int_t^{\infty}f(\tau)\dd{\tau}.\]
\bpr
\[\ba
\mathcal{L}\left\{\int_t^{\infty}f(\tau)\dd{\tau}\right\}&=\int_{-\infty}^{\infty}e^{-st}\int_t^{\infty}f(\tau)\dd{\tau}\dd{t}\\
...
\[\mathcal{L}\left\{\int_t^{\infty}f(\tau)\dd{\tau}\right\}=-\frac{1}{s}F(s),\]
and the ROC is $F(s)$ if there is a zero at $0$ of $F(s)$ and $F(s)\cap\{s\in\bbC\mid\Re(s)<0\}$ otherwise.
\sssc{Differentiation in frequency domain}
\[\mathcal{L}\{tf(t)\}=-\dv{}{s}F(s),\]
\[\mathcal{L}\left\{t^nf(t)\right\}=(-1)^n\dv[n]{}{s}F(s).\]
\begin{proof}
...
For two-sided Laplace transform:
\[\mathcal{L}\{f(at)\} = \frac{1}{a} F\left(\frac{s}{a}\right), \quad a\neq 0.\]
\bpr
For functional:
...
By Fubini's theorem,
\[\ba
\mathcal{L}\{(f*g)(t)\}&=\int_Re^{-st}\int_Rf(\tau)g(t-\tau)\dd{\tau}\dd{t},\quad u=t-\tau\\
&=\int_Re^{-s\tau}f(\tau)\dd{\tau}\int_Re^{-su}g(u)\dd{u}\\
&=F(s)G(s)
\ea\]
\epr
For functional and distribution:
\[\mathcal{L}\{f*T\}=F(s)\mathcal{L}\{T\}.\]
\bpr
By Fubini's theorem,
\[\ba
\mathcal{L}\{f*T\}&=\int_R\langle T(x),f(t-x)\rangle e^{-st}\dd{t}\\
&=\langle T(x),\int_Rf(t-x)e^{-st}\dd{t}\rangle
\ea\]
\[u=t-x,\quad\dd{t}=\dd{u}\]
\[\ba
\int_Rf(t-x)e^{-st}\dd{t}&=e^{-sx}\int_Rf(u)e^{-su}\dd{u}\\
&=e^{-sx}F(s)
\ea\]
\[\langle T,e^{-sx}F(s)\rangle=F(s))\mathcal{L}\{T\}\]
\epr
For distributions:
\[\mathcal{L}\{S*T\}=\mathcal{L}\{S\}\mathcal{L}\{T\}.\]
\bpr
By Fubini's theorem,
\[\ba
\mathcal{L}\{S*T\}&=\langle S(t),\langle T(x),e^{-s(t+x)}\rangle\rangle\\
&=\langle S(t),e^{-st}\langle T(x),e^{-sx}\rangle\rangle\\
&=\mathcal{L}\{S\}\mathcal{L}\{T\}
\ea\]
\epr
\sssc{Cross-correlation theorem for two-sided Laplace transform}
For two-sided Laplace transform of functionals:
\[\mathcal{L}\left\{(f\star g)(t)\right\}=\ol{F(s)}G(s).\]
\bpr
By Fubini's theorem,
\[\ba
\mathcal{L}\left\{(f\star g)(t)\right\}&=\int_{-\infty}^{\infty}e^{-st}\int_{-\infty}^{\infty}\ol{f(-\tau)}g(t-\tau)\dd{\tau}\dd{t},\quad u=t-\tau\\
&=\int_{-\infty}^{\infty}e^{-s\tau}\ol{f(-\tau)}\dd{\tau}\int_{-\infty}^{\infty}e^{-su}g(u)\dd{u}\\
&=\ol{F(s)}G(s)
\ea\]
\epr
TODO: For two-sided Laplace transform of functional and distribution:
TODO: For two-sided Laplace transform of distributions:
\sssc{Initial Value Theorem (IVT) for one-sided Laplace transform of functional}
...
\ea\]
\end{proof}
\sssc{Final Value Theorem (FVT) for one-sided Laplace transform of functional}
...
For any complex-valued functional $f$ defined on $\bbR$ such that $f(t)e^{-\gamma t}\in L^1(\bbR)$, the inverse Laplace transform of the two-sided Laplace transform of $f(t)$ is equal to $f(t)$ a.e.
\begin{proof}
...
\ea\]
\end{proof}
For any complex-valued functional $f$ defined on $\bbR$ such that $f(t)=0$ for all $\Re(t)<0$ and $f(t)e^{-\gamma t}\in L^1(\bbR_{\geq0})$, the inverse Laplace transform of the one-sided Laplace transform of $f(t)$ is equal to $f(t)$ a.e.
\sssc{Inverse Laplace transform of constant}
For any $c\in\bbC$,
\[\mathcal{L}^{-1}\{c\}=0\]
\bpr
...
For $c\ne0$,
\[\ba
\lim_{T\to\infty}\int_{-iT}^{iT}e^{st}\dd{s}&=\lim_{T\to\infty}\evlp(\frac{1}{s}e^{st})_{-iT}^{iT}\\
&=\lim_{T\to\infty}\frac{e^{iT}+e^{-iT}}{iT}\\
&=\lim_{T\to\infty}\frac{2\cos(T)}{iT}\\
&=0
\ea\]
\epr
\sssc{Cover-up method for partial fraction decomposition}
Suppose $P(s)$ is a polynomial of $s$ and is not divisible by any of $(s-a_i)$, $a_i\in\bbC,b_i\in\bbN$ and $i\ne j\implies a_i\ne a_j$, and
\[\frac{P(s)}{\prod_{i=1}^n(s-a_i)^{b_i}}=\sum_{i=1}^n\sum_{j=1}^{b_i}\frac{A_{i,j}}{(s-a_i)^j}.\]
Then
\[A_{k,b_k-m}=\frac{1}{m!}\evlp{\dv[m]{s}(\frac{P(s)}{\prod_{\substrack{i=1\\i\ne k}}^n(s-a_i)^{b_i}})}_{s=a_k}.\]
\sssc{Transform of a periodic function}
Suppose $f(t)$ is periodic with period $T>0$ and of exponential order.

For one-sided Laplace transform:
\[\calL\{f(t)\}=\frac{1}{1-e^{-sT}}\int_0^Te^{-st}f(t)\dd{t}\]
\bpr
\[\calL\{f(t)\}=\int_0^Te^{-st}f(t)\dd{t}+\int_T^{\infty}e^{-st}f(t)\dd{t}\]
Let $t=u+T$.
\[\int_T^{\infty}e^{-st}f(t)\dd{t}=\int_0^{\infty}e^{-s(u+T)}f(u+T)\dd{u}=e^{-sT}\calL\{f(t)\}\]
\epr
For two-sided Laplace transform: The two-sided Laplace transform of a periodic function of period $T$ such that $\int_0^Te^{-su}f(u)\dd{u}\ne 0$ must diverge.
\bpr
\[\calL\{f(t)\}=\sum_{k=-\infty}^{\infty}\int_{kT}^{(k+1)T}e^{-st}f(t)\dd{t}\]
Let $u=kT$.
\[\sum_{k=-\infty}^{\infty}\int_{kT}^{(k+1)T}e^{-st}f(t)\dd{t}=\int_0^Te^{-su}f(u)\dd{u}\sum_{k=-\infty}^{\infty}e^{-skT}\]
$\sum_{k=-\infty}^{\infty}e^{-skT}$ diverges for all $s$.
\epr
...
TODO
TODO: mb as Laplace transform special case?
