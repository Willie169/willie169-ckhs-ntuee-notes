...
Laplace transform of sum of distribution and functional should be interpreted as the sum of the Laplace transform of them, and the convergence of Laplace transform is defined as the convergence of Laplace transform of the functional.
...
where for all $j$, we can always find a complex-valued (or real-valued) function $\chi(t)$ such that for all $i$, $\chi(A_{ji})=a_{ji}$ and that $e^{-st}\chi(t)\in\mathcal{D}(R)$, and the integral of $e^{-st}\chi(t)\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji})$ is to be interpreted as
\[\langle\sum_{i\in I_j}\abs{a_{ji}}\dv[j]{}{t}\delta(t-A_{ji}),e^{-st}\chi(t)\rangle,\]
and the derivative of $\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji})$ is taken distributionally, that is,
\[\dv{}{t}(\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji}))=\sum_{i\in I_j}a_{ji}\dv[j+1]{}{t}\delta(t-A_{ji}).\]

Dirac delta can also be interpreted as Dirac measure or Stieltjes integral over $u(x)$ if no derivative is involved. As improper Riemann–Stieltjes integral, the one-sided Laplace transform can be defined as
\[\lim_{\varepsilon\to 0^+}\int_{-\varepsilon}^{\infty}e^{-st}\qty(f(t)+\sum_{j=0}^{\infty}\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji}))\dd{t},\]
where $\lim_{\varepsilon\to 0^+}\int_{-\varepsilon}^{\infty}$ is also denoted as $\int_{0^-}^{\infty}$, and $e^{-zt}f(t)\in L^1(R)$ can be replaced with the absolute convergence of the improper Riemann–Stieltjes integral with $s=z$.

When denoting $f(t)+\sum_{j=0}^{\infty}\sum_{i\in I_j}a_{ji}\dv[j]{}{t}\delta(t-A_{ji})$ as a single notation, say $g(t)$, for all $n\in\bbN_0$, $\dv[n]{}{t}g(0)$, $\dv[n]{}{t}g(0^-)$, or $\dv[n]{}{t}g(0^+)$ are to be interpreted as $\dv[n]{}{t}f(0)$.
...
A complex-valued (or real-valued) function $f(t)$ defined on $[0,\infty)$ is one-sided Laplace-transformable if it is piecewise continuous on all closed subintervals of $[0,\infty)$ and of exponential order.
\begin{proof}
...
\end{proof}
A complex-valued (or real-valued) function $f(t)$ defined on $\bbR$ is two-sided Laplace-transformable if it is piecewise continuous on all closed intervals and of exponential order and that $f(-t)$ is of exponential order.
