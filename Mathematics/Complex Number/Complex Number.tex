...
\sssc{Complex conjugate (共軛)}
The (complex) conjugate of a complex number $z=a+bi$ with $a,b\in\bbR$, denoted as $\ol{z}$ or $z^*$ and called z bar, is defined as $a-bi$.
\sssc{Division}
...
For a complex number $z\neq 0$, the argument of $z$, denoted as $\arg(z)$ or $\angle z$, is definited as any real number $\varphi $ such that
\[z=|z|e^{i\varphi}.\]
...
The complex plane is the plane formed by the complex numbers, with a Cartesian coordinate system such that the horizontal x-axis, called the real axis, is formed by the real numbers, and the vertical y-axis, called the imaginary axis, is formed by the imaginary numbers. Half-plane with positive real parts is called right Half-plane (RHP). Half-plane with negative real parts is called left Half-plane (LHP).
