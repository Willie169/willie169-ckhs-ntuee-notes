...
\item $G$ is a deterministic next-output function.
\eit
\sssc{Moore machine}
...
\item $s_0\in S$ is the starting state.
\item $\delta\colon S\times\Sigma\to S$ is a deterministic transition function.
\item $G$ is a deterministic next-output function, which is a partial function
\[G\colon S\to\Gamma\]
...
\item $S$ is a finite non-empty set of all states.
\item $s_0\in S$ is the starting state.
\item $\delta\subseteq S\times\Gamma\times S\times(\Gamma\setminus\{l,r\})\times\{L,S,R\}$ is a transition function with the constraints that
