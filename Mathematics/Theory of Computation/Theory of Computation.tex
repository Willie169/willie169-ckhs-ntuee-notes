...
\[w_0\vdash w_1\vdash\ldots\vdash w_n,\]
then
\[w_0\vdash^*w_n,\]
...
\sssc{Context-free language (CFL)}
...
\sssc{Context-sensitive language (CFL)}
...
\tb{Theorem.} The set of all recognizable languages of all finite-state machine is the set of all regular language.
...
\tb{Theorem.} The set of all recognizable languages of all deterministic pushdown automata is the set of all deterministic context-free language.
\sssc{Transducer}
...
\item $\Gamma$ is the output alphabet,
\item $S$ is a set of all states,
\item $\delta$ is the deterministic or nondeterministic next-state function or transition function,
