...
normalizing means dividing $H(s)$ by $K$, called pass gain. The following filter definitions are normalized. Multiple by $K$ for pass gain $K$. 
\sssc{Logarithmic scale, Bode plot (波德圖), and decibel (dB)}
Normalized frequency response are often expressed in logarithmic scale:
\bit
...
\item Magnitude response, input, and output can be expressed in decibel (dB) as $20\log(\abs{H(i\omega)})$, $20\log(x)$, and $20\log(y)$.
\eit
\sssc{Cutoff (截止), break, or corner (angular) frequency}
For a normalized transfer function, cutoff frequency $\omega_c$ is a frequency such that
\[\abs{H(i\omega_c)}=\frac{1}{\sqrt{2}}.\]
...
\sssc{Standard band-pass (BP) filter (BPF)}
...
where $\omega_L<\omega_H$ are called band edge, $\omega_H-\omega_L$ is called bandwidth, and $\omega_0=\sqrt{\omega_L\omega_H}$ is called central passed frequency.
\sssc{Standard band-stop (BS) or notch filter (BSF)}
...
where $\omega_L<\omega_H$ are called band edge, $\omega_H-\omega_L$ is called bandwidth, and $\omega_0=\sqrt{\omega_L\omega_H}$ is called central rejected frequency.
\sssc{Normalized low-pass prototype filter}
A normalized low-pass prototype filter is a low-pass filter with cutoff frequency $\omega_c=1$. Suppose the transfer function of it is $H_P(s)$.
...
\[H_{BP}(s)=H_P\qty(\frac{Bs}{s^2+\omega_0^{\pht{0}2}}).\]
...
\[H_{BS}(s)=H_P\qty(\frac{s^2+\omega_0^{\pht{0}2}}{Bs}).\]
