...
Given an $n$th-order system of linear ODEs
\[\sum_{i=0}^na_i(x)\mb{y}^{(i)}=\mb{f}(x),\]
\[\sum_{i=0}^na_i(x)\mb{y}^{(i)}=0,\]
...
If $\mb{y}_1(x)$ and $\mb{y}_2(x)$ are both solutions to a system of linear ODEs on an interval, then $c_1\mb{y}_1(x)+c_2\mb{y}_2(x)$ is also a solution to it on the interval for any $c_1,c_2\in\bbR$.
