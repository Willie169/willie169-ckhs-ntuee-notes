...
All $c_i=0$ if $W(f_1,f_2,\ldots,f_n)(x)\ne 0$ by Cramer rule, thus $W(f_1,f_2,\ldots,f_n)(x)=0$.
\end{proof}
...
If $y_1(x),y_2(x),\ldots y_n(x)$ are $n$ linearly independent solutions of an $n$th-order homogeneous linear ODE on an interval, then they are a basis of the vector space of the general solutions of the ODE on the interval, and $\sum_{i=1}^nc_iy_i(x)$ is the family of general solutions to it on the interval with $n$ parameter $c_1,c_2,\ldots c_n$.
\sssc{Solutions of non-homogeneous linear ODE as affine space}
...
For any solution $f(t)$ of any non-homogeneous linear ODE of one variable $t$, there exists some $M,T\geq 0$ and $\alpha\in\bbR$ such that for all $t>T$ $\abs{f(t)}\leq e^{M\alpha t}$, and the smallest such $\alpha$ is the largest real part among all roots of its characteristic equation.
...
All $c_i=0$ if $W(\mb{y}_1,\ldots,\mb{y}_n)\ne 0$ by Cramer rule, thus $W(\mb{y}_1,\ldots,\mb{y}_n)=0$.
\end{proof}
\sssc{Homogeneity}
...
\sssc{Bounded-input bounded-output (BIBO) stable}
A LTI system is called asymptotically stable iff bounded input implies bounded response, iff all poles of it are on the left half-plane.
\sssc{Marginally stable}
A LTI system is called marginally stable iff there is no pole of it on the right half-plane and no multiple pole of it on the imaginary axis, iff its impulse response converges to an equilibrium.
\sssc{Unstable}
A LTI system is called unstable iff response may not be bounded, iff all poles of it are on the left half-plane.
