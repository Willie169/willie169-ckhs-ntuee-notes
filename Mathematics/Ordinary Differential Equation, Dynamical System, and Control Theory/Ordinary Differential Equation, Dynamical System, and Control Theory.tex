...
If $y_1(x)$ and $y_2(x)$ are both solutions to a linear ODE on an interval, then $c_1y_1(x)+c_2y_2(x)$ is also a solution to it on the interval for any $c_1,c_2\in\bbR$.
...
\end{vmatrix}.\]
\sssc{Wrońskian and linear independence}
If $f_1,f_2,\ldots,f_n$ are not linearly independent, then $W(f_1,f_2,\ldots,f_n)=0$.
...
\epr
\sssc{Solutions of linear ODE as vector space}
...
\epr
\sssc{Wrońskian}
Suppose $\mb{y}_i\colon\Omega_i\subseteq\bbR\to\bbR^n$ for all $i\in\bbN_{\le n}$ are solution vectors to a homogeneous system on an interval and
...
\sssc{Wrońskian and linear independence}
If $\mb{y}_1,\ldots,\mb{y}_n$ are not linearly independent, then $W(\mb{y}_1,\ldots,\mb{y}_n)=0$.
\sssc{Non-zero Wrońskian implies linear independence}
