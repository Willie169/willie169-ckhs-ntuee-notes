...
A frequency $\omega_c$ such that
\[\abs{H(j\omega_c)}=\frac{1}{\sqrt{2}}.\]
\sssc{Low-pass (低通) (LP), high-cut, or bass-cut filter (LPF)}
A continuous-time linear time-invariant system is a low-pass filter iff $H(i\omega)$ as a function of a real variable $\omega$ is such that
\[\lim_{\omega\to 0}\abs{H(i\omega)}=K\in\bbR_{>0}\]
and
\[\lim_{\omega\to\infty}\abs{H(i\omega)}=0,\]
where $K$ is called low-pass gain.
\sssc{High-pass (高通) (HP), low-cut, or treble-cut filter (HPF)}
A continuous-time linear time-invariant system is a high-pass filter iff $H(i\omega)$ as a function of a real variable $\omega$ is such that
\[\lim_{\omega\to 0}\abs{H(i\omega)}=0\]
and
\[\lim_{\omega\to\infty}\abs{H(i\omega)}=K\in\bbR_{>0},\]
where $K$ is called high-pass gain.
\sssc{Band-pass (BP) filter (BPF)}
A continuous-time linear time-invariant system is a band-pass filter iff $H(i\omega)$ as a function of a real variable $\omega$ is such that
\[\lim_{\omega\to 0}\abs{H(i\omega)}=0\]
\[\lim_{\omega\to\infty}\abs{H(i\omega)}=0\]
and
\[\max_{\omega}\abs{H(i\omega)}=K\in\bbR_{>0},\]
where $K$ is called band-pass gain.
\sssc{Band-stop (BS) or notch filter (BSF)}
A continuous-time linear time-invariant system is a band-stop filter iff $H(i\omega)$ as a function of a real variable $\omega$ is such that
\[\lim_{\omega\to 0}\abs{H(i\omega)}=K_1\in\bbR_{>0},\]
\[\lim_{\omega\to\infty}\abs{H(i\omega)}=K_2\in\bbR_{>0},\]
and
\[\min_{\omega}\abs{H(i\omega)}=0,\]
where $K_1$ is called low-pass notch gain and $K_2$ is called high-pass notch gain.
