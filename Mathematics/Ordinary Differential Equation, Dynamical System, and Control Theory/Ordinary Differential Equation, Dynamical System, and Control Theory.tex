...
\[Y(s)=X(s)\cdot H(s),\]
where $H(s)$ is a rational function called transfer function (轉移函數), and its absolute value is called gain, or attenuation if $<1$.
...
\[Y(s)=X(s)\cdot H(s),\]
where $H(s)$ is a rational function called transfer function, and its absolute value is called gain (增益), or attenuation (衰減量) if $<1$.
