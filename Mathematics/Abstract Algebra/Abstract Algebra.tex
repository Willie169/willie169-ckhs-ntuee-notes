...
\ssc{Operation (運算)}
An $n$-ary operation on a set $A$ is a function from a subset of $A^n$ to $A$.
\ssc{Algebraic structures (代數結構) or algebraic system (代數系統)}
An algebraic structure or algebraic system consists of a nonempty set $A$, called the underlying set, carrier (set), or domain, a collection of operations on $A$, and a finite set of identities called axioms that these operations must satisfy.
\ssc{Common axioms}
...
A magma or binar is a set $M$ with a binary operation $*$ such that
\[\forall a,b\colon a,b\in M\implies a*b\in M.\]
\sssc{Semigroup (半群)}
A semigroup is a magma equipped with a binary operation that is associative.
\sssc{Unital magma}
A unital magma is a magma equipped with a binary operation that has an identity element.
\sssc{Monoid (么半群, 單群, or 亞群)}
A monoid is a magma equipped with a binary operation that is associative and has an identity element.
\sssc{Commutative monoid or abelian monoid}
A commutative monoid or abelian monoid is a monoid equipped with a binary operation that is commutative.
\sssc{Quasigroup (擬群)}
A quasigroup is a magma equipped with a binary operation that has Latin square property.
\sssc{Loop (么擬群 or 圈)}
A loop is a magma equipped with a binary operation that has Latin square property and has an identity element.
\sssc{Associative quasigroup}
An associative quasigroup is a magma equipped with a binary operation that is associative and has Latin square property.
\sssc{Group (群)}
A group is a magma equipped with a binary operation that has an identity element, is associative, and every element has an inverse element, that is, a nonempty associative quasigroup.
\sssc{Abelian group (阿貝爾群) or commutative group (交換群)}
An abelian group, also called a commutative group, is a group equipped with a binary operation that is commutative.
...
A map $\phi\colon\bbR\to G$ to some group equipped with a binary operation $*$ is called a one-parameter subgroup of $G$ iff
\[\phi(t)*\phi(s)=\phi(t+s).\]
