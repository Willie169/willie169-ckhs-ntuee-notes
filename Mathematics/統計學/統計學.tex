...
\eam
\ssc{仿射變換}
$\mathbf{X}$的仿射變換$\mathbf{Y}=(y_i| y_i=ax_i+b,i=1,2,\ldots,n)$，記作$\mathbf{Y}=a\mathbf{X}+b$。
...
$\mathbf{Z}$，定義為：
\[\mb{Z}=\left\{z_i|z_i=\frac{x_i-\mu}{\sigma},\quad i=1,2,\ldots,n\right\}\]
...
\sct{二維數據分析}
今有由小到大排列的實數序列\(\mathbf{X}=(x_1,x_2,\ldots,x_n)\)與\(\mathbf{Y}=(y_1,y_2,\ldots,y_n)\)，標準差分別為$\sigma_\mathbf{X},\sigma_\mathbf{Y}$，算術平均數分別為$\mu_\mathbf{X},\mu_\mathbf{Y}$，標準分數分別為\(\mathbf{X}'=\qty(x_i' | x_i'=\frac{x_i-\mu_\mathbf{X}}{\sigma_\mathbf{X}},i=1,2,\ldots,n)\)與\(\mathbf{Y}'=\qty(y_i' | y_i'=\frac{y_i-\mu_\mathbf{Y}}{\sigma_\mathbf{Y}},i=1,2,\ldots,n)\)。
\ssc{散布圖}
