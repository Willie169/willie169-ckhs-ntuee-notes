...
\sssc{熱力學狀態（Thermodynamic state）}
指一組描述熱力學系統的狀態。
\sssc{狀態函數（State function）/狀態變數（State variable）}
是系統狀態的函數，其值僅取決於系統目前的狀態，而與系統如何到達該狀態的路徑無關。
\sssc{熱力學平衡狀態}
指除非被引發熱力學過程的熱力學操作中斷，否則將保持不變的熱力學狀態。

系統的每個平衡狀態分別由一組適當的熱力學狀態函數完全指定，只要一熱力學系統中有足夠多的已知狀態函數，其他的就已被確定，而所需要的狀態函數數量視系統複雜程度而定。
\sssc{狀態方程式（Equation of state）}
描述流體熱力學狀態的方程式，形式為：
\[f(p,V,T)=0,\]
其中$p$為壓力，$V$為體積，$T$為絕對溫度。

例如理想氣體方程式為：
\[pV-nRT=0\]
\subsection{熱力學過程（Thermodynamic process）/熱力學路徑（Thermodynamic path）}
\sssc{熱力學過程/熱力學路徑}
指系統熱力學狀態改變的過程，系統的改變是由從初始熱力學平衡狀態到最終狀態的轉變來定義的，例如所有化學反應都是熱力學過程。在經典熱力學中實際過程常被忽略，但非平衡熱力學考慮的是系統狀態接近熱力學平衡的過程。
\sssc{路徑函數（Path function）/過程函數（Process function）}
是系統從一個狀態變到另一個狀態的過程的函數。
\sssc{熱力學循環（Thermodynamic cycle）}
一熱力學系統經過一系列熱力學過程，最終回到初始狀態的過程。
\subsection{常數}
...
\item \tb{可逆過程}：$\mathrm{d}S = \frac{\delta Q}{T}$，此式又稱熵的克勞修斯定義。
\eit
\ssc{吉布斯能（Gibbs energy）與亥姆霍茲能（Helmholtz energy）}
\sssc{吉布斯能變化判斷定壓過程}
對於定壓封閉系統：
...
為熵的波茲曼定義。
\ssc{熱傳導（Thermal conduction）與熱容（Heat capacity）}
\sssc{傅立葉定律（Fourier's law）}
...
\sssc{熱導率（Thermal conductivity）}
$\kappa$ (W m$^{-1}$ K$^{-1}$)。定義為使得：
\[\mb{q}=-\kappa\nabla T.\]
\sssc{牛頓冷卻定律（Newton's law of cooling）}
...
相圖是描述物質在不同狀態下之相的圖表，顯示了不同相之間的平衡條件。相圖一般以溫度為橫軸，壓力（指該物質的氣相分壓，即蒸氣壓）為縱軸，亦有另增體積為$z$軸者，在圖上將異相平衡的條件以曲線（對於有體積軸者，曲面）繪出，並標示彼等圍出的各區域之各相。
