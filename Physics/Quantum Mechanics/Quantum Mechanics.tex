...
\[\psi(x)=Ae^{ikx}\]
\[\abs{\psi(x)}^2=A^2\]
\sssc{Reflection from a potential step}
A potential step with boundary at $x=0$ is
...
Solve TISE for incoming and reflected at $x=0$ waves in region I $x<0$ and transmitted at $x=0$ and reflected at $x=a$ waves in region II $0\le x\le a$ and transmitted wave in region III $x>a$, called tunnelled through the potential barrier.
\[\psi_I(x)=Ae^{ikx}+Be^{-ikx}\]
\[k=\frac{\sqrt{2mE}}{\hbar}\]
...
\item $Ce^{\kappa x}$: transmitted at $x=0$ wave
