...
\sssc{Electron in a one-dimensional infinite square (potential) well/trap}
...
\sssc{Electron in a two-dimensional infinite square (potential) well or rectangular corral}
...
\sssc{Electron in a three-dimensional infinite square (potential) well or rectangular box}
A three-dimensional infinite square well with boundary at $x=0$, $x=L_x$, $y=0$, $y=L_y$, $yz=0$, and $z=L_z$ is
...
\sssc{Quantum numbers}
For an electron trapped in a $k$-dimensional infinite square well, it has $k+1$ quantum numbers $n_1$ to $n_k$ and spin quantum number $m_s$.
\subsubsection{Pauli exclusion principle (包立不相容原理)}
No two fermions of the same type confined to the same trap can have the same set of values for their quantum numbers.
