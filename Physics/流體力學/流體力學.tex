...
\item $\mathbf{v}$：流速場
\item $\rho$：密度
\item $\boldsymbol{\sigma}$: (Cauchy/true) stress tensor
\item $V$：控制體積
\item $M$：流體分子分子量
\item $n$：流體分子分子數
...
\end{itemize}
\ssc{Fluid Motion}
\sssc{Material derivative (物質導數) or Substantial derivative(隨質導數)}
\[\frac{\mathrm{D}}{\mathrm{D}t}=\pdv{}{t}+\mathbf{v}\cdot\nabla.\]
\sssc{Continuity equation (連續性方程)}
Describe the conversation of mass.
\[\frac{\partial\rho}{\partial t}+\nabla\cdot(\rho\mathbf{v})=0.\]
\sssc{(Cauchy/true) stress tensor}
\[\boldsymbol{\sigma}\]
PLACEHOLDER
\sssc{Navier–Stokes Equations (納維-斯托克斯方程式) or Cauchy momentum equation}
Describe the Newton's second law.
\[\rho\frac{\mathrm{D}\mathbf{v}}{\mathrm{D} t}=\nabla\cdot\boldsymbol{\sigma}+\rho\mb{a}.\]
...
\[R_m=\rho R_v=\rho Av=\tx{a constnat.}\]
...
\[\mb{F}=-6\pi\mu a\mb{v}.\]
\sssc{(Hagen–)Poiseuille equation or (Hagen–)Poiseuille law}
The assumptions are that the fluid is incompressible and Newtonian with constant viscosity; the laminar flow is steady, axisymmetric, fully developed, and with zero radial and azimuthal components of velocity through a pipe of constant circular cross-section that is substantially longer than its diameter and with no-slip boundary; and pressure varies only along the pipe.
\[v(r)=\frac{\delta p}{4\mu L}\qty(R^2-r^2),\]
\[Q=\frac{\pi R^4}{8\mu L}\delta p=\frac{\qty(2\pi R^2)^2}{8\pi\mu L}\delta p,\]
where
\bit
\item $v(r)$ is the axial velocity of the fluid at radial position $r$,
\item $\delta p$ is the pressure difference between the two ends,
\item $L$ is the length of pipe,
