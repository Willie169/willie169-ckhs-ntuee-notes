...
\end{longtable}
\ect
\subsubsection{薄透鏡公式（Thin lens formula or thin lens equation）}
Let both sides of lens be in the same medium, focal length be $f$ with convex lens being positive and concave lens being negative, object distance be $u$ in front of mirror, image distance be $v$ with real image being positive and virtual image being negative, object height be $h_o$ with upward being positive and downward being negative, image height be $h_i$ with upward being positive and downward being negative. The mirror formula gives
\[\frac{1}{u}+\frac{1}{v}=\frac{1}{f}.\]
Laterial (orthogonal to main axis) magnification $m=\frac{h_i}{h_o}$ and longitudinal (parallel to main axis) magnification $m_p$ are
\[m=\frac{h_i}{h_o}=-\frac{v}{u}\]
\[m_p=\dv{v}{u}=-m^2\]
...
\subsubsection{Thin lens formula between mediums}
Same setup as previous one but the object is in medium of refractive index $n_1$ and the other side of lens is in medium of refractive index $n_2$.
PLACEHOLDER
\subsubsection{造鏡者公式（Lens maker's formula）}PLACEHOLDER
