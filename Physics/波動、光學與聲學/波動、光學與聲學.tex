...
\sssc{楊氏（雙狹縫）干涉實驗（Young's (double-slit) inference experiment）}
以通過單狹縫產生同調波，進行雙狹縫干涉。
\[I(\theta)=4I_0\cos^2(\frac{\pi b\sin\theta}{\lambda}).\]
\subsubsection{折射率效應}
...
\subsubsection{亮暗紋}
亮紋（bright band, bright fringe, or maximum）指同相處，即光強度極大值處；暗紋（dark band, dark fringe, or minimum）指反相處，即光強度極小值處；亮帶（bright band or bright fringe）中線指兩相鄰暗紋之中線；暗帶（dark band or dark fringe）；此種圖樣稱干涉圖樣（inference pattern）。
\begin{itemize}
\item 中央亮紋：$\theta=0$。
...
\sssc{Setup}
波長$\lambda$同調波通過平面上$k$個相鄰狹縫射向屏幕，縫寬$a>\lambda$、兩相鄰狹縫中線相距$b>\lambda$，屏幕平面平行狹縫平面，兩者距離$L$使得$L\gg a$、$L\gg b$，中央亮紋場強$I_0$。對於屏幕上某點，令其與狹縫平面中線之最短連線與該點上狹縫平面法線之夾角$\theta$。
\sssc{場強分布}
場強分布$I(\theta)$為：
\[I(\theta)=I_0\qty(\frac{\sin(\frac{\pi a\sin\theta}{\lambda})}{\frac{\pi a\sin\theta}{\lambda}})^2\qty(\frac{\sin(k\frac{\pi b\sin\theta}{\lambda})}{\sin(\frac{\pi b\sin\theta}{\lambda})})^2.\]
\sssc{楊氏（雙狹縫）干涉實驗（Young's (double-slit) inference experiment）}
以通過單狹縫產生同調波，進行雙狹縫干涉。
\[I(\theta)=4I_0\qty(\frac{\sin(\frac{\pi a\sin\theta}{\lambda})}{\frac{\pi a\sin\theta}{\lambda}})^2\cos^2(\frac{\pi b\sin\theta}{\lambda}).\]
\subsubsection{折射率效應}
折射率$n$的介質，因波長為真空$\frac{1}{n}$倍，故$m$為真空$n$倍。
\subsubsection{非單色光效應}
入射光為白光時，中央亮紋兩側可見黃色，因紫色$m$最小，而其互補色為黃色。
