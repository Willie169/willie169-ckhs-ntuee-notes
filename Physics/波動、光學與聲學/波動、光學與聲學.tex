...
平行光反射匯聚於焦點（focal point or focus）。
...
令焦距$f$凹面鏡為正、凸面鏡為負，物距$p$實物（即鏡前）為正、虛物（即鏡後）為負，像距$q$實像為正、虛像為負，物高$h_o$正立為正、倒立為負，像高$h_i$正立為正、倒立為負，橫向放大率（lateral magnification）（指垂直主軸）$m=\frac{h_i}{h_o}$，縱向放大率（指平行主軸）$m_p=\dv{q}{p}$。
\[m=\frac{h_i}{h_o}=\frac{q}{p}=\frac{f}{p-f}=\frac{q-f}{f}\]
...
\subsubsection{球面鏡近軸近似}
球面鏡對於近軸光線之反射可近似為焦距為其曲率半徑二分之一的拋物面鏡。
...
\subsubsection{凹（Concave）透鏡}
可散光，可分為雙凹、平凹、凸凹。
\subsubsection{凸（Convex）透鏡}
可聚光，可分為雙凸、平凸、凹凸。
\subsubsection{球面像差（Spherical aberration, SA）}
...
\end{longtable}
\ect
\subsubsection{薄透鏡公式/高斯公式}
此為薄透鏡近似，厚透鏡有偏差。

令焦距$f$凸透鏡為正、凹透鏡為負，物距$p$實物（即鏡前）為正、虛物（即鏡後）為負，像距$q$實像為正、虛像為負，物高$h_o$正立為正、倒立為負，像高$h_i$正立為負、倒立為正，橫向（指垂著主軸）放大率$m=\frac{h_i}{h_o}$，縱向（指平行主軸）放大率$m_p$。
\[m=\frac{h_i}{h_o}=\frac{q}{p}=\frac{f}{p-f}=\frac{q-f}{f}\]
\[\frac{1}{p}+\frac{1}{q}=\frac{1}{f}\]
\[m_p=\dv{q}{p}=-m^2\]
