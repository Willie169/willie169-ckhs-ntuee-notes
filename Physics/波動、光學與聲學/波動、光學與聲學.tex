...
\section{惠更斯（－菲涅耳）原理（Huygens(–Fresnel) principle）與干涉（Inference）}
\ssc{惠更斯（－菲涅耳）原理（Huygens(–Fresnel) principle）}
...
\sssc{Setup}
同調光通過狹縫射向屏幕。令有$k$個狹縫，狹縫寬忽略，相鄰狹縫間距$b\gg\lambda$，屏幕距狹縫片$L$，$L\gg b$。屏幕平行任二狹縫各自中線之公垂線，對於屏幕上某點，令其在中央狹縫中線上之垂線與屏幕平面之法線過中央狹縫中線者之夾角$\theta$，取正，屏幕上某點距狹縫片中線在屏幕上正射影距離$y=L\tan\theta$。令$\beta=\frac{\pi b\sin\theta}{\lambda}$。
\sssc{場強分布}
\[\ba
...
\ea\]
\bpr
Consider electric field in the center
...
\ssc{透鏡}
可用匯聚透鏡在狹縫面與屏幕之間替代$L\gg a$、$L\gg b$，達成相同效果。
\ssc{阿拉戈（Arago）/帕松
