...
\item $k$：（角）波數（(Angular) wave number）
\item $\omega$：角頻率（Angular frequency）量值
\item $\nu$：頻率（Frequency）。介質質點在每秒內振動的次數。
\item $T$：週期（Period）。介質上質點在每秒內振動的次數。
\item $E$：能量
\item $u$：能量密度
\item $\varphi$：相位（常數）（Phase (constant)）
\item $\varphi$：相位角（常數）（Phase angle (constant)）
\item $D$：波程差
\end{itemize}
\sssc{向量}
\begin{itemize}
...
\item $\varphi_l$：光通量（Luminous flux）(lm lumen = 流明 = cd sr)
...
\sssc{純量}
\begin{itemize}
\item $F$：繩張力（tension）在波前進方向的分量量值
\item $m$：質量
\item $\mu=\dv{m}{x}$：（繩子等的）線密度
\item $K$：動能
\item $U$：位能
\end{itemize}
\subsection{聲波}
...
\subsection{波的形狀}
\begin{itemize}
\item 波形（Waveform/wave form）：波動作為時間函數的圖形形狀。
...
\item 波前（Wavefront/wave front）：等相位處形成的幾何對象，$i$維波的波前是$i-1$維幾何對象，始終與波的行進方向垂直。
\end{itemize}
\subsection{數學描述}
...
同調波源（Coherent source）：指頻率相同的波源。可以形成穩定的干涉圖樣。對於光即單色（monochromatic）光。
...
一個行進的正弦波的波函數可以表示成：
\[\begin{aligned}
y &= A \cdot \sin\qty(\mathbf{k} \cdot \mathbf{x} \pm \omega t + \varphi) \\
&= \frac{A}{2i} \left(e^{i(\mathbf{k} \cdot \mathbf{x} \pm \omega t + \varphi)} - e^{-i(\mathbf{k} \cdot \mathbf{x} \pm \omega t + \varphi)} \right)
\end{aligned}\]
...
\begin{proof}
We want to prove:
\[ \qty(y = A \cdot \sin\qty(\mathbf{k} \cdot \mathbf{x} \pm \omega t + \varphi))\implies\qty(\pdv[2]{y}{t}=v^2\nabla^2y)\]
Compute \(\frac{\partial^2 y}{\partial t^2}\):
\[\frac{\partial y}{\partial t} = A \pm\omega \cdot \cos\left(\mathbf{k} \cdot \mathbf{x} \pm \omega t + \varphi\right).\]
\[\begin{aligned}
\frac{\partial^2 y}{\partial t^2} &= -A \omega^2 \cdot \sin\left(\mathbf{k} \cdot \mathbf{x} \pm \omega t + \varphi\right)\\
&= -\omega^2 y.
\end{aligned}\]
Compute \(\nabla^2 y\):
\[
\frac{\partial y}{\partial x_i} = Ak_i \cdot \cos\left(\mathbf{k} \cdot \mathbf{x} \pm \omega t + \varphi\right).
\]
\[
\frac{\partial^2 y}{\partial x_i^2} = -Ak_i^2 \cdot \sin\left(\mathbf{k} \cdot \mathbf{x} \pm \omega t + \varphi\right).
\]
\[
...
\subsection{一維波函數}
\[\begin{aligned}
y &=A\cdot\sin(kx\pm\omega t+\varphi)\\
&= A\cdot\sin\qty(2\pi\qty(\frac{x}{\lambda}\pm\frac{t}{T})+\varphi)\\
&= \frac{A}{2i} \left(e^{i(kx \pm \omega t + \varphi)} - e^{-i(kx \pm \omega t + \varphi)} \right)
\end{aligned}\]
其中：$x$為位置；$y$為質點振動位移；$\varphi$為相位角；若$\pm$取負表波往$x$軸$\frac{k}{\abs{k}}$方向傳遞，若$\pm$取正表波往$x$軸$-\frac{k}{\abs{k}}$方向傳遞。
\subsection{介質質點振動速度}
波函數：
\[y=A\cdot\sin(\mathbf{k}\cdot\mathbf{x}-\omega t+\varphi)\]
...
\[ \pdv{y}{t}=-\pdv{y}{x}v \]
\subsection{合成波振幅}
令波$f(x,t)=A\cdot\sin\qty(\mathbf{k}\mathbf{x}-\omega t-\varphi_1)$與$g(x,t)=B\cdot\sin\qty(\mathbf{k}\mathbf{x}-\omega t-\varphi_2)$，合成波$f(t)+g(t)$振幅$C$：
\[ C = \sqrt{A^2+B^2+2AB\cos\qty(\varphi_1-\varphi2)} \]
\subsection{駐波（Standing wave）}
駐波指兩個波數、頻率和振幅皆相同的正弦波相向行進干涉而成的合成波。
\subsubsection{數學描述}
令兩波$f(\mathbf{x},t)=A\sin\qty(\mathbf{k}\cdot\mathbf{x}+\omega t+\varphi_1)$、$g(\mathbf{x},t)=A\sin\qty(\mathbf{k}\cdot\mathbf{x}-\omega t+\varphi_2)$，其合成波為：
\[ f(\mathbf{x},t)+g(\mathbf{x},t) = 2A\sin\qty(\mathbf{k}\cdot\mathbf{x} + \frac{\varphi_1 + \varphi_2}{2})\cos\qty(\omega t + \frac{\varphi_1 - \varphi_2}{2}) \]
\begin{proof}
\[
f(\mathbf{x},t) + g(\mathbf{x},t) = A\sin\qty(\mathbf{k}\cdot\mathbf{x} + \omega t + \varphi_1) + A\sin\qty(\mathbf{k}\cdot\mathbf{x} - \omega t + \varphi_2)
\]
正弦的和化積公式：
\[
...
設：
\[
u = \mathbf{k}\cdot\mathbf{x} + \omega t + \varphi_1, \quad v = \mathbf{k}\cdot\mathbf{x} - \omega t + \varphi_2
\]
則：
\[\begin{aligned}
&f(\mathbf{x},t) + g(\mathbf{x},t) \\
=& 2A\sin\qty(\frac{\mathbf{k}\cdot\mathbf{x} + \omega t + \varphi_1 + \mathbf{k}\cdot\mathbf{x} - \omega t + \varphi_2}{2})\cos\qty(\frac{\mathbf{k}\cdot\mathbf{x} + \omega t + \varphi_1 - (\mathbf{k}\cdot\mathbf{x} - \omega t + \varphi_2)}{2})\\
=& 2A\sin\qty(\mathbf{k}\cdot\mathbf{x} + \frac{\varphi_1 + \varphi_2}{2})\cos\qty(\omega t + \frac{\varphi_1 - \varphi_2}{2})
\end{aligned}\]
\end{proof}
\subsubsection{駐波性質}
\begin{itemize}
...
\subsubsection{可逆性}
幾何光學的光具有可逆性。
\subsubsection{像（image）}
An image is a reproduction of an object via light. If image can form on a surface, it is a real image (實像) and can exist even if no observer is present. If the image requires the visual system of an observer, it is a virtual image (虛像).
...
\end{longtable}
\FB
\subsubsection{司乃耳定律（Snell's law）/折射定律（Law of refraction）}
...
\end{proof}
當$k_1<k_2$，對於光即$n_1<n_2$，週期入射波反射，其反射波與入射波相位角差$\pi$。
\ssc{側位移/旁位移}
...
s 偏振指振動方向垂直於入射平面，p 偏振指振動方向平行於入射平面。對於電磁波，振動方向指電場方向。
\sssc{Electromagnetic wave}
The plane containing $\mb{E}$ is called plane of oscillation and the wave is plane-polarized in the axis of $\mb{E}$. The electromagnetic waves emitted by any common source of light such as the Sun or a bulb are polarized randomly, also called unpolarized. We can split an electromagnetic wave into two perpendicular polarization directions.
\sssc{偏振與反射}
s-偏振反射係數$R_s$，s-偏振透射係數$T_s$，p-偏振反射係數$R_p$，s-偏振透射係數$T_p$：
...
\end{proof}
\subsubsection{偏振片/偏振器（Polarizer or polarizing sheet）}
...
\ssc{Extended object and ray tracing}
一橫線，稱主軸，一面鏡或透鏡垂直主軸。Extended object 一般畫為一垂直主軸、自主軸向上的箭頭，對於點光源常畫作與之等高。一點或平行光源，ray tracing 繪製自光源的 rays，射到 extended object 頂與底、鏡中等位置，以得到像。
\subsection{平面鏡（Plane mirror）}
\sssc{Image}
For a plane mirror, the image is a virtual image of the same size of the object at the opposite side of the mirror with same image distance (像距) as object distance (物距).
...
\sssc{平面鏡應用}
日常生活鏡子、潛望鏡、反射式望遠鏡副鏡。
\subsection{拋物面鏡（Parabolic mirror）與球面鏡（Spherical mirror）}
\subsubsection{凹（Concave）面鏡/匯聚（Converging）面鏡}
平行光反射匯聚於（實）焦點（(real) focal point or focus）。
\subsubsection{凸（Convex）面鏡/發散（Diverging）面鏡}
平行光反射延長線交於焦點，稱虛焦點。
\subsubsection{成像表}
\bct
\begin{longtable}[c]{|p{0.05\tw}|p{0.15\tw}|p{0.15\tw}|p{0.05\tw}|p{0.05\tw}|p{0.1\tw}|p{0.1\tw}|}
\hline
鏡別  & 物距        & 像的位置      & 像的虛實 & 像的正倒 & 像與物相比之大小 & 像與物相比之移動速率 \\\hline\endhead
...
平行入射光，光線反射線（對於凹面鏡）或延長線（對於凸面鏡）與主軸之交點距鏡心之距離，近軸光線較遠軸光線小，因此光線不能交於一個理想的焦點。拋物面鏡則可，但對於與軸夾較大角度者又與上理想公式不同。
\subsubsection{球面鏡近軸（paraxial）焦距}
球面鏡對於近軸光線之反射焦距為其曲率半徑二分之一。
\begin{proof}
...
\end{proof}
\subsubsection{面鏡公式（Mirror equation or mirror formula）}
Let focal length be $f$ with concave mirror being positive and convex mirror being negative, object (or light producing real image) distance be $u$ relative to the mirror with before being positive and after being negative, image distance be $v$ with real image being positive and virtual image being negative, object height be $h_o$ with upward being positive and downward being negative, image height be $h_i$ with upward being positive and downward being negative. The mirror formula gives
\[\frac{1}{u}+\frac{1}{v}=\frac{1}{f}.\]
Lateral (orthogonal to main axis) magnification $m=\frac{h_i}{h_o}$ and longitudinal (parallel to main axis) magnification $m_p$ are
\[m=\frac{h_i}{h_o}=-\frac{v}{u}\]
\[m_p=\dv{v}{u}=-m^2\]
\begin{proof}
Draw a right triangle with object height and object distance as two sides and a right triangle with image height and image distance as two sides. The two are similar. Thus,
\[\frac{h_i}{h_o}=-\frac{v}{u}.\]
Let a ray be emitted from two times focal length in front of mirror. Draw a right triangle with object top, object bottom, and two times focal length in front of mirror as vertices and a right triangle with image top, image bottom, and two times focal length in front of mirror as vertices. The two are similar. Thus,
\[\frac{h_i}{h_o}=-\frac{v-2f}{2f-u}\]
\[2fu-uv=uv-2fv\]
\[fu+fv=uv\]
\[\frac{1}{u}+\frac{1}{v}=\frac{1}{f}\]
\[v=\qty(\frac{1}{f}-\frac{1}{u})^{-1}\]
\[\dv{v}{u}=-\qty(\frac{1}{f}-\frac{1}{u})^{-2}u^{-2}=-\frac{v^2}{u^2}\]
\end{proof}
The least distance between real object and real image is $4f$, at which $u=v=2f$.
\begin{proof}
By Cauchy–Schwarz inequality
\[\qty(\qty(\frac{1}{\sqrt{u}})^2+\qty(\frac{1}{\sqrt{v}})^2)\qty(\qty(\sqrt{u})^2+\qty(\sqrt{v})^2)\geq (1+1)^2\]
\[\qty(\frac{1}{u}+\frac{1}{v})(u+v)=\frac{u+v}{f}\geq 4\]
\end{proof}
共軛成像：指實物與實像可互換位置，發生於$u+v\geq 4f$。
\sssc{凸面鏡應用}
路口安全鏡、車輛後視鏡、商店防盜鏡。
\sssc{凹面鏡應用}
反射式望遠鏡主鏡、化妝鏡、聚光鏡。
...
\end{proof}
\subsection{透鏡（Lens）}
\subsubsection{凹透鏡/發散透鏡}
可散光，平行入射光延長線匯聚於虛焦點，可分為雙凹、平凹、凸凹。
\subsubsection{凸透鏡/匯聚透鏡}
可聚光，平行入射光匯聚於實焦點，可分為雙凸、平凸、凹凸。
\subsubsection{球面}
同面鏡者。
...
\end{longtable}
\ect
\sssc{Refraction at spherical surface}
Suppose a region with spherical surface of curvature radius $R$ whose center of curvature is on the main axis has refractive index $n_2$. Suppose the medium in front of it has refractive index $n_1$. An object (or light producing real image) has height $h_o\gtrapprox0$ (paraxial) relative to the main axis and distance $u$ relative to the spherical surface with before being positive and after being negative. Let the image of it in the region has image height $h_i$ with upward being positive and downward being negative and image distance $v$ relative to the spherical surface with real image being positive and virtual image being negative.
\[\frac{n_1}{u}+\frac{n_2}{v}=\frac{n_2-n_1}{R}\]
\[\frac{h_i}{h_o}=-\frac{v}{u}\]
\bpr
Consider the object bottom. A ray from it travels with angle $\alpha$ relative to the main axis and reaches a point of height $h$ on the spherical surface, and then reaches the main axis with angle $\beta$ after a refraction on the surface with angle of incidence $\theta_1$ and angle of refraction $\theta_2$.
\[\theta_1=\alpha+\frac{h}{R}\]
\[\theta_2=\frac{h}{R}-\beta\]
\[n_1\theta_1=n_2\theta_2\]
\[\alpha=\frac{h}{u}\]
\[\beta=\frac{h}{v}\]
\[n_1\qty(\frac{1}{u}+\frac{1}{R})=n_2\qty(\frac{1}{R}-\frac{1}{v})\]
\[\frac{n_1}{u}+\frac{n_2}{v}=\frac{n_2-n_1}{R}\]
\epr
\subsubsection{Lens maker's formula (造鏡者公式)}
Suppose a lens consists of two spherical surfaces whose centers of curvature are on the main axis and whose intersection is a cricle on a plane $E$ perpendicular to the main axis and whose distances to $E$ are negligible (thin lens). Let the front surface has curvature radius of $R_1$, and the back surface has curvature radius $R_2$, both of sign convention that a radius is positive if the center of curvature is behind the lens and negative if the center of curvature is in front of the lens. Suppose the medium in front of the lens has refractive index $n_1$, the lens has refractive index $n_l$, and the meidum behind the lens has refractive index $n_2$. An object has height $h_o\gtrapprox0$ (paraxial) relative to the main axis and distance $u$ relative to $E$. Let the image of it in the region has image height $h_i$ with upward being positive and downward being negative and image distance $v$ relative to $E$ with real image being positive and virtual image being negative.
\[\frac{n_1}{u}+\frac{n_2}{v}=\frac{n_l-n_1}{R_1}+\frac{n_2-n_l}{R_2}\]
\bpr
Consider image of first refraction with height $h_l$ and distance $w$.
\[\frac{n_1}{u}+\frac{n_l}{w}=\frac{n_l-n_1}{R_1}\]
Consider final image.
\[\frac{n_l}{-w}+\frac{n_2}{v}=\frac{n_2-n_l}{R_2}\]
Combine:
\[\frac{n_1}{u}+\frac{n_2}{v}=\frac{n_l-n_1}{R_1}+\frac{n_2-n_l}{R_2}\]
\epr
Lateral (orthogonal to main axis) magnification $m=\frac{h_i}{h_o}$ and longitudinal (parallel to main axis) magnification $m_p$ are
\[m=\frac{h_i}{h_o}=-\frac{v}{u}\]
\[m_p=\dv{v}{u}=-\frac{n_1}{n_2}m^2\]
\bpr
\[\frac{n_1}{u}+\frac{n_2}{v}=\qty(\frac{n_l-n_1}{R_1}+\frac{n_2-n_l}{R_2})\]
\[-n_1u^{-2}\dd{u}-n_2v^{-2}\dd{v}=0\]
\[\dv{v}{u}=-\frac{n_1}{n_2}m^2\]
\epr
If $n_1=n_2=n$, let focal length be $f$:
\[\frac{1}{u}+\frac{1}{v}=\frac{1}{f}=\qty(\frac{n_l}{n}-1)\qty(\frac{1}{R_1}-\frac{1}{R_2})\]
\[m_p=-m^2\]
The least distance between real object and real image is $4f$, at which $u=v=2f$.
...
If $n_1=n_2=1$:
\[\frac{1}{u}+\frac{1}{v}=\frac{1}{f}=\qty(n_l-1)\qty(\frac{1}{R_1}-\frac{1}{R_2})\]
\subsubsection{菲涅耳透鏡（Fresnel lens）}
由多層同心的環形光學面組成，其曲率和角度根據透鏡的總焦距來設計，旨在減少光學系統中的厚度和重量，而達到相同的透鏡效果。
\sssc{Angular magnification}
The angular magnification $m_{\theta}$ is the angular size $\theta_1$ of an object viewed through an optical instrument so that the final image is at infinity (relaxed eye) divided by the angular size $\theta_0$ of an object seen by the unaided eye at near point $D$, conventionally taken as $D=25$ cm. Let the object have height $h$. The angular size of an object seen by the unaided eye at near point $D$ is approximately $\theta_0\approx\frac{h}{D}$ under small angle approximation.
\subsubsection{（簡單）放大鏡（(simple) magnifying glass or magnifier）}
凸透鏡，物體置於鏡前焦點內，鏡前得正立放大虛像。考慮物置於焦點$f$時，像位於無限遠處，$\theta_1=\frac{h}{f}$, $m_{\theta}=\frac{D}{f}$ under small angle approximation.
\sssc{Compound microscope (複式顯微鏡)}
A compound microscope is an optical instrument that uses two converging lenses to magnify very small nearby objects. They are
\bit
\item Objective (lens): Close to the object, very short focal length $f_o$, tube length (lens to object length) $s$, lateral magnification $m=-\frac{s}{f_o}$, produces a real inverted enlarged intermediate image.
\item Eyepiece (ocular) (lens): Close to the eye, a simple magnifier with focal length $f_e$, produces a virtual enlarged image of the immediate image, with angular magnification $m_{\theta}=\frac{D}{f_e}$.
\eit
The overall magnification $M$ is
\[M=mm_{\theta}=-\frac{s}{f_o}\frac{D}{f_e}.\]
\sssc{Newtonian (reflecting) telescope/reflector (牛頓（反射）式望遠鏡)}
A Newtonian reflecting telescope is an optical instrument that uses two mirrors and one lens to magnify distant objects and produce a virtual inverted enlarged image. They are
\bit
\item Primary/objective mirror: A concave parabolic mirror at the bottom of the tube, large focal length $f_o$, collects incoming nearly parallel light and forms a real inverted enlarged image near focal point. The image has height $h=-f_o\theta_0$.
\item Secondary mirror: Flat mirror that reflects light to the side of the tube.
\item Eyepiece (ocular) (lens): Close to the eye, a simple magnifier with focal length $f_e$, produces a virtual enlarged image of the image of the secondary mirror, with angular size $\theta=\frac{h}{f_e}$.
\eit
The (overall) angular magnification $m_{\theta}$ is
\[m_{\theta}=\frac{\theta}{\theta_0}=-\frac{h}{f_e}\frac{f_o}{h}=-\frac{f_o}{f_e}\]
\sssc{Keplerian (/) refracting telescope/refractor (克卜勒（/）折射式望遠鏡)}
A Keplerian refracting telescope is an optical instrument that uses two lenses to magnify distant objects and produce a virtual inverted enlarged image. They are
\bit
\item Objective (lens): A convex (converging) lens with focal length $f_o$, collects incoming nearly parallel light and forms a real inverted enlarged intermediate image near focal point. The image has height $h=-f_o\theta_0$.
\item Eyepiece (ocular) (lens): Close to the eye, a simple magnifier with focal length $f_e$, produces a virtual enlarged image of the intermediate image, with angular size $\theta=\frac{h}{f_e}$.
\eit
The (overall) angular magnification $m_{\theta}$ is
\[m_{\theta}=\frac{\theta}{\theta_0}=-\frac{h}{f_e}\frac{f_o}{h}=-\frac{f_o}{f_e}\]
\sssc{Galilean (refracting) telescope/refractor (伽俐略（折射）式望遠鏡)}
A Keplerian refracting telescope is an optical instrument that uses two lenses to magnify distant objects and produce a virtual upright enlarged image. They are
\bit
\item Objective (lens): A convex (converging) lens with focal length $f_o$, collects incoming nearly parallel light and forms a real inverted enlarged intermediate image near focal point. The image has height $h=f_o\theta_0$.
\item Eyepiece (ocular) (lens): Close to the eye, a concave (diverging) lens with focal length $f_e<0$ placed before the focal plane of the objective, produces a virtual inverted shrinked image of the intermediate image, with angular size $\theta=-\frac{h}{f_e}$.
\eit
The (overall) angular magnification $m_{\theta}$ is
\[m_{\theta}=\frac{\theta}{\theta_0}=\frac{h}{f_e}\frac{f_o}{h}=-\frac{f_o}{f_e}\]
...
\section{惠更斯原理（Huygens principle）、惠更斯－菲涅耳/菲涅耳－惠更斯原理（Huygens–Fresnel/Fresnel－Huygens principle）與干涉（Inference）}
\ssc{惠更斯原理（Huygens principle）}
惠更斯指出當波行進時，波前上的每一點都可以視為新的點波源，以其為球心，各自發出與原波同樣維度的球面子波（Wavelet），稱惠更斯原理。在某一時刻，和這些子波相切的流形，稱為包絡（線/面）（Envelope），形成新的波前。

惠更斯原理可以解釋反射、折射、干涉等現象。
\ssc{惠更斯－菲涅耳/菲涅耳－惠更斯原理（Huygens–Fresnel/Fresnel－Huygens principle）}
...
\sssc{惠更斯原理解釋司乃耳定律}
考慮一頻率$f$入射光以入射角$\frac{\pi}{2}-\theta_1$從折射率$n_1$介質進入折射率$n_2$介質，折射角$\frac{\pi}{2}-\theta_2$。考慮一波前，一點在界面上，一點距界面在光方向上距離$\lambda_1=\frac{c}{n_1f}$。考慮下一波前，時間相同可見
\[\lambda_2\frac{n_2}{c}=\lambda_1\frac{n_1}{c}\]
兩共斜邊直角三角形可見
\[\frac{\lambda_1}{\sin\theta_1}=\frac{\lambda_2}{\sin\theta_2}\]
合併得
\[n_1\sin\theta_1=n_2\sin\theta_2.\]
\sssc{Phase difference}
考慮折射率$n_1$、$n_2$介質均為長方體，一頻率$f$光自外平行兩者界面射入它們，原始光為同相，通過它們長$L$後，前者波長$\lambda_1=\frac{c}{n_1f}$相位差$\frac{L}{\lambda_1}=\frac{Ln_1f}{c}$，後者波長$\lambda_2=\frac{c}{n_2f}$相位差$\frac{L}{\lambda_2}=\frac{Ln_2f}{c}$，故出射光兩者有相位差
\[\frac{Lf}{c}(n_1-n_2)\]
這可以解釋彩虹內側等出現的supernumeraries，即主虹外側因path difference造成的建設性干涉處。
\ssc{同頻同調波源}
做光的繞射與干涉實驗時，通常使用同頻同調波源，同頻常使用濾光器（Optical filter）達成，即可以過濾入射光僅使特定波長區間通過的光學元件，同調則常使用狹縫與透鏡達成。
\ssc{Two beams with phase angle difference inference}
The resultant beam of two coherent same; frequency beams with electric field amplitudes $A,B$, intensities $I_A=\frac{\varepsilon c}{2}A^2$ and $I_B=\frac{\varepsilon c}{2}B^2$, and phase angle difference $\delta$ has amplitude phasor $\calC$:
\[\calC=A+Be^{i\delta},\]
amplitude $C$
\[C=\abs{\calC}=\sqrt{A^2+B^2+2AB\cos(\delta)},\]
and intensity
\[I(\delta)=\frac{\varepsilon c}{2}C^2=\frac{\varepsilon c}{2}\qty(A^2+B^2+2AB\cos(\delta))=I_A+I_B+2\sqrt{I_AI_B}\cos(\delta).\]
Thus
\[\max_{\delta}I(\delta)=I_A+I_B+2\sqrt{I_AI_B}\]
\[\arg_{\delta}\max_{\delta}I(\delta)=2k\pi,\quad k\in\bbZ\]
\[\min_{\delta}I(\delta)=I_A+I_B-2\sqrt{I_AI_B}\]
\[\arg_{\delta}\min_{\delta}I(\delta)=(2k+1)\pi,\quad k\in\bbZ\]
If $I_A=I_B=I_i$,
\[I(\delta)=2I_i\qty(1+\cos(\delta))=4I_i\cos^2(\frac{\delta}{2})\]
\[\max_{\delta}I(\delta)=4I_i\]
\[\min_{\delta}I(\delta)=0\]
Let $I_0=4I_i$,
\[I(\delta)=\frac{I_0}{2}\qty(1+\cos(\delta))=I_0\cos^2(\frac{\delta}{2})\]
\subsection{光柵（Grating）}
使反射光（稱反射光柵）或/與透射光（稱透射光柵）的振幅（稱振幅光柵）或/與相位（稱相位光柵）相對於入射光受到週期性空間調製的光學元件。
\subsection{忽略繞射之雙狹縫干涉}
同調光通過狹縫射向屏幕。令狹縫間距$b\gg\lambda$，狹縫寬忽略，屏幕距狹縫片$L$。屏幕平行兩狹縫各自中線之公垂線，對於屏幕上某點，令其在兩狹縫中線上之垂線與屏幕平面之法線過兩狹縫中線者之夾角$\theta$，取正，$y=L\tan\theta$。
...
\end{itemize}
\subsection{遠場忽略繞射之雙狹縫干涉（Double slits interference）/楊氏（雙狹縫）干涉實驗（Young's (double-slit) inference experiment）}
同調光通過狹縫射向屏幕。令狹縫間距$b\gg\lambda$，狹縫寬忽略，屏幕距狹縫片$L$，$L\gg b$。屏幕平行兩狹縫各自中線之公垂線，對於屏幕上某點，令其在兩狹縫中線上之垂線與屏幕平面之法線過兩狹縫中線者之夾角$\theta$，取正，屏幕上某點距狹縫片中線在屏幕上正射影距離$y=L\tan\theta$。
\subsubsection{亮暗紋}
光程差$D\approx b\sin\theta$（若非$b\gg \lambda$不可為此近似），令$m=\frac{b\sin\theta}{\lambda}$。亮紋（bright band, bright fringe, or maximum）指同相處，即光強度極大值處；暗紋（dark band, dark fringe, or minimum）指反相處，即光強度極小值處；亮帶（bright band or bright fringe）中線指兩相鄰暗紋之中線；暗帶（dark band or dark fringe）；此種圖樣稱干涉圖樣（inference pattern）。
\begin{itemize}
\item 亮紋：
\[(m+1)\in\mathbb{N}\]
，稱第$m$亮紋（$m$th-order maximum），$m=0$為中央亮紋（Central maximum）。$m$稱order number.
\item 暗紋：
...
第$m$亮紋發生於：
\[y=m\frac{\lambda L}{b},\quad m\in\mathbb{N}_0\]
第$m$暗紋發生於：
\[y=\qty(m-\frac{1}{2})\frac{\lambda L}{b},\quad m\in\bbN\]
\subsubsection{相位角差}
相位角差$\varphi$：
\[\varphi=\frac{2\pi b\sin\theta}{\lambda}\]
\subsubsection{光強度}
令$I$為光強度：
\[\begin{aligned}
I(\theta) &= I_0\qty(\frac{\sin\varphi}{\sin\frac{\varphi}{2}})^2\\
&= I_0\qty(\frac{\sin\qty(\frac{2\pi b\sin\theta}{\lambda})}{\sin\qty(\frac{\pi b\sin\theta}{\lambda})})^2\\
&=4I_0\cos^2\frac{\varphi}{2}\\
&=4I_0\cos^2\qty(\frac{\pi b\sin\theta}{\lambda})
\end{aligned}\]
對於小角度令$\tan\theta=\sin\theta$：
\[I(y)=4I_0\cos^2\qty(\frac{\pi by}{L\lambda})\]
\subsection{遠場忽略繞射之多狹縫/光柵（grating）干涉}
\sssc{Setup}
同調光通過狹縫射向屏幕。令有$k$（called the number of rulings）個狹縫，其中$k$為偶數，狹縫寬忽略，相鄰狹縫間距$b\gg\lambda$，屏幕距狹縫片$L$，$L\gg b$。屏幕平行任二狹縫各自中線之公垂線，對於屏幕上某點，令其在中央狹縫中線上之垂線與屏幕平面之法線過中央狹縫中線者之夾角$\theta$，取正，屏幕上某點距狹縫片中線在屏幕上正射影距離$y=L\tan\theta$。令$\beta=\frac{\pi b\sin\theta}{\lambda}$。
\sssc{場強分布}
\( I_0 \) 是入射光的場強，場強分布為：
\[\ba
I(\theta)=&I_0\qty(\frac{\sin(k\beta)}{\sin\beta})^2\\
=& I_0\qty(\frac{\sin\qty(k\frac{\pi b\sin\theta}{\lambda})}{\sin\qty(\frac{\pi b\sin\theta}{\lambda})})^2
\ea\]
\bpr
\[\ba
D=&\sqrt{\qty(L\tan\theta+\frac{c}{2})^2+L^2}-\sqrt{\qty(L\tan\theta)^2+L^2}\\
=&\sqrt{L^2\sec^2\theta+cL\tan\theta+\frac{c^2}{4}}-L\sec\theta\\
\approx&\frac{cL\tan\theta}{2L\sec\theta}\\
\approx&\frac{c}{2}\sin\theta
\ea\]
\[c=(i-0.5)b\]
\[E_0=E\sin(\omega t+\psi)\]
Let electric field through the $i$th slit with top half being positive and bottom half be negative being $E_i$. For $i\in\bbN$,
\[E_i=E\sin(\omega t+\psi+\frac{2\pi(i-0.5)b\sin\theta}{\lambda}+\psi)=E\sin(\omega t+\psi+(2i-1)\beta+\psi)\]
\[E_{-i}=E\sin(\omega t+\psi-\frac{2\pi(i-0.5)b\sin\theta}{\lambda}+\psi)=E\sin(\omega t-\psi+(2i-1)\beta+\psi)\]
\[E_i+E_{-i}=2E\sin(\omega t+\psi)\cos((2i-1)\beta)\]
\[\ba
\sum_{i=1}^{\frac{k}{2}}E_i+E_{-i}=&2E\sin(\omega t+\psi)\sum_{i=1}^{\frac{k}{2}}\cos((2i-1)\beta)\\
=&2E\sin(\omega t+\psi)\frac{\sin(k\beta)}{2\sin\beta}\\
=&E\sin(\omega t+\psi)\frac{\sin(k\beta)}{\sin\beta}
\ea\]
\[\frac{I}{I_0}=\qty(\frac{\sum_{i=1}^{\frac{k}{2}}E_i+E_{-i}}{E\sin(\omega t+\psi)})^2=\qty(\frac{\sin(k\beta)}{\sin\beta})^2\]
\epr
\subsubsection{折射率效應}
...
\subsubsection{夾角效應}
屏幕面與狹縫面夾角$\psi$時，狹縫有效間距為$b\cos\psi$。
\sssc{Angular separation}
Angular separation $\Delta\theta$:
\[b\sin\theta=m\lambda\]
\[b\cos\theta\dd{\theta}=m\dd{\lambda}\]
\[\dv{\theta}{\lambda}=\frac{m}{b\cos\theta}\]
\[\Delta\theta\approx\frac{m}{b\cos\theta}\Delta\lambda\]
\sssc{Dispersion}
The dispersion $D$ of a diffraction grating is defined as
\[D=\dv{\theta}{\lambda}\]
where $\theta$ is the angular separation of lines produced.

The dispersion of a diffraction grating at angle $\theta$ is
\[D=\frac{m}{b\cos\theta}.\]
\sssc{Angular half-width}
Angular half-width of the bright fringe whose center is at $\theta$ is
\[\Delta\theta_{\tx{hw}}\approx\frac{\lambda}{mkb\cos\theta}.\]
\bpr
TODO 
\epr
\sssc{Instrinct peak width}
\[\approx \frac{\lambda}{kb\cos\theta}\]
\sssc{Resolving power}
Rayleigh's criterion: two waves with average wavelength $\bar{\lambda}$ and wavelength difference $\Delta\lambda$ are barely distinguishable when their angular separation $\approx\frac{m}{b\cos\theta}\Delta\lambda$ is the same as instrinct peak width $\approx \frac{\bar{\lambda}}{kb\cos\theta}$.

For two waves that are barely distinguishable, the resolving power $R$ of a diffraction grating is defined as
\[R=\frac{\bar{\lambda}}{\Delta\lambda}=km.\]
\sssc{透鏡}
可用焦距$f$匯聚透鏡在狹縫面與屏幕之間讓$y\approx f\theta$，使不依賴$L$。
\ssc{薄膜（Thin-film）干涉}
\sssc{平行表面薄膜}
折射率$n_1$介質一波長$\lambda$入射光入射薄膜上表面，薄膜折射率$n_2>n_1$、厚$L$，裡面光波長$\lambda_2=\frac{n_1}{n_2}\lambda$，折射角$\frac{\pi}{2}-\theta_t\approx\frac{\pi}{2}$，下表面以下為折射率$n_3<n_2$介質，波程差：
\[2L\cos\theta_t\approx 2L\]
此時上表面反射相位角差$\pi$，下表面反射則同相，故亮紋發生於
\[2L\cos\theta_t=\qty(m+\frac{1}{2})\frac{n_1}{n_2}\lambda,\quad m\in\bbN_0\]
暗紋發生於
\[2L\cos\theta_t=m\frac{n_1}{n_2}\lambda,\quad m\in\bbN_0\]
當$L\ll\frac{n_1}{n_2}\lambda$（常取$L<0.1\frac{n_1}{n_2}\lambda$），恰為$m=0$，整個薄膜均暗。
\sssc{Newton's ring (牛頓環)}
Place a thin slightly convex upward lens with curvature radius $R$ on a flat mirror. The two make contact only at the center. Consider a monochromatic light incident from the top that reflects both the bottom surface of the top lens and the top surface of the bottom lens. At a position with radial distance $r$ from the center, the air thickness $t(r)$ is
\[t(r)\approx R-\sqrt{R^2-r^2}\approx\frac{r^2}{2R}.\]
The path difference between the two waves is
\[2t(r).\]
Thus the total phase angle difference between the two waves is
\[\delta(r)=\frac{2\pi}{\lambda}2t(r)+\pi.\]
The intensity is
\[I(r)=\frac{I_0}{2}\qty(1+\cos(\delta(r)))=I_0\cos^2(\frac{\delta}{2})=I_0\sin^2(\frac{2\pi}{\lambda}t(r)).\]
The center $r=0$ is dark. The $m$th dark ring (center as $m=0$) is at
\[\frac{2\pi}{\lambda}t(r)=m\pi,\quad m\in\bbN_0\]
\[\frac{r^2}{\lambda R}\approx m\]
\[r_{m,\tx{dark}}\approx\sqrt{m\lambda R}\]
The $m$th bright ring is at
\[\frac{2\pi}{\lambda}t(r)=(m+0.5)\pi,\quad m\in\bbN_0\]
\[\frac{r^2}{\lambda R}\approx m+0.5\]
\[r_{m,\tx{bright}}\approx\sqrt{(m+0.5)\lambda R}\]
\ssc{Beam splitter (分光鏡)}
A beam splitter is an optical component that divides an incoming light beam into two beam, one transmitted and one reflected, and often 50\% each.
\ssc{Michelson('s) Interferometer （邁克生干涉儀）}
An interferometer is a device that can be used to measure lengths or changes in length with great accuracy by means of interference fringes. We describe the form originally devised and built by A. A. Michelson in 1881.

On a $xy$ plane, the interferometer consists of a point light source $P=(-p,0)$ with $p>0$ with wavelength $\lambda$ in air to a beam splitter $M=(0,0)$ facing towards $(-1,1)$ direction. At $M$, part of the light reflects along arm 2 to a movable flat mirror $M_2=(0,d_2)$, part of the light transmits along arm 1 to a fixed flat mirror $M_1=(d_1,0)$. $M_2$ reflects the light back to $M$, part of it reflects to $-x$ direction, part of it transmits to $-y$ direction. $M_1$ reflects the light back to $M$, part of it reflects to $-y$ direction, part of it transmits to $-x$ direction. The observer observes the two lights from a telescope in $-y$ direction at $Q=(0,-q)$ with $q>0$.

The observer will see a pattern of curved or approximately straight interference fringes.

Mirror shift: The path length difference for the two waves when they recombine at the telescope is $2(d_2-d_1)$, and anything that changes this path length difference will cause a change in the phase difference between the two waves at the eye.

Insertion: The insertion of a thin transparent material into the optical path of $M_1$. If the material has thickness $L$ and index of refraction $n$, the number of wavelength along the light's path in the material is
\[N_m=\frac{2Ln}{\lambda}.\]
The original number of wavelength along the light's path in the air before the insetion with index of refraction $n_1$ is
\[N_a=\frac{2L}{\lambda}.\]
The light returned from $M_1$ undergoes a phase change in terms of wavelengths of
\[N_m-N_a=\frac{2L}{\lambda}(n-1).\]
For each phase change of one wavelength, the fringe pattern is shifted by one fringe. Thus, by counting the number of fringes through which the marital cause the pattern to shift. We can determine $L$ or $n$ in terms of $\lambda$ and the other.
...
\end{itemize}
\ssc{公式}
\sssc{菲涅耳-基爾霍夫繞射公式（Fresnel–Kirchhoff diffraction formula）}
...
\sssc{菲涅耳繞射（Fresnel Diffraction）}
...
\sssc{夫朗和斐繞射（Fraunhofer Diffraction）}
...
\subsection{遠場（夫朗和斐）單狹縫繞射（Single slit diffraction）}
\sssc{Setup}
同調光通過狹縫射向屏幕。令縫寬$a\gg\lambda$，屏幕距狹縫片$L$，$L\gg a$。屏幕平行狹縫兩邊緣之公垂線，對於屏幕上某點，令其在狹縫中線上之垂線與屏幕平面之法線過狹縫中線者之夾角$\theta$，取正，屏幕上某點距狹縫片中線在屏幕上正射影距離$y=L\tan\theta$。令$\alpha=\frac{\pi a\sin\theta}{\lambda}$，$m=\frac{a\sin\theta}{\lambda}$。
\subsubsection{亮暗紋}
亮紋（bright band, bright fringe, or maximum）指同相處，即光強度極大值處，其中中央者稱中央亮紋（central maximum），其餘者稱次（secondary）/側（side）亮紋；暗紋（dark band, dark fringe, or minimum）指反相處，即光強度極小值處；亮帶（bright band or bright fringe）中線指兩相鄰暗紋之中線；暗帶（dark band or dark fringe）；此種圖樣稱繞射圖樣（diffraction pattern）。
\begin{itemize}
\item 中央亮紋：$\theta=0$。
\item 暗紋：$m\in\mathbb{N}$，稱第$m$暗紋，狹縫兩邊緣光程差$m$。將狹縫分為$2m$等分，自上起第$k$與$k+1$等分將完全相消干涉，由此可知。$m$稱order number.
\item 亮紋：$\alpha=\tan\alpha$時。
...
\bpr
\[\ba
D=&\sqrt{\qty(L\tan\theta+y)^2+L^2}-\sqrt{\qty(L\tan\theta)^2+L^2}\\
=&\sqrt{L^2\sec^2\theta+2yL\tan\theta+y^2}-L\sec\theta\\
\approx&\frac{yL\tan\theta}{L\sec\theta}\\
\approx&y\sin\theta
\ea\]
Let electric field at $\theta$ be $E(\theta)$ and $E$ be the total electric field of incident light. Consider the infinitesimal phasors of the electric field of the light from each point in th slit. They form an arc of arc length $E$, angle $2\pi\frac{a\sin\theta}{\lambda}=2\alpha$, radius $R=\frac{E}{2\alpha}$, and length beaten the two endpoints $E(\theta)$. Thus
\[E(\theta)=2R\sin\alpha=\frac{E}{\alpha}\sin\alpha\]
\[\frac{I}{I_0}=\qty(\frac{E(\theta)}{E})^2=\left( \frac{\sin\alpha}{\alpha} \right)^2\]
\epr
\subsubsection{觀察}
...
入射光為白光時，中央亮紋兩側可見黃色，因紫色$m$最小，而其互補色為黃色。
\subsubsection{夾角效應}
屏幕面與狹縫面夾角$\psi$時，狹縫有效間距為$b\cos\psi$，有效縫寬為$a\cos\psi$。
\sssc{透鏡}
可用焦距$f$匯聚透鏡在狹縫面與屏幕之間讓$y\approx f\theta$，使不依賴$L$。
\subsection{遠場（夫朗和斐）多狹縫/（繞射）光柵（(diffraction) grating）繞射與干涉}
\sssc{Setup}
波長$\lambda$同調波通過平面上$k$個相鄰狹縫射向屏幕，縫寬$a\gg\lambda$、兩相鄰狹縫中線相距$b\gg\lambda$，屏幕平面平行狹縫平面，兩者距離$L$使得$L\gg a$、$L\gg b$，中央亮紋場強$I_0$。對於屏幕上某點，令其與狹縫平面中線之最短連線與該點上狹縫平面法線之夾角$\theta$。令$\alpha=\frac{\pi a\sin\theta}{\lambda}$，$\beta=\frac{\pi b\sin\theta}{\lambda}$。
\sssc{場強分布}
\( I_0 \) 是中心極大處的場強，場強分布$I(\theta)$為：
\[\ba
I(\theta)=&I_0\qty(\frac{\sin\alpha}{\alpha})^2\qty(\frac{\sin(k\beta)}{\sin\beta})^2\\
=&I_0\qty(\frac{\sin(\frac{\pi a\sin\theta}{\lambda})}{\frac{\pi a\sin\theta}{\lambda}})^2\qty(\frac{\sin(k\frac{\pi b\sin\theta}{\lambda})}{\sin(\frac{\pi b\sin\theta}{\lambda})})^2
\ea\]
\sssc{亮暗紋等性質}
同忽略繞射之多狹縫干涉。
\ssc{阿拉戈（Arago）/帕松
（Poisson）/菲涅耳（Fresnel）光斑（(light) spot）}
一點光源照射一直徑$a$之圓球之中心，圓球後距離$b$處一屏幕，在屏幕上的影子正中呈現一光斑，菲涅耳繞射下（即進場近似下），$\frac{b^2}{b^2+a^2}$愈大，光斑愈小，強度愈強。
\ssc{Diffraction by a circular aperture and Rayleigh's criterion}
\sssc{Diffraction by a circular aperture (光圈)}
波長$\lambda$光自一直徑$d$光圈正中入射，將產生同心圓干涉圖樣，正中為較大亮斑，第一最小值發生於
\[\sin\theta\approx\frac{1.22\lambda}{d}.\]
\bpr
TODO 
\epr
\sssc{Rayleigh's criterion}
Two objects that are barely resolvable for a aperture (e.g. pupil) with diameter $d$ have angular separation
\[\theta_R=\arcsin(\frac{1.22\lambda}{d})\]
For small angle,
\[\theta_R\approx\frac{1.22\lambda}{d}\]


\section{Doppler effect (都卜勒效應)}
\subsection{Non-relativistic Doppler effect}
\sssc{Non-relativistic Doppler effect}
Let the unit vector in the direction from the source to the receiver be $\hat{\mb{n}}$, $v_m$ be the speed of the wave in the medium, $s$ be the part of the velocity of the source relative to its local medium frame in the direction of $\hat{\mb{n}}$, $r$ be the part of the velocity of the receiver relative to its local medium frame in the direction of $\hat{\mb{n}}$, $f_s$, $\lambda_s$, $v_s$ be the frequency, wavelength, and speed of the wave as observed by the source, and $f_r$, $\lambda_r$, $v_r$ be the frequency, wavelength, and speed of the wave as observed by the receiver. Non-relativistic Doppler effect states that
\[v_s=v_m-s\]
\[v_r=v_m-r\]
\[\lambda_s=\lambda_r=\frac{v_s}{f_s}=\frac{v_m-s}{f_s}\]
\[f_r=\frac{v_m-r}{\lambda_r}=\frac{v_m-r}{v_m-s}f_s\]
\bpr
The source move $\frac{s}{f_s}$ per period towards the receiver. Thus
\[\lambda_r=\frac{v_m-s}{f_s}.\]
The receiver moves through the wave pattern at speed $r$ away from the source. Thus
\[v_r=v_m-r\]
and
\[f_r=\frac{v_m-r}{\lambda_r}=\frac{v_m-r}{v_m-s}f_s.\]
\epr
\sssc{Remarks}
$\lambda_r$, called 視波長, is equal to $\lambda_s$ and is only dependent on the medium and the source. The quicker the source move towards the receiver, the shorter the wavelength.

$f_r$, called 視頻率, is dependent on both. The quicker the source move towards the receiver, the larger the frequency. The quicker the receiver move away from the source, the larger the frequency.
\ssc{Blue shift or blueshift (藍移) and red shift or redshift (紅移)}
The increasing of frequency due to Doppler effect is called blue shift or blueshift. The decreasing of frequency due to Doppler effect is called red shift or redshift.
...
\[\dv{K}{x}=\frac{1}{2}\mu\qty(\pdv{y}{t})^2\]
Potential energy density:
\[\dv{U}{x}=\frac{1}{2}v^2\mu\qty(\pdv{y}{x})^2\]
Energy density:
\[\dv{E}{x}=\dv{K}{x}+\dv{U}{x}\]
\begin{proof}
    \[\dd{K}=\frac{1}{2}\dd{m}\qty(\pdv{y}{t})^2\]
    \[\dv{K}{x}=\frac{1}{2}\mu\qty(\pdv{y}{t})^2\]
    \[\dd{U}=\frac{1}{2}F\dd{x}\qty(\pdv{y}{x})^2\]
    \[\dv{U}{x}=\frac{1}{2}v^2\mu\qty(\pdv{y}{x})^2\]
\end{proof}
\subsubsection{正弦繩波}
設一條張緊的繩子上有一個弦繩波，波的位移函數 \( y(x,t) = A \sin(kx - \omega t) \)，則：
\[ \dv{K}{x} = \dv{U}{x} = \frac{1}{2}\mu A^2 \omega^2 \cos^2(kx - \omega t) \]
\[ \dv{E}{x} = \mu A^2 \omega^2 \cos^2(kx - \omega t) \]
The average rate at which kinetic and potential energy are transported:
\[\left\langle\dv{K}{t}\right\rangle=\left\langle\dv{U}{t}\right\rangle=\frac{1}{4}\mu vA^2\omega^2\]
The average rate at which energy is transported:
\[\left\langle\dv{E}{t}\right\rangle=\frac{1}{2}\mu vA^2\omega^2\]
\begin{proof}
\[ \frac{\partial y}{\partial t} = -A \omega \cos(kx - \omega t) \]
\[ \frac{\partial y}{\partial x} = A k \cos(kx - \omega t) \]
\[ \omega = k v \]
\[\dv{K}{x}=\frac{1}{2}\mu\qty(\pdv{y}{t})^2=\frac{1}{2}\mu A^2\omega^2\cos^2(kx - \omega t)\]
\[\dv{U}{x}=\frac{1}{2}\mu A^2\omega^2\cos^2(kx - \omega t)\]
\[\left\langle\dv{K}{t}\right\rangle=\left\langle\dv{K}{x}\dv{x}{t}\right\rangle=\frac{1}{2}\mu vA^2\omega^2\frac{\omega}{2\pi}\int_0^{\frac{2\pi}{\omega}}\cos^2(kx-\omega t)\dd{t}=\frac{1}{2}\mu vA^2\omega^2\frac{\omega}{2\pi}\frac{\pi}{\omega}=\frac{1}{4}\mu vA^2\omega^2\]
\[\left\langle\dv{U}{t}\right\rangle=\left\langle\dv{U}{x}\dv{x}{t}\right\rangle=\frac{1}{4}\mu vA^2\omega^2\]
\end{proof}
\subsection{反射與透射}
繩波是一維波，必為垂直入射。
...
\begin{equation}
\frac{\partial \varphi}{\partial z} = 0 \quad \text{ at } z = -h.\quad\tx{\tb{(2)}}
\end{equation}
...
\begin{equation}
\frac{\partial \eta}{\partial t} = \frac{\partial \varphi}{\partial z} \quad \text{ at } z = \eta(x,t).\quad\tx{\tb{(3)}}
\end{equation}
...
\begin{equation}
\frac{\partial \varphi}{\partial t} + g \eta = 0 \quad \text{ at } z = \eta(x,t).\quad\tx{\tb{(4)}}
\end{equation}
Because this is a linear theory, in both free-surface boundary conditions – the kinematic and the dynamic one, equations (3) and (4) – the value of \( \varphi \) and \( \frac{\partial \varphi}{\partial z} \) at the fixed mean level \( z = 0 \) is used.
...
\[
\varphi = \frac{\omega}{k} a \frac{\cosh k (z+h) }{\sinh k h} \sin ( k x - \omega t),
\]
with \( \sinh \) and \( \cosh \) the hyperbolic sine and hyperbolic cosine function, respectively. But \( \eta \) and \( \varphi \) also have to satisfy the dynamic boundary condition, which results in non-trivial (non-zero) values for the wave amplitude \( a \) only if the linear dispersion relation is satisfied:
\[
\omega^2 = g k \tanh k h,
\]
...
\[\begin{aligned}
p_s &= A \cdot \sin\qty(\mathbf{k} \cdot \mathbf{x} \pm \omega t + \varphi) \\
y &= A \cdot \cos\qty(\mathbf{k} \cdot \mathbf{x} \pm \omega t + \varphi) 
\end{aligned}\]
\subsubsection{一維波函數}
平均位移對聲壓之相位角移為$\frac{\pi}{2}$，即平均位移較聲壓（時間上）領先$\frac{1}{4}$週期，即聲壓較平均位移（空間上）快$\frac{\lambda}{4}$，即：
\[\begin{aligned}
p_s &=A\cdot\sin(kx\pm\omega t+\varphi)\\
y &=A\cdot\cos(kx\pm\omega t+\varphi)
\end{aligned}\]
