...
\ea\]
\sssc{亮暗紋}
條紋間距、angular separation等同忽略繞射之多狹縫干涉。

Angular half-width of the bright fringe whose center is at $\theta$ is
\[\Delta\theta_{\tx{hw}}=\frac{\lambda}{mkb\cos\theta}.\]
\sssc{Dispersion}
...
Rayleigh's criterion: two waves with average wavelength $\bar{\lambda}$ and wavelength difference $\Delta\lambda$ are barely distinguishable when their angular separation $\frac{m}{b\cos\theta}\Delta\lambda$ is the same as instrinct peak width $\frac{\bar{\lambda}}{kb\cos\theta}$.
...
\ssc{Diffraction by a circular aperture (光圈)}
...
For small angle,
\[\theta_R\approc\frac{1.22\lambda}{d}\]
