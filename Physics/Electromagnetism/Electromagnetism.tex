...
\ssc{勞侖茲力功率密度}
一點有體積電荷密度 $\rho$、自由電流密度 $\mb{J}_f$、速度 $\mb{v}$，受電場 $\mb{E}$ 與磁場 $\mb{B}$ 之勞侖茲力密度 $\mb{f}$ 與功率密度（獲得為正）$\dv{P}{V}$：
\[\dv{P}{V}=\mb{f}\cdot\mb{v}=\rho\mb{E}\cdot\mb{v}+\mb{J}_f\times\mb{B}\cdot\mb{v}=\mathbf{J}_f\cdot\mathbf{E}\]
\ssc{電磁場的能量}
電磁場的能量密度$u$為：
\[u=\frac{1}{2}(\mathbf{E}\cdot\mathbf{D}+\mathbf{B}\cdot\mathbf{H})=\frac{\varepsilon}{2}|\mathbf{E}|^2+\frac{1}{2\mu}|\mathbf{B}|^2.\]
This is justified by Poynting's theorem as power density into matter plus time derivative of energy density plus net outward EM energy flux (divergence of Poynting vector) is 0.
...
PLACEHOLDER: non linear also work, generalized force, dW=H\cdot dB
