...
\sssc{Displacement current}
Displacement current is defined as
\[I_d=\varepsilon\pdv{\Phi_E}{t}\]
and is not an actual current, a transfer of charge.
...
For a capacitor consists of two plates, the current $I$ at the terminals is the same in direction and magnitude as the displacement current $I_d$ between them.
\bpr
\[\Phi_E=\frac{q}{\varepsilon}\]
\[I_d=\varepsilon\dv{\Phi_E}{t}=\dv{q}{t}=I\]
\epr
\subsubsection{Parallel plate capacitor}
Two parallel circular metal plate of radius $R$ with area $A=\pi R^2$ and charges $q$ and $-q$, in distance $d$ seperated by dielectric medium with permittivity $\varepsilon$. Let the unit vector from the former to the later be $\hat{k}$. Let $r$ be the radial distance relative to their center axis. Edge field effect ignored.

Electric field between them:
\[\mb{E}=\frac{q\hat{k}}{A\varepsilon}\]
Potential difference (later relative to inner):
\[V=\frac{qd}{A\varepsilon}\]
Capacitance:
...
Two coaxial cylindrical metal shell with radius $a<b$, charges $q$ and $-q$, and length $L$, seperated by dielectric medium with permittivity $\varepsilon$. Let $\mb{r}$ be the vector relative to a point on the axis such that $\mb{r}$ is perpendicular to the axis. Edge field effect ignored.

Electric field between them:
\[\mb{E}=\frac{q\hat{\mb{r}}}{2\pi\varepsilon L\abs{\mb{r}}}\]
Potential difference (outer relative to inner):
\[V=\frac{q}{2\pi\varepsilon L}\ln(\frac{b}{a})\]
Capacitance:
\[C=\frac{2\pi\varepsilon L}{\ln(\frac{b}{a})}\]
\subsubsection{Spherical capacitor}
Two concentric spherical metal shell with radius $a<b$ and charges $q$ and $-q$, seperated by dielectric medium with permittivity $\varepsilon$. Let their center be the origin.

Electric field between them:
\[\mb{E}=\frac{q\hat{\mb{r}}}{4\pi\varepsilon\abs{\mb{r}}^2}\]
Potential difference (outer relative to inner):
\[V=\frac{q}{4\pi\varepsilon}\qty(\frac{1}{a}-\frac{1}{b})\]
Capacitance:
\[C=\frac{4\pi\varepsilon}{\frac{1}{a}-\frac{1}{b}}\]
\sssc{Single sphere capacitor}
A spherical metal shell with radius $R$ and charge $q$ surrounded by an infinite medium with with permittivity $\varepsilon$. Let its center be the origin.
