...
Two parallel metal plate with area $A$ and charges $q$ and $-q$, in distance $d$ seperated by dielectric medium with permittivity $\varepsilon$. Let the unit vector from the former to the later be $\hat{k}$. Edge field effect ignored.
...
\subsection{（永久（Permanent））磁鐵（Magnet）}
A material that produces a persistent magnization $\mb{M}$ on its own due to microscopic magnetic moments.
