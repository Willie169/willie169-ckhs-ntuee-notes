...
考慮一載流固體導體在外加磁場 $\mb{B}$ 中以速度 $\mb{v}$ 移動，其中載流子帶電荷 $q$，漂移速度 $\mb{v}_d$，勞侖茲力平衡條件為當其內靜電場 $\mb{E}$ 使得：
\[\mb{E}=(\mb{v}+\mb{v}_d)\times\mb{B}\]
此時路徑 $l$ 兩端之霍爾電壓$V_H$為：
\[V_H=\int_l\mathbf{E}\cdot\dd{\mb{l}=\int_{l}\qty((\mb{v}+\mb{v}_d)\times\mb{B})\cdot\dd{\mb{l}.\]
對於電流與磁場垂直且 $\mb{v}$ 與 $\mb{v}_d$ 平行的長 $d$ 長方體導體，有：
\[V_H=(\abs{\mb{v}+\abs{\mb{v}_d)\abs{\mb{B}}=\abs{\mb{E}}d.\]
考慮電流與磁場方向垂直、電流方向截面積 $A$、磁場方向寬 $w$ 的靜止長方體導體載電流 $I$，其電流密度量值為：
\[j=\frac{I}{A}.\]
