...
即：
\[\nabla\cdot\mathbf{E}=\frac{\rho}{\varepsilon_0}=\frac{\rho_f}{\varepsilon}\]
...
即：
\[\oiint_{S}\mathbf {E} \cdot \mathrm{d}\mathbf{S}=\frac{1}{\varepsilon_0}\iiint_V\rho\,\mathrm{d}V=\frac{1}{\varepsilon}\iiint_V\rho_f\,\mathrm{d}V=\frac{q}{\varepsilon_0}=\frac{q_f}{\varepsilon}.\]
\subsection{靜電感應與起電}
...
$\mu_0\varepsilon_0\frac{\partial\mathbf{E}}{\partial t}$由馬克士威貢獻。
...
\sssc{勞侖茲力解釋}
載流子在磁場 $\mathbf{B}$ 中以速度 $\mathbf{v}$ 移動時，所受磁場力等效於電場 $\mb{E}$ 產生的電場力：
\[\mb{E}=\mathbf{v}\times\mathbf{B}\]
此時路徑 $l$ 兩端因此產生之電動勢為：
\[\mathcal{E}=\int_l\mathbf{v}\times\mathbf{B}\cdot\mathrm{d}\mathbf{l}\]
\sssc{感應電動勢解釋}
考慮外加磁場 $\mb{B}$ 中一定向簡單連接緊緻曲面 $S$，其邊界的一部分為一射線 $\mb{l}$ 且始終位於導體中，並以速度 $\mb{v}$ 移動，有：
...
故有感應電動勢
\[\mathcal{E}=-\dv{\Phi_B}{t}=\int_l\left(\mathbf{v}\times\mathbf{B}\right)\cdot\dd{\mathbf{l}}.\]
等效感應電場為：
\[\mathbf{E}=\mathbf{v}\times\mathbf{B}\]
\sssc{相對性}
兩種解釋相當於取載流子相對磁場運動與磁場相對載流子的兩個慣性參考系，可見動生電動勢與慣性參考系的選取無關。
...
\sssc{霍爾效應}
外加磁場中的固體導體有電流通過時，載流子受動生電動勢偏向一邊，繼而產生電位差，最終當其內電荷載子產生的靜電場與外加磁場的勞侖茲力平衡時達到穩態，使後來的電流可順利通過而非聚集，此時的電位差稱霍爾電壓（Hall voltage）。

考慮一載流固體導體在外加磁場 $\mb{B}$ 中，漂移速度 $\mb{v}_d$，勞侖茲力平衡條件為當其內靜電場 $\mb{E}$ 使得：
\[\mb{E}=\mb{v}_d\times\mb{B}\]
此時路徑 $l$ 兩端之霍爾電壓$V_H$為：
\[V_H=\int_l\mathbf{E}\cdot\dd{\mb{l}}=\int_{l}\qty(\mb{v}_d\times\mb{B})\cdot\dd{\mb{l}}.\]
對於電流與磁場垂直的長 $d$ 長方體導體，有：
\[V_H=\abs{\mb{v}}_d\abs{\mb{B}}=\abs{\mb{E}}d.\]
考慮電流與磁場方向垂直、電流方向截面積 $A$、磁場方向寬度 $w$ 的長方體導體載電流 $I$，其電流密度量值為：
\[j=\frac{I}{A}.\]
霍爾係數（Hall coefficient）$R_H$ 定義為：
\[R_H=\pm\frac{\abs{\mb{E}}}{j\abs{\mb{B}}}=\pm\frac{1}{jw}\dv{V_H}{\abs{\mb{B}}},\]
其中 $\pm$ 使得當載流子帶正電荷 $R_H$ 為正；當載流子帶負電荷 $R_H$ 為負。

載流子帶電荷 $q$、濃度 $n$，有：
\[n=\frac{1}{|q|R_H}.\]
為避免測量電位差的兩點在電流方向有差距，從而測得包含電流之電位差，可以相同接點正向與反向測量，再計算其差除以二作為霍爾電壓。
\subsubsection{金屬}
...
\[0\le k_c\le 1\]
\sssc{Winding resistance of inductor}
An ideal inductor has zero resistance. However, the wire used to wind the inductor's coil has resistance, which makes an inductor with positive resistance, called winding resistance.
...
\section{電磁場的能量}
\ssc{勞侖茲力功率密度}
一體積元 $\dd{V}$ 有體積電荷密度 $\rho$、自由電流密度 $\mb{J}_f$、速度 $\mb{v}$，受電場 $\mb{E}$ 與磁場 $\mb{B}$ 之勞侖茲力密度 $\mb{f}$ 有功率（獲得為正）$\delta P$：
\[\delta P=\mb{f}\cdot\mb{v}\dd{V}=\qty(\rho\mb{E}\cdot\mb{v}+(\mb{J}_f\times\mb{B})\cdot\mb{v})\dd{V}=\mathbf{J}_f\cdot\mathbf{E}\dd{V}\]
\ssc{坡印廷定理（Poynting's theorem）–微分形式}
\[\mathbf{J}_f\cdot\mathbf{E}+\mathbf{E}\cdot\pdv{D}{t}+\mathbf{H}\cdot\pdv{\mathbf{B}}{t}+\nabla\cdot(\mathbf{E}\times\mathbf{H})=0.\]
其中 $\mathbf{E}\times\mathbf{H}$ 稱坡印廷向量，又作 $\mb{S}$。
\begin{proof}
\[\begin{aligned}
\mathbf{E}\cdot\pdv{D}{t}+\mathbf{H}\cdot\pdv{\mathbf{B}}{t}&=\mathbf{E}\cdot(\nabla\times\mathbf{H})-\mathbf{E}\cdot\mathbf{J}_f-\mathbf{H}\cdot\nabla \times \mathbf{E}
&=-\nabla\cdot(\mathbf{E}\times\mathbf{H})-\mathbf{J}_f\cdot\mathbf{E}
\end{aligned}\]
\end{proof}
\ssc{坡印廷定理–積分形式}
$S=\partial V$，
\[\iiint_V\mathbf{J}_f\cdot\mathbf{E}+\mathbf{E}\cdot\pdv{D}{t}+\mathbf{H}\cdot\pdv{\mathbf{B}}{t}\dd{V}+\oiint_S(\mathbf{E}\times\mathbf{H})\cdot\dd{\mathbf{S}}=0\]
\ssc{電磁場的能量}
對於線性磁化、線性極化、各向同性、非色散材料，電磁場的能量密度$u$為：
\[u=\frac{1}{2}(\mathbf{E}\cdot\mathbf{D}+\mathbf{B}\cdot\mathbf{H})=\frac{\varepsilon}{2}|\mathbf{E}|^2+\frac{1}{2\mu}|\mathbf{B}|^2.\]
\bpr
\[\pdv{u}{t}=\mathbf{E}\cdot\pdv{D}{t}+\mathbf{H}\cdot\pdv{\mathbf{B}}{t}\]
\epr
...
\item 光強度$I_l$：指電磁波垂直波向量截面上單位面積的輻射通量，即該面的輻照度與輻出度，即坡印廷向量的垂直分量的時間平均，即平均功率表面密度。
\eit
\[I_l=\langle\dv{p}{S}\rangle=\frac{\varepsilon c|\mathbf{E}_0|^2}{2}=\frac{c|\mathbf{B}_0|^2}{2\mu}\]
\sssc{點光源}
...
\section{電純量勢（Electric scalar potential）與磁向量勢（Magnetic vector potential）}
TODO
relativity?
