...
Suppose the primary and secondary coils have $N_p$ and $N_s$ turns and no resistance. The primary coil is connected to a source with emf $\calE=\calE_m\sin(\omega t)$. The secondary circuit is connected to a resistive load $R$. The equivalent resistance of secondary coil, as seen by generator, is $R_{eq}$. The emf per turn is
\[\calE_t=-\dv{\Phi_B}{t}=\frac{V_p}{N_p}=\frac{V_s}{N_s}\]
Thus
\[V_s=V_p\frac{N_s}{N_p}\]
The transformer is called set-up transformer if $N_s>N_p$, and set-down transformer if $N_s<N_p$.
The power transfered is
\[P=I_pV_p=I_sV_s\]
Thus
\[I_s=I_p\frac{N_p}{N_s}\]
And
\[V_s=I_sR\]
Therefore, the equivalent resistance, as seen by the source, $R_{eq}$, is
\[R_{eq}=\frac{V_p}{I_p}=\qty(\frac{N_s}{N_p})^2R\]
...
The power into matter is
\[P=f\cdot\mb{v}=\rho\mb{E}\cdot\mb{v}+\mb{J}_f\times\mb{B}\cdot\mb{v}=\mathbf{J}_f\cdot\mathbf{E}\]
\ssc{電磁場的能量}
電磁場的能量密度$u$為：
...
指的是某些材料相鄰電子自旋呈相同方向排列，使其在無外加磁場時可以具有自發磁化（spontaneous magnetization），且該磁化方向在施加足夠強的外加磁場時可以翻轉（switchability），且翻轉後的磁矩方向與外磁場之點積為正。Ferromagnetism subjects to thermal agitation a lot. 磁域/磁田/磁疇（Magnetic domain）指鐵磁性物質的一小區域的磁原子經交換作用成群向同一方向排列，其邊界稱磁田壁（domain wall）。未經磁化時各磁田磁矩方向散亂分布，無淨巨觀磁矩。磁化過的鐵磁性物質即使外加磁場移除仍保有磁矩，稱自發磁矩。具之之物質稱ferromagnetic material or magnetic material（磁性物質）。\ce{Fe}、\ce{Co}、\ce{Ni} 具有鐵磁性。具有鐵磁性的物質較具有鐵電性的物質多。

將未被磁化的鐵磁性物質置於一可施加外加  處，隨著外加磁場逐漸增強，各磁域盡量與外加磁場同方向，自發磁化方向和外加磁場方向相近的磁域擴大，總磁化強度也隨之增加；反之則縮小。當所有磁域的磁矩方向一致，則達到飽和磁化（saturation magnetization），此時再加強外加磁場其磁化強度不再增加。再逐漸減小外加磁場，則磁化強度亦隨之減小，但程度較緩，並不沿原磁化曲線（magnetization curve） 返回，而是沿另一斜率較小之曲線變化，直到外加磁場降為零，而鐵磁性物質仍保有磁矩，此即磁滯現象（hysteresis），該磁矩稱該物質之殘磁（remanence）。接著再通以反向之外加磁場並逐漸加大，當鐵磁物質之磁性為零時之外加 H-field 大小即為此物質之抗磁力或矯頑力（coercive force）或抗磁場強度（coercive field intensity）。反向磁場大到某一程度，鐵磁性物質磁化強度亦會達到飽和，再減小反向磁場至零點，鐵磁性物質仍具磁性。再通以正向磁場，直到飽和。整個過程紀錄在 M-H 圖上的封閉曲線即稱為磁滯曲線（hysteresis curve or loop）。
