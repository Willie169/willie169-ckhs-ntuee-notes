...
其中$\oint_{l}\mathbf{H}\cdot\mathrm{d}\mathbf{l}$與$\oint_{l}\mathbf{B}\cdot\mathrm{d}\mathbf{l}$稱磁環場積、當$l$為線圈稱安培環（Ampèrian loop）。
...
長直載流導線上半徑$R$載有電流強度$I$，在徑向距離軸$r$產生磁場量值$B$：
...
\ea\]
\epr
Let $\mb{m}\cdot\mb{B}=0$ be potential $U=0$ and $\theta=\pi$.
\[U=-\mb{m}\cdot\mb{B}\]
\bpr
\[\ba
