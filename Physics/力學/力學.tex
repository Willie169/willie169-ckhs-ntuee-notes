...
We can find a set of $x$-, $y$-, and $z$-axes such that $I_{xy}=I_{xz}=I_{yz}=0$, that is, $\mb{I}$ is diagonal.
...
\ssc{Rotation using body-fixed frame}
Below, we'll consider rotation of a rigid body in the body-fixed frame, that is, the rotating frame of reference such that any point of the body remains at the same position in the frame.
\sssc{Euler's rotation equations (歐拉（轉動）方程)}
Euler's rotation equations are a first-order ordinary differential equation describing the rotation of a rigid body using body-fixed frame.
\[\mb{I}\dot{\bomega}+\bomega\times(\mb{I}\bomega=\btau,\]
that is,
\[\dv{L}{t}+\bomega\times\mb{L}=\btau.\]
...
Suppose a top is subjected to a constant force $\mb{F}$ on the COM, which is on its rotational axis, and another force $-\mb{F}$ on the fulcrum (支點), which is also on its rotational axis. Let the relative position of the COM from the fulcrum be $\mb{r}$.
