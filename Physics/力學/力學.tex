...
\ssc{Elasticity (彈性)}
\sssc{Introduction}
Elasticity is the ability of a body to resist a distorting external force/stress (外力/外應力) and to return to its original size and shape when that force/stress is removed. Such force and stress are called elastic force/stress, resisting force/stress, or restoring force/stress, and are the negation of the net external force and stress applied when the two achieves static equilibrium.

Linear elastic region or simply elastic region is a region of a type external stress in which the strain of the elastic object is proportional to stress, called linear elasticity. The bounds of the linear elastic region are called elastic limits (彈性限度) or yield points.
...
Four modulo are used to describe elasticity or stiffness of an elastic object within the elastic region, which are given by four stress-strain relations, collectively called generalized Hooke's law (虎克定律).
\sssc{Young's modulus or Young modulus (楊氏模量/楊氏模數) (E) for axial elasticity}
When an object is under uniaxial stress (i.e. tension or compression in one direction), the uniaxial stress–strain relation during static equilibrium is given by
\[p=\frac{F}{A}=E\frac{\Delta L}{L},\]
where $p$ is the external axial (i.e. tensile or compressive) stress applied, postive when pointing outward the object and negative when pointing inward the object, $F$ is the external axial force, with same orientationas $p$, and $A$ is the cross-sectional of the object perpendicular to $p$, $\Delta L$ is the change of the axial length of the object, $L$ is the original axial length of the object, $\frac{\Delta L}{L}$ is called the axial strain of the object, and $E$ is a positive constant called Young's modulus or Young modulus, which is only dependent on the material of the object but not $A$ or $L$.

The original Hooke's law for linear spring is an implicate of the above relation, written
\[-F=-k(\Delta L),\]
