...
\[\mathbf{r}_c=\frac{\int_V\rho(\mb{r})\mathbf{r}\dd{V}}{\int_V\rho(\mb{r})\dd{V}},\]
...
\ssc{轉動（Rotation）}
Below only consider rotation in three-dimensional space.
\sssc{Rotaion}
Rotations is a rigid body movement which, unlike a translation, keeps at least one point fixed. We call a rotation to be around or about the fixed point or axis (when all points on a line are fixed).
...
\sssc{Rotation around a fixed axis or axial rotation}
The angular position of a point $\mathbf{P}$ with respect to a given reference axis, which is a line oriented along a unit vector $\hat{\mathbf{n}}$, relative to a given polar axis on some plane $F$ perpendicular to the reference axis, which defines a polar coordinate system whose positive sense is determined by looking upward along $\hat{\mathbf{n}}$, is the polar angle $\theta$ of the projection of $\mathbf{P}$ on $F$ (scalar) or $\boldsymbol{\theta}=\theta\hat{\mb{n}}$ (three-dimensional vector).

The angular displacement is the angular position at the end subtracted by the angular position at the begining. It is linear for rotations around a same axis and as a vector is linear for infinisimal rotations. However, it as a vector is not linear for rotations around arbitrary axes.
