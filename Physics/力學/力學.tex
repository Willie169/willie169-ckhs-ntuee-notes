...
For a system of $N$ particles with masses $m_i$ at postion $\mathbf{r}_i$, the (position of) center of mass $\mathbf{r}$ is
\[\mathbf{r}=\frac{\sum_{i=1}^Nm_i\mathbf{r}_i}{\sum_{i=1}^Nm_i}.\]

For a system of volumne $V$ of density field $\rho(\mathbf{r})$ of continuous mass distribution with (mass) density $\rho(\mathbf{r})$, the (position of) center of mass $\mathbf{r}$
\[\mathbf{r}=\frac{\int_V\mathbf{r}\dd{V}}{V}.\]
\sssc{向量}
\bit
...
where $\theta$ is the polar angle of the projection of $\mathbf{P}$ on $F$.
