...
給定速率$v$下最大射程$x_m\coloneq\max(x)$為
\[x_m=\frac{v^2}{g}\]
發生於
\[\tan\theta=\frac{v}{\sqrt{v^2+2gh}}.\]
\begin{proof}
\[x_m=\frac{v\cos\theta}{g}\qty(v\sin\theta+\sqrt{v^2\sin^2\theta+2gh})\]
Let
\[s=\sin\theta,\quad c=\cos\theta,\quad T=\tan\theta\]
\[S(\theta)=\sqrt{v^2s^2+2gh}\]
\[S'=\frac{v^2sc}{S}\]
\[x_m=\frac{vc}{g}\qty(vs+S)\]
\[\ba
x_m'&=\frac{-vs}{g}\qty(vs+S)+\frac{vc}{g}\qty(vc+S')\\
&=\frac{v}{g}\qty(-vs^2-sS+vc^2+S'c)\\
&=\frac{v}{g}\qty(-vs^2-sS+vc^2+\frac{v^2sc^2}{S})\\
&=\frac{v}{gS}\qty(-vs^2S-sS^2+vc^2S+v^2sc^2)\\
{{
