...
\eit
\ssc{Center of mass (COM) (質心)}
\sssc{Density (密度)}
The density $\rho(\mb{r})$ at a point in an object is a scalar field defined as the derivative of mass $m$ with respect to volume $V$ at that point, that is, $\rho(\mb{r})=\dv{m}{V}$.

The average density of an abject is the total mass of it divided by the volume of it.
\sssc{Center of mass (COM) (質心)}
...
For a system of $N$ particles with masses $m_i$ at postion $\mb{r}_i$, the (position of) center of mass $\mb{r}$ is
\[\mb{r}=\frac{\sum_{i=1}^Nm_i\mb{r}_i}{\sum_{i=1}^Nm_i}.\]

For a system of volumne $V$ of density field $\rho(\mb{r})$ of continuous mass distribution with (mass) density $\rho(\mb{r})$, the (position of) center of mass $\mb{r}$
\[\mb{r}=\frac{\int_V\mb{r}\dd{V}}{V}.\]
\sssc{向量}
\bit
...
令時間$t_1$與$t_2$時$\mb{v}$與$m$分別為$\mb{v}_1$、$m_1$與$\mb{v}_2$、$m_2$。積分前式：
\[\mb{v}_2-\mb{v}_1=\mathbf{v}_{rel}\int_{t_1}^{t_2}\frac{R}{m}\dd{t}.\]
由於：
\[\dv{\ln|f(x)|}{x}=\frac{f'(x)}{f(x)}.\]
我們有：
\[\mb{v}_2-\mb{v}_1=\mathbf{v}_{rel}\ln\frac{M_2}{M_1}.\]
\sssc{質量減少與火箭方程}
令無外力下：
\begin{itemize}
...
\[F=-kx,\]
其中$x$與$F$以正負號表示一維方向，$k$為一正常數，稱力常數（force constant）或彈力常數/彈性常數（spring constant），依賴於材料與截面積；又可以寫作
\[p=Ex,\]
其中$p$為材料承受之軸向應力（stress）（壓力），$E$為一正常數，稱楊氏模量/楊氏模數（Young's modulus or Young modulus）。
...
\end{itemize}
兩質點質量$m_1$、$m_2$，碰撞前，速度$v_1$、$v_2$，質心速度$v_c$，內動能$K_{int}$，令
兩質點縮減質量$\mu$，兩者彈性碰撞，無外力，碰撞後，$m_1$之速度$v'_1$、$m_2$之速度$v'_2$、內動能$K'_{int}$：
\[\begin{aligned}
v_c &= \frac{m_1v_1+m_2v_2}{m_1+m_2} \\
K_{int} &= \frac{\mu\qty(v_1-v_2)^2}{2}\\
v'_1 &= 2v_c - v_1 = \f{(m_1-m_2)v_1+2m_2v_2}{m_1+m_2} \\
v'_2 &= 2v_c - v_2 = \f{(m_2-m_1)v_2+2m_1v_1}{m_2+m_1} \\
K'_{int} &= \f{\mu \qty(v'_1-v'_2)^2}{2} = \f{\mu \qty(v_1-v_2)^2}{2} = K_{int}
\end{aligned}\]
add euqal mass, m1 >>, m2 >>, v_2=0 cases
