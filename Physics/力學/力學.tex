...
\item\tb{位移（Displacement）$\Delta\mb{r}$}：系統末位置減去初位置。
\item\tb{（瞬間/瞬時（Instantaneous））速度（Velocity）$\mb{v}$}：系統位置對時間的導數。
\item\tb{平均速度（Average velocity）}：系統位移，與末時刻減去初時刻，的比值。
\item\tb{（瞬間/瞬時）加速度（Acceleration）$\mb{a}$}：系統速度對時間的導數。
\item\tb{（瞬間/瞬時）加加速度（Jerk or jolt）}：系統加速度對時間的導數。
\item\tb{平均加速度（Average acceleration）}：系統末速度減去初速度，與末時刻減去初時刻，的比值。
\item\tb{動量（Momentum）$\mb{p}$}：系統質量與速度的乘積。
\item\tb{衝量（Impulse）$\mb{J}$}：系統末動量減去初動量。
\end{itemize}
\subsubsection{純量}
\begin{itemize}
...
\[\mathbf{r}_c=\frac{\iiint_V\rho(\mb{r})\mathbf{r}\dd{V}}{\iiint_V\rho(\mb{r})\dd{V}},\]
...
\[\rho(\mb{r})=\sum_{i=1}^Nm_i\delta(\mb{r}-\mb{r}_i),\]
...
\sssc{Velocity of a point in a rotating rigid body}
If a rigid body is rotating at angular velocity $\boldsymbol{\omega}$ and angular acceleration $\balpha$ around a rotational axis, and the axis is translating at velocity $\mb{v}_t$, then a mass point in a rigid body at position $\mb{r}$ from the axis is translating at velocity
\[\dot{\mb{r}}=\boldsymbol{\omega}\times\mb{r}+\mb{v}_t.\]
\sssc{Moment of inertia (轉動慣量) or rotational inertia (scalar)}
The moment of inertia or rotational inertia scalar $I$ of an object of volume $V$ about its rotational axis is a measure of the object's resistance to changes to its rotation, defined with the density field $\rho(\mb{r})$ with some point on the axis as origin and unit vector in the direction of axis $\hat{\mb{n}}$ as:
\[I=\iiint_V\rho(\mb{r})\abs{\mb{r}\times\hat{\mb{n}}}^2\dd{V}.\]
\sssc{Moment of inertia or rotational inertia tensor}
The moment of inertia or rotational inertia tensor $\mb{I}$ of an object of volume $V$ is a measure of the object's resistance to changes to its rotation independent from axes, defined with the density field $\rho(\mb{r})$:
\[\mb{I}=\begin{pmatrix}
    I_{xx} & I_{xy} & I_{xz}\\
    I_{yx} & I_{yy} & I_{yz}\\
    I_{zx} & I_{zy} & I_{zz}
\end{pmatrix},\]
where:
\[I_{xx}=\iiint_V\rho(\mb{r})\qty(\qty(\mb{r}\cdot\hat{\mb{j}})^2+\qty(\mb{r}\cdot\hat{\mb{k}})^2)\dd{V},\]
\[I_{yy}=\iiint_V\rho(\mb{r})\qty(\qty(\mb{r}\cdot\hat{\mb{i}})^2+\qty(\mb{r}\cdot\hat{\mb{k}})^2)\dd{V},\]
\[I_{zz}=\iiint_V\rho(\mb{r})\qty(\qty(\mb{r}\cdot\hat{\mb{i}})^2+\qty(\mb{r}\cdot\hat{\mb{j}})^2)\dd{V},\]
\[I_{xy}=I_{yx}=-\iiint_V\rho(\mb{r})\qty(\mb{r}\cdot\hat{\mb{i}})\cdot\qty(\mb{r}\cdot\hat{\mb{j}})\dd{V},\]
\[I_{yz}=I_{zy}=-\iiint_V\rho(\mb{r})\qty(\mb{r}\cdot\hat{\mb{j}})\cdot\qty(\mb{r}\cdot\hat{\mb{z}})\dd{V},\]
\[I_{xz}=I_{zx}=-\iiint_V\rho(\mb{r})\qty(\mb{r}\cdot\hat{\mb{i}})\cdot\qty(\mb{r}\cdot\hat{\mb{z}})\dd{V},\]
where $\hat{\mb{i}},\hat{\mb{j}},\hat{\mb{k}}$ are the unit vectors in the positive $x$-, $y$-, and $z$-axes.
\sssc{Angular momentum (角動量)}
The angular momentum $\mb{L}$ is
\[\mb{L}=\mb{I}\bomega=I\bomega=\iint_V\rho{\mb{r}}\mb{r}\times\dot{\mb{r}}\dd{V}.\]
\sssc{Torque (力矩)}
The torque $\btau$ is the twisting effect of a force $F$ applied to a rigid body at position $\mb{r}$ from its axis of rotation given by
\[\btau=\mb{r}\times\mb{F},\]
where $\mb{r}$ is called the lever arm (力臂) of the force.

Sometimes
\[\tau=\abs{\btau}\]
is used as torque instead.
...
\[W=\int_{t_0}^{t_1}\btau\cdot\bomega\dd{t}=\int_{\theta_0}^{\theta_1}\tau\dd{\theta}.\]

The kinetic energy $K_r$ due to the rotation, called rotational kinetic energy, is given by
\[K_r=\frac{1}{2}I|\bomega|^2.\]
The translational kinetic energy $K_t$ associated with the motion of the rotation axis is given by
\[K_t=\frac{1}{2}m|\mb{v}|^2\]
where $m$ is the total mass of the rigid body and $\mb{v}$ is the velocity of the axis of rotation.

The total kinetic energy $K$ of the rigid body is given by
\[K=\frac{1}{2}\iint_V\rho{\mb{r}}\abs{\dot{\mb{r}}}^2\dd{V}=K_r+K_t.\]
...
指一系統達移動平衡且轉動平衡。
\sssc{Precession (進動) and nutation (章動)}
The (instantaneous) precession rate (（瞬時）進動率) $\bOmega$ is defined as
\[\bOmega=\frac{\mb{L}\times\btau}{\abs{\mb{L}}^2}\]
to describe the angular velocity of the axis of rotation.

Sometimes $\Omega=\abs{\bOmega}$ is used as the instantaneous precession rate.

Suppose a rigid body of which the mass is distributed axially symmetric about the rotational axis fixed to the body, called a spinning top or simply top, is subjected to a constant force $\mb{F}$ on the COM, which is on the axis, and another force $-\mb{F}$ on a fulcrum (支點) that is also on the axis. Let the relative position of the COM from the fulcrum be $\mb{r}$. The torque $\btau$ is
\[\btau=\mb{r}\times\mb{F}.\]
The steady solution is such that the precession rate is a constant $\bOmega_c$ called steady procession dependent on the angular momentum $\mb{L}$
\[\bOmega_c=
