...
\ssc{Rotation (轉動)}
\sssc{Rotaion}
Rotations is a rigid body movement which, unlike a translation, keeps at least one point fixed. We call a rotation to be around or about the fixed point or axis (when all points on a line are fixed), and call such point or axis point or axis of the rotation or rotational point or axis.
...
Any motion of a rigid body such that a point on it remains fixed, is equivalanet to a single rotaion about some axis that passes through the fixed point.
...
\sssc{Moment of inertia (轉動慣量) or rotational inertia}
The moment of inertia or rotational inertia $I$ of an object of volume $V$ about its rotational axis is a measure of the object's resistance to changes to its rotation, defined with the density field $\rho(\mb{r})$ with some point on the axis as origin and unit vector in the direction of axis $\hat{\mb{n}}$ as:
\[I=\iiint_V\rho(\mb{r})\abs{\mb{r}\times\hat{\mb{n}}}^2\dd{V}.\]
\sssc{Angular momentum (角動量)}
The angular momentum $\mb{L}$ is
\[\mb{L}=I\bomega=\iint_V\rho{\mb{r}}\mb{r}\times\dot{\mb{r}}\dd{V}.\]
\sssc{Torque (力矩)}
...
\sssc{Linearity}
Qunatities of a rotation about an axis or describing the rotational property of a rigid body to do such rotations, such as angular displacement, are linear only for rotations around a same axis and, as three-dimensional, linear for infinisimal rotations. However, they are not linear for arbitrary rotations.
